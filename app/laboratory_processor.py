...
from app.development_journal import list_records
...
def analyze_development_journal() -> Dict[str, Any]:
    records = [r for r in list_records(include_test=False, include_archived=False) if _norm(r.get('status')) not in {'closed'}]
...
def build_laboratory_response(result: Dict[str, Any] | None = None) -> Dict[str, Any]:
    ...
    lines = [
        ...
        '',
        f"Проанализировано Production-записей: **{result.get('records_analyzed')}**",
