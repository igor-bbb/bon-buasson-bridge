...
def _build_all_block(level: str, rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], period: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    try:
        return _build_drain(level, rows, business_metrics, period, limit=None)
    except TypeError:
        # compatibility with tests/monkeypatches expecting old signature
        items, next_level = _build_drain(level, rows, business_metrics, period)
        if isinstance(items, list):
            items = sorted(items, key=lambda item: _to_float(item.get("effect_money")))
