...
def _build_network_focus(drain_block: List[Dict[str, Any]]) -> Dict[str, Any]:
    top_item = (drain_block or [{}])[0] or {}
    return {
        "object_name": top_item.get("object_name"),
        "effect_money": round_money(_to_float(top_item.get("effect_money"))),
    }


def _build_network_decision(structure: Dict[str, Any], goal: Dict[str, Any]) -> List[Dict[str, Any]]:
    negatives = []
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        item = structure.get(key) or {}
        effect_money = _to_float(item.get("effect_money"))
        if effect_money < 0:
            negatives.append((key, abs(effect_money)))
    negatives.sort(key=lambda x: x[1], reverse=True)
    decisions: List[Dict[str, Any]] = []
    for key, effect_money in negatives[:3]:
        decisions.append({
            "action": ACTION_LABELS_RU.get(key, key),
            "effect_money": round_money(effect_money),
        })

    target = abs(_to_float((goal or {}).get("value_money")))
    if decisions and target > 0:
        current_sum = sum(_to_float(item.get("effect_money")) for item in decisions)
        if current_sum > 0:
            scale = target / current_sum
            scaled_sum = 0.0
            for item in decisions[:-1]:
                scaled = round_money(_to_float(item.get("effect_money")) * scale)
                item["effect_money"] = scaled
                scaled_sum += _to_float(scaled)
            decisions[-1]["effect_money"] = round_money(max(target - scaled_sum, 0.0))
    return decisions
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], business_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "navigation": {**_build_navigation(level, drain_block, adaptive_next_level, has_all=bool(all_block), has_causes=bool(cause_block)), "mode": "default"},
        "all_block": all_block,
    ...
    if level == "network":
        payload["focus_block"] = _build_network_focus(drain_block)
        payload["decision_block"] = _build_network_decision(structure, payload.get("goal") or {})
    return payload
