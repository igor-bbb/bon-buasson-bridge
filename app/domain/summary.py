...
def _build_network_adaptive_drain(object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], str]:
    ...
    return drain_items, selected_level
...
def _build_drain(level: str, object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], period: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    if level == "network":
        return _build_network_adaptive_drain(object_rows, business_metrics)
...
def _build_navigation(level: str, drain_block: List[Dict[str, Any]], next_level: Optional[str] = None) -> Dict[str, Any]:
    top_items = [item.get("object_name") for item in drain_block[:3]]
    all_items = [
        {"object_name": item.get("object_name"), "effect_money": round_money(_to_float(item.get("effect_money")))}
...
def _build_cause_navigation(structure: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        ...
        if effect_money < 0:
            causes.append({
                "article": ARTICLE_LABELS_RU.get(key, key),
                "effect_money": round_money(effect_money),
            })
    causes.sort(key=lambda item: _to_float(item.get("effect_money")))
    return causes


def _build_network_focus(structure: Dict[str, Any]) -> Dict[str, Any]:
    negatives = []
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        item = structure.get(key) or {}
        effect_money = _to_float(item.get("effect_money"))
    ...
    negatives.sort(key=lambda x: _to_float(x.get("effect_money")))
    return {"items": negatives[:2]}

def _build_sku_focus(structure: Dict[str, Any]) -> Dict[str, Any]:
    negatives = []
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        item = structure.get(key) or {}
        effect_money = _to_float(item.get("effect_money"))
...
def _build_sku_decision(structure: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        ...
        effect_money = _to_float(item.get("effect_money"))
        if effect_money < 0:
            negatives.append((key, effect_money))
    negatives.sort(key=lambda x: x[1])
    decisions = []
    for key, effect_money in negatives[:2]:
        decisions.append({
            "action": ACTION_LABELS_RU.get(key, key),
            "effect_money": round_money(abs(effect_money)),
        })
    return decisions


def _build_sku_prompt(focus_block: Dict[str, Any]) -> List[str]:
    prompts: List[str] = []
    for item in (focus_block.get("items") or [])[:2]:
        article = item.get("article") or "статья"
        effect_money = abs(_to_float(item.get("effect_money")))
        prompts.append(f"скажи: что сделаешь, чтобы вернуть {round_money(effect_money)} по статье '{article}'?")
    return prompts
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], business_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    payload = {
        ...
        "navigation": _build_navigation(level, drain_block, adaptive_next_level),
    ...
    if level == "sku":
        ...
        payload["cause_navigation"] = _build_cause_navigation(structure)
        ...
        payload["drain_block"] = []
        payload["navigation"] = _build_navigation(level, [], adaptive_next_level)
        return payload

    payload["structure"] = structure
    payload["cause_navigation"] = _build_cause_navigation(structure)
    payload["drain_block"] = drain_block[:3]
    if level == "network":
