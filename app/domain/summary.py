...
def _build_metrics(current_metrics: Dict[str, float], prev_metrics: Dict[str, float], level: str) -> Dict[str, Any]:
    revenue_metric = _metric_money(_to_float(current_metrics.get("revenue")), _to_float(prev_metrics.get("revenue")))
    markup_metric = _metric_percent(
        _safe_markup_percent(current_metrics),
        _safe_markup_percent(prev_metrics),
    )
    finrez_pre_metric = _metric_money(_to_float(current_metrics.get("finrez_pre")), _to_float(prev_metrics.get("finrez_pre")))
    margin_metric = _metric_percent(
        _to_float(current_metrics.get("margin_pre")),
        _to_float(prev_metrics.get("margin_pre")),
    )
    metrics: Dict[str, Any] = {
        "revenue": revenue_metric,
        "markup_percent": markup_metric,
        "finrez_pre": finrez_pre_metric,
        "margin_percent": margin_metric,
        # response aliases without changing core logic
        "markup": markup_metric,
        "finrez": finrez_pre_metric,
...
def _build_grouping_type(rows: List[Dict[str, Any]], period: str) -> str:
    try:
        sku_total = len({clean_text(row.get("sku", "")) for row in rows if clean_text(row.get("sku", ""))})
        tmc_total = len({clean_text(row.get("tmc_group", "")) for row in rows if clean_text(row.get("tmc_group", ""))})
        category_total = len({clean_text(row.get("category", "")) for row in rows if clean_text(row.get("category", ""))})
        if category_total and category_total <= 3:
            return "category"
        if tmc_total and tmc_total <= 6:
            return "tmc"
        return "sku"
    except Exception:
        return "sku"
