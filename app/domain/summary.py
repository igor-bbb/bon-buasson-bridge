...
def get_business_summary(period: str, filter_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    effective_filter = dict(filter_payload or {})
    effective_filter["period"] = period
    rows, _ = _run_filter(all_rows, period=period, **{k: v for k, v in effective_filter.items() if k != "period" and v})
    prev_rows = []
    prev_period = _previous_year_period(period)
    if prev_period:
        prev_filter = dict(effective_filter)
        prev_filter["period"] = prev_period
        prev_rows, _ = _run_filter(all_rows, period=prev_period, **{k: v for k, v in prev_filter.items() if k != "period" and v})
    payload = _summary_from_rows("business", "business", period, rows, prev_rows, rows)
...
def _object_summary(level: str, field: str, object_name: str, period: str, filter_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    effective_filter["period"] = period
    if object_name:
        effective_filter[field] = object_name

    current_kwargs = {k: v for k, v in effective_filter.items() if k != "period" and v}
    rows, _ = _run_filter(all_rows, period=period, **current_kwargs)

    business_filter = {k: v for k, v in effective_filter.items() if k not in {"manager_top", "manager", "network", "category", "tmc_group", "sku", "period"} and v}
    business_rows, _ = _run_filter(all_rows, period=period, **business_filter)
    ...
    if prev_period:
        prev_filter = dict(effective_filter)
        prev_filter["period"] = prev_period
        prev_kwargs = {k: v for k, v in prev_filter.items() if k != "period" and v}
        prev_rows, _ = _run_filter(all_rows, period=prev_period, **prev_kwargs)

    payload = _summary_from_rows(level, object_name, period, rows, prev_rows, business_rows)
    payload["filter"] = effective_filter
    return payload
