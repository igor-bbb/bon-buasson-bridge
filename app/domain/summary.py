...
def _product_tmc_decision(level: str, rows: List[Dict[str, Any]], all_rows: List[Dict[str, Any]], period: str) -> Optional[Dict[str, Any]]:
    ...
    TMC Group is not a mandatory navigation level. On Category screens VECTRA
    still analyses TMC groups internally so the assistant can decide whether a
    category-level recommendation is enough or whether an additional TMC
    drilldown is needed. This does not change the navigation tree.
    """
    ...
    denominator = abs(parent_delta) if abs(parent_delta) > 1e-9 else sum(
        abs(_profit_delta_money(aggregate_metrics(chunk), _child_prev_metrics("tmc_group", chunk, period)))
    ...
    for name, chunk in grouped.items():
        ...
        prev = _child_prev_metrics("tmc_group", chunk, period)
        profit_delta = _profit_delta_money(current, prev)
        business_rows, _ = _run_filter(all_rows, period=period, tmc_group=name)
...
def _decision_workspace_category_intelligence(all_block: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    categories.sort(key=lambda item: _to_float(item.get("navigation_money")), reverse=True)
...
def _sku_role(revenue: float, share_network: float, share_business: float) -> str:
    if share_network >= 20:
        return "якорный SKU контракта"
    if share_business >= 5:
        return "значимый SKU бизнеса"
    if revenue > 0:
        return "рабочий SKU"
    return "низкая роль"
...
def _build_contract_assortment_analysis(rows: List[Dict[str, Any]], all_rows: List[Dict[str, Any]], period: str) -> Dict[str, Any]:
    ...
    for sku, sku_rows in _group_rows_by(rows, "sku").items():
        ...
        sku_leaders.append({
            ...
            "role": _sku_role(revenue, share_network, share_business),
        })
    ...
    for sku, sku_rows in _group_rows_by(business_period_rows, "sku").items():
        ...
        missing_business_leaders.append({
            ...
            "reason": "SKU входит в продажи бизнеса за период, но отсутствует в выбранной сети.",
        })
...
def _build_decision_workspace(
    ...
    category_intelligence = _decision_workspace_category_intelligence(all_block)
    assortment_analysis = assortment_analysis or {}
    negotiation_package = {
        "goal": "подготовить развитие контракта на основании экономики, категорий и SKU",
        ...
        "arguments": [
            "категории и SKU подтверждены данными текущего периода",
    ...
    return {
        ...
        "product_diagnostics": {
            "summary": "Product Layer используется как доказательная база Decision Engine, а не как обязательный следующий экран.",
