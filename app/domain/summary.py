...
def _structure_item(current_money: float, base_money: float, current_revenue: float, base_revenue: float, is_expense: bool, scale_base_to_current_revenue: bool = False) -> Dict[str, float]:
    current_signed_money = _signed_money(current_money, is_expense)
    raw_base_signed_money = _signed_money(base_money, is_expense)
    current_percent = _signed_percent(current_signed_money, current_revenue)
    base_percent = _signed_percent(raw_base_signed_money, base_revenue)
    effective_base_money = raw_base_signed_money
    if scale_base_to_current_revenue:
        effective_base_money = round_money((base_percent / 100.0) * current_revenue) if abs(current_revenue) > 1e-9 else 0.0
    ...
    return {
        ...
        "base_money": round_money(effective_base_money),
...
def _build_structure(current_metrics: Dict[str, float], base_metrics: Dict[str, float], scale_base_to_current_revenue: bool = False) -> Dict[str, Any]:
    ...
    for public_key, metric_key, is_expense in STRUCTURE_KEYS:
        ...
        structure[public_key] = _structure_item(
            current_money,
            base_money,
            current_revenue,
            base_revenue,
            is_expense,
            scale_base_to_current_revenue=scale_base_to_current_revenue,
        )
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], base_rows: List[Dict[str, Any]], previous_state: Optional[Dict[str, Any]] = None, propagated_effect: Optional[float] = None) -> Dict[str, Any]:
    ...
    structure = _build_structure(current_metrics, structure_base_metrics, scale_base_to_current_revenue=(level != "business"))
