...
def _build_network_adaptive_drain(object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], limit: Optional[int] = 3) -> Tuple[List[Dict[str, Any]], str]:
    ...
    for sku_name, rows in sku_groups.items():
        ...
        margin_percent = _to_float(metrics.get("margin_pre"))
        ...
        sku_items.append({
            ...
            "effect_money": _to_float(_drain_item(sku_name, rows, business_metrics).get("effect_money")),
        })
    ...
    median_margin = _percentile(sorted_margins, 0.5)
    # Q1/Q2 = lower half by margin signal. Q3/Q4 are filtered out.
    signal_filtered = [
        item for item in sku_items
        if item["margin_percent"] <= median_margin and _to_float(item["effect_money"]) < 0
    ]
    if not signal_filtered:
        signal_filtered = [item for item in sku_items if _to_float(item["effect_money"]) < 0]
    ...
    drain_items = [
        {"object_name": name, "effect_money": round_money(value)}
...
def _build_reasons_block(structure: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        ...
        reasons.append({
            ...
            "metric": key,
            "effect_money": round_money(_to_float(item.get("effect_money"))),
            "source_level": "sku" if key == "markup" else "network",
        })
    reasons.sort(key=lambda x: _to_float(x.get("effect_money")))
    return reasons


def _top_negative_structure(structure: Dict[str, Any], limit: int = 3) -> List[Dict[str, Any]]:
    items: List[Dict[str, Any]] = []
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        item = structure.get(key) or {}
        ...
        if effect_money < 0:
            items.append({
                "metric": key,
                "effect_money": round_money(effect_money),
                "solution_level": SOLUTION_LEVEL_BY_METRIC.get(key, "network"),
                "direction": "bad",
                "source_level": "sku" if key == "markup" else "network",
            })
...
def _build_network_focus(structure: Dict[str, Any]) -> List[Dict[str, Any]]:
    return _top_negative_structure(structure, limit=3)
...
def _build_decision_block(structure: Dict[str, Any], level: str) -> List[Dict[str, Any]]:
    block: List[Dict[str, Any]] = []
    for item in _top_negative_structure(structure, limit=3):
        metric = item.get("metric")
        solution_level = item.get("solution_level")
        if level == "network" and solution_level not in {"network", "sku"}:
            continue
        if level == "sku" and solution_level != "sku":
            continue
        block.append({
            "metric": metric,
            "direction": item.get("direction", "bad"),
            "solution_level": solution_level,
            "effect_money": round_money(abs(_to_float(item.get("effect_money")))),
            "source_level": item.get("source_level"),
        })
    return block


def _build_grouping_type(object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any]) -> str:
    drain_items, selected_level = _build_network_adaptive_drain(object_rows, business_metrics, limit=None)
    if selected_level == "category":
        return "category"
    negative_sku_count = 0
    for sku_name, rows in _group(object_rows, "sku").items():
        if _to_float(_drain_item(sku_name, rows, business_metrics).get("effect_money")) < 0:
            negative_sku_count += 1
    if negative_sku_count <= 3:
        return "sku"
    return "tmc" if drain_items else "sku"
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], business_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    reasons_block = _build_reasons_block(structure)
    navigation = _build_navigation(level, drain_block, adaptive_next_level, has_all=bool(all_block), has_causes=bool(reasons_block))
    navigation["mode"] = "default"
    navigation["vector"] = [item.get("object_name") for item in drain_block[:3]]
    ...
    if level == "sku":
        ...
        payload["decision_block"] = _build_decision_block(structure, level="sku")
        payload["drain_block"] = []
        payload["navigation"]["vector"] = []
        return payload
    ...
    if level == "network":
        ...
        payload["decision_block"] = _build_decision_block(structure, level="network")
        payload["grouping_type"] = _build_grouping_type(rows, business_metrics if business_rows else current_metrics)
    return payload
...
def get_category_summary(category: str, period: str) -> Dict[str, Any]:
    return _object_summary("category", "category", category, period)


def get_tmc_group_summary(tmc_group: str, period: str) -> Dict[str, Any]:
    return _object_summary("tmc_group", "tmc_group", tmc_group, period)
