...
def _impact_label(delta_money: float) -> str:
    if delta_money > 0:
        return "улучшили"
    if delta_money < 0:
        return "ухудшили"
...
def _structure_item(
    ...
    current_signed_money = -abs(current_money) if is_expense else current_money
    base_signed_money = -abs(base_money) if is_expense else base_money
    current_percent = round_percent((current_signed_money / current_revenue) * 100.0) if abs(current_revenue) > 1e-9 else 0.0
    base_percent = round_percent((base_signed_money / base_revenue) * 100.0) if abs(base_revenue) > 1e-9 else 0.0
    ...
    return {
        ...
        "impact": _impact_label(delta_money),
    }
