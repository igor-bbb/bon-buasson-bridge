...
def _build_network_adaptive_drain(object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], str]:
    ...
    for item in sku_effect_items:
        effect_money = _to_float(item["effect_money"])
        total_negative_effect += effect_money
        category_totals[item["category"]] += effect_money
        group_totals[item["tmc_group"]] += effect_money

    total_abs = abs(total_negative_effect)
    category_shares = {name: (abs(value) / total_abs) if total_abs > 1e-9 else 0.0 for name, value in category_totals.items()}
