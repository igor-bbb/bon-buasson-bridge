...
def _build_network_adaptive_drain(object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], limit: Optional[int] = 3) -> Tuple[List[Dict[str, Any]], str]:
    ...
    if not signal_filtered:
        ...
        prepared = _sort_by_navigation_money(prepared)
        fallback_items = [{
    ...
    drain_items = _sort_by_navigation_money(drain_items)
    ...
    if limit is not None and len(drain_items) < limit:
        ...
        fillers = _sort_by_navigation_money(fillers)
...
def _build_drain(level: str, object_rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], period: str, limit: Optional[int] = 3) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    items = _sort_by_navigation_money(items)
...
def _build_all_block(level: str, rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], period: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    # It should include every object of the next level, sorted by Opportunity Money DESC.
    if level == "network":
    ...
    for name, child_rows in grouped.items():
        ...
        items.append(_lock_money_contract_item({
...
def _object_result_money(current_metrics: Dict[str, Any], prev_metrics: Dict[str, Any]) -> float:
...
def _opportunity_from_effect(effect_money: Any) -> float:
    """V1.2 Opportunity Money: positive return potential, never KPI result."""
    value = _to_float(effect_money)
    return round_money(abs(value)) if value < 0 else 0.0


def _lock_money_contract_item(item: Dict[str, Any]) -> Dict[str, Any]:
    """Attach v1.2 navigation money to child item without changing KPI formulas."""
    out = dict(item or {})
    opportunity = _opportunity_from_effect(out.get("effect_money"))
    out["opportunity_money"] = opportunity
    out["navigation_money"] = opportunity
    return out


def _sort_by_navigation_money(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    return sorted(
        [_lock_money_contract_item(item) for item in (items or []) if isinstance(item, dict)],
        key=lambda item: (_to_float(item.get("navigation_money")), -_to_float(item.get("effect_money"))),
        reverse=True,
    )


def _rating(items: List[Dict[str, Any]], *, field: str, limit: int = 10) -> List[Dict[str, Any]]:
    prepared = []
    for item in items or []:
        if not isinstance(item, dict):
            continue
        prepared.append({
            "object_name": item.get("object_name"),
            field: round_money(_to_float(item.get(field))),
            "effect_money": round_money(_to_float(item.get("effect_money"))),
        })
    prepared.sort(key=lambda item: _to_float(item.get(field)), reverse=True)
    return prepared[:limit]


def _attach_bottom_up_drain(payload: Dict[str, Any], current_metrics: Dict[str, Any], prev_metrics: Dict[str, Any], drain_block: List[Dict[str, Any]], all_block: List[Dict[str, Any]], level: str) -> None:
    """
    V12 Bottom-Up Drain Layer.

    object_result_money = performance of the object as a whole vs previous year.
    internal_drain_money = sum of negative child effects only. Positive child
    objects are context, not compensation.

    Navigation must be guided by internal_drain, not by object_result alone.
    """
    object_result = _object_result_money(current_metrics, prev_metrics)
    # V12.3: all_block is the canonical full child dataset. drain_block is
    # only top-route navigation and must not define the screen focus.
    source_items = all_block if all_block else drain_block
    net_total = _sum_net_effect(source_items)
    negative_total = _sum_negative_effect(source_items)

    opportunity = abs(negative_total)

    payload["object_result_money"] = object_result
    payload["opportunity_money"] = opportunity
    payload["navigation_money"] = opportunity
    payload["net_drain_money"] = net_total
    payload["gross_loss_money"] = negative_total
    payload["internal_drain_money"] = negative_total
    payload["drain_total"] = -opportunity

    locked_items = _sort_by_navigation_money(source_items)
    payload["opportunity_rating"] = _rating(locked_items, field="opportunity_money")
    payload["business_result_rating"] = _rating(locked_items, field="effect_money")

    # Business is the root CEO screen, not a management object. It may expose
    # business opportunity data, but it must not create or display Path Goal,
    # Focus Money, Coverage, Goal Block or Vector Block.
    if level == "business":
        payload.pop("focus_money", None)
        payload.pop("path_goal_money", None)
        payload.pop("vector_block", None)
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], business_rows: List[Dict[str, Any]], compare_base: str = "business") -> Dict[str, Any]:
    ...
    payload = {
        ...
        "goal": None if level == "business" else _build_goal(level, current_metrics, prev_metrics, business_metrics),
    ...
    _attach_bottom_up_drain(payload, current_metrics, prev_metrics, drain_block, all_block, level)
    if level == "business":
        payload["decision_block"] = _build_decision_block(structure, source_level="business", top_n=1)
    ...
    if level == "sku":
        ...
        sku_focus_money = 0.0
        for _focus in focus_block or []:
            sku_focus_money += abs(_to_float(_focus.get("effect_money")))
        if sku_focus_money <= 0:
            sku_focus_money = abs(_to_float(current_metrics.get("finrez_pre")))
        payload["drain_block"] = [{
            "object_name": _human_object_name(level, object_name),
            "effect_money": -round_money(sku_focus_money),
            "value_money": -round_money(sku_focus_money),
            "level": "sku",
            "self_drain": True,
        }] if sku_focus_money > 0 else []
        payload["drain_total"] = -round_money(sku_focus_money) if sku_focus_money > 0 else 0
        return payload
