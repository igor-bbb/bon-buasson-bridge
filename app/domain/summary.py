...
def _kpi_metric(current_value: float, prev_value: float) -> Dict[str, float]:
    return {
...
def _build_metrics(current_metrics: Dict[str, float], prev_year_metrics: Dict[str, float], level: str) -> Dict[str, Any]:
    metrics: Dict[str, Any] = {
        "revenue": _kpi_metric(_to_float(current_metrics.get("revenue")), _to_float(prev_year_metrics.get("revenue"))),
        "markup": _kpi_metric(_safe_markup_percent(current_metrics), _safe_markup_percent(prev_year_metrics)),
        "finrez_pre": _kpi_metric(_to_float(current_metrics.get("finrez_pre")), _to_float(prev_year_metrics.get("finrez_pre"))),
        "margin_pre": _kpi_metric(_to_float(current_metrics.get("margin_pre")), _to_float(prev_year_metrics.get("margin_pre"))),
    }
    if level == "business":
        metrics["finrez_final"] = _kpi_metric(_to_float(current_metrics.get("finrez_final")), _to_float(prev_year_metrics.get("finrez_final")))
    return metrics
...
def _build_focus_decision(structure: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if ordered and _to_float((ordered[0][1] or {}).get("effect_money")) < 0:
        ...
        for key, payload in ordered:
            ...
            if effect < 0:
                money = round_money(abs(effect))
                actions.append({"action": label_map.get(key, key), "effect_money": money})
                total += money
            if len(actions) >= 3:
                break
