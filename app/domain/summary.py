...
def _attach_bottom_up_drain(payload: Dict[str, Any], current_metrics: Dict[str, Any], prev_metrics: Dict[str, Any], drain_block: List[Dict[str, Any]], all_block: List[Dict[str, Any]]) -> None:
    """
    ...
    # V12.3: internal drain is the sum of all negative child effects.
    # Use all_block as the canonical child dataset when it exists; drain_block is
    # only a focus/top-list view.
    negative_total = _sum_negative_effect(all_block) if all_block else _sum_negative_effect(drain_block)
    ...
    payload["focus_money"] = abs(negative_total)
    payload["drain_total"] = negative_total

    # Keep existing vector path goal if already fixed by state, but current
    # focus should be internal drain for this screen.
    vector = payload.get("vector_block") if isinstance(payload.get("vector_block"), dict) else {}
    if vector is None:
        vector = {}
    if negative_total:
        vector["current_focus_money"] = abs(negative_total)
        payload["path_goal_money"] = vector.get("path_goal_money") or abs(negative_total)
        if not vector.get("path_goal_money"):
            vector["path_goal_money"] = abs(negative_total)
        try:
            vector["coverage_percent"] = min(100, int(round(abs(negative_total) / abs(float(vector.get("path_goal_money"))) * 100)))
        except Exception:
            pass
    payload["vector_block"] = vector
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], business_rows: List[Dict[str, Any]], compare_base: str = "business") -> Dict[str, Any]:
    ...
    _attach_bottom_up_drain(payload, current_metrics, prev_metrics, drain_block, all_block)
