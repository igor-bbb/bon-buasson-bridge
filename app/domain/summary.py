...
def _sort_navigation_items(items: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Stage 2 Navigation Contract: one ordering for Top-3 and all.

    Navigation is sorted only by lower-level money potential, not by KPI result,
    margin, markup or signal.
    """
    return sorted(
        items or [],
        key=lambda item: (
            -_to_float(item.get("navigation_money")),
...
def _build_all_block(level: str, rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], period: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    ...
    # Sorting is navigation_money DESC.
    if level == "network":
        items, next_level = _build_network_adaptive_drain(rows, business_metrics, limit=None)
        return _ensure_navigation_contract(items), next_level
    ...
    for idx, (name, child_rows) in enumerate(grouped.items(), start=1):
        ...
        effect_money = round_money(_to_float(item.get("effect_money")))
        child_structure = _build_structure(aggregate_metrics(child_rows), business_metrics)
        ...
        items.append({
            ...
            "navigation_money": opportunity_money,
        })
