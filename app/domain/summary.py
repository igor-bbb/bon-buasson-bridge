...
def _previous_article_values(key: str, prev_metrics: Dict[str, Any]) -> Dict[str, float]:
    ...
    if key == "markup":
        ...
        return {"previous_money": money, "previous_percent": percent}
...
def _build_cause_block(structure: Dict[str, Any], prev_metrics: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    for key in ["markup", "retro", "logistics", "personnel", "other"]:
        ...
        previous_percent = round_percent(_to_float(previous.get("previous_percent")))
        delta_vs_business = round_percent(value_percent - base_percent)
        delta_vs_previous = round_percent(value_percent - previous_percent)
        causes.append({
