...
def _build_all_block(level: str, rows: List[Dict[str, Any]], business_metrics: Dict[str, Any], period: str) -> Tuple[List[Dict[str, Any]], Optional[str]]:
    """Build full list for current level without top-3 truncation.

    Important: all_block must not depend on navigation.items because those contain only top-3.
    It also must not filter only negative values; it should return the full level sorted by effect_money ASC.
    """
    child = LEVEL_CHILD.get(level)
    if not child:
        return [], None
