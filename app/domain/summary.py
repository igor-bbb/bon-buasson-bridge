...
def _build_metrics(current_metrics: Dict[str, float], base_metrics: Dict[str, float], level: str) -> Dict[str, Any]:
    ...
    metrics: Dict[str, Any] = {
        ...
        "markup": _metric_percent(
            ...
            _to_float(base_metrics.get("markup")),
...
def _structure_item(name: str, current_money: float, base_money: float, current_revenue: float, base_revenue: float) -> Dict[str, float]:
    current_percent = round_percent((current_money / current_revenue) * 100.0) if abs(current_revenue) > 1e-9 else 0.0
    base_percent = round_percent((base_money / base_revenue) * 100.0) if abs(base_revenue) > 1e-9 else 0.0
    return {
...
def _build_structure(current_metrics: Dict[str, float], base_metrics: Dict[str, float]) -> Dict[str, Any]:
    ...
    for public_key, metric_key in STRUCTURE_KEYS:
        ...
        structure[public_key] = _structure_item(public_key, current_money, base_money, current_revenue, base_revenue)
    return structure
...
def _summary_from_rows(level: str, object_name: str, period: str, rows: List[Dict[str, Any]], prev_rows: List[Dict[str, Any]], business_rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    return {
        ...
        "metrics": _build_metrics(current_metrics, base_metrics, level),
