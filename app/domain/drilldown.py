...
def _navigation_money_from_item(item: Dict[str, Any]) -> float:
    if not isinstance(item, dict):
        return 0.0
    value = item.get('navigation_money')
    if value is None:
        impact = item.get('impact') if isinstance(item.get('impact'), dict) else {}
        value = impact.get('gap_loss_money')
    try:
        money = float(value or 0.0)
    except Exception:
        money = 0.0
...
def _item_sort_key(item: Dict[str, Any]) -> tuple:
    return (
        -_navigation_money_from_item(item),
...
def _build_items(grouped: Dict[str, List[Dict[str, Any]]], level: str, business_metrics: Dict[str, float], period: str) -> List[Dict[str, Any]]:
    ...
    for name, chunk in grouped.items():
        ...
        item['object_id'] = len(items) + 1
        item['navigation_money'] = _navigation_money_from_item(item)
        items.append(item)
