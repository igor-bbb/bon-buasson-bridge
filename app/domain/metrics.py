...
def aggregate_metrics(rows: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    finrez_pre = round_money(sum(_to_float(r.get('finrez_pre')) for r in rows))
    finrez_values = [_to_float(r.get('finrez')) for r in rows]
    finrez_final = round_money(sum(finrez_values))
...
def build_expected_metrics(
    ...
    negative_benchmark = _to_float(business_metrics.get('finrez_pre')) < 0
...
def interpret_effect(metric: str, effect_value: float) -> Dict[str, Any]:
    ...
    return {
        ...
        'type': 'income' if metric == 'finrez_pre' else 'cost',
...
def detect_next_step(level: str) -> str:
    chain = {
        ...
        'network': 'спуститься до позиций',
        'category': 'спуститься до позиций',
        'tmc_group': 'спуститься до позиций',
        'sku': 'принять решение по позиции',
    }
...
def detect_suggested_action(status: str, priority: str, top_drain_metric: Optional[str], level: str) -> str:
    ...
    if status == 'critical' and level == 'sku':
        return 'проверить цену, контракт и экономику позиции'
