...
def _weighted_percent(rows: List[Dict[str, Any]], value_field: str, weight_field: str = 'revenue') -> Optional[float]:
    weighted_sum = 0.0
    total_weight = 0.0
    has_value = False
    ...
    for row in rows:
        ...
        has_value = True
        if weight > 0:
            weighted_sum += float(value or 0.0) * weight
            total_weight += weight

    if has_value and total_weight > 0:
        return round_percent(weighted_sum / total_weight)
...
def aggregate_margin_pre_from_rows(rows: List[Dict[str, Any]]) -> Optional[float]:
    return _weighted_percent(rows, 'margin_pre', 'revenue')
...
def aggregate_metrics(rows: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    weighted_margin_pre = _weighted_percent(rows, 'margin_pre', 'revenue')
    weighted_markup = _weighted_percent(rows, 'markup', 'revenue')

    margin_pre = weighted_margin_pre if weighted_margin_pre is not None else _safe_percent(finrez_pre, revenue)
    markup = weighted_markup if weighted_markup is not None else _safe_percent(gross_profit, cost)
