...
def aggregate_metrics(rows: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    has_explicit_margin = any(r.get('margin_pre') not in {None, ''} for r in rows)
    weighted_margin_num = sum((r.get('margin_pre', 0.0) or 0.0) * (r.get('revenue', 0.0) or 0.0) for r in rows)
    weighted_margin_den = sum((r.get('revenue', 0.0) or 0.0) for r in rows)
    if has_explicit_margin and weighted_margin_den:
        margin_pre = round_percent(weighted_margin_num / weighted_margin_den)
    else:
        margin_pre = _safe_percent(finrez_pre, revenue)

    has_explicit_markup = any(r.get('markup') not in {None, ''} for r in rows)
    weighted_markup_num = sum((r.get('markup', 0.0) or 0.0) * (r.get('revenue', 0.0) or 0.0) for r in rows)
    weighted_markup_den = sum((r.get('revenue', 0.0) or 0.0) for r in rows)
    if has_explicit_markup and weighted_markup_den:
        markup = round_percent(weighted_markup_num / weighted_markup_den)
    elif cost > 0 and gross_profit != 0:
