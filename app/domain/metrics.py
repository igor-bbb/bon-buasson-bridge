...
def build_solutions_from_effects(
    ...
    for metric, value in effects.items():
        ...
        if metric == 'logistics_cost':
            ...
            effect = simulate_cost_reduction(object_metrics.get('other_costs'), 5)
            solutions.append({
                'metric': 'Прочие',
    ...
    seen = set()
    for item in sorted(solutions, key=lambda x: -_to_float(x.get('effect'))):
        key = item.get('metric')
        if key in seen:
            continue
        seen.add(key)
