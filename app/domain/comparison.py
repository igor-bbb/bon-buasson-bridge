...
SKU_PARENT_FIELDS = ('manager_top', 'manager', 'network')


def _has_required_sku_parent_context(filter_payload: Optional[Dict[str, Any]]) -> bool:
    """SKU in management flow is only valid inside the current parent contour.

    v6.1 acceptance rule: a SKU must never be calculated globally for a period.
    The minimal valid parent context is the current network. Manager / manager_top
    may also be present and must be preserved when provided.
    """
    if not filter_payload:
        return False
    network = filter_payload.get('network')
    return network not in (None, '', 'business')
...
def build_comparison_payload(
    ...
    period_signal = _build_level_signal(level, period, object_name, object_metrics, object_rows=object_rows)
    status = period_signal.get('status', 'ok')
...
def _single_object_comparison(
    ...
    **filters: Any
    ...
    merged_filters = dict(filter_payload or {})
...
def get_sku_comparison(sku: str, period: str, filter_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    if not _has_required_sku_parent_context(filter_payload):
        return {
            'error': 'no data after filtering',
            'reason': 'sku requires parent context',
        }
