...
def build_comparison_payload(
    ...
    return {
        ...
        'navigation': {'kpi_gap': object_metrics.get('kpi_gap'), 'median_gap': median_gap, 'kpi_zone': kpi_zone},
        'context': {
...
def get_business_comparison(period: str) -> Dict[str, Any]:
    ...
    previous_rows, _ = _run_filter(rows, period=str(int(period[:4]) - 1) + period[4:]) if isinstance(period, str) and len(period) >= 7 and period[:4].isdigit() else ([], {})
    previous_metrics = aggregate_metrics(previous_rows) if previous_rows else {}
    payload['previous_object_metrics'] = previous_metrics
    payload['filter'] = {}
    return payload
...
def _single_object_comparison(
    ...
    **filters: Any,
    ...
    previous_rows, _ = _run_filter(rows, period=str(int(period[:4]) - 1) + period[4:], **merged_filters) if isinstance(period, str) and len(period) >= 7 and period[:4].isdigit() else ([], {})
    previous_metrics = aggregate_metrics(previous_rows) if previous_rows else {}
    payload['previous_object_metrics'] = previous_metrics
