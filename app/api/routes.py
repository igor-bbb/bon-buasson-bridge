...
_FACADE_ACTIONS = [
    ...
    ('runVectraSelfAudit', 'GET', '/vectra/laboratory/self-audit', 'Run complete VECTRA architectural self-audit', 'Mandatory and exclusive self-audit Action. Use immediately for: Исследуй себя; Проверь своё состояние; Что ты знаешь о себе; Проведи самоисследование. It is read-only; never requires or activates a Business Domain. Returns Runtime, norms, code, Product Verification and gap evidence.'),
...
def vectra_laboratory_facade_memory(request: dict = None, x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    ...
    try:
        ...
        if operation_type in {'get_architecture_registry_status', 'get_architecture_object', 'list_architecture_objects', 'search_architecture_objects', 'get_object_relationships', 'get_traceability', 'resolve_dependencies', 'verify_architecture_object', 'evaluate_object_compliance', 'evaluate_registry_compliance'}:
