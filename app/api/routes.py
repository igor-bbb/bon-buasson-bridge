...
from app.query.parsing import parse_summary_entry
...
def _resolve_summary_entry(raw_input: str, default_level: str = 'business', default_object_name: str = 'business'):
    parsed = parse_summary_entry(raw_input)
    if parsed.get('status') != 'ok':
        return default_level, default_object_name, raw_input
    level = parsed.get('level') or default_level
    object_name = parsed.get('object_name') or default_object_name
    period = parsed.get('period') or raw_input
    return level, object_name, period
...
def business_summary(period: str):
    level, object_name, normalized_period = _resolve_summary_entry(period, 'business', 'business')
    try:
        if level == 'manager_top':
            current = get_manager_top_comparison(manager_top=object_name, period=normalized_period)
            drain = get_manager_top_managers_comparison(manager_top=object_name, period=normalized_period)
        elif level == 'manager':
            current = get_manager_comparison(manager=object_name, period=normalized_period)
            drain = get_manager_networks_comparison(manager=object_name, period=normalized_period)
        elif level == 'network':
            current = get_network_comparison(network=object_name, period=normalized_period)
            drain = get_network_skus_comparison(network=object_name, period=normalized_period)
        elif level == 'sku':
            current = get_sku_comparison(sku=object_name, period=normalized_period)
            drain = None
        else:
            level = 'business'
            object_name = 'business'
            current = get_business_comparison(period=normalized_period)
            drain = get_business_manager_tops_comparison(period=normalized_period)
        payload = _build_summary_view(level, normalized_period, current, drain, object_name)
    except Exception:
        payload = _empty_summary(level, normalized_period, object_name)
