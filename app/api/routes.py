...
def _enforce_public_response_budget(payload: dict) -> dict:
    ...
    if has_canonical_workspace:
        ...
        for key in (
            ...
            out.pop(key, None)
        out['screen_order'] = ['workspace_markdown', 'navigation_block']
...
def _make_full_workspace_public_payload(rendered_payload: dict) -> dict:
    ...
    Sprint W14.1 — Full Workspace Rendering Contract.

    `workspace_markdown` is the only user-visible source of truth for full
    Workspace screens. The API must not expose competing legacy blocks that let
    Custom GPT rebuild, shorten, reorder or paraphrase the Workspace.

    `workspace_primary_block` is intentionally not returned in the public
    payload when `workspace_markdown` exists: it is a duplicate transport copy
    and can trigger response-budget fallbacks. State still stores the full
    rendered payload before this public projection.
    """
    if not isinstance(rendered_payload, dict):
        return rendered_payload

    primary = rendered_payload.get('workspace_primary_block')
    ...
    return {
        ...
        'workspace_render_instruction': (
            rendered_payload.get('workspace_render_instruction')
            or 'Показывать workspace_markdown полностью как основной рабочий стол.'
        ) + ' Любое сокращение, перестановка, переформулирование или изменение таблиц считается Engineering Bug.',
