...
def _apply_stage51_render_overrides(payload):
    ...
    if render_mode not in {'list_only', 'reasons', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace'} and level:
        try:
    ...
    try:
        ...
        if isinstance(drain, dict):
            ...
            if render_mode not in {'list_only', 'reasons', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace'}:
...
def _attach_sku_passport_if_missing(payload):
    ...
    try:
        ...
        for key in ('sku_passport', 'sku_passport_block', 'business_context', 'business_context_block', 'category_workspace', 'category_workspace_block', 'business_opportunity', 'business_opportunity_block', 'recommendation_engine', 'recommendation_block', 'narrative_engine', 'narrative_block', 'product_workspace', 'product_workspace_block'):
            if rebuilt.get(key):
...
def vectra_query(request: VectraQueryRequest):
    ...
    if render_only_payload.get('status') != 'error' and render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace'}:
        try:
