...
from app.assistant_runtime.repository import (
    ...
    memory_overview as get_vectra_memory_overview,
    list_evolution_journal as list_vectra_evolution_journal,
...
from app.assistant_runtime.command_guidance import (
    command_guidance_model as get_vectra_command_guidance_model,
    resolve_natural_command as resolve_vectra_natural_command,
    guide_and_read as guide_and_read_vectra_memory,
)
...
# Product Owner should not remember internal commands. VECTRA accepts human
# intent, opens the correct memory area, and proves what is actually stored.

@router.get('/vectra/memory', summary='Show VECTRA Memory Overview')
def vectra_memory_overview():
    return json_response(get_vectra_memory_overview())


@router.get('/vectra/state', summary='Show VECTRA Runtime State')
def vectra_state_readback():
    return json_response(get_vectra_assistant_current_state())


@router.get('/vectra/evolution-journal', summary='Show VECTRA Evolution Journal')
def vectra_evolution_journal_readback(limit: int = 50):
    return json_response(list_vectra_evolution_journal(limit=limit))


@router.get('/vectra/decisions', summary='Show VECTRA Product Decisions')
def vectra_decisions_readback(limit: int = 50):
    return json_response(list_vectra_product_decisions(limit=limit))


@router.get('/vectra/knowledge', summary='Show VECTRA Knowledge Repository')
def vectra_knowledge_readback():
    return json_response(list_vectra_assistant_knowledge_documents())


@router.get('/vectra/recovery', summary='Show VECTRA Recovery Bundle')
def vectra_recovery_readback():
    return json_response(get_vectra_assistant_recovery_bundle())


@router.get('/vectra/snapshots', summary='Show VECTRA Recovery Snapshots')
def vectra_snapshots_readback(limit: int = 20):
    return json_response(list_vectra_recovery_snapshots(limit=limit))


@router.get('/vectra/runtime-reports', summary='Show VECTRA Runtime Reports')
def vectra_runtime_reports_readback(limit: int = 20):
    return json_response(list_vectra_runtime_execution_reports(limit=limit))


@router.get('/vectra/pending-approvals', summary='Show VECTRA Pending Approvals')
def vectra_pending_approvals_readback():
    return json_response(get_vectra_runtime_pending_approvals())


@router.get('/vectra/command-guidance/model', summary='Show Natural Command Guidance Model')
def vectra_command_guidance_model():
    return json_response(get_vectra_command_guidance_model())


@router.post('/vectra/command-guidance/resolve', summary='Resolve Human Request to VECTRA Action')
def vectra_command_guidance_resolve(request: dict):
    if not isinstance(request, dict):
        request = {}
    return json_response(resolve_vectra_natural_command(request))


@router.post('/vectra/read', summary='Read VECTRA Memory by Natural Language')
def vectra_guided_readback(request: dict):
    if not isinstance(request, dict):
        request = {}
    return json_response(guide_and_read_vectra_memory(request))


@router.post('/vectra/verify-write-readback', summary='Verify that VECTRA Can Read Back Stored Memory')
def vectra_verify_write_readback(request: dict):
    if not isinstance(request, dict):
        request = {}
    # This endpoint does not mutate data. It proves that already-written memory
    # can be read back from the same runtime repository.
    overview = get_vectra_memory_overview()
    missing = []
    counts = overview.get('counts') if isinstance(overview.get('counts'), dict) else {}
    for key in ['evolution_journal_entries', 'product_decisions', 'knowledge_documents', 'recovery_snapshots']:
        if counts.get(key, 0) is None:
            missing.append(key)
    return json_response({
        'status': 'ok' if not missing else 'degraded',
        'render_mode': 'vectra_write_readback_verification',
        'verified': not missing,
        'memory_overview': overview,
        'missing_readback_sections': missing,
        'product_owner_report': {
            'what_i_checked': 'Я открыла память VECTRA и проверила, что основные разделы доступны для чтения.',
            'what_i_found': overview.get('human_summary'),
            'what_requires_your_decision': 'Ничего не требуется.' if not missing else 'Нужно доработать чтение некоторых разделов памяти.',
        },
    })
...
def vectra_query(request: VectraQueryRequest):
    ...
    if is_self_evolution_command(request.message):
        result = run_self_evolution_cycle(
    ...
    if rendered_payload.get('render_mode') == 'self_evolution':
        render_only_payload = {
            'status': rendered_payload.get('status', 'ok'),
            'reason': rendered_payload.get('reason'),
            'context': rendered_payload.get('context'),
