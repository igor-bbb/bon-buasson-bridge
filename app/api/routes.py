...
from app.query.orchestration import orchestrate_vectra_query, save_last_payload, update_session

router = APIRouter()
logger = logging.getLogger(__name__)


PUBLIC_TOP_LEVEL_KEYS = ('object_result_money', 'internal_drain_money', 'compare_base', 'context', 'goal', 'metrics', 'structure', 'drain_block', 'all_block', 'navigation', 'reasons_block', 'decision_block', 'decision_block_render', 'reasons_block_render', 'kpi_block', 'structure_block', 'main_driver', 'drain_block_render', 'drain_total', 'navigation_block', 'goal_block', 'summary_block', 'path', 'vector_block')
...
def _normalize_drain(payload):
    ...
    explicit_total = None
    if raw is None:
        # Full-list screens carry objects in items, not drain_block.
    ...
    items = sorted(items, key=lambda x: x['effect_money'])

    if not is_all_mode and len(items) < 3:
        # V11.4: rendered drain is a focus list, but it must still give the user
        # up to 3 choices. If strict drain has fewer than 3 objects, fill from
        # all_block (negative first, then neutral/positive context).
        seen = {str(item.get('object_name')) for item in items}
        all_items = payload.get('all_block') or []
        fillers = []
        for idx, entry in enumerate(all_items, start=1):
            if not isinstance(entry, dict):
                continue
            name = entry.get('object_name') or entry.get('name')
            if not name or str(name) in seen:
                continue
            effect = entry.get('effect_money')
            if effect is None:
                effect = entry.get('potential_money', entry.get('gap_loss_money'))
            fillers.append({
                'object_name': name,
                'object_id': entry.get('object_id', idx),
                'effect_money': _intnum(effect),
            })
        fillers = sorted(fillers, key=lambda x: x['effect_money'])
        items.extend(fillers[:max(0, 3 - len(items))])

    if not is_all_mode:
        items = items[:3]

    if is_all_mode:
        # Full-list screen is a navigation mode. Positive objects must not compensate losses.
        total = sum(item['effect_money'] for item in items if item.get('effect_money', 0) < 0)
    elif explicit_total is not None:
        total = _num(explicit_total)
    else:
        # Total loss must include only negative focus values; positive context
        # objects are shown for choice, not as compensation.
        total = sum(item['effect_money'] for item in items if item.get('effect_money', 0) < 0)
...
ACTION_TEXT_MAP = {
    'raise_margin': 'пересмотреть цены / условия контракта',
    'reduce_personnel': 'пересмотреть нагрузку / штат',
    'reduce_logistics': 'оптимизировать логистику',
    'reduce_retro': 'пересмотреть бонусы / ретро',
    'reduce_other': 'требуется дополнительный разбор',
    'reduce_markup_gap': 'пересмотреть цены / условия контракта',
    'markup': 'пересмотреть цены / условия контракта',
    'margin': 'пересмотреть цены / условия контракта',
    'personnel': 'пересмотреть нагрузку / штат',
    'logistics': 'оптимизировать логистику',
    'retro': 'пересмотреть бонусы / ретро',
    'other': 'требуется дополнительный разбор',
}


DECISION_LEVELS = {'network', 'category', 'tmc_group', 'sku'}
...
def _decision_action_text(action_key, level):
    ...
    if level in {'category', 'tmc_group'}:
        category_map = {
            'raise_margin': 'пересмотреть цены на ключевые SKU внутри категории',
            'reduce_retro': 'проверить бонусы/ретро внутри категории',
            'reduce_logistics': 'проверить логистическую нагрузку по SKU категории',
            'reduce_personnel': 'пересчитать цену / убрать убыточные SKU из промо',
            'reduce_other': 'дополнительный разбор статьи внутри категории',
            'markup': 'пересмотреть цены на ключевые SKU внутри категории',
            'margin': 'пересмотреть цены на ключевые SKU внутри категории',
            'retro': 'проверить бонусы/ретро внутри категории',
            'logistics': 'проверить логистическую нагрузку по SKU категории',
            'personnel': 'пересчитать цену / убрать убыточные SKU из промо',
            'other': 'дополнительный разбор статьи внутри категории',
        }
        return category_map.get(str(action_key), base)
    if level == 'sku':
        sku_map = {
            'raise_margin': 'поднять цену / убрать из убыточного промо',
            'reduce_retro': 'пересмотреть бонус по SKU',
            'reduce_logistics': 'проверить логистическую нагрузку SKU',
            'reduce_personnel': 'поднять цену или убрать SKU из убыточного промо',
            'reduce_other': 'разобрать прочие затраты по SKU',
            'markup': 'поднять цену / убрать из убыточного промо',
            'margin': 'поднять цену / убрать из убыточного промо',
            'retro': 'пересмотреть бонус по SKU',
            'logistics': 'проверить логистическую нагрузку SKU',
            'personnel': 'поднять цену или убрать SKU из убыточного промо',
            'other': 'разобрать прочие затраты по SKU',
        }
...
def public_summary(payload):
    ...
    response = {
        ...
        'vector_block': payload.get('vector_block') or {},
    ...
    # V12.1: make summary explain positive object with internal drain.
    try:
        ctx_level = str((rendered.get('context') or {}).get('level') or '').strip().lower()
        obj_result = rendered.get('object_result_money')
        int_drain = rendered.get('internal_drain_money')
        if ctx_level != 'business' and obj_result is not None and int_drain is not None:
            if _num(obj_result) > 0 and _num(int_drain) < 0:
                rendered['summary_block'] = 'Объект в целом даёт плюс, но внутри есть управляемые потери.'
            elif _num(obj_result) < 0 and _num(int_drain) < 0:
...
def _render_result_block(response):
    """V12.1 UX: render object result and internal drain as separate concepts."""
    ...
    result = response.get('object_result_money')
    internal = response.get('internal_drain_money')
    lines = []
    if result is not None:
        label = 'Общий вклад'
        lines.append(f'{label}: {_fmt_signed_int(result)}')
    if internal is not None and _num(internal) != 0:
        lines.append(f'Внутренний дренаж: {_fmt_signed_int(internal)}')
    return lines


def _render_goal_block(goal, vector_block=None):
    vector = vector_block if isinstance(vector_block, dict) else {}
    path_goal = vector.get('path_goal_money')
    current_focus = vector.get('current_focus_money')
    coverage = vector.get('coverage_percent')
    if path_goal is not None and current_focus is not None:
        lines = [
            f'Цель пути: закрыть {abs(_intnum(path_goal))}',
            f'Фокус экрана: закрыть {abs(_intnum(current_focus))}',
        ]
        if coverage is not None:
            lines.append(f'Покрытие цели: {_intnum(coverage)}%')
        return '\n'.join(lines)
...
def _focus_money_for_decision(response):
    vector = response.get('vector_block') if isinstance(response.get('vector_block'), dict) else {}
    focus_money = vector.get('current_focus_money')
    if focus_money is None:
        drain = response.get('drain_block') if isinstance(response.get('drain_block'), dict) else {}
        focus_money = drain.get('total_effect')
    return abs(_intnum(focus_money))


def _price_percent_text(response):
    focus_money = _focus_money_for_decision(response)
    revenue = _metric_fact_revenue(response)
    if focus_money <= 0 or revenue <= 0:
        return 'проверить цену и промо-условия по объекту'
    percent = min(100.0, (focus_money / revenue) * 100.0)
    # Price action is an orientation, not an automatic command.
    return f'ориентир по цене: компенсировать около +{percent:.1f}% к обороту уровня'
...
def _render_decision_block(response):
    ...
    if level == 'network':
        if object_name:
            lines.append(f'Фокус контракта: {object_name} → закрыть {focus_money}')
        if main_driver:
            lines.append(f'Сигнал: объект хуже внутреннего рынка по статье «{main_driver}»')
        lines.append('Действие: причины — разбор контракта')
        return lines

    if level in {'category', 'tmc_group'}:
        if object_name:
            lines.append(f'Фокус цены: {object_name} → закрыть {focus_money}')
        lines.append(_price_percent_text(response))
        lines.append('Действие: поднять цену / убрать убыточные SKU из промо / проверить SKU-лист')
        return lines
    ...
    if level == 'sku':
        if object_name:
            lines.append(f'Фокус цены: {object_name} → закрыть {focus_money}')
        lines.append(_price_percent_text(response))
        lines.append('Действие: поднять цену / убрать SKU из убыточного промо / проверить цену')
        return lines
...
def _attach_render_blocks(response, payload):
    ...
    vector_block = response.get('vector_block') if isinstance(response.get('vector_block'), dict) else {}

    drain_total = _intnum(response.get('internal_drain_money') if response.get('internal_drain_money') is not None else drain.get('total_effect'))
    ctx_level_for_goal = str((response.get('context') or {}).get('level') or '').strip().lower()
    if ctx_level_for_goal != 'business' and drain_total:
        vector_block = dict(vector_block or {})
        if not vector_block.get('path_goal_money'):
            vector_block['path_goal_money'] = abs(drain_total)
        else:
            vector_block['path_goal_money'] = abs(_intnum(vector_block.get('path_goal_money')))
        vector_block['current_focus_money'] = abs(drain_total)
        try:
            vector_block['coverage_percent'] = min(100, int(round(abs(drain_total) / abs(float(vector_block['path_goal_money'])) * 100)))
        except Exception:
            pass
    ...
    if render_mode == 'list_only':
        ...
        # Strictly align focus with current drain_total when the screen has child drain.
        if vector_block and drain_total:
            vector_block = dict(vector_block)
            vector_block['current_focus_money'] = abs(drain_total)
            path_goal = vector_block.get('path_goal_money')
            try:
                if path_goal:
                    vector_block['path_goal_money'] = abs(_intnum(path_goal))
                    vector_block['coverage_percent'] = min(100, int(round(abs(drain_total) / abs(float(vector_block['path_goal_money'])) * 100)))
            except Exception:
                pass
        ...
        response['main_driver'] = ('Цена / ассортимент' if ctx_level_for_goal in {'category', 'tmc_group', 'sku'} else _render_main_driver(structure))
        response['result_block'] = _render_result_block(response)
        # V12.1: keep goal compact; the main UX block is result_block.
        response['goal_block'] = _render_goal_block(goal, vector_block)
    ...
    if ctx_level == 'sku':
        ...
        if not rendered_sku_drain:
            sku_focus = 0
            try:
                sku_focus = _intnum((vector_block or {}).get('current_focus_money'))
            except Exception:
                sku_focus = 0
            sku_name = str((response.get('context') or {}).get('object_name') or payload.get('object_name') or 'SKU')
            if sku_focus:
                rendered_sku_drain = [f'{sku_name} → {-abs(sku_focus)}']
            response['drain_total'] = -abs(sku_focus) if sku_focus else drain_total
        else:
    ...
    response['vector_block'] = vector_block
...
def _is_render_ready_payload(payload):
    if not isinstance(payload, dict):
        return False
    render_keys = {'kpi_block', 'structure_block', 'navigation_block', 'goal_block', 'drain_block_render'}
...
def _force_product_navigation(payload):
    ...
    # Reasons must always be available on analytical screens.
    if level == 'network':
        add('причины — разбор контракта')
    elif level and level not in {'start'}:
        add('причины — разбор')
...
def _v9_level_next_from_screen(screen):
    if not isinstance(screen, dict):
        return None
    if screen.get('children_level'):
        return screen.get('children_level')
    nav = screen.get('navigation') if isinstance(screen.get('navigation'), dict) else {}
    if nav.get('next_level'):
        return nav.get('next_level')
...
def _v9_effect_money(item):
    if not isinstance(item, dict):
        return 0
    for key in ('effect_money', 'potential_money', 'gap_loss_money', 'selected_focus_money'):
        if item.get(key) is not None:
            return item.get(key)
    return 0


def _v9_all_mode_screen(screen):
    base = json.loads(json.dumps(screen, ensure_ascii=False)) if isinstance(screen, dict) else {}
    block = base.get('all_block') if isinstance(base.get('all_block'), list) else []
    next_level = _v9_level_next_from_screen(base)
    losses = []
    neutral = []
    positive = []
    active = []
    ...
    for item in block:
        ...
        if not name:
            continue
        effect = _v9_effect_money(item)
        try:
            effect_num = int(round(float(effect or 0)))
        except Exception:
            effect_num = 0
        active.append({
            'object_name': name,
            'level': item.get('level') or next_level,
            # Positive object may still contain internal drain. Do not convert
            # positive contribution into path goal; next screen will define focus
            # from its own drain/API цифры.
            'selected_focus_money': effect_num if effect_num < 0 else None,
        })
        entry = (len(active), name, effect_num)
        if effect_num < 0:
            total_losses += effect_num
            losses.append(entry)
        elif effect_num > 0:
    ...
    if losses:
        lines.append('Потери:')
        for idx, name, effect_num in losses:
            lines.append(f'{idx} — {name} → {_fmt_signed_int(effect_num)}')
    if neutral:
        lines.append('Ноль:')
        for idx, name, effect_num in neutral:
            lines.append(f'{idx} — {name} → {_fmt_signed_int(effect_num)}')
    if positive:
        lines.append('Положительный вклад:')
        for idx, name, effect_num in positive:
            lines.append(f'{idx} — {name} → {_fmt_signed_int(effect_num)}')
    ...
    base.update({
        ...
        'drain_total': total_losses,
        'navigation_block': ['причины — разбор', 'назад — вверх'],
    })
    return base, active


def _v9_reasons_mode_screen(screen):
    base = json.loads(json.dumps(screen, ensure_ascii=False)) if isinstance(screen, dict) else {}
    reasons = base.get('reasons_block_render') if isinstance(base.get('reasons_block_render'), list) else []
    if not reasons:
        reasons = _render_reasons_block(base.get('reasons_block') or [])
    if not reasons:
...
def _v10_norm_text(value):
    return ''.join(ch.lower() for ch in str(value or '') if ch.isalnum())


def _v10_screen_object_names(screen):
    names = []
    if not isinstance(screen, dict):
        return names
    ctx = screen.get('context') if isinstance(screen.get('context'), dict) else {}
    if ctx.get('object_name'):
        names.append(str(ctx.get('object_name')))
    for item in screen.get('path') or []:
...
def _v10_return_to_screen(session, message):
    normalized = _v10_norm_text(message)
    for prefix in ('верниськ', 'вернутьсяк', 'перейдик', 'назадк'):
    ...
    if not target:
        return None
    candidates = []
    for item in (session.get('stack') or []):
        if isinstance(item, dict):
            candidates.append(item.get('current_screen') or item.get('last_payload') or item)
    candidates.append(session.get('current_screen') or session.get('last_payload'))
    for screen in reversed([c for c in candidates if isinstance(c, dict)]):
        for name in _v10_screen_object_names(screen):
            if target in _v10_norm_text(name):
                out = json.loads(json.dumps(screen, ensure_ascii=False))
                out['render_mode'] = ''
                return _force_product_navigation(out)
    return None
...
def vectra_query(request: VectraQueryRequest):
    ...
    # V9 state display commands. These commands are UI modes, not API analysis calls.
    from app.query.orchestration import SESSION_STORE

    session = SESSION_STORE.get(session_id, {})
    last = session.get('current_screen') or session.get('last_payload')

    return_to = _v10_return_to_screen(session, message)
    if return_to:
        update_session(session_id, {'current_screen': return_to, 'last_payload': return_to, 'view_mode': 'drain', 'show_all': False})
        return JSONResponse(return_to)

    if message in ["все", "причины"] and last:
        if message == "все":
            view, active = _v9_all_mode_screen(last)
            update_session(session_id, {
                'view_mode': 'all',
                'show_all': True,
                'last_list_items': active,
            })
            return JSONResponse(view)
        if message == "причины":
            view = _v9_reasons_mode_screen(last)
            if view.get('status') != 'error':
                update_session(session_id, {'view_mode': 'reasons', 'show_all': False})
            return JSONResponse(view)

    if message == "назад":
        # If user is inside a display mode, return to numbers for the same object.
        # Only real drilldown should pop stack inside orchestration.
        if session.get('view_mode') in {'all', 'reasons'} and last:
            update_session(session_id, {'view_mode': 'drain', 'show_all': False})
            clean = json.loads(json.dumps(last, ensure_ascii=False))
            clean['render_mode'] = ''
            clean = _force_product_navigation(clean)
    ...
    render_only_payload = {
        ...
        'vector_block': rendered_payload.get('vector_block', {}),
    ...
    # Persist the final rendered screen at the API boundary. UI-only commands
    # (все / причины / назад) enter through the same endpoint and must read
    # the exact previous screen from state, not a partial domain payload.
    # Never overwrite a valid screen with an error response.
    if render_only_payload.get('status') != 'error':
        try:
