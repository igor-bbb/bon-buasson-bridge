...
def _main_factor_sentence(response: dict) -> str:
    ...
    strong = _best_positive_reason(response)
    if risk:
        return f'Главный фактор давления на прибыль: {_reason_display_name(risk).lower()} ({_render_money_value(_reason_effect(risk))}).'
    if strong:
        return f'Главный положительный фактор: {_reason_display_name(strong).lower()} ({_render_money_value(_reason_effect(strong))}).'
    main_driver = str(response.get('main_driver') or '').strip()
    if main_driver:
        return f'Главный фактор диагностики: {main_driver.lower()}.'
    return 'Главный фактор по доступным данным не выделен.'
...
def _turnover_or_margin_sentence(response: dict) -> str:
    ...
    if fin_delta < 0 and rev_delta < 0 and margin_delta >= 0:
        return 'Главная природа проблемы — оборот: доходность улучшилась или удержалась, но не компенсировала потерю продаж.'
    if fin_delta < 0 and margin_delta < 0:
        return 'Главная природа проблемы — доходность: маржа снизилась и давит на финрез.'
    if fin_delta > 0 and rev_delta > 0:
        return 'Рост прибыли поддержан оборотом.'
    if fin_delta > 0 and margin_delta > 0:
        return 'Рост прибыли поддержан доходностью.'
    if rev_delta < 0:
        return 'Оборот ниже прошлого года — нужно проверить, не теряется ли объём продаж.'
    if margin_delta < 0:
        return 'Маржа ниже прошлого года — нужно проверить доходность.'
...
def _benchmark_diagnosis_sentence(response: dict) -> str:
    ...
    lines = response.get('benchmark_diagnostic_block') or []
    if lines:
        return f'Относительно бизнеса: {lines[0]}'
    return 'Относительно бизнеса отдельный критичный разрыв по доступным данным не выделен.'
...
def _build_assistant_diagnosis_block(response: dict) -> list:
    ...
    if layer == 'business':
        ...
        lines.append('Это Product Layer: детальная причина по цене, объёму, ассортименту и SKU Mix будет доступна после подключения VECTRA Data Mart.')
    elif layer == 'sku':
...
def _stage7_screen_order(response: dict) -> list:
    ...
    if str(ctx.get('level') or '').strip().lower() == 'business':
        return ['result_block', 'diagnosis_block', 'recommended_next_step_block', 'factor_change_block', 'opportunity_rating_block', 'decision_block_render', 'navigation_block']
    return ['result_block', 'diagnosis_block', 'recommended_next_step_block', 'factor_change_block', 'benchmark_diagnostic_block', 'opportunity_rating_block', 'decision_block_render', 'navigation_block']
