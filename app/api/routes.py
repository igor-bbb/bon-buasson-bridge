...
def _pr_business_workspace_block(response: dict) -> list:
    ...
    if managers:
        ...
        lines.extend(['','💵 Где находятся деньги',f'Подтверждённый потенциал по текущей модели: {_fmt_int(total_potential)} грн' if total_potential else 'Потенциал определяется по объектам ниже.', 'Объект | Сигнал | Δ прибыли | Доля оборота | Доля прибыли | Потенциал | Контрактов | SKU'])
        ...
        for item in managers[:6]:
            ...
            lines.append(f"{name} | {sig} | {_fmt_signed_int(item['delta_profit'])} | {_fmt_percent_value(item['share_revenue'])} | {_fmt_percent_value(item['share_profit'])} | {_fmt_int(opp_map.get(name, 0))} | {item['network_count']} | {item['sku_count']}")
    lines.extend(['','🚨 Management Radar','Зона | Объект | Почему важно | Что делать'])
    if managers:
        ...
        reserve=max(managers, key=lambda x: _num(opp_map.get(x['name'],0))) if managers else None
        best=max(managers, key=lambda x: x['delta_profit'])
        if risk: lines.append(f'🔴 Главный риск | {risk["name"]} | Δ прибыли {_fmt_signed_int(risk["delta_profit"])}; доля прибыли {_fmt_percent_value(risk["share_profit"])} | открыть Workspace и найти источник просадки')
        if reserve: lines.append(f'🟠 Главный резерв | {reserve["name"]} | потенциал {_fmt_int(opp_map.get(reserve["name"],0))}; доля оборота {_fmt_percent_value(reserve["share_revenue"])} | разобрать портфель ниже')
        if best: lines.append(f'🟢 Лучшая практика | {best["name"]} | прирост прибыли {_fmt_signed_int(best["delta_profit"])} | использовать как пример решений')
...
def _w4_set_primary_workspace(payload: dict, block_key: str) -> dict:
    """Sprint W4 Information Recovery: make the recovered Workspace block explicit.

    Custom GPT can otherwise summarize older short legacy blocks. This helper
    exposes a single primary Workspace artifact with full evidence tables.
    """
...
def _trim_default_public_payload(payload: dict) -> dict:
    ...
    if isinstance(primary, list) and primary:
        ...
        # information-dense Workspace in Custom GPT rendering.
        for key in (
