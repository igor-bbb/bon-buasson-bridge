...
from app.presentation.views import build_screen_view

router = APIRouter()


PUBLIC_SUMMARY_KEYS = ('context', 'metrics', 'structure', 'drain_block', 'goal', 'all_block', 'cause_block', 'navigation')

def public_summary(payload):
    if not isinstance(payload, dict):
        return payload
    allowed = set(PUBLIC_SUMMARY_KEYS) | {'focus_block', 'decision_block', 'provocation_block'}
    if any(key in payload for key in allowed):
        compact = {key: payload.get(key) for key in payload.keys() if key in allowed or key in {'context', 'metrics', 'structure', 'drain_block', 'goal', 'navigation', 'all_block', 'cause_block'}}
        return build_screen_view(compact)
    return payload
