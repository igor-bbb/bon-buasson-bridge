...
def _stage7_screen_order(response: dict) -> list:
    ...
    if level == 'network' and response.get('decision_workspace'):
        return ['result_block', 'diagnosis_block', 'decision_workspace_block', 'factor_change_block', 'benchmark_diagnostic_block', 'opportunity_explanation_block', 'navigation_block']
...
def _render_decision_workspace_block(response):
    ...
    if main_question:
        lines.append(str(main_question))
    ...
    if actions:
        ...
        for idx, action in enumerate(actions[:3], start=1):
            text = action.get('action') or 'Проверить фактор'
            effect = action.get('expected_effect_money')
            lines.append(f'{idx}. {text} → ожидаемый эффект {_render_money_value(effect)}')
    ...
    if items:
        names = [str(item.get('category')) for item in items if isinstance(item, dict) and item.get('category')]
        if names:
            lines.append('Доказательства: ' + ', '.join(names[:3]))
    return lines
