...
def _decision_action_text(action_key, level):
    ...
    if level in {'category', 'tmc_group'}:
        category_map = {
            ...
            'reduce_personnel': 'перераспределить нагрузку по SKU категории',
            ...
            'personnel': 'перераспределить нагрузку по SKU категории',
    ...
    if level == 'sku':
        sku_map = {
            ...
            'reduce_personnel': 'снизить трудозатраты на SKU',
            ...
            'personnel': 'снизить трудозатраты на SKU',
...
def _render_navigation_block(payload, navigation, drain):
    ...
    if 'reasons' in action_types:
        lines.append('причины — разбор контракта' if level == 'network' else 'причины — разбор')
...
def _render_decision_block(response):
    ...
    first = decision[0] if isinstance(decision[0], dict) else {}
    action_hint = str(first.get('text') or '').strip()
    effect_money = first.get('effect_money')

    lines = []
    if object_name:
        focus_value = focus_money if focus_money is not None else effect_money
        lines.append(f'Фокус решения: {object_name} → закрыть {abs(_intnum(focus_value))}')
    if main_driver:
        lines.append(f'Основная проблема: {main_driver}')
    if action_hint:
        lines.append(f'Рекомендованное действие: {action_hint}')
    return lines
