...
def _normalize_metrics(payload):
    ...
    if isinstance(raw, list):
        for entry in raw:
            ...
            if normalized_name == 'Финрез итог' and level != 'business':
                continue
            is_percent = ('fact_percent' in entry or 'pg_percent' in entry or normalized_name in {'Маржа'})
            base = {'name': normalized_name, 'is_primary': normalized_name == 'Финрез до'}
            if is_percent:
                base.update({
    ...
    for key, title, kind in mapping:
        ...
        if not isinstance(entry, dict):
            continue
        base = {'name': title, 'is_primary': title == 'Финрез до'}
        if kind == 'percent':
            base.update({
...
def _normalize_drain(payload):
    ...
    for idx, entry in enumerate(source_items, start=1):
        if not isinstance(entry, dict):
            continue
        eff = _intnum(entry.get('effect_money'))
        items.append({
...
def _build_vectra_query_render_only_payload(payload):
    payload = _ensure_vectra_query_render_contract(payload)
    ...
    return {
        'status': payload.get('status', 'ok') if isinstance(payload, dict) else 'error',
        'reason': payload.get('reason'),
        'context': context,
        'goal_block': payload.get('goal_block', ''),
        'kpi_block': payload.get('kpi_block', []),
        'structure_block': payload.get('structure_block', []),
        'main_driver': payload.get('main_driver', ''),
        'drain_block_render': payload.get('drain_block_render', []),
        'drain_total': _intnum(payload.get('drain_total')),
        'navigation_block': payload.get('navigation_block', []),
    }
...
def vectra_query(request: VectraQueryRequest):
    ...
    public_payload = public_summary(payload)
    render_payload = _build_vectra_query_render_only_payload(public_payload)
    logger.info(
        ...
        session_id,
        'kpi_block' in render_payload,
        'structure_block' in render_payload,
        'drain_block_render' in render_payload,
        'navigation_block' in render_payload,
        'goal_block' in render_payload,
    )
    _log_vectra_query_payload(session_id, render_payload)
    return json_response(render_payload)
