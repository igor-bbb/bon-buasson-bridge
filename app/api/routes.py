...
def _normalize_drain(payload):
    ...
    for idx, entry in enumerate(source_items, start=1):
        ...
        nav = _intnum(navigation_money)
        items.append({
            'object_name': entry.get('object_name'),
            'object_id': entry.get('object_id', idx),
            'effect_money': -abs(nav),
            'navigation_money': nav,
        })

    items = sorted(items, key=lambda x: (-_num(x.get('navigation_money')), str(x.get('object_name') or '')))
    ...
    if not is_all_mode and len(items) < 3:
        ...
        for idx, entry in enumerate(all_items, start=1):
            ...
            if navigation_money is None:
                ...
                effect_num = _intnum(effect)
                navigation_money = abs(effect_num) if effect_num < 0 else 0
            nav = _intnum(navigation_money)
            fillers.append({
                'object_name': name,
                'object_id': entry.get('object_id', idx),
        ...
        fillers = sorted(fillers, key=lambda x: (-_num(x.get('navigation_money')), str(x.get('object_name') or '')))
    ...
    if is_all_mode:
        # Full-list screen is a navigation mode.
...
def _render_priority_action(response):
    ...
    if not isinstance(action, dict) or not action:
        return []
    text = str(action.get('text') or action.get('action') or action.get('metric') or 'Приоритетное действие').strip()
...
def _render_decision_block(response):
    ...
    if isinstance(decision, list) and decision:
        ...
        for item in decision:
            if not isinstance(item, dict):
                continue
            text = str(item.get('text') or item.get('action') or 'Действие').strip()
...
def _attach_render_blocks(response, payload):
    ...
    if render_mode == 'list_only':
        ...
        response['main_driver'] = ('Цена / ассортимент' if ctx_level_for_main_driver in {'category', 'tmc_group', 'sku'} else _render_main_driver(structure))
