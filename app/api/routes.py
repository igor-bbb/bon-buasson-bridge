...
def _empty_summary_contract(*, level: str, period: str, object_name: str, next_level: str | None = None):
    navigation = {
        'current_level': level,
    ...
    return {
        'context': {
            'path': [{'level': level, 'object_name': object_name}],
            'period': period,
            'level': level,
...
def _build_summary_view(
    ...
    *,
    ...
    if not isinstance(current, dict) or 'error' in current:
        return _empty_summary_contract(level=level, period=period, object_name=object_name, next_level=next_level)
...
@router.get(
    ...
    description='Preferred CEO business screen.',
)
def business_summary(period: str):
    ...
    return json_response(_build_summary_view(current, drain, level='business', period=period, object_name='business', next_level='manager_top'))


@router.get(
    '/manager_top_summary',
    summary='Manager Top Summary Screen',
    description='Preferred top-manager screen.',
)
def manager_top_summary(manager_top: str, period: str):
    ...
    return json_response(_build_summary_view(current, drain, level='manager_top', period=period, object_name=manager_top, next_level='manager'))


@router.get(
    '/manager_summary',
    summary='Manager Summary Screen',
    description='Preferred manager screen.',
)
def manager_summary(manager: str, period: str):
    ...
    return json_response(_build_summary_view(current, drain, level='manager', period=period, object_name=manager, next_level='network'))


@router.get(
    '/network_summary',
    summary='Network Summary Screen',
    description='Preferred network screen.',
)
def network_summary(network: str, period: str):
    ...
    return json_response(_build_summary_view(current, drain, level='network', period=period, object_name=network, next_level='sku'))


@router.get(
    '/sku_summary',
    summary='SKU Summary Screen',
    description='Preferred SKU final screen.',
)
def sku_summary(sku: str, period: str):
    current = get_sku_comparison(sku=sku, period=period)
    return json_response(_build_summary_view(current, None, level='sku', period=period, object_name=sku, next_level=None))
...
@router.post(
    ...
    description='Preferred single entrypoint for ChatGPT.',
)
