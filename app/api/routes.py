...
PUBLIC_TOP_LEVEL_KEYS = ('context', 'goal', 'metrics', 'structure', 'drain_block', 'navigation', 'reasons_block', 'decision_block', 'kpi_block', 'structure_block', 'main_driver', 'drain_block_render', 'drain_total', 'navigation_block', 'goal_block', 'summary_block', 'path', 'vector_block')
...
def _normalize_drain(payload):
    ...
    mode = payload.get("view_mode") or (nav.get("mode") if isinstance(nav, dict) else None)
    full_mode = mode == "all"
    items = []
    total = 0.0
...
def _normalize_navigation(payload, drain):
    ...
    def _has_action(action_type: str) -> bool:
        return any(a.get('type') == action_type for a in actions if isinstance(a, dict))

    def _append_service_actions():
        if not isinstance(raw, dict):
            return
        # Support both new and legacy flags:
        # new: has_all / has_causes / has_back
        # legacy presentation: all / reasons / back
        if (raw.get('all') or raw.get('has_all') or payload.get('all_block')) and not _has_action('all'):
            actions.append({'type': 'all'})
        if (raw.get('reasons') or raw.get('has_causes') or raw.get('has_reasons') or payload.get('reasons_block')) and not _has_action('reasons'):
            actions.append({'type': 'reasons'})
        if (raw.get('back') or raw.get('has_back')) and not _has_action('back'):
...
def _render_navigation_block(payload, navigation, drain):
    lines = []
    raw_nav = payload.get("navigation") or {}
    mode = payload.get("view_mode") or (raw_nav.get("mode") if isinstance(raw_nav, dict) else None)
    full_mode = mode == "all"
    names = _extract_navigation_names(payload, drain)
    max_items = len(names) if full_mode else 3
    ...
    if "all" in action_types and not full_mode:
