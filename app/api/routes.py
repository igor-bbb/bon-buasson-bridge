...
def _build_recommended_next_step_block(response: dict) -> list:
    ...
    if level == 'network' and response.get('decision_workspace'):
        return ['➡ Рекомендуемый следующий шаг: выбрать приоритетное действие Decision Engine или открыть доказательства по категории/SKU.']
...
def _render_decision_workspace_block(response):
    """Render Network as a full Contract Workspace, not as a forced next step.

    The block is deliberately advisory: it lays the tools on the desk and
    highlights possible work directions, while the KAM keeps control of the
    decision. Detailed KPI/factor/benchmark blocks are rendered separately in
    the screen order before this block.
    """
    ...
    lines = [
        ...
        'Это полный разбор бизнеса с контрагентом. VECTRA не ограничивает пользователя одним маршрутом: контракт можно разбирать через экономику, категории, ассортимент, переговоры или задачи.',
    ]

    # Work directions, not forced navigation.
    lines.append('🧩 Доступные направления работы')
    lines.append('1. Экономика контракта — проверить маржу, наценку, ретро, логистику и прочие факторы.')
    lines.append('2. Категории — понять, какие категории формируют результат и где есть резерв развития.')
    lines.append('3. Ассортимент / SKU — найти отсутствующие или недопредставленные позиции.')
    lines.append('4. Переговоры — подготовить позицию по контракту или ассортименту.')
    lines.append('5. Задачи — зафиксировать действия после принятого решения.')

    if categories:
        lines.append('📂 Категории в контракте')
        for idx, item in enumerate(categories[:7], start=1):
            name = item.get('category') or item.get('object_name') or 'Категория'
            delta = item.get('profit_delta_money')
            opportunity = item.get('opportunity_money')
            nav = item.get('navigation_money')
            parts = [f'{idx}. {name}']
            if delta is not None:
                parts.append(f'Δ прибыли {_fmt_signed_int(delta)} грн')
            if opportunity is not None and _num(opportunity) > 0:
                parts.append(f'потенциал {_fmt_int(opportunity)} грн')
            elif nav is not None:
                parts.append(f'навигационный вес {_fmt_signed_int(nav)} грн')
            lines.append(' | '.join(parts))

    if sku_leaders:
        lines.append('⭐ SKU-лидеры внутри контракта')
        for idx, item in enumerate(sku_leaders[:5], start=1):
            sku = item.get('sku') or 'SKU'
            revenue = item.get('revenue')
            share_network = item.get('share_network_percent')
            share_business = item.get('share_business_percent')
            role = item.get('role') or 'роль не определена'
            lines.append(f'{idx}. {sku} | оборот {_fmt_int(revenue)} грн | доля в контракте {_fmt_percent(share_network)}% | доля в бизнесе {_fmt_percent(share_business)}% | {role}')

    if missing_sku:
        lines.append('➕ SKU-лидеры бизнеса, отсутствующие в контракте')
        for idx, item in enumerate(missing_sku[:5], start=1):
            sku = item.get('sku') or 'SKU'
            revenue = item.get('business_revenue')
            finrez = item.get('business_finrez_pre')
            lines.append(f'{idx}. {sku} | оборот бизнеса {_fmt_int(revenue)} грн | финрез до {_fmt_signed_int(finrez)} грн | статус: отсутствует')

    if sku_leaders or missing_sku:
        lines.append('📦 Ассортимент')
        lines.append(f'В контракте найдено SKU: {_fmt_int(assortment.get("contract_sku_count"))}. Отсутствующих SKU-лидеров бизнеса: {_fmt_int(assortment.get("missing_business_leader_count"))}.')
    ...
    if negotiation:
        ...
        if sku_package:
            lines.append('Пакет SKU для обсуждения: ' + ', '.join(str(x) for x in sku_package[:5]) + '.')
    ...
    # Advisory priority, not a single mandatory next button.
    lines.append('🎯 Приоритетное направление ассистента')
    if categories:
        best = categories[0]
        best_name = best.get('category') or best.get('object_name') or 'категория'
        lines.append(f'Если цель — развитие продаж и прибыли по контракту, я бы первым разобрал направление «{best_name}».')
    elif actions:
        first = actions[0]
        action_text = first.get('action') or 'проверить фактор'
        effect = first.get('expected_effect_money')
        effect_text = f' с ориентиром до {_fmt_int(effect)} грн' if effect is not None else ''
        lines.append(f'Если цель — улучшить экономику контракта, я бы начал с действия: {action_text}{effect_text}.')
    else:
        lines.append('Приоритет не навязывается: выберите направление работы по цели встречи или задайте ассистенту свободный вопрос.')

    lines.append('💬 Свободный диалог')
    lines.append('На этом рабочем столе можно спросить: «что бы ты сделал дальше?», «как подготовить переговоры?», «какую категорию разобрать?», «какие SKU стоит предложить?»')
