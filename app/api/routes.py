...
def _reason_previous_percent(item):
    ...
    return _fmt_percent(item.get('previous_percent', item.get('prev_percent')))
...
def _render_factor_change_block(reasons):
    ...
    for item in sorted_reasons:
        ...
        lines.append(
            ...
            f'эффект: {_fmt_signed_int(item.get("effect_money"))}\n'
            f'сигнал: {signal}'
        )
    return lines
