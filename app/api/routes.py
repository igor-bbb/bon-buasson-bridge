...
def _benchmark_diagnosis_sentence(response: dict) -> str:
    ...
    if risk:
        ...
        return f'Относительно бизнеса слабое место: {_reason_display_name(risk).lower()} ({_fmt_pp_delta(delta)}, эффект {_render_money_value(_reason_effect_vs_business(risk))}). Это benchmark-диагностика, а не причина изменения к прошлому году.'
