...
from app.workspace_runtime import apply_runtime_contract, refresh_active_workspace_state, runtime_action_map_consistent
...
def _w15_insert_history_block(payload: dict, block: list) -> list:
    """W15.0 Historical Context: one 6-month horizon and one short conclusion.

    The helper is applied to every primary Workspace block. Existing blocks that
    already contain the dynamics section are left unchanged.
    """
    if not isinstance(block, list) or not block:
        return block or []
    if any('Динамика 6 месяцев' in str(x) for x in block):
        return block
    ctx = payload.get('context') if isinstance(payload.get('context'), dict) else {}
    level = str(ctx.get('level') or '').strip().lower()
    period = str(ctx.get('period') or '').strip()
    if not level or not period or level in {'start'}:
        return block
    filters = _pr_context_filters(payload)
    months = _pr_months_back(period, 6)
    if not any(_pr_rows(p, **filters) for p in months):
        return block
    history = _pr_trend_lines(period, filters, 6)
    comment = _w5_trend_comment(period, filters, ctx.get('object_name') or 'объекта')
    history_block = [''] + history
    if comment:
        history_block.append(comment)
    # Insert before next-actions section; otherwise append near the end.
    idx = None
    for i, line in enumerate(block):
        if 'Что делаем дальше' in str(line) or '➡️' in str(line):
            idx = i
            break
    if idx is None:
        return block + history_block
    return block[:idx] + history_block + [''] + block[idx:]
...
def _w4_set_primary_workspace(payload: dict, block_key: str) -> dict:
    ...
    if not block:
        return payload
    block = _w15_insert_history_block(payload, block)
    payload[block_key] = block
...
def _hydrate_session_from_explicit_runtime_context(session_id: str, request: VectraQueryRequest) -> None:
    """W15.0 Context Engine bridge for Custom GPT Actions.

    Custom GPT Actions do not provide hidden chat history automatically. When
    GPT explicitly sends active_workspace_state / runtime_context, use it as the
    primary context source and hydrate server-side State before routing. This
    lets follow-up free-text, local and numeric commands remain scoped to the
    last displayed Workspace even if the client session_id changes.
    """
    try:
        ...
        if not isinstance(active_state, dict):
            if isinstance(runtime_context, dict) and isinstance(runtime_context.get('active_workspace_state'), dict):
                active_state = runtime_context.get('active_workspace_state')
            else:
                active_state = {}
        if not active_state:
            return
        level = active_state.get('workspace_level') or active_state.get('level')
        object_name = active_state.get('object_name') or active_state.get('object')
        period = active_state.get('period') or active_state.get('period_current')
        action_map = active_state.get('action_map')
        ...
        current_screen = {
            ...
            'path': active_state.get('path') if isinstance(active_state.get('path'), list) else [],
            'active_workspace_state': active_state,
            'workspace_action_map': action_map if isinstance(action_map, list) else [],
            'render_mode': active_state.get('render_mode') or '',
        }
        update_session(session_id, {
            ...
            'last_payload': current_screen,
            'view_mode': 'drain',
            'show_all': False,
        })
    except Exception:
        logger.exception('w15_runtime_context_hydration_failed session_id=%s', session_id)
...
def _apply_stage51_render_overrides(payload):
    ...
    if render_mode not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export', 'architecture_complete_gate', 'product_review', 'sprint_candidate', 'decision_capture', 'task_capture', 'feedback_capture', 'corporate_memory', 'closed_loop_status', 'product_intelligence'} and level:
        try:
    ...
    try:
        ...
        if isinstance(drain, dict):
            ...
            if render_mode not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export', 'architecture_complete_gate', 'product_review', 'sprint_candidate', 'decision_capture', 'task_capture', 'feedback_capture', 'corporate_memory', 'closed_loop_status', 'product_intelligence'}:
...
def vectra_query(request: VectraQueryRequest):
    ...
    _hydrate_session_from_explicit_runtime_context(session_id, request)
    ...
    render_only_payload = refresh_active_workspace_state(render_only_payload)
    if render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export', 'architecture_complete_gate', 'product_review', 'sprint_candidate', 'decision_capture', 'task_capture', 'feedback_capture', 'corporate_memory', 'closed_loop_status', 'product_intelligence'}:
        ...
        if render_only_payload.get('status') == 'error' and render_only_payload.get('reason') == 'workspace_generation_error':
            ...
            render_only_payload = refresh_active_workspace_state(render_only_payload)
            if not render_only_payload.get('workspace_action_map'):
            ...
            elif not runtime_action_map_consistent(render_only_payload):
                _record_runtime_rendering_issue(session_id, render_only_payload, 'navigation_contract_violation', 'workspace_action_map does not match final workspace_markdown visible menu.', 'workspace_action_map_mismatch')
    ...
    should_save_rendered_state = (
        ...
        and render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export', 'architecture_complete_gate', 'product_review', 'sprint_candidate', 'decision_capture', 'task_capture', 'feedback_capture', 'corporate_memory', 'closed_loop_status', 'product_intelligence'}
    )
