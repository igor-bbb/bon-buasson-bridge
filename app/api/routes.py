...
def _render_reasons_block(reasons):
    ...
    for item in reasons or []:
        ...
        lines.append(
            f'{name}: факт {money} грн / {percent}%; бизнес {base_percent}%; '
            f'прошлый год {prev_money} грн / {prev_percent}%; '
            f'отклонение от бизнеса {delta_b_text} п.п.; '
            f'отклонение к прошлому году {delta_p_text} п.п.; '
            f'эффект {effect}; сигнал: {signal}'
        )
    return lines
...
def _render_navigation_block(payload, navigation, drain):
    ...
    if level in {'network', 'category', 'tmc_group'} and names:
        lines.append('искать — идти вглубь по списку 1/2/3')
...
def _render_decision_block(response):
    ...
    if level == 'network':
        ...
        lines.append('Действие: причины — разбор контракта; искать — работа с ценой и продуктом')
        return lines
    ...
    if level == 'sku':
        # SKU is a verification/detail level, not the management decision point.
        return []
...
def _force_product_navigation(payload):
    ...
    # Search is not a separate calculation; it is the guided drilldown by numbers.
    if level in {'network', 'category', 'tmc_group'} and has_numeric_items:
        add('искать — идти вглубь по списку 1/2/3')
...
def vectra_query(request: VectraQueryRequest):
    ...
    last = session.get('last_payload') or session.get('current_screen')

    if message in ["все","причины","назад"] and last:
        if message == "все":
            last["render_mode"] = "all"
        elif message == "причины":
            last["render_mode"] = "reasons"
        elif message == "назад":
            last["render_mode"] = "numbers"
