...
PUBLIC_SUMMARY_KEYS = ('context', 'metrics', 'structure', 'drain_block', 'goal', 'focus_block', 'decision_block', 'navigation')
...
def public_summary(payload):
    ...
    if required.issubset(set(payload.keys())):
        return {key: payload.get(key) for key in PUBLIC_SUMMARY_KEYS if key in payload}
    return payload
