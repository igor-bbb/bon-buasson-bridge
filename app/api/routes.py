...
def _build_next_step_block(response: dict) -> list:
    ...
    if _is_product_layer_level(level):
        return [
            'Что делаем дальше: подготовить пакет развития категории, разобрать форматы/SKU или собрать переговорный аргумент.',
            'Можно задать вопрос ассистенту свободно: «какие SKU предложить первыми?» или «где быстрый эффект по категории?»',
        ]
...
def _assortment_skew_lines(assortment: dict, categories: list) -> list:
    ...
    if sku_leaders:
        ...
        if top_share >= 65:
            lines.append(f'Высокая концентрация: ТОП-5 SKU дают около {_fmt_percent(top_share)}% оборота контракта.')
        elif top_share > 0:
            lines.append(f'ТОП-5 SKU дают около {_fmt_percent(top_share)}% оборота контракта — это основа текущей матрицы.')
    missing_count = _intnum(assortment.get('missing_business_leader_count'))
    if missing_count > 0:
        lines.append(f'Есть ассортиментное окно: отсутствует {missing_count} SKU-лидеров бизнеса.')
    if categories:
...
def _render_decision_workspace_block(response):
    ...
    if revenue_current is not None:
        lines.append(f'Масштаб контракта по текущему обороту: {_fmt_int(revenue_current)} грн. Это база, от которой считаем доли категорий и SKU.')
    ...
    if sku_leaders:
        lines.append('')
        lines.append('⭐ ТОП SKU контракта')
        lines.append('SKU | Оборот | Доля контракта | Доля бизнеса | Роль')
        for item in sku_leaders[:10]:
            ...
            lines.append(
                ...
                f'{_fmt_percent(item.get("share_business_percent"))}% | '
                f'{item.get("role") or "роль не определена"}'
            )

    if missing_sku:
        lines.append('')
        lines.append('➕ ТОП-10 SKU-лидеров бизнеса, которых нет в контракте')
        lines.append('SKU | Оборот бизнеса | Финрез до бизнеса | Почему важно')
    ...
    if missing_sku:
        lines.append('2. Ассортимент: собрать короткий пакет из ТОП-10 отсутствующих SKU-лидеров бизнеса, а не открывать весь длинный список.')
    if categories:
        lines.append(f'3. Категории: начать с «{categories[0].get("category") or "ключевой категории"}» и проверить, какие форматы/SKU дают следующий прирост.')
    ...
    if negotiation:
        ...
        if sku_package:
            lines.append('SKU-пакет для первой встречи: ' + ', '.join(str(x) for x in sku_package[:10]) + '.')
    ...
    if categories:
        lines.append(f'2. Разобрать категорию «{categories[0].get("category") or "ключевую категорию"}» — посмотреть форматы, SKU и потенциал.')
    else:
        lines.append('2. Разобрать категории контракта.')
    if missing_sku:
        lines.append('3. Собрать пакет SKU для ввода — начать с ТОП-10 отсутствующих лидеров бизнеса.')
    else:
        lines.append('3. Посмотреть ассортиментные возможности.')
    lines.append('4. Создать задачи по контракту после выбора направления.')
    lines.append('5. Или задай вопрос ассистенту своими словами: «какие SKU предложить первыми?», «как говорить с байером?», «где быстрый эффект?»')
