...
def _build_product_layer_block(response):
    """V1.3 product economics presentation for levels below Network.
    ...
    if margin:
        ...
        if base_margin is not None:
            benchmark_text = f'Эффективность SKU относительно бизнеса: маржа базы {_fmt_percent_value(base_margin)}'
        elif compare_base and compare_base != 'product_baseline_missing':
            benchmark_text = 'Эффективность SKU относительно бизнеса: активна'
    elif compare_base and compare_base != 'product_baseline_missing':
        benchmark_text = 'Эффективность SKU относительно бизнеса: активна'

    # DATA contract currently has turnover, but not physical volume/price.
    # Revenue is kept in KPI; Product economics view does not invent price or volume.
    if not revenue:
        volume_text = 'данных нет'
...
def _build_next_step_block(response: dict) -> list:
    ...
    if _is_product_layer_level(level):
        return ['Рекомендуемый следующий шаг: открыть SKU ниже и проверить эффективность SKU.']
