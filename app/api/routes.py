...
def _navigation_money_text(item):
    ...
    if item.get('profit_delta_money') is not None:
        return _fmt_signed_int(item.get('profit_delta_money'))
...
def _benchmark_sentence(response: dict) -> str:
    obj_result = _num(response.get('object_result_money'))
    if obj_result < 0:
        return f'Относительно среднего уровня бизнеса объект ниже на {_fmt_int(abs(obj_result))} результата.'
    if obj_result > 0:
        return f'Относительно среднего уровня бизнеса объект добавляет {_fmt_int(obj_result)} результата.'
    return 'Относительно среднего уровня бизнеса объект находится около нуля.'
...
def _build_benchmark_driven_summary(response: dict) -> str:
    ...
    if layer == 'business':
        ...
        if result_names:
            parts.append(f'Лучший вклад относительно бизнеса: {result_names}.')
        if potential_names:
    ...
    if layer in {'object', 'contract'}:
        fact = _profit_first_fact_sentence(response)
        benchmark = _benchmark_sentence(response)
        ...
        opportunity_line = f'Управляемый резерв внутри объекта: {_fmt_int(opportunity)} грн.' if opportunity > 0 else 'Существенный управляемый резерв внутри объекта не выявлен.'
        return ' '.join([x for x in [fact, factor_line, benchmark, strong_line, opportunity_line] if x])
...
def _build_explanation_block(response: dict) -> list:
    ...
    if layer == 'business':
        ...
        return [
            ...
            f'Лучший вклад относительно бизнеса: {result_names or "данных нет"}.',
            f'Главные резервы возврата: {potential_names or "данных нет"}.',
            'Benchmark и Opportunity используются как диагностические инструменты после факта прибыли.',
        ]
...
def _render_business_result_block(response):
    return [
        f'Что произошло с прибылью: {_profit_first_fact_sentence(response)}',
        f'Результат бизнеса к прошлому году: {_render_money_value(response.get("business_result_money"))} грн',
    ]


def _render_object_result_block(response):
    return [
        f'Что произошло: {_profit_first_fact_sentence(response)}',
        f'Вклад относительно бизнеса: {_render_money_value(response.get("object_result_money"))} грн',
    ]


def _render_opportunity_block(response):
    value = response.get("opportunity_money")
    return [f'Потенциал прибыли внутри объекта: {_fmt_int(abs(_num(value)))} грн']


def _render_result_block(response):
    """CHANGE-005: render profit first, then benchmark result, then opportunity."""
    ctx = response.get('context') if isinstance(response.get('context'), dict) else {}
