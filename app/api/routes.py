...
def _is_product_team_research_request(message: str) -> bool:
    ...
    has_object = (
        any(token in raw_text for token in ('vectra', 'product team assistant', 'workspace', 'release'))
        or any(token in text for token in ('вектра', 'продукт теам ассистант', 'ассистент', 'воркспаце', 'релиз', 'продукт'))
    )
...
def _research_object_name(message: str) -> str:
    ...
    if 'workspace' in raw_text or 'воркспаце' in text:
        return 'Workspace'
    if 'релиз' in text or 'release' in raw_text:
        return 'Release'
    return 'VECTRA'
