...
def _laboratory_knowledge_openapi_schema() -> dict:
    return _laboratory_split_openapi_schema(
        ...
        version='FOUNDATION-0013-KNOWLEDGE-SELF-EVOLUTION',
...
def _facade_operation_request_schema() -> dict:
    return {
        ...
        'properties': {
            ...
            'payload': {'type': 'object', 'description': 'Operation-specific payload. For operation_type=capitalize_confirmed_knowledge, may include prepared_knowledge_package prepared by VECTRA; Runtime will then skip raw-context reanalysis and execute storage, diff, write, readback, snapshot and report.', 'additionalProperties': True},
...
def _laboratory_facade_openapi_schema() -> dict:
    ...
    schema = {
        ...
        'info': {
            ...
            'version': 'LABORATORY-KNOWLEDGE-0006-PREPARED-KNOWLEDGE-PACKAGE',
            'description': 'Official compact OpenAPI schema for VECTRA Laboratory GPT Actions. Product Owner imports this single URL. The schema exposes professional facade Actions only; Runtime performs detailed routing to internal services. LABORATORY-KNOWLEDGE-0006 adds prepared_knowledge_package support for autonomous capitalization: VECTRA performs intellectual analysis, Runtime performs persistence and verification.',
        },
        'servers': [{'url': server_url}],
        'components': {
            'schemas': {},
        ...
        'x-vectra-release': 'LABORATORY-KNOWLEDGE-0006',
...
def _build_laboratory_facade_action_manifest() -> dict:
    ...
    return {
        ...
        'render_mode': 'vectra_laboratory_action_manifest',
        'release_version': 'LABORATORY-KNOWLEDGE-0005',
        'schema_url': '/vectra/laboratory/openapi.json',
...
def vectra_laboratory_facade_knowledge(request: dict = None, x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    ...
    try:
        ...
        if operation_type == 'create_report':
            return json_response(_facade_response(operation_type, 'knowledge_capitalization.list_reports', '/vectra/knowledge/capitalization/reports', list_vectra_knowledge_capitalization_reports(limit=int(payload.get('limit') or 20), include_failed=bool(payload.get('include_failed', True)))))
        if operation_type == 'capitalize_confirmed_knowledge':
            result = auto_capitalize_vectra_confirmed_knowledge(payload)
            next_action = 'Capitalization completed. Read back capitalized knowledge through Professional or Business Knowledge API.'
            ...
            return json_response(_facade_response(operation_type, 'knowledge_capitalization.AUTO_CAPITALIZATION_PIPELINE', '/vectra/laboratory/facade/knowledge', result, next_action=next_action))
