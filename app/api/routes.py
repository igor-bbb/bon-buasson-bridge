...
def json_response(payload):
    if not isinstance(payload, dict):
        payload = {'context': {}, 'metrics': {}, 'structure': {}, 'drain_block': [], 'goal': {}, 'navigation': {}}
...
def _summary_from_query(query: dict, raw_input: str):
    if not ensure_runtime_ready():
            ensure_runtime_ready(force=True, retries=3)
    level = str(query.get('level') or 'business')
    object_name = query.get('object_name') or ('business' if level == 'business' else None)
    period_value = query.get('period_current') or query.get('period') or raw_input

    try:
        if level == 'business':
            current = get_business_comparison(period=period_value)
            drain = get_business_manager_tops_comparison(period=period_value)
            return _build_summary_view('business', period_value, current, drain, 'Бизнес')
        if level == 'manager_top':
            current = get_manager_top_comparison(manager_top=object_name, period=period_value)
            drain = get_manager_top_managers_comparison(manager_top=object_name, period=period_value)
            return _build_summary_view('manager_top', period_value, current, drain, object_name)
        if level == 'manager':
            current = get_manager_comparison(manager=object_name, period=period_value)
            drain = get_manager_networks_comparison(manager=object_name, period=period_value)
            return _build_summary_view('manager', period_value, current, drain, object_name)
        if level == 'network':
            current = get_network_comparison(network=object_name, period=period_value)
            drain = get_network_skus_comparison(network=object_name, period=period_value)
            return _build_summary_view('network', period_value, current, drain, object_name)
        if level == 'sku':
            current = get_sku_comparison(sku=object_name, period=period_value)
            return _build_summary_view('sku', period_value, current, None, object_name)
    except Exception:
        return _empty_summary(level, period_value, object_name)
...
@router.get(
    ...
    description='Preferred CEO business screen. Supports deterministic entry parsing and returns the normalized summary contract.',
)
def business_summary(period: str):
    try:
        if not ensure_runtime_ready():
            ensure_runtime_ready(force=True, retries=3)
        parsed = parse_query_intent(period)
        if isinstance(parsed, dict) and parsed.get('status') == 'ok':
            query = parsed.get('query') or {}
            if query.get('query_type') == 'summary':
                return json_response(_summary_from_query(query, period))
            if query.get('query_type') == 'drill_down':
                # direct summary endpoint should not guess from numeric/state-less commands
                return json_response({
                    'context': {'level': 'business', 'object_name': 'business', 'period': period},
                    'metrics': {},
                    'structure': {},
                    'drain_block': [],
                    'goal': {},
                    'navigation': {'current_level': 'business', 'next_level': None, 'items': [], 'all': True, 'reasons': True, 'back': True},
                    'error': 'Для цифровой команды сначала откройте экран со списком объектов.',
                })
        # fallback for plain valid period values if parser unexpectedly fails
...
def manager_summary(manager: str, period: str):
    ...
    return json_response(_build_summary_view('manager', period, current, drain, manager))
...
def network_summary(network: str, period: str):
    ...
    return json_response(_build_summary_view('network', period, current, drain, network))
