...
def _stage7_screen_order(response: dict) -> list:
    ...
    if level == 'network' and response.get('decision_workspace'):
        # Stage 8.2: Network is no longer rendered as a normal Object Screen.
        # It opens the Decision Workspace; old KPI/factor/navigation fields remain
        # available as raw API data but are not the primary screen order.
        return ['decision_workspace_block']
...
def _render_decision_workspace_block(response):
    ...
    ctx = response.get('context') if isinstance(response.get('context'), dict) else {}
    contract = workspace.get('contract') or ctx.get('object_name') or 'контракт'
    ...
    product = workspace.get('product_diagnostics') if isinstance(workspace.get('product_diagnostics'), dict) else {}
    ...
    if period:
        lines.extend(['📅 Период', str(period)])

    main_question = workspace.get('main_question')
    if main_question:
        lines.extend(['🎯 Главный вопрос', str(main_question)])

    lines.append('🧠 Диагноз контракта')
    if diagnostics.get('summary'):
        lines.append(str(diagnostics.get('summary')))
    profit_delta = diagnostics.get('profit_delta_money')
    if profit_delta is not None:
        lines.append(f'Результат периода: {_fmt_signed_int(profit_delta)} грн к прошлому году.')
    revenue_current = diagnostics.get('revenue_current')
    revenue_previous = diagnostics.get('revenue_previous_year')
    if revenue_current is not None and revenue_previous is not None:
        revenue_delta = _num(revenue_current) - _num(revenue_previous)
        lines.append(f'Оборот: {_fmt_int(revenue_current)} грн | Δ {_fmt_signed_int(revenue_delta)} грн.')
    ...
    if margin_current is not None and margin_business is not None:
        lines.append(f'Маржа: {_fmt_percent(margin_current)}% | бизнес {_fmt_percent(margin_business)}%.')

    lines.append('📊 Contract Diagnostics')
    factor_rows = response.get('benchmark_diagnostic_table') if isinstance(response.get('benchmark_diagnostic_table'), list) else []
    if factor_rows:
        for row in factor_rows[:5]:
            factor = row.get('factor') or 'Фактор'
            delta = row.get('delta_to_business') or ''
            effect = row.get('effect') or ''
            lines.append(f'{factor}: отклонение к бизнесу {delta}, эффект {effect}.')
    else:
        lines.append('Контрактная диагностика построена на KPI, факторах и benchmark, переданных API.')

    lines.append('📦 Product Diagnostics')
    if product.get('summary'):
        lines.append(str(product.get('summary')))
    category_count = product.get('category_count')
    if category_count is not None:
        lines.append(f'Категорий в доказательной базе: {category_count}.')

    if categories:
        lines.append('📂 Category Intelligence')
        for idx, item in enumerate(categories[:5], start=1):
            ...
            if opportunity is not None and _num(opportunity) > 0:
                lines.append(f'{idx}. {name}: Δ прибыли {_fmt_signed_int(delta)} грн, потенциал {_fmt_int(opportunity)} грн.')
            else:
                lines.append(f'{idx}. {name}: Δ прибыли {_fmt_signed_int(delta)} грн.')

    if assortment:
        lines.append('📑 Assortment Analysis')
        summary = assortment.get('summary')
        if summary:
            lines.append(str(summary))

    opportunity = response.get('opportunity_money')
    if opportunity is not None:
        lines.extend(['💰 Потенциал прибыли', f'{_fmt_int(abs(_num(opportunity)))} грн.'])

    lines.append('📌 Рекомендуемые действия')
    if actions:
        for idx, action in enumerate(actions[:5], start=1):
            action_text = action.get('action') or 'Проверить фактор'
            problem = action.get('problem') or ''
            reason = action.get('reason') or ''
            effect = action.get('expected_effect_money')
            head = f'{idx}. {action_text} → ожидаемый эффект {_fmt_int(effect)} грн.'
            if problem:
                head += f' Объект воздействия: {problem}.'
            lines.append(head)
            if reason:
                lines.append(str(reason))
    else:
        lines.append('API не передал готовые рекомендации. GPT не формирует их самостоятельно.')

    if evidence_items:
        lines.append('📎 Доказательства')
        for idx, item in enumerate(evidence_items[:5], start=1):
            name = item.get('category') or item.get('object_name') or item.get('name')
            delta = item.get('profit_delta_money')
            if name:
                lines.append(f'{idx}. {name}: Δ прибыли {_fmt_signed_int(delta)} грн.')
        lines.append('Category, TMC и SKU открываются только как подтверждение рекомендации.')
