...
def _build_product_tmc_decision_block(response):
    ...
    if mode == 'dominant' and dominant:
        ...
        if markup_delta > 0:
            ...
            lines.append('Группа концентрирует результат; следующий шаг — подтвердить устойчивость на уровне позиций.')
        return lines
...
def _build_product_layer_block(response):
    ...
    dimensions will be populated by the future витрины данных VECTRA.
    """
    return [
        'Что влияет на результат продукта?',
        'Цена — данные будут доступны после подключения витрины данных VECTRA.',
        'Объём — данные будут доступны после подключения витрины данных VECTRA.',
        'Ассортимент — данные будут доступны после подключения витрины данных VECTRA.',
        'структуры ассортимента — данные будут доступны после подключения витрины данных VECTRA.',
    ]
...
def _build_recommended_next_step_block(response: dict) -> list:
    ...
    if level == 'sku':
        return ['➡ Рекомендуемый следующий шаг: перейти к причинам и проверить доступные факторные сигналы позиции.']
...
def _render_decision_workspace_block(response):
    ...
    if sku_leaders:
        lines.append('')
        lines.append('⭐ Лидеры ассортимента в контракте')
        lines.append('Позиция | Оборот | Доля контракта | Доля бизнеса | Роль')
    ...
    lines.append('5. Или задай вопрос ассистенту своими словами: «какие позиции предложить первыми?», «как говорить с байером?», «где быстрый эффект?»')
...
def _apply_stage51_render_overrides(payload):
    ...
    if render_mode not in {'list_only', 'reasons', 'voice_diagnostic', 'action_package'} and level:
        try:
    ...
    try:
        ...
        if isinstance(drain, dict):
            ...
            if render_mode not in {'list_only', 'reasons', 'voice_diagnostic', 'action_package'}:
...
def vectra_query(request: VectraQueryRequest):
    ...
    if render_only_payload.get('status') != 'error' and render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'voice_diagnostic', 'action_package'}:
        try:
