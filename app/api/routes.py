...
from app.assistant_runtime.architecture_registry_runtime import (
    execute_architecture_registry_operation as execute_vectra_architecture_registry_operation,
)
...
from app.assistant_runtime.session_runtime import (
    execute_session_runtime_operation as execute_vectra_session_runtime_operation,
)
from app.assistant_runtime.runtime_supervisor import (
    execute_runtime_supervisor_operation as execute_vectra_runtime_supervisor_operation,
)
from app.assistant_runtime.runtime_recovery import (
    execute_runtime_recovery_operation as execute_vectra_runtime_recovery_operation,
)
from app.assistant_runtime.runtime_capability_registry import (
    execute_runtime_capability_registry_operation as execute_vectra_runtime_capability_registry_operation,
)
from app.assistant_runtime.runtime_dependency_graph import (
    execute_runtime_dependency_graph_operation as execute_vectra_runtime_dependency_graph_operation,
)
...
def _pr_management_workspace_block(response: dict) -> list:
    ...
    if level == 'manager':
        lines.append('7. Выбрать другой контракт / сеть из своего портфеля.')
    return [x for x in lines if str(x or '').strip()]
...
def _pr_contract_workspace_block(response: dict) -> list:
    ...
    lines.extend(['','✅ Что делаем дальше?','1. Подготовить переговоры по контракту.','2. Собрать пакет SKU для ввода.','3. Разобрать категорию с наибольшим эффектом.','4. Показать причины по экономике контракта.','5. Создать задачи КАМ / трейд-маркетингу / аналитикам.','6. Спросить ассистента: «какие SKU предложить первыми и почему?»','7. Открыть продуктовую ветку и выбрать SKU.'])
    return [x for x in lines if str(x or '').strip()]
...
def _w3_category_workspace_block(response: dict) -> list:
    ...
    lines.extend(['','➡️ Что делаем дальше?','1. Подготовить пакет развития категории.','2. Показать все SKU категории.','3. Подготовить переговорный аргумент.','4. Открыть SKU-лидера как доказательство.','5. Создать задачи.','6. Отфильтровать SKU по формату.'])
    return [x for x in lines if str(x or '').strip()]
...
def vectra_runtime_status(x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    ...
    product_verification = collect_release_47_product_verification(domain_id='bon_buasson')
    public_openapi = _laboratory_facade_openapi_schema()
    public_operation_count = _count_openapi_operations(public_openapi)
    servers = public_openapi.get('servers') if isinstance(public_openapi, dict) else []
    production_server = servers[0].get('url') if isinstance(servers, list) and servers and isinstance(servers[0], dict) else None
    ...
    action_manifest_verification = {
        'status': 'PASS' if not manifest_missing else 'FAIL',
        'missing_actions': manifest_missing or [],
        'registered_actions_count': action_manifest.get('public_facade_actions_count') if isinstance(action_manifest, dict) else None,
    }
    combined_failures = list(product_verification.get('summary', {}).get('failed') or [])
    if openapi_verification['status'] == 'FAIL':
        combined_failures.append('openapi')
    if action_manifest_verification['status'] == 'FAIL':
        combined_failures.append('action_manifest')
    product_verification['openapi'] = openapi_verification
    ...
    return json_response({
        ...
        'laboratory_access': {
            ...
            'compact_product_verification_embedded': True,
        },
    })
...
def vectra_laboratory_facade_business_data(request: dict = None, x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    ...
    try:
        ...
        if operation_type == 'get_network_sku_package':
            result = get_vectra_network_sku_package(
                period,
                str(payload.get('network') or payload.get('object_id') or payload.get('object_name') or ''),
                limit=int(payload.get('limit') or 50),
            )
            return json_response(_facade_response(
                operation_type,
                'network_sku_package.build_network_sku_package',
                '/vectra/laboratory/facade/business-data',
                result,
...
def vectra_laboratory_facade_memory(request: dict = None, x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    ...
    try:
        ...
        if operation_type in {'get_architecture_registry_status', 'get_architecture_object', 'list_architecture_objects', 'search_architecture_objects', 'get_object_relationships', 'get_traceability', 'resolve_dependencies', 'verify_architecture_object', 'evaluate_object_compliance', 'evaluate_registry_compliance', 'list_normative_sources', 'get_normative_source', 'verify_normative_sources', 'trace_normative_usage'}:
            result = execute_vectra_architecture_registry_operation(operation_type, payload)
            next_actions = {
                'get_architecture_registry_status': 'Read an architecture object or evaluate Registry Compliance.',
                'get_architecture_object': 'Inspect relationships, traceability, Verification or computed Compliance.',
                'list_architecture_objects': 'Select an object_id for detailed inspection.',
                'search_architecture_objects': 'Select an object_id from the Registry search results.',
                'get_object_relationships': 'Inspect traceability or resolve dependencies for the selected object.',
                'get_traceability': 'Verify the object and evaluate computed Compliance.',
                'resolve_dependencies': 'Review direct, reverse, transitive and critical dependencies.',
                'verify_architecture_object': 'Evaluate computed Compliance for the same object.',
            ...
            return json_response(_facade_response(operation_type, f'architecture_registry_runtime.{operation_type}', '/vectra/laboratory/facade/memory', result, next_action=next_actions.get(operation_type)))
        if operation_type in {'get_verification_runtime_status', 'get_verification_status', 'list_verification_results', 'verify_runtime_object', 'verify_registry', 'get_verification_evidence', 'search_verification_results'}:
            result = execute_vectra_verification_runtime_operation(operation_type, payload)
            next_actions = {
                'get_verification_runtime_status': 'Verify one architecture object or the complete Registry.',
                'get_verification_status': 'Inspect Verification Evidence for the selected execution.',
                'list_verification_results': 'Select an execution_id for status or Evidence inspection.',
                'verify_runtime_object': 'Read Verification Status and Evidence, then evaluate object Compliance.',
                'verify_registry': 'Review aggregation and Registry Compliance integration.',
                'get_verification_evidence': 'Use published Evidence for Product Verification.',
                'search_verification_results': 'Select an execution_id from the search results.',
            }
            return json_response(_facade_response(operation_type, f'verification_runtime.{operation_type}', '/vectra/laboratory/facade/memory', result, next_action=next_actions.get(operation_type)))
        if operation_type in {'get_execution_runtime_status', 'start_execution', 'run_execution', 'get_execution_status', 'get_execution_history', 'search_execution_results'}:
            result = execute_vectra_execution_runtime_operation(operation_type, payload)
            next_actions = {
                'get_execution_runtime_status': 'Start a registered execution for a verified architecture object.',
                'start_execution': 'Run the prepared execution when execution_status is READY.',
                'run_execution': 'Inspect Execution Status and Execution History.',
                'get_execution_status': 'Inspect Execution History or search related executions.',
                'get_execution_history': 'Use the published history for Product Verification.',
        ...
        if operation_type in {'start_runtime_session', 'restore_runtime_session', 'get_runtime_session_status', 'list_runtime_sessions', 'search_runtime_sessions', 'close_runtime_session'}:
            result = execute_vectra_session_runtime_operation(operation_type, payload)
            next_actions = {
                'start_runtime_session': 'Use the active professional Runtime session and close it when work is complete.',
                'restore_runtime_session': 'Continue work from the restored session state.',
                'get_runtime_session_status': 'Inspect the active session or close it with saved state.',
                'list_runtime_sessions': 'Select a session_id for status inspection or restoration.',
                'search_runtime_sessions': 'Select a matching session_id for status inspection or restoration.',
        ...
        if operation_type in {'get_runtime_supervisor_status', 'get_runtime_readiness', 'get_runtime_events', 'search_runtime_events', 'get_runtime_diagnostics'}:
            ...
            next_actions = {
                ...
                'get_runtime_health': 'Inspect readiness or diagnostics for any non-HEALTHY state.',
                'get_runtime_readiness': 'Use diagnostics when readiness is DEGRADED or BLOCKED.',
                'get_runtime_events': 'Search events by component, type, reason code or evaluation id.',
                'search_runtime_events': 'Inspect Runtime diagnostics for the selected condition.',
                'get_runtime_diagnostics': 'Resolve blocking conditions before resuming the operational contour.',
            }
            return json_response(_facade_response(operation_type, f'runtime_supervisor.{operation_type}', '/vectra/laboratory/facade/memory', result, next_action=next_actions.get(operation_type)))
        if operation_type in {'get_runtime_recovery_status', 'start_runtime_recovery', 'get_runtime_recovery_history', 'search_runtime_recovery', 'get_runtime_recovery_plan'}:
            result = execute_vectra_runtime_recovery_operation(operation_type, payload)
            next_actions = {
                'get_runtime_recovery_status': 'Inspect the registered Recovery Plan before starting recovery.',
                'get_runtime_recovery_plan': 'Start recovery only when Runtime Supervisor reports BLOCKED or UNHEALTHY.',
                'start_runtime_recovery': 'Inspect Recovery History and the Supervisor reevaluation.',
                'get_runtime_recovery_history': 'Search Recovery executions by plan, target component or result.',
                'search_runtime_recovery': 'Inspect the selected Recovery execution Evidence.',
            }
            return json_response(_facade_response(operation_type, f'runtime_recovery.{operation_type}', '/vectra/laboratory/facade/memory', result, next_action=next_actions.get(operation_type)))
        if operation_type in {'get_runtime_capabilities', 'get_runtime_capability', 'search_runtime_capabilities', 'verify_runtime_capability', 'get_runtime_capability_registry_status'}:
            result = execute_vectra_runtime_capability_registry_operation(operation_type, payload)
            next_actions = {
                'get_runtime_capability_registry_status': 'Inspect the published Runtime capabilities.',
                'get_runtime_capabilities': 'Select or search a registered Runtime capability.',
                'get_runtime_capability': 'Verify the selected capability against its current publisher.',
                'search_runtime_capabilities': 'Inspect or verify a matching Runtime capability.',
                'verify_runtime_capability': 'Use the verified capability through its publishing Runtime component.',
            }
            return json_response(_facade_response(operation_type, f'runtime_capability_registry.{operation_type}', '/vectra/laboratory/facade/memory', result, next_action=next_actions.get(operation_type)))
        if operation_type in {'get_runtime_dependency_graph', 'search_runtime_dependencies', 'trace_runtime_dependency', 'verify_runtime_dependency', 'get_runtime_dependency_graph_status'}:
            result = execute_vectra_runtime_dependency_graph_operation(operation_type, payload)
            next_actions = {
                'get_runtime_dependency_graph_status': 'Inspect the published Runtime dependency graph.',
                'get_runtime_dependency_graph': 'Search or trace a published Runtime dependency.',
                'search_runtime_dependencies': 'Trace or verify a matching Runtime dependency.',
                'trace_runtime_dependency': 'Verify the dependencies in the returned path.',
                'verify_runtime_dependency': 'Use the verified dependency evidence for Runtime diagnostics.',
            }
            return json_response(_facade_response(operation_type, f'runtime_dependency_graph.{operation_type}', '/vectra/laboratory/facade/memory', result, next_action=next_actions.get(operation_type)))
        if operation_type in {'get_runtime_observability_status', 'get_runtime_observations', 'search_runtime_observations', 'trace_runtime_observation', 'verify_runtime_observation'}:
            result = execute_vectra_runtime_observability_operation(operation_type, payload)
            next_actions = {
                'get_runtime_observability_status': 'Inspect aggregated published Runtime observations.',
                'get_runtime_observations': 'Search or trace an aggregated Runtime observation.',
                'search_runtime_observations': 'Trace or verify a matching Runtime observation.',
                'trace_runtime_observation': 'Verify a selected Runtime observation against its publisher.',
