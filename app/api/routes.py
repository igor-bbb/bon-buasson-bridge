...
def _pr_business_workspace_block(response: dict) -> list:
    ...
    if response.get('structure'):
        lines.extend(['','💰 Почему изменилась прибыль','Фактор | Контекст | Эффект'])
        for item in response.get('structure') or []:
            if not isinstance(item, dict): continue
            lines.append(f"{item.get('name')} | {_fmt_percent_value(item.get('percent'))} vs {_fmt_percent_value(item.get('base_percent'))} | {_fmt_signed_int(item.get('effect_money'))} грн")
    # money owners
...
def _pr_contract_workspace_block(response: dict) -> list:
    ...
    if response.get('structure'):
        lines.extend(['','💰 Экономика контракта vs бизнес','Фактор | Контракт | Бизнес | Эффект'])
        for item in response.get('structure') or []:
            if not isinstance(item,dict): continue
            lines.append(f"{item.get('name')} | {_fmt_percent_value(item.get('percent'))} | {_fmt_percent_value(item.get('base_percent'))} | {_fmt_signed_int(item.get('effect_money'))} грн")
...
def _attach_product_recovery_blocks(payload: dict) -> dict:
    if not isinstance(payload, dict):
        return payload
    if payload.get('render_mode') in {'list_only','reasons','kpi_only','voice_diagnostic'}:
        return payload
    ctx=payload.get('context') if isinstance(payload.get('context'), dict) else {}
    level=str(ctx.get('level') or '').strip().lower()
    if level=='business':
