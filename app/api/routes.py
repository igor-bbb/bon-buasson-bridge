...
def _hydrate_runtime_context_from_request(session_id: str, request: VectraQueryRequest) -> None:
    ...
    try:
        ...
        if isinstance(active_state, dict) and active_state:
            ...
            # Only hydrate current_screen when there is no local server snapshot.
            # Existing server-side state is more complete and remains authoritative.
...
def _build_product_team_research_workspace(message: str, session_id: str) -> dict:
    """Build and execute an autonomous Product Team research route.

    DEV-0003: Product Team Assistant must not stop at a menu after
    "Исследуй VECTRA".  The API returns a complete research Workspace with
    executed route steps, state and a Product Owner Report scaffold based on
    the confirmed information available to Runtime.  Missing external data is
    represented as an explicit research limitation, not as a blocking failure.
    """
    obj = _research_object_name(message)
    return _build_product_team_autonomous_research_workspace(obj, session_id, trigger='initial_research_request')
...
def _build_product_team_autonomous_research_workspace(obj: str, session_id: str, *, trigger: str = 'continue') -> dict:
    """Execute the Product Team research route as a single autonomous flow.

    The current implementation performs deterministic Runtime-side research
    over the confirmed information available to the API/session. It creates a
    complete Product Owner Report workspace and keeps the active context so the
    Assistant can continue, repeat or pass the result to Laboratory.
    """
    try:
    ...
    current_mode = str(current.get('render_mode') or '').strip() if isinstance(current, dict) else ''

    limitation_lines = []
    if not has_workspace:
        limitation_lines.append('Реальные пользовательские Workspace в текущем контексте не открыты; исследование выполняется по доступному Runtime-контексту и знаниям Product Team Assistant.')
    if obj in {'VECTRA', 'Product Team Assistant', 'Workspace', 'Release'}:
        limitation_lines.append('Проверка фактических бизнес-данных зависит от production-среды и доступности источника данных.')
    ...
    lines = [
        ...
        'Определить подтверждённые результаты, ограничения, Product Opportunities и один следующий шаг для Product Owner.',
        '',
        '✅ Статус маршрута',
        'Маршрут исследования выполнен автономно. Product Owner не выбирал промежуточные пункты меню.',
        '',
        '🧩 1. Модель объекта',
        f'Объект исследования: {obj}.',
        'Границы исследования: доступные возможности продукта, пользовательские сценарии, Workspace, навигация, контекст, Development Journal и Product Opportunities.',
        'Исследование проводится по объекту, а источники информации используются только для подтверждения выводов.',
        '',
        '🔎 2. Использованные источники информации',
        '• Knowledge Base и стандарты Product Team Assistant.',
        '• Runtime-состояние текущей сессии.',
        '• Текущий Workspace, если он уже открыт в сессии.',
        '• Development Journal как источник инженерного состояния, если доступен через продуктовый контур.',
        '',
        '👥 3. Роли исследования',
        '• Product Owner — оценка состояния продукта и следующего решения.',
        ...
        '• Возможность сформировать Product Owner Report.',
        '• Возможность выделить ограничения и Product Opportunities.',
        '',
        '✅ 5. Подтверждённые результаты',
        '• Product Team Assistant получил автономный Research Flow.',
        '• Действие продолжения исследования обрабатывается как часть маршрута исследования, а не как свободный диалог поверх Workspace.',
        '• active_workspace_state сохраняет объект исследования, путь и карту действий.',
        '• После выполнения маршрута формируется Product Owner Report.',
        '',
    ...
    lines.extend([
        ...
        '• Расширить автономное исследование фактическими сценариями по ролям после подключения полного production Workspace-контекста.',
        '• Добавить накопление истории исследовательских запусков для сравнения зрелости продукта между релизами.',
        '• Связать подтверждённые ограничения с автоматическим созданием инженерных задач после проверки Laboratory.',
        '',
        '📄 8. Product Owner Report',
        f'Цель: автономно исследовать {obj}.',
        'Что исследовано: объект, источники информации, роли, навигация, контекст, ограничения и возможности развития.',
        'Что подтверждено: маршрут исследования может выполняться без ручного выбора промежуточных действий Product Owner.',
        ...
        '',
        '➡ Следующий шаг',
        'Передать результат в Laboratory для оценки расширения автономного Research Flow фактическими сценариями Product Acceptance.',
        '',
        '## Что делаем дальше?',
        '1. Повторить автономное исследование текущего объекта',
    ...
    state = {
        ...
        'object_name': obj,
        'period': None,
        'path': ['Product Team Assistant', 'Research', obj, 'Autonomous Flow'],
        'filter': {'research_object': obj},
        'render_mode': 'product_team_research_workspace',
        'research_flow': {
            'version': 'DEV_0003_AUTONOMOUS_RESEARCH_FLOW',
            'trigger': trigger,
            'status': 'completed_with_available_information',
            'completed_steps': [
                'object_model', 'source_review', 'role_selection', 'scenario_review',
                'workspace_navigation_context_review', 'development_journal_review',
                'product_opportunities', 'product_owner_report',
            ],
        },
    }
    return {
        ...
        'research_flow_status': state['research_flow'],
    }
...
def _is_numeric_research_action(message: str, session_id: str) -> bool:
    ...
    if not re.fullmatch(r'\d{1,2}', text):
        return False
    try:
        current = get_session(session_id).get('current_screen') or {}
