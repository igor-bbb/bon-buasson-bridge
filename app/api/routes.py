...
from app.query.orchestration import derive_session_id, orchestrate_vectra_query

router = APIRouter()


PUBLIC_SUMMARY_KEYS = ('context', 'metrics', 'structure', 'drain_block', 'goal', 'navigation')

def public_summary(payload):
    if not isinstance(payload, dict):
        return payload
    if all(key in payload for key in PUBLIC_SUMMARY_KEYS):
        return {key: payload.get(key) for key in PUBLIC_SUMMARY_KEYS}
    return payload
...
def vectra_query(request: VectraQueryRequest, http_request: Request):
    explicit_session_id = getattr(request, 'session_id', None)
    if explicit_session_id:
        session_id = derive_session_id(explicit_session_id)
    else:
        ...
        client_host = getattr(getattr(http_request, 'client', None), 'host', '') or ''
        seed = '|'.join(part for part in [forwarded_for, real_ip, client_host, user_agent, origin, referer] if part)
