...
def _render_category_workspace_block(response):
    ...
    lines.append('3. Перейти к Product Workspace по ключевому SKU или подготовить переговорный аргумент по категории.')
    return [line for line in lines if str(line or '').strip()]
...
def _render_management_workspace_block(response):
    ...
    lines = [
        f'🧭 Management Workspace: {mi.get("object_name") or ctx.get("object_name") or "объект"}',
...
def _render_product_workspace_block(response):
    ...
    if str(level).lower() not in {'network', 'category', 'tmc_group', 'sku'}:
        return []
    lines = [
        f'📦 Product Workspace: {workspace.get("object_name") or ctx.get("object_name") or "объект"}',
...
# Business = commercial director desktop; Network = Contract Workspace for KAM.
...
def _pr_metric_text(response: dict, name: str) -> str:
    ...
    if name in {'Маржа', 'Наценка'}:
        return f"{_fmt_percent_value(item.get('fact_percent'))} | LY {_fmt_percent_value(item.get('pg_percent'))} | Δ {_fmt_pp_delta(item.get('delta_percent'))}"
    return f"{_fmt_signed_int(item.get('fact_money'))} | LY {_fmt_signed_int(item.get('pg_money'))} | Δ {_fmt_signed_int(item.get('delta_money'))}"
...
def _pr_trend_lines(period: str, filters: dict, limit: int = 6) -> list:
    lines=['📈 Динамика 6 месяцев','Период | Оборот | Финрез ДО | Маржа | Наценка']
...
def _pr_business_workspace_block(response: dict) -> list:
    ...
    filters={}
    lines=[
        f'📍 Business Workspace — {period}',
        '👤 Workspace: Бизнес',
        '🤖 Ассистент: стратегический ассистент коммерческого директора',
        '',
        '🧠 Executive Summary',
    ]
    ...
    lines.append(f'Финрез ДО изменился на {_fmt_signed_int(fin_delta)} грн при изменении оборота на {_fmt_signed_int(rev_delta)} грн.')
    if fin_delta > 0 and rev_delta < 0:
        lines.append(f'Бизнес заработал больше не за счёт объёма, а за счёт качества экономики: маржа {_fmt_pp_delta(margin_delta)}, наценка {_fmt_pp_delta(markup_delta)}.')
    elif fin_delta > 0:
        lines.append('Бизнес улучшил результат; нужно отделить эффект экономики от эффекта структуры и масштаба.')
    else:
        lines.append('Бизнес требует управленческого вмешательства: сначала проверяем структуру, затем факторы и владельцев результата.')
    # KPI
    lines.extend(['','📊 KPI бизнеса','KPI | Текущий / LY / Δ'])
    for name in ['Оборот','Финрез до','Маржа','Наценка','Финрез итог']:
        val=_pr_metric_text(response,name)
        if val!='—': lines.append(f'{name} | {val}')
    # structural
    sitems=_pr_structural_items('business', period, filters)
    if sitems:
        lines.extend(['','🏗 Структурный анализ бизнеса','Структура | Сейчас | LY | Δ'])
        for it in sitems:
            lines.append(f"{it['name']} | {it['current']} | {it['previous']} | {it['delta']:+d}")
        changed=[it for it in sitems if it.get('delta')]
        if changed:
            lines.append('Комментарий: финансовый результат нужно читать с учётом изменения структуры бизнеса, а не как чистое изменение экономики.')
    lines.extend([''] + _pr_trend_lines(period, filters, 6))
    # factors
    if response.get('structure'):
        lines.extend(['','💰 Почему изменилась прибыль относительно прошлого года','Фактор | Текущий период | Прошлый год / база | Δ | Денежный эффект'])
        for item in response.get('structure') or []:
            if not isinstance(item, dict): continue
            cur=_num(item.get('percent')); base=_num(item.get('base_percent')); gap=cur-base
            lines.append(f"{item.get('name')} | {_fmt_percent_value(cur)} | {_fmt_percent_value(base)} | {_fmt_pp_delta(gap)} | {_fmt_signed_int(item.get('effect_money'))} грн")
    # money owners
    managers=_pr_group_table(period,'manager_top',filters,8)
    if managers:
        total_potential=sum(max(0,_num(x.get('opportunity_money'))) for x in (response.get('opportunity_rating') or []) if isinstance(x,dict))
        lines.extend(['','💵 Где находятся деньги',f'Подтверждённый потенциал по текущей модели: {_fmt_int(total_potential)} грн' if total_potential else 'Потенциал определяется по объектам ниже.', 'Объект | Сигнал | Δ прибыли | Доля оборота | Доля прибыли | SKU'])
        ...
        for item in managers[:6]:
            name=item['name']; sig='управленческий риск' if item['delta_profit']<0 else ('крупный резерв' if opp_map.get(name,0)>0 else 'рост')
            lines.append(f"{name} | {sig} | {_fmt_signed_int(item['delta_profit'])} | {_fmt_percent_value(item['share_revenue'])} | {_fmt_percent_value(item['share_profit'])} | {item['sku_count']}")
    lines.extend(['','🚨 Management Radar'])
    if managers:
        risk=next((x for x in managers if x['delta_profit']<0), None)
        reserve=max(managers, key=lambda x: _num({str(o.get('object_name')):_num(o.get('opportunity_money')) for o in (response.get('opportunity_rating') or []) if isinstance(o,dict)}.get(x['name'],0))) if managers else None
        best=max(managers, key=lambda x: x['delta_profit'])
        if risk: lines.append(f'🔴 Главный риск: {risk["name"]} — Δ прибыли {_fmt_signed_int(risk["delta_profit"])}, доля прибыли {_fmt_percent_value(risk["share_profit"])}.')
        if reserve: lines.append(f'🟠 Главный резерв: {reserve["name"]} — требует разбор ниже по портфелю.')
        if best: lines.append(f'🟢 Лучшая практика: {best["name"]} — прирост прибыли {_fmt_signed_int(best["delta_profit"])}.')
    lines.extend(['','🎯 Приоритет коммерческого директора'])
    if managers:
        first=next((x for x in managers if x['delta_profit']<0), managers[0])
        lines.append(f'№1. Открыть {first["name"]}: это ближайшая точка управленческого влияния на результат бизнеса.')
    lines.extend(['','➡️ Что делаем дальше?','1. Открыть Workspace главного риска.','2. Открыть Workspace крупнейшего резерва.','3. Показать полную витрину руководителей.','4. Показать причины.','5. Показать просадки по контрактам/категориям/SKU.','6. Создать задачи.','7. Спросить ассистента: «что бы ты сделал первым и почему?»'])
...
def _pr_contract_workspace_block(response: dict) -> list:
    ...
    lines=[f'📍 Contract Workspace — {contract} | {period}','👤 Workspace: Контракт','🤖 Ассистент: цифровой помощник KAM по развитию клиента','','🧠 Executive Summary']
    fin_delta=_pr_metric_num(response,'Финрез до','delta_money')
    rev_delta=_pr_metric_num(response,'Оборот','delta_money')
    lines.append(f'Финрез ДО изменился на {_fmt_signed_int(fin_delta)} грн, оборот — на {_fmt_signed_int(rev_delta)} грн.')
    if fin_delta>0:
        lines.append('Контракт показывает положительную динамику; задача KAM — понять, что закрепить, где расширить матрицу и какие условия контролировать.')
    else:
    ...
    lines.extend(['','📊 KPI контракта','KPI | Текущий / LY / Δ'])
    for name in ['Оборот','Финрез до','Маржа','Наценка']:
        val=_pr_metric_text(response,name)
        if val!='—': lines.append(f'{name} | {val}')
    # economics vs business
    if response.get('structure'):
        lines.extend(['','💰 Экономика контракта относительно бизнеса','Фактор | Контракт | Бизнес | Gap | Эффект'])
        for item in response.get('structure') or []:
            if not isinstance(item,dict): continue
            cur=_num(item.get('percent')); base=_num(item.get('base_percent')); gap=cur-base
            lines.append(f"{item.get('name')} | {_fmt_percent_value(cur)} | {_fmt_percent_value(base)} | {_fmt_pp_delta(gap)} | {_fmt_signed_int(item.get('effect_money'))} грн")
    ...
    if sitems:
        lines.extend(['','🏗 Структура контракта','Показатель | Сейчас | LY | Δ'])
        for it in sitems:
            lines.append(f"{it['name']} | {it['current']} | {it['previous']} | {it['delta']:+d}")
    lines.extend(['']+_pr_trend_lines(period,filters,6))
    # categories
    ...
    if sku_items:
        lines.extend(['','⭐ SKU-лидеры контракта','SKU | Оборот | Доля контракта | Финрез | Роль'])
        for s in sku_items[:8]:
            role='флагман' if s['share_revenue']>=15 else 'рабочая позиция'
            lines.append(f"{s['name']} | {_fmt_int(s['revenue'])} | {_fmt_percent_value(s['share_revenue'])} | {_fmt_signed_int(s['finrez'])} | {role}")
    if missing:
        lines.extend(['','➕ Лидеры бизнеса, которых нет в контракте','SKU | Ранг в бизнесе | Оборот бизнеса | Сетей где есть | Почему предложить'])
        for s in missing[:10]:
            lines.append(f"{s['sku']} | №{s['rank']} | {_fmt_int(s['revenue'])} | {s['network_count']} | лидер бизнеса отсутствует в контракте")
    ...
    lines.extend(['','🤝 Переговорный пакет KAM','Цель: перейти от общего разговора о контракте к пакету развития: категория → формат → SKU → условия.'])
    if missing:
        lines.append('Аргумент: предлагаемые позиции уже доказаны бизнесом — имеют оборот, покрытие сетей и рейтинг в бизнесе.')
    lines.extend(['','✅ Что делаем дальше?','1. Подготовить переговоры по контракту.','2. Собрать пакет SKU для ввода.','3. Разобрать категорию с наибольшим эффектом.','4. Показать причины по экономике контракта.','5. Создать задачи KAM / трейд-маркетингу / аналитикам.','6. Спросить ассистента: «какие SKU предложить первыми и почему?»'])
    return [x for x in lines if str(x or '').strip()]



def _pr_share_of_business(period: str, filters: dict) -> dict:
    rows = _pr_rows(period, **filters)
    biz_rows = _pr_rows(period)
    m = aggregate_metrics(rows) if rows else {}
    bm = aggregate_metrics(biz_rows) if biz_rows else {}
    revenue = _num(m.get('revenue')); finrez = _num(m.get('finrez_pre'))
    biz_revenue = _num(bm.get('revenue')); biz_finrez = _num(bm.get('finrez_pre'))
    return {
        'revenue': revenue,
        'finrez': finrez,
        'share_revenue': (revenue / biz_revenue * 100.0) if biz_revenue else 0.0,
        'share_finrez': (finrez / biz_finrez * 100.0) if abs(biz_finrez) > 1e-9 else 0.0,
    }


def _pr_format_table(period: str, filters: dict, business_filters: dict = None, top: int = 12) -> list:
    rows = _pr_rows(period, **filters)
    biz_rows = _pr_rows(period, **(business_filters or {}))
    grouped = {}; biz_grouped = {}
    for r in rows:
        fmt = _pi72_format_name(r.get('tmc_group') or r.get('sku'))
        grouped.setdefault(fmt, []).append(r)
    for r in biz_rows:
        fmt = _pi72_format_name(r.get('tmc_group') or r.get('sku'))
        biz_grouped.setdefault(fmt, []).append(r)
    total = aggregate_metrics(rows) if rows else {}
    biz_total = aggregate_metrics(biz_rows) if biz_rows else {}
    total_rev = _num(total.get('revenue')); biz_total_rev = _num(biz_total.get('revenue'))
    names = set(grouped) | set(biz_grouped)
    out=[]
    for fmt in names:
        rs=grouped.get(fmt) or []; brs=biz_grouped.get(fmt) or []
        m=aggregate_metrics(rs) if rs else {}; bm=aggregate_metrics(brs) if brs else {}
        rev=_num(m.get('revenue')); brev=_num(bm.get('revenue'))
        out.append({
            'format': fmt,
            'revenue': rev,
            'finrez': _num(m.get('finrez_pre')),
            'share_local': (rev/total_rev*100.0) if total_rev else 0.0,
            'share_business': (brev/biz_total_rev*100.0) if biz_total_rev else 0.0,
            'sku_count': len({str(r.get('sku') or '').strip() for r in rs if str(r.get('sku') or '').strip()}),
            'business_sku_count': len({str(r.get('sku') or '').strip() for r in brs if str(r.get('sku') or '').strip()}),
        })
    out.sort(key=lambda x: (x.get('revenue') or 0), reverse=True)
    return out[:top]


def _pr_category_workspace_block(response: dict) -> list:
    ctx=response.get('context') if isinstance(response.get('context'), dict) else {}
    if str(ctx.get('level') or '').strip().lower()!='category':
        return []
    category=str(ctx.get('object_name') or '').strip(); period=str(ctx.get('period') or '').strip()
    filters=_pr_context_filters(response)
    network=filters.get('network') or '—'
    category_filters={'category': category}
    lines=[f'📦 Рабочий стол категории: {category}', f'Контракт: {network}', f'Период: {period}', '', '🧾 Паспорт категории']
    share=_pr_share_of_business(period, filters)
    rows=_pr_rows(period, **filters)
    lines.extend(['Показатель | Значение', f'Оборот | {_fmt_int(share.get("revenue"))} грн', f'Финрез ДО | {_fmt_signed_int(share.get("finrez"))} грн', f'Доля бизнеса по обороту | {_fmt_percent_value(share.get("share_revenue"))}', f'Доля бизнеса по прибыли | {_fmt_percent_value(share.get("share_finrez"))}', f'Форматов | {len({ _pi72_format_name(r.get("tmc_group") or r.get("sku")) for r in rows })}', f'SKU | {len({str(r.get("sku") or "").strip() for r in rows if str(r.get("sku") or "").strip()})}'])
    lines.extend(['','📊 KPI категории','KPI | Текущий период | Прошлый год | Δ'])
    for name in ['Оборот','Финрез до','Маржа','Наценка']:
        val=_pr_metric_text(response,name)
        if val!='—': lines.append(f'{name} | {val}')
    sitems=_pr_structural_items('category', period, filters)
    if sitems:
        lines.extend(['','🏗 Структура категории','Показатель | Сейчас | LY | Δ'])
        for it in sitems:
            lines.append(f"{it['name']} | {it['current']} | {it['previous']} | {it['delta']:+d}")
    if response.get('structure'):
        lines.extend(['','💰 Экономика категории относительно бизнеса','Фактор | Категория в контракте | Категория в бизнесе | Gap | Эффект'])
        for item in response.get('structure') or []:
            if not isinstance(item,dict): continue
            cur=_num(item.get('percent')); base=_num(item.get('base_percent'))
            lines.append(f"{item.get('name')} | {_fmt_percent_value(cur)} | {_fmt_percent_value(base)} | {_fmt_pp_delta(cur-base)} | {_fmt_signed_int(item.get('effect_money'))} грн")
    formats=_pr_format_table(period, filters, category_filters, 12)
    if formats:
        lines.extend(['','📐 Форматы категории','Формат | Оборот | Доля категории | Доля бизнеса | SKU | Что делать'])
        for f in formats:
            action='защитить' if f['share_local']>=30 else ('ввести/развить' if f['share_business']>0 and f['share_local']==0 else 'оценить')
            lines.append(f"{f['format']} | {_fmt_int(f['revenue'])} | {_fmt_percent_value(f['share_local'])} | {_fmt_percent_value(f['share_business'])} | {f['sku_count']} | {action}")
    sku_items=_pr_group_table(period,'sku',filters,12)
    biz_sku=_pr_group_table(period,'sku',category_filters,80)
    biz_sku_map={x['name']:x for x in biz_sku}
    if sku_items:
        lines.extend(['','⭐ SKU-лидеры категории','SKU | Формат | Оборот | Доля категории | Доля бизнеса | Финрез | Роль'])
        for s in sku_items[:8]:
            bs=biz_sku_map.get(s['name'],{})
            fmt=_pi72_format_name(s['name'])
            role='якорь' if s['share_revenue']>=15 else 'рабочая позиция'
            lines.append(f"{s['name']} | {fmt} | {_fmt_int(s['revenue'])} | {_fmt_percent_value(s['share_revenue'])} | {_fmt_percent_value(bs.get('share_revenue'))} | {_fmt_signed_int(s['finrez'])} | {role}")
    current_skus={x['name'] for x in sku_items}
    missing=[x for x in biz_sku if x['name'] not in current_skus]
    if missing:
        lines.extend(['','➕ Отсутствующие SKU / форматы','SKU | Формат | Оборот в бизнесе | Доля категории бизнеса | Сетей где есть | Почему важно'])
        leaders=_pr_business_sku_leaders(period,200)
        cover={x['sku']:x.get('network_count') for x in leaders}
        for s in missing[:10]:
            fmt=_pi72_format_name(s['name'])
            lines.append(f"{s['name']} | {fmt} | {_fmt_int(s['revenue'])} | {_fmt_percent_value(s['share_revenue'])} | {cover.get(s['name'], '—')} | закрывает ассортиментный пробел")
    lines.extend(['','🧠 Комментарий ассистента'])
    if formats:
        missing_formats=[f for f in formats if f['share_local']==0 and f['share_business']>0]
        if missing_formats:
            lines.append(f'Перед выбором отдельных SKU нужно проверить формат: в контракте отсутствуют форматы, которые уже работают в бизнесе ({", ".join(f["format"] for f in missing_formats[:3])}).')
        else:
            lines.append('Категория уже имеет рабочую форматную основу; следующий шаг — выбрать SKU, которые дадут лучший переговорный аргумент.')
    lines.extend(['','✅ Что делаем дальше?','1. Подготовить пакет развития категории.','2. Разобрать форматы категории.','3. Посмотреть отсутствующие SKU.','4. Подготовить переговорный аргумент по категории.','5. Создать задачи по развитию категории.'])
    return [x for x in lines if str(x or '').strip()]


def _pr_sku_workspace_block(response: dict) -> list:
    ctx=response.get('context') if isinstance(response.get('context'), dict) else {}
    if str(ctx.get('level') or '').strip().lower()!='sku':
        return []
    sku=str(ctx.get('object_name') or '').strip(); period=str(ctx.get('period') or '').strip()
    filters=_pr_context_filters(response)
    rows=_pr_rows(period, **filters)
    sku_business_rows=_pr_rows(period, sku=sku)
    all_rows=_pr_rows(period)
    m=aggregate_metrics(rows) if rows else {}; bm=aggregate_metrics(sku_business_rows) if sku_business_rows else {}; total=aggregate_metrics(all_rows) if all_rows else {}
    total_revenue=_num(total.get('revenue')); total_finrez=_num(total.get('finrez_pre'))
    category=filters.get('category') or (rows[0].get('category') if rows else '')
    group=(rows[0].get('tmc_group') if rows else '')
    fmt=_pi72_format_name(group or sku)
    leaders_rev=_pr_business_sku_leaders(period,300)
    rev_rank=next((x['rank'] for x in leaders_rev if x['sku']==sku), None)
    leaders_profit=[]
    grouped={}
    for r in all_rows:
        name=str(r.get('sku') or '').strip()
        if name: grouped.setdefault(name,[]).append(r)
    for name,rs in grouped.items():
        mm=aggregate_metrics(rs); leaders_profit.append({'sku':name,'finrez':_num(mm.get('finrez_pre'))})
    leaders_profit.sort(key=lambda x:x['finrez'], reverse=True)
    profit_rank=next((i for i,x in enumerate(leaders_profit,1) if x['sku']==sku), None)
    coverage=len({str(r.get('network') or '').strip() for r in sku_business_rows if str(r.get('network') or '').strip()})
    all_networks=len({str(r.get('network') or '').strip() for r in all_rows if str(r.get('network') or '').strip()})
    lines=[f'🧾 Паспорт SKU: {sku}', f'Период: {period}', '', '🏷 Паспорт позиции', 'Поле | Значение', f'Категория | {category or "—"}', f'Группа ТМС | {group or "—"}', f'Формат | {fmt}', f'Ранг по обороту в бизнесе | №{rev_rank}' if rev_rank else 'Ранг по обороту в бизнесе | —', f'Ранг по прибыли в бизнесе | №{profit_rank}' if profit_rank else 'Ранг по прибыли в бизнесе | —', f'Представленность | {coverage} из {all_networks} контрактов']
    lines.extend(['','📊 KPI SKU','KPI | Текущий период | Прошлый год | Δ'])
    for name in ['Оборот','Финрез до','Маржа','Наценка']:
        val=_pr_metric_text(response,name)
        if val!='—': lines.append(f'{name} | {val}')
    lines.extend(['','🏢 Роль SKU в бизнесе','Метрика | Значение', f'Оборот SKU в бизнесе | {_fmt_int(_num(bm.get("revenue")))} грн', f'Финрез SKU в бизнесе | {_fmt_signed_int(_num(bm.get("finrez_pre")))} грн', f'Доля оборота бизнеса | {_fmt_percent_value((_num(bm.get("revenue"))/total_revenue*100) if total_revenue else 0)}', f'Доля прибыли бизнеса | {_fmt_percent_value((_num(bm.get("finrez_pre"))/total_finrez*100) if abs(total_finrez)>1e-9 else 0)}', f'Покрытие контрактов | {coverage} из {all_networks}'])
    # Top networks
    grouped_n={}
    for r in sku_business_rows:
        n=str(r.get('network') or '').strip()
        if n: grouped_n.setdefault(n,[]).append(r)
    nets=[]
    for n,rs in grouped_n.items():
        mm=aggregate_metrics(rs); nets.append({'network':n,'revenue':_num(mm.get('revenue')),'finrez':_num(mm.get('finrez_pre')),'margin':_num(mm.get('margin_pre'))})
    nets.sort(key=lambda x:x['revenue'], reverse=True)
    if nets:
        lines.extend(['','⭐ Где SKU работает лучше всего','Сеть | Оборот | Финрез ДО | Маржа | Роль'])
        for n in nets[:8]:
            role='топ-клиент' if n['network']==filters.get('network') else 'доказательство продаж'
            lines.append(f"{n['network']} | {_fmt_int(n['revenue'])} | {_fmt_signed_int(n['finrez'])} | {_fmt_percent_value(n['margin'])} | {role}")
    if all_networks and coverage < all_networks:
        present={n['network'] for n in nets}
        category_networks={str(r.get('network') or '').strip() for r in _pr_rows(period, category=category) if str(r.get('network') or '').strip()}
        absent=[n for n in category_networks if n not in present]
        if absent:
            lines.extend(['','➕ Где SKU отсутствует','Сеть | Почему важно | Приоритет'])
            for n in sorted(absent)[:10]:
                lines.append(f'{n} | сеть уже работает с категорией, но SKU отсутствует | проверить ввод')
    lines.extend(['','🗣 Переговорная позиция по SKU','Аргумент | Доказательство | Ответ на возражение', f'SKU уже доказан бизнесом | ранг №{rev_rank or "—"} по обороту, покрытие {coverage} сетей | предложить тестовый ввод и оценку результата через 1–2 периода'])
    lines.extend(['','✅ Что делаем дальше?','1. Использовать SKU как доказательство в переговорах.','2. Создать задачу по защите/развитию позиции.','3. Посмотреть, где SKU отсутствует.','4. Вернуться к категории или контракту.'])
    return [x for x in lines if str(x or '').strip()]

def _attach_product_recovery_blocks(payload: dict) -> dict:
    if not isinstance(payload, dict):
        return payload
    if payload.get('render_mode') in {'list_only','reasons','kpi_only','voice_diagnostic','action_package','task_workspace','post_meeting_workspace'}:
        return payload
    ctx=payload.get('context') if isinstance(payload.get('context'), dict) else {}
    level=str(ctx.get('level') or '').strip().lower()
    if level=='business':
        ...
        payload['category_workspace_block']=_pr_category_workspace_block(payload)
        order=payload.get('screen_order') if isinstance(payload.get('screen_order'), list) else []
        payload['screen_order']=['category_workspace_block']+[x for x in order if x!='category_workspace_block']
    elif level=='sku':
        payload['sku_passport_v2_block']=_pr_sku_workspace_block(payload)
        order=payload.get('screen_order') if isinstance(payload.get('screen_order'), list) else []
        payload['screen_order']=['sku_passport_v2_block']+[x for x in order if x!='sku_passport_v2_block']
    return payload
...
def _stage7_screen_order(response: dict) -> list:
    ...
    if level == 'network':
        # Stage 8.4: Network is the full Contract Workspace.
...
def _render_decision_workspace_block(response):
    """Render Network as Contract Workspace 2.0.
    ...
    if sku_leaders:
        lines.append('')
        lines.append('⭐ SKU Intelligence: лидеры ассортимента в контракте')
...
def _enforce_public_response_budget(payload: dict) -> dict:
    ...
    Routes and orchestration should already render scoped payloads. This guard is
    intentionally placed at the API boundary to prevent a single oversized
    Workspace from breaking Custom GPT Actions. It does not change calculations;
    it only removes or compacts non-essential public blocks.
    """
    if not isinstance(payload, dict):
        return payload
    ...
    if render_mode == 'list_only':
        ...
        # Keep the decision-critical top-level blocks and remove heavy future /
        # product / narrative sections first. These blocks can always be opened
        # by focused commands later.
        for key in (
    ...
    if _json_len(out) > VECTRA_PUBLIC_RESPONSE_HARD_BUDGET:
        # Hard fallback: minimal decision screen. This is preferable to a failed
        # Action call and keeps the user inside the current Workspace context.
        out = {
...
def _make_list_only_public_payload(rendered_payload: dict) -> dict:
    ...
    Showcase must not leak full Workspace blocks. It is a list/navigation mode,
...
def _make_kpi_only_public_payload(rendered_payload: dict) -> dict:
    ...
    return {
        ...
        'summary_block': 'KPI текущего Workspace.',
...
def _apply_stage51_render_overrides(payload):
    ...
    if render_mode not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace'} and level:
        try:
    ...
    try:
        ...
        if isinstance(drain, dict):
            ...
            if render_mode not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace'}:
...
def _attach_management_workspace_if_missing(payload):
    ...
    try:
        if level == 'business':
            ...
            return payload
        for key in ('management_intelligence', 'management_workspace', 'management_passport', 'management_workspace_block', 'business_workspace_block', 'contract_workspace_block', 'sku_passport_v2_block'):
            if rebuilt.get(key):
                payload[key] = rebuilt.get(key)
...
def _prepare_vectra_query_payload(payload):
    ...
    if _is_render_ready_payload(payload):
        ...
        if ready.get('render_mode') == 'start' or str(ctx.get('level') or '').strip().lower() == 'start':
...
def _force_product_navigation(payload):
    ...
    if render_mode == 'kpi_only':
        ...
        payload['navigation_block'] = ['причины — разобрать факторы', 'все — витрина текущего уровня', 'назад — вернуться к Workspace']
    ...
    # Action-first navigation: after a Workspace, user should see working actions,
...
def vectra_query(request: VectraQueryRequest):
    ...
    if rendered_payload.get('render_mode') == 'list_only':
        ...
        'sku_passport_v2_block': rendered_payload.get('sku_passport_v2_block', []),
    ...
    should_save_rendered_state = (
        ...
        and render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace'}
    )
    # Explicit object-scoped KPI requests (`Покажи Варус KPI`) return a KPI-only
    # public payload, but State must keep the full rendered Workspace for the
    ...
    # must not overwrite the active Workspace.
    if response_scope == 'kpi' and str(rendered_payload.get('render_mode') or '').strip().lower() != 'kpi_only':
        should_save_rendered_state = True

    if should_save_rendered_state:
        try:
            # State must keep the full rendered payload, not the public render-only
            # response. The public response intentionally hides technical fields
            # such as metrics/filter, but State needs them for correct `назад`,
            # named drilldown and local UI modes without recalculating or losing
            # context.
