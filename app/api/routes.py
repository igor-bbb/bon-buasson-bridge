...
DECISION_LEVELS = {'business', 'network', 'category', 'tmc_group', 'sku'}
...
def public_summary(payload):
    ...
    response = {
        ...
        'business_result_rating': payload.get('business_result_rating') or [],
        'opportunity_rating': payload.get('opportunity_rating') or [],
        ...
        'goal': None if (payload.get('context') or {}).get('level') == 'business' or payload.get('level') == 'business' else _normalize_goal(payload, metrics),
        'metrics': metrics,
        'structure': structure,
        'drain_block': drain,
...
def _focus_money_for_decision(response):
    opportunity_money = response.get('opportunity_money')
    if opportunity_money is None:
        drain = response.get('drain_block') if isinstance(response.get('drain_block'), dict) else {}
        opportunity_money = drain.get('total_effect')
    return abs(_intnum(opportunity_money))
...
def _render_decision_block(response):
    ...
    if level == 'network':
        if object_name:
            lines.append(f'Приоритет контракта: {object_name} → закрыть {focus_money}')
        if main_driver:
    ...
    if level in {'category', 'tmc_group'}:
        if object_name:
            lines.append(f'Приоритет цены: {object_name} → закрыть {focus_money}')
...
def _attach_render_blocks(response, payload):
    ...
    # v1.2: Path Goal / Focus / Coverage are legacy presentation concepts.
    # Keep Opportunity Money in explicit fields instead of rebuilding vector_block.
    vector_block = {}
    ...
    if ctx_level == 'sku':
        ...
        if not rendered_sku_drain:
            sku_focus = 0
            try:
                sku_focus = _intnum(response.get('opportunity_money'))
            except Exception:
                sku_focus = 0
...
def vectra_query(request: VectraQueryRequest):
    ...
    render_only_payload = {
        ...
        'business_result_rating': rendered_payload.get('business_result_rating', []),
        'opportunity_rating': rendered_payload.get('opportunity_rating', []),
