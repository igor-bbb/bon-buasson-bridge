...
def _render_business_result_block(response):
    lines = ['📊 Что произошло с прибылью']
    lines.extend(_profit_first_metric_lines(response))
    lines.append(f'Итоговое изменение финреза до: {_render_money_value(response.get("business_result_money"))} грн')
    return lines


def _render_object_result_block(response):
    lines = ['📊 Что произошло с объектом']
    lines.extend(_profit_first_metric_lines(response))
    lines.append(f'🏆 Вклад относительно среднего уровня бизнеса: {_render_money_value(response.get("object_result_money"))} грн')
    return lines
