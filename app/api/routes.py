...
def _laboratory_full_openapi_schema() -> dict:
    ...
    schema['info'] = {
        'title': 'VECTRA Laboratory Complete Actions',
        'version': 'LABORATORY-ACTIONS-0002-FULL-LABORATORY-ACTIONS',
        'description': 'Canonical single complete OpenAPI schema for VECTRA Laboratory GPT Actions. It aggregates all supported Laboratory Actions and is checked by the Laboratory Action Manifest so previously published Actions cannot disappear silently.',
    }
    schema['x-vectra-scope'] = 'laboratory_complete_actions'
    schema['x-vectra-release'] = 'LABORATORY-ACTIONS-0002'
    schema['x-vectra-action-manifest'] = '/vectra/laboratory/actions/manifest'
    schema['x-vectra-action-completeness-verification'] = '/vectra/laboratory/actions/verify'
    schema['x-vectra-full-schema-policy'] = 'single_complete_schema_required; split schemas are backward-compatible diagnostic exports only'
...
def vectra_laboratory_action_manifest(x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    _verify_laboratory_api_key(x_vectra_laboratory_key)
    return json_response(_build_laboratory_action_manifest())
...
def vectra_laboratory_action_completeness_verify(x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    _verify_laboratory_api_key(x_vectra_laboratory_key)
    return json_response(_verify_laboratory_action_completeness())
