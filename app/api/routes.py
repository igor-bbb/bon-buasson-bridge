...
def _build_product_insight_block(response):
    ...
    object_label = object_name or ('SKU' if level == 'sku' else 'продуктовый объект')
    ...
    if opportunity > 0:
        opportunity_line = f'Однако внутри остаётся {_fmt_int(opportunity)} потенциальной прибыли.'
        next_line = 'Для поиска источника потерь необходимо перейти к анализу SKU.' if level != 'sku' else 'Необходимо проверить продуктовые факторы SKU: цену, объём, ассортимент и SKU Mix.'
    else:
        opportunity_line = 'Существенный потенциал прибыли внутри продуктового уровня не выявлен.'
...
def _build_benchmark_driven_summary(response: dict) -> str:
    ...
    if level == 'business':
        if fin_delta > 0:
            return f'Бизнес улучшил результат к прошлому году на {_fmt_int(fin_delta)}. Основной потенциал прибыли показывают объекты в рейтинге возможностей.'
        if fin_delta < 0:
            return f'Бизнес просел к прошлому году на {_fmt_int(abs(fin_delta))}. Нужно открыть рейтинг возможностей и причины бизнеса.'
        return 'Бизнес находится примерно на уровне прошлого года. Основной фокус — рейтинг возможностей и причины бизнеса.'

    yoy_text = _metric_delta_text(response)
    obj_result = _num(response.get('object_result_money'))
    opportunity = _num(response.get('opportunity_money'))
    product_level = _is_product_layer_level(level)

    if product_level:
        if obj_result < 0:
            result_text = f'Относительно эффективности SKU по бизнесу остаётся недозаработанный результат {_fmt_int(abs(obj_result))}.'
        elif obj_result > 0:
            result_text = f'Относительно эффективности SKU по бизнесу объект даёт дополнительный результат {_fmt_int(obj_result)}.'
        else:
            result_text = 'Отклонение относительно эффективности SKU по бизнесу не выявлено.'
        if opportunity > 0:
            opportunity_text = f'Внутри продуктового уровня находится потенциал прибыли {_fmt_int(opportunity)} через проверку эффективности SKU.'
        else:
            opportunity_text = 'Существенный потенциал прибыли на этом продуктовом уровне не выявлен.'
        return f'{yoy_text} {result_text} {opportunity_text}'

    if obj_result < 0:
        result_text = f'Однако относительно бизнеса остаётся недозаработанный результат {_fmt_int(abs(obj_result))}.'
    elif obj_result > 0:
        result_text = f'Относительно бизнеса объект даёт дополнительный результат {_fmt_int(obj_result)}.'
    else:
        result_text = 'Относительно бизнеса существенного отклонения результата не выявлено.'

    if opportunity > 0:
        opportunity_text = f'Внутри объекта находится потенциал прибыли {_fmt_int(opportunity)}.'
    else:
        opportunity_text = 'Существенный потенциал прибыли внутри объекта не выявлен.'
    return f'{yoy_text} {result_text} {opportunity_text}'
...
def _build_explanation_block(response: dict) -> list:
    ...
    lines = []
    if level == 'business':
        result = response.get('business_result_money')
        lines.extend([
            'Результат бизнеса — что это означает?',
            f'Это изменение результата бизнеса относительно прошлого года: {_fmt_int(result)}.',
        ])
        return lines
    ...
    opportunity = _num(response.get('opportunity_money'))
    product_level = _is_product_layer_level(level)
    benchmark_name = 'эффективности SKU относительно бизнеса' if product_level else 'среднего уровня бизнеса'
    benchmark_subject = 'Эффективность SKU относительно бизнеса' if product_level else 'Средний уровень бизнеса'
    if obj_result < 0:
        result_text = f'{benchmark_subject} выше эффективности объекта. При текущем обороте это соответствует разнице результата {_fmt_int(abs(obj_result))}.'
    elif obj_result > 0:
        result_text = f'Эффективность объекта выше {benchmark_name}. При текущем обороте это даёт дополнительный результат {_fmt_int(obj_result)}.'
    else:
        result_text = f'Объект находится около уровня {benchmark_name}; существенная разница результата не выявлена.'
    lines.extend(['Вклад в результат бизнеса — что это означает?', result_text])

    if opportunity > 0:
        if product_level:
            opportunity_text = f'Внутри продуктового уровня есть позиции ниже эффективности SKU относительно бизнеса. Проверка SKU может вернуть до {_fmt_int(opportunity)} результата.'
        else:
            opportunity_text = f'Внутри объекта есть точки ниже среднего уровня бизнеса. При доведении их до среднего уровня бизнеса можно вернуть до {_fmt_int(opportunity)} результата.'
    else:
        opportunity_text = 'Внутри объекта не найден существенный денежный потенциал прибыли.'
    lines.extend(['Потенциал прибыли — что это означает?', opportunity_text])
    return lines
...
def _render_object_result_block(response):
    return [f'Вклад в результат бизнеса: {_render_money_value(response.get("object_result_money"))}']


def _render_opportunity_block(response):
    return [f'Потенциал прибыли: {_render_money_value(response.get("opportunity_money"))}']
