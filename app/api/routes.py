...
def public_summary(payload):
    ...
    if payload.get('status') == 'error':
        if (payload.get('reason') or '') == 'show_start_screen':
            return {
                'status': 'ok',
                'reason': None,
                'context': {'level': None, 'object_name': None, 'period': None, 'parent_object': None},
                'goal_block': '',
                'kpi_block': [],
                'structure_block': [],
                'main_driver': '',
                ...
                'drain_total': 0,
                'navigation_block': [],
                'summary_block': 'Введите ФИО менеджера и период или только период. Примеры: 2026-02; Бурденко Денис 2026-02',
                'path': []
            }
