...
PUBLIC_TOP_LEVEL_KEYS = ('context', 'goal', 'metrics', 'structure', 'drain_block', 'navigation', 'reasons_block', 'decision_block', 'kpi_block', 'structure_block', 'main_driver', 'drain_block_render', 'drain_total', 'navigation_block', 'goal_block', 'summary_block', 'path')
...
def _normalize_goal(payload, metrics_out):
    ...
    # VECTRA VECTOR LOCK:
    # If orchestration provided goal.effect_money, this is the selected parent
    # vector and MUST NOT be overwritten by the current level KPI delta.
    effect = goal.get('effect_money')
    explicit_goal = effect is not None
    if effect is None:
    ...
    if gtype in ('close_gap', 'close'):
        gtype = 'close'
    elif gtype in ('keep', 'hold', 'keep_growth'):
        gtype = 'hold'
    else:
...
def public_summary(payload):
    ...
    context_path = (payload.get('context') or {}).get('path') if isinstance(payload.get('context'), dict) else None
    response = {
        'context': context,
        'path': payload.get('path') or context_path or [],
