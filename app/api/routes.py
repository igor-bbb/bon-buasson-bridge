...
def _pr_business_workspace_block(response: dict) -> list:
    ...
    # factors: Evidence First — current, LY, delta, effect.
    factors = _w3_factor_evidence_rows(response, business=True)
    if factors:
        lines.extend(['','💰 Почему изменилась прибыль относительно прошлого года','Фактор | Текущий период | Прошлый год | Δ | Денежный эффект | Сигнал'])
        for item in factors:
...
def _pr_contract_workspace_block(response: dict) -> list:
    ...
    if econ_rows:
        lines.extend(['','💰 Экономика контракта относительно бизнеса','Фактор | Контракт | Бизнес | Gap | Денежный эффект | Сигнал'])
...
# Sprint W3 Workspace Intelligence: Evidence First, Object Passport and SKU-FIRST blocks.
...
def _w3_category_workspace_block(response: dict) -> list:
    ...
    lines = [
        ...
        f'Период: {period}',
        '🤖 Ассистент: помощник KAM по развитию категории',
        '',
        '🧾 Паспорт категории',
        'Показатель | Значение',
        ...
        '',
        '📊 KPI категории',
        'KPI | Текущий период | Прошлый год | Δ | Доля контракта | Доля бизнеса',
    ]
    ...
    if factors:
        lines.extend(['','💰 Экономика категории: доказательства','Фактор | Текущий период | LY | Δ | Эффект | Сигнал'])
        for item in factors:
            lines.append(f"{item['name']} | {item['current_text']} | {item['previous_text']} | {item['delta_text']} | {_fmt_signed_int(item['effect'])} грн | {item['signal']}")
...
def _w3_sku_passport_block(response: dict) -> list:
    ...
    lines = [
        ...
        '🤖 Ассистент: помощник по развитию продукта',
        '',
        '🧬 Идентификация SKU',
        'Поле | Значение',
        f'Категория | {ident.get("category") or "—"}',
        ...
        'KPI | Текущий период | Прошлый год | Δ / комментарий',
...
def _attach_sku_passport_if_missing(payload):
    if not isinstance(payload, dict):
        return payload
    ctx = payload.get('context') if isinstance(payload.get('context'), dict) else {}
    if payload.get('render_mode') in {'action_package', 'task_workspace', 'post_meeting_workspace', 'list_only', 'reasons', 'kpi_only'}:
        return payload
    if str(ctx.get('level') or '').strip().lower() != 'sku':
        return payload
