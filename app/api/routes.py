...
def _compact_public_all_block(items):
    ...
    allowed = {
        ...
        'network_count', 'margin', 'margin_pre', 'markup', 'priority_signal',
    }
...
def _attach_management_workspace_if_missing(payload):
    ...
    try:
        if level == 'business':
            ...
            return payload
        for key in ('management_intelligence', 'management_workspace', 'management_passport', 'management_workspace_block'):
            if rebuilt.get(key):
                payload[key] = rebuilt.get(key)
