...
def _pr_business_workspace_block(response: dict) -> list:
    ...
    # KPI
    lines.extend(['','📊 Ключевые показатели бизнеса','Показатель | Текущий период | Прошлый год | Изменение'])
    ...
    if sitems:
        lines.extend(['','🏗 Структурный анализ бизнеса','Структура | Сейчас | Прошлый год | Δ'])
        for it in sitems:
    ...
    if managers:
        ...
        potential_rows = [{'name': item['name'], 'potential': opp_map.get(item['name'], 0)} for item in managers]
        comment = _w5_potential_comment(potential_rows)
        if comment:
            lines.append(comment)
    lines.extend(['','🚨 Приоритеты руководителя','Зона | Объект | Почему важно | Что делать'])
    if managers:
        ...
        if risk: lines.append(f'🔴 Главный риск | {risk["name"]} | Δ прибыли {_fmt_signed_int(risk["delta_profit"])}; доля прибыли {_fmt_percent_value(risk["share_profit"])} | открыть рабочий стол и найти источник просадки')
    ...
    lines.extend(['','➡️ Что делаем дальше?','1. Открыть рабочий стол главного риска.','2. Открыть рабочий стол крупнейшего резерва.','3. Показать полную витрину руководителей.','4. Показать причины.','5. Показать просадки по контрактам/категориям/SKU.','6. Создать задачи.','7. Спросить ассистента: «что бы ты сделал первым и почему?»'])
...
def _enforce_public_response_budget(payload: dict) -> dict:
    ...
    рабочий стол from breaking Custom GPT Actions. It does not change calculations;
    ...
    if _json_len(out) > VECTRA_PUBLIC_RESPONSE_HARD_BUDGET:
        ...
        # Action call and keeps the user inside the current рабочий стол context.
        out = {
