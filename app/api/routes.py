...
def _laboratory_public_openapi_schema() -> dict:
    ...
    return {
        ...
        'info': {
            ...
            'version': os.getenv('VECTRA_LABORATORY_SCHEMA_VERSION', 'FOUNDATION-0010-KNOWLEDGE-WRITE-ACCESS'),
            'description': 'Public Action-facing API for VECTRA Laboratory. Includes read-only Business Data, Repository Inspection and explicit Product Owner approved Knowledge Capitalization write endpoints required for FOUNDATION-0010 Product Verification.',
        },
...
# FOUNDATION-0011: Split Laboratory OpenAPI schemas for GPT Actions 30-operation limit
# OpenAI GPT Actions currently rejects a single imported schema with more than
# 30 operations. Keep the legacy Laboratory schema available as Core and expose
# separate schemas for Business Data and Knowledge / Self Evolution.
...
def _laboratory_split_openapi_schema(paths_to_include: set[str], *, title: str, version: str, description: str, scope: str) -> dict:
    ...
    schema['x-vectra-foundation'] = 'FOUNDATION-0011'
...
def _laboratory_core_openapi_schema() -> dict:
    return _laboratory_split_openapi_schema(
        ...
        description='Core VECTRA Laboratory Actions for Runtime state restoration, verification, VOS, Life Model, Professional Model, Professional Body, Evolution and Recovery checks. This schema is intentionally below the GPT Actions 30-operation limit.',
...
def _laboratory_knowledge_openapi_schema() -> dict:
    return _laboratory_split_openapi_schema(
        ...
        description='Knowledge Capitalization, Repository Inspection and Self Evolution Actions for VECTRA Laboratory. This schema is intentionally below the GPT Actions 30-operation limit.',
...
@router.get('/vectra/laboratory/openapi.json', summary='Read VECTRA Laboratory Core OpenAPI schema')
def vectra_laboratory_openapi_schema():
    return JSONResponse(content=_laboratory_core_openapi_schema())


@router.get('/laboratory/openapi.json', summary='Alias: Read VECTRA Laboratory Core OpenAPI schema')
...
@router.get('/vectra/knowledge/professional', summary='Read capitalized VECTRA Professional Knowledge')
def vectra_knowledge_professional(x_vectra_laboratory_key: str | None = Header(default=None, alias='X-VECTRA-LABORATORY-KEY')):
    _verify_laboratory_api_key(x_vectra_laboratory_key)
    return json_response(get_vectra_professional_knowledge())
