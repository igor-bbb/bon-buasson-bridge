...
def _build_product_priority_action_block(response):
    ...
    if markup:
        ...
        effect = _reason_effect_vs_business(markup)
        if delta < 0:
            return [f'Проверить цену/наценку → потенциал до {_fmt_int(abs(effect))} относительно {_product_compare_base_label(response)}']
        if delta > 0:
...
def _stage7_screen_order(response: dict) -> list:
    ...
    if str(ctx.get('level') or '').strip().lower() in {'category', 'tmc_group'}:
        return ['result_block', 'diagnosis_block', 'recommended_next_step_block', 'factor_change_block', 'benchmark_diagnostic_block', 'product_tmc_decision_block', 'opportunity_rating_block', 'decision_block_render', 'navigation_block']
