...
def _apply_large_workspace_rendering(payload: dict) -> dict:
    ...
    payload['screen_order'] = ['workspace_primary_block', 'navigation_block']
    return payload
...
def _make_full_workspace_public_payload(rendered_payload: dict) -> dict:
    ...
    W10 rendering restoration: for full object screens the Custom GPT must not
    reconstruct a shorter BI-style answer from legacy fields. The public payload
    exposes one user-visible artifact (workspace_markdown / workspace_primary_block)
    plus navigation. State still stores the full rendered_payload separately.
    """
    primary = rendered_payload.get('workspace_primary_block') if isinstance(rendered_payload, dict) else []
    if not (isinstance(primary, list) and primary):
    ...
    return {
        ...
        'workspace_render_instruction': rendered_payload.get('workspace_render_instruction') or 'Показывать workspace_markdown полностью как основной рабочий стол.',
        'workspace_markdown': markdown,
        'workspace_primary_block': primary,
        'workspace_sections': rendered_payload.get('workspace_sections', []),
        'large_workspace_rendering': rendered_payload.get('large_workspace_rendering', {}),
        'navigation_block': rendered_payload.get('navigation_block', []),
        'screen_order': ['workspace_markdown', 'navigation_block'],
    }
...
def _apply_stage51_render_overrides(payload):
    ...
    if render_mode not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export'} and level:
        try:
    ...
    try:
        ...
        if isinstance(drain, dict):
            ...
            if render_mode not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export'}:
...
def vectra_query(request: VectraQueryRequest):
    ...
    if render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export'}:
    ...
    should_save_rendered_state = (
        ...
        and render_only_payload.get('render_mode') not in {'list_only', 'reasons', 'kpi_only', 'voice_diagnostic', 'action_package', 'task_workspace', 'post_meeting_workspace', 'development_journal', 'development_journal_capture', 'development_journal_export'}
    )
