...
def _build_product_insight_block(response):
    ...
    if obj_result > 0:
        result_line = f'{object_label} показывает положительный вклад в результат бизнеса.'
    elif obj_result < 0:
        result_line = f'{object_label} работает ниже среднего уровня бизнеса на {_fmt_int(abs(obj_result))}.'
    else:
        result_line = f'{object_label} находится около среднего уровня бизнеса.'

    if opportunity > 0:
        opportunity_line = f'Однако внутри продукта остаётся {_fmt_int(opportunity)} потенциала прибыли.'
        next_line = 'Основной источник нужно искать на уровне отдельных SKU.' if level != 'sku' else 'Нужно проверить текущие KPI SKU и подтвердить данные Data Mart по цене, объёму, ассортименту и SKU Mix.'
    else:
        opportunity_line = 'Существенный продуктовый потенциал прибыли не выявлен.'
        next_line = 'Следующий шаг — проверить устойчивость результата по отдельным SKU.' if level != 'sku' else 'Следующий шаг — подтвердить, что SKU работает не хуже среднего уровня бизнеса.'
...
def _build_product_priority_action_block(response):
    ...
    if compare_base == 'product_baseline_missing':
        return ['Проверить эффективность SKU → данных для базы нет']
    if margin and _num(margin.get('delta_percent')) < 0:
        return ['Улучшить маржу']
    return ['Проверить эффективность SKU']
...
def _navigation_money_text(item):
    ...
    return _fmt_int(abs(_num(value)))
...
def _best_positive_reason(response: dict):
    reasons = [r for r in (response.get('reasons_block') or response.get('object_reasons') or []) if isinstance(r, dict)]
    positives = [r for r in reasons if _reason_effect(r) > 0]
    if not positives:
        return None
    return max(positives, key=lambda r: _reason_effect(r))


def _worst_negative_reason(response: dict):
    reasons = [r for r in (response.get('reasons_block') or response.get('object_reasons') or []) if isinstance(r, dict)]
    negatives = [r for r in reasons if _reason_effect(r) < 0]
    if not negatives:
        return None
    return min(negatives, key=lambda r: _reason_effect(r))
...
def _build_benchmark_driven_summary(response: dict) -> str:
    ...
    if layer == 'business':
        if fin_delta > 0:
            base = f'Бизнес заработал на {_fmt_int(fin_delta)} больше, чем год назад.'
        elif fin_delta < 0:
            base = f'Бизнес заработал на {_fmt_int(abs(fin_delta))} меньше, чем год назад.'
        else:
            base = 'Бизнес находится примерно на уровне прошлого года.'
        ...
        risk_line = f'Главный риск бизнеса — {_reason_display_name(risk).lower()}.' if risk else 'Главный риск бизнеса по причинам не выделен.'
        result_line = f'Основной вклад в результат создают {result_names}.' if result_names else 'Основной вклад в результат нужно уточнить через рейтинг результата.'
        potential_line = f'Наибольший потенциал прибыли находится у {potential_names}.' if potential_names else 'Основной потенциал прибыли нужно уточнить через рейтинг возможностей.'
        return f'{base} {result_line} {potential_line} {risk_line}'

    if layer == 'object':
        yoy = _metric_delta_text(response)
        if obj_result < 0:
            result_line = f'Эффективность объекта ниже среднего уровня бизнеса; вклад в результат бизнеса составляет {_fmt_signed_int(obj_result)} грн.'
        elif obj_result > 0:
            result_line = f'Эффективность объекта выше среднего уровня бизнеса; вклад в результат бизнеса составляет {_fmt_signed_int(obj_result)} грн.'
        else:
            result_line = 'Эффективность объекта находится около среднего уровня бизнеса.'
        strong_line = f'Главная сильная сторона — {_reason_display_name(strong).lower()}.' if strong else 'Главная сильная сторона по причинам не выделена.'
        risk_line = f'Главный источник потерь — {_reason_display_name(risk).lower()}.' if risk else 'Критичный источник потерь по причинам не выделен.'
        opportunity_line = f'Внутри объекта остаётся потенциал прибыли {_fmt_int(opportunity)} грн.' if opportunity > 0 else 'Существенный потенциал прибыли внутри объекта не выявлен.'
        return f'{yoy} {result_line} {strong_line} {risk_line} {opportunity_line}'

    if layer == 'contract':
        if obj_result < 0:
            result_line = f'Сеть работает ниже среднего уровня бизнеса на {_fmt_int(abs(obj_result))} результата.'
        elif obj_result > 0:
            result_line = f'Сеть работает выше среднего уровня бизнеса и добавляет {_fmt_int(obj_result)} результата.'
        else:
            result_line = 'Сеть работает около среднего уровня бизнеса.'
        strong_line = f'Сильная сторона контракта — {_reason_display_name(strong).lower()}.' if strong else 'Сильная сторона контракта по причинам не выделена.'
        risk_line = f'Главный риск контракта — {_reason_display_name(risk).lower()}.' if risk else 'Критичный контрактный риск не выделен.'
        opportunity_line = f'Потенциал прибыли внутри контракта составляет {_fmt_int(opportunity)} грн.' if opportunity > 0 else 'Существенный потенциал прибыли внутри контракта не выявлен.'
        return f'{result_line} {strong_line} {risk_line} {opportunity_line}'

    if layer == 'product':
        if obj_result < 0:
            result_line = f'Продуктовый объект работает ниже среднего уровня бизнеса на {_fmt_int(abs(obj_result))} результата.'
        elif obj_result > 0:
            result_line = f'Продуктовый объект показывает положительный вклад в результат бизнеса {_fmt_int(obj_result)}.'
        else:
            result_line = 'Продуктовый объект находится около среднего уровня бизнеса.'
        opportunity_line = f'Внутри продукта остаётся {_fmt_int(opportunity)} грн потенциала прибыли.' if opportunity > 0 else 'Существенный продуктовый потенциал прибыли не выявлен.'
        return f'{result_line} {opportunity_line} Вероятный источник нужно искать на уровне отдельных SKU. Детальный анализ цены, объёма, ассортимента и SKU Mix будет доступен после подключения VECTRA Data Mart.'

    if layer == 'sku':
        return f'{_sku_metric_sentence(response)} Для полного анализа SKU не хватает данных Data Mart. Доступна только оценка по текущим KPI. Нужно проверить цену, промо и условия продажи SKU.'
...
def _build_explanation_block(response: dict) -> list:
    ...
    if layer == 'business':
        finrez = _metric_by_name(response.get('metrics') or [], 'Финрез до')
        fin_delta = _delta_money_for_metric(finrez)
        result_names = _top_names(response.get('business_result_rating') or [], 2)
        potential_names = _top_names(response.get('opportunity_rating') or [], 2)
        risk = _worst_negative_reason(response)
        lines.extend([
            ...
            f'Изменение к прошлому году: {_fmt_signed_int(fin_delta)}.',
            f'Основной вклад в результат: {result_names or "данных нет"}.',
            f'Основной потенциал прибыли: {potential_names or "данных нет"}.',
            f'Главный риск бизнеса: {_reason_display_name(risk) if risk else "не выделен"}.',
        ])
        return lines
    ...
    if layer == 'object':
        if obj_result < 0:
            result_text = f'Объект работает ниже среднего уровня бизнеса. При текущем обороте это соответствует разнице результата {_fmt_int(abs(obj_result))} грн.'
        elif obj_result > 0:
            result_text = f'Объект работает выше среднего уровня бизнеса. При текущем обороте это даёт дополнительный результат {_fmt_int(obj_result)} грн.'
        else:
            result_text = 'Объект находится около среднего уровня бизнеса.'
        lines.extend([
            'Почему объект лучше или хуже бизнеса?',
            result_text,
            f'Главная сильная сторона: {_reason_display_name(strong) if strong else "не выделена"}.',
            f'Главный источник потерь: {_reason_display_name(risk) if risk else "не выделен"}.',
            f'Потенциал прибыли: {_fmt_int(opportunity)} грн.' if opportunity > 0 else 'Существенный потенциал прибыли не выявлен.',
        ])
        return lines

    if layer == 'contract':
        if obj_result < 0:
            result_text = f'Сеть работает ниже среднего уровня бизнеса на {_fmt_int(abs(obj_result))} результата.'
        elif obj_result > 0:
            result_text = f'Сеть работает выше среднего уровня бизнеса и добавляет {_fmt_int(obj_result)} результата.'
        else:
            result_text = 'Сеть работает около среднего уровня бизнеса.'
        lines.extend([
            'Что происходит внутри контракта?',
            result_text,
            f'Сильная сторона контракта: {_reason_display_name(strong) if strong else "не выделена"}.',
            f'Главный риск контракта: {_reason_display_name(risk) if risk else "не выделен"}.',
            f'Потенциал прибыли внутри контракта: {_fmt_int(opportunity)} грн.' if opportunity > 0 else 'Существенный потенциал прибыли внутри контракта не выявлен.',
        ])
        return lines
    ...
    if layer == 'product':
        lines.extend([
            ...
            f'Вклад продукта в результат бизнеса: {_fmt_signed_int(obj_result)} грн.',
            f'Потенциал прибыли внутри продукта: {_fmt_int(opportunity)} грн.' if opportunity > 0 else 'Существенный продуктовый потенциал прибыли не выявлен.',
            'Основной источник нужно искать среди отдельных SKU внутри продукта.',
            'Детальный анализ цены, объёма, ассортимента и SKU Mix будет доступен после подключения VECTRA Data Mart.',
        ])
        return lines

    if layer == 'sku':
        lines.extend([
            'Почему SKU зарабатывает или теряет деньги?',
            _sku_metric_sentence(response),
            'Для полного анализа SKU не хватает данных Data Mart. Доступна только оценка по текущим KPI.',
            'Нужно проверить цену, промо и условия продажи SKU.',
        ])
        return lines
...
def _build_next_step_block(response: dict) -> list:
    ...
    if response.get('render_mode') in {'list_only', 'reasons'}:
        return []
    if level == 'business':
        rating = response.get('opportunity_rating') or []
        first = rating[0].get('object_name') if rating and isinstance(rating[0], dict) else None
        if first:
            return [f'Рекомендуемый следующий шаг: открыть {first} из рейтинга возможностей и проверить источник потенциала.']
        return ['Рекомендуемый следующий шаг: открыть рейтинг возможностей и определить основной источник потенциала.']
    if _is_product_layer_level(level):
        return [
            'Основной потенциал находится на уровне отдельных SKU.',
            'Следующий шаг:',
            'Определить позиции, которые работают хуже среднего уровня бизнеса.',
        ]
    if _num(response.get('opportunity_money')) > 0:
        return ['Рекомендуемый следующий шаг: открыть объекты ниже и определить основной источник потенциала.']
    return ['Рекомендуемый следующий шаг: проверить причины и подтвердить, что значимый потенциал прибыли отсутствует.']
...
def _render_business_result_block(response):
    return [f'Результат бизнеса: {_render_money_value(response.get("business_result_money"))} грн']


def _render_object_result_block(response):
    return [f'Вклад в результат бизнеса: {_render_money_value(response.get("object_result_money"))} грн']


def _render_opportunity_block(response):
    value = response.get("opportunity_money")
    return [f'Потенциал прибыли: {_fmt_int(abs(_num(value)))} грн']
...
def _attach_render_blocks(response, payload):
    ...
    if render_mode == 'list_only':
        ...
        if _is_product_layer_level(ctx_level_for_main_driver):
            ...
            response['object_reasons_block'] = _render_reasons_block(response.get('object_reasons') or [], ctx_level_for_main_driver)
...
def _replace_user_terms(value):
    replacements = {
        'Результат объекта': 'Вклад в результат бизнеса',
