...
def _build_product_team_research_workspace(message: str, session_id: str) -> dict:
    ...
    lines = [
        f'📍 Рабочий стол исследования — {obj}',
        '',
        '🧭 Цель',
        f'Исследовать объект: {obj}.',
        'Определить подтверждённые возможности, ограничения, Product Opportunities и один следующий шаг.',
        '',
        '🧩 Модель объекта',
        'Перед исследованием Assistant должен определить состав объекта, его назначение, участников, жизненный цикл и связи с другими объектами.',
        '',
        '🔎 Маршрут исследования',
        '1. Определить объект исследования',
        '2. Построить модель объекта',
        '3. Определить доступные источники информации',
        '4. Проверить роли и пользовательские сценарии',
        '5. Проверить Workspace, навигацию и контекст',
        '6. Проверить Development Journal',
        '7. Определить ограничения и Product Opportunities',
        '8. Сформировать Product Owner Report',
        '',
        '## Что делаем дальше?',
        '1. Построить модель объекта исследования',
        '2. Составить маршрут исследования',
        '3. Проверить Development Journal',
        '4. Сформировать Product Owner Report',
        '5. назад — вернуться к предыдущему Workspace',
    ]
    return {
        'status': 'ok',
        ...
        'path': ['Product Team Assistant', 'Research', obj],
...
def _build_product_team_research_action_workspace(message: str, session_id: str) -> dict:
    ...
    actions = {
        ...
        5: ('Возврат', 'Вернуться к рабочему столу исследования.'),
    }
    title, description = actions.get(number, ('Действие недоступно', 'Выбранного действия нет в текущем рабочем столе исследования.'))
    if number == 5:
    ...
    return {
        ...
        'render_mode': 'product_team_research_workspace',
        'context': {'level': 'product_team_research', 'object_name': obj, 'period': None, 'parent_object': None},
        'path': ['Product Team Assistant', 'Research', obj, title],
        'workspace_primary_block': lines,
        'workspace_markdown': '\n'.join(lines),
        'screen_order': ['workspace_markdown'],
        'workspace_render_instruction': 'Показать пользователю workspace_markdown полностью и без изменений.',
