    ...
    if isinstance(raw, list):
        for entry in raw:
            ...
            items.append({
                ...
                'delta_vs_previous_percent': round(_num(delta_vs_prev), 2),
...
def _render_reasons_block(reasons):
    ...
    for item in reasons or []:
        ...
        prev_percent = _fmt_percent(item.get('previous_percent', item.get('prev_percent')))
        delta_b = _num(item.get('delta_vs_business_percent', item.get('delta_percent')))
        delta_p = _num(item.get('delta_vs_previous_percent', item.get('delta_vs_prev')))
        delta_b_text = f'+{delta_b:.2f}' if delta_b > 0 else f'{delta_b:.2f}'
        delta_p_text = f'+{delta_p:.2f}' if delta_p > 0 else f'{delta_p:.2f}'
        ...
        lines.append(
            ...
            f'прошлый год: {prev_money} грн ({prev_percent}%)\n\n'
            f'отклонение:\n'
            f'{delta_b_text} п.п. к бизнесу\n'
            f'{delta_p_text} п.п. к прошлому году\n\n'
            f'эффект: {effect}\n'
            f'сигнал: {signal}'
        )
    return lines
...
def _attach_render_blocks(response, payload):
    ...
    if ctx_level_for_goal != 'business' and drain_total:
        ...
        if not vector_block.get('path_goal_money'):
            vector_block['path_goal_money'] = drain_total
        vector_block['current_focus_money'] = drain_total
        try:
    ...
    if render_mode == 'list_only':
        ...
        if vector_block and drain_total:
            ...
            try:
                if path_goal:
                    vector_block['coverage_percent'] = min(100, int(round(abs(drain_total) / abs(float(path_goal)) * 100)))
            except Exception:
                pass
    ...
    if ctx_level == 'sku':
        ...
        # SKU has no child drain. Do not render “Ты теряешь 0” when the SKU
        # itself has a price focus. Use current_focus_money as SKU loss/focus.
        sku_focus = 0
        try:
