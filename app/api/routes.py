...
def public_summary(payload):
    ...
    if level == 'sku':
        allowed = ('context', 'metrics', 'structure', 'drain_block', 'goal', 'focus_block', 'decision_block', 'provocation_block', 'all_block', 'cause_block', 'navigation')
        return {key: payload.get(key) for key in allowed if key in payload}
    if level == 'network':
        allowed = ('context', 'metrics', 'structure', 'drain_block', 'goal', 'focus_block', 'navigation')
        return {key: payload.get(key) for key in allowed if key in payload}
