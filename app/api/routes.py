...
def _render_reasons_block(reasons):
    ...
    for item in reasons or []:
        ...
        suffix = f'; сигнал: {signal}' if signal else ''
        lines.append(f'{name}: {percent} vs {base_percent} → {delta_text} п.п.; эффект {effect}{suffix}')
    return lines
...
def _render_decision_block(response):
    ...
    if level == 'sku':
        if object_name:
            lines.append(f'Проверка SKU: {object_name} → закрыть {focus_money}')
        lines.append(_price_percent_text(response))
        lines.append('Действие: поднять цену или убрать SKU из убыточного промо')
        return lines
