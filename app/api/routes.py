...
def _build_benchmark_driven_summary(response: dict) -> str:
    ...
    if level == 'business':
        if fin_delta > 0:
            return f'Бизнес улучшил результат к прошлому году на {_fmt_int(fin_delta)}. Основной потенциал возврата показывают объекты в рейтинге возможностей.'
        if fin_delta < 0:
    ...
    if product_level:
        if obj_result < 0:
            result_text = f'Относительно SKU Benchmark остаётся недозаработанный результат {_fmt_int(abs(obj_result))}.'
        elif obj_result > 0:
            result_text = f'Относительно SKU Benchmark объект даёт дополнительный результат {_fmt_int(obj_result)}.'
        else:
            result_text = 'Отклонение относительно SKU Benchmark не выявлено.'
        if opportunity > 0:
            opportunity_text = f'Внутри продуктового уровня находится потенциал возврата {_fmt_int(opportunity)} через проверку SKU Benchmark.'
        else:
            opportunity_text = 'Существенный потенциал возврата на этом продуктовом уровне не выявлен.'
    ...
    if opportunity > 0:
        opportunity_text = f'Внутри объекта находится потенциал возврата прибыли {_fmt_int(opportunity)}.'
    else:
        opportunity_text = 'Существенный потенциал возврата внутри объекта не выявлен.'
    return f'{yoy_text} {result_text} {opportunity_text}'
...
def _build_explanation_block(response: dict) -> list:
    ...
    opportunity = _num(response.get('opportunity_money'))
    product_level = _is_product_layer_level(level)
    benchmark_name = 'SKU Benchmark' if product_level else 'средней эффективности бизнеса'
    benchmark_subject = 'SKU Benchmark' if product_level else 'Средняя эффективность бизнеса'
    if obj_result < 0:
    ...
    lines.extend(['Результат объекта — что это означает?', result_text])

    if opportunity > 0:
        if product_level:
            opportunity_text = f'Внутри продуктового уровня есть позиции ниже SKU Benchmark. Проверка SKU может вернуть до {_fmt_int(opportunity)} результата.'
        else:
            opportunity_text = f'Внутри объекта есть точки ниже средней эффективности бизнеса. При доведении их до бизнес-уровня можно вернуть до {_fmt_int(opportunity)} результата.'
    else:
        opportunity_text = 'Внутри объекта не найден существенный денежный потенциал для возврата прибыли.'
    lines.extend(['Потенциал возврата прибыли — что это означает?', opportunity_text])
    return lines
...
def _build_next_step_block(response: dict) -> list:
    ...
    if _is_product_layer_level(level):
        return ['Рекомендуемый следующий шаг: открыть SKU ниже и проверить отклонение от SKU Benchmark.']
    ...
    return ['Рекомендуемый следующий шаг: проверить причины и подтвердить, что значимый потенциал возврата отсутствует.']
