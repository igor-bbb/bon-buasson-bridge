...
def _stage7_screen_order(response: dict) -> list:
    ...
    if level == 'sku':
        return ['sku_passport_block', 'factor_change_block', 'benchmark_diagnostic_block', 'decision_block_render', 'navigation_block']
...
def _attach_sku_passport_if_missing(payload):
    ...
    try:
        ...
        for key in ('sku_passport', 'sku_passport_block'):
            if rebuilt.get(key):
