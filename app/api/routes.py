...
def _build_opportunity_explanation_block(response: dict) -> list:
    ...
    for reason in reasons:
        ...
        if effect < 0:
            negative_gaps.append((abs(effect), reason))
    ...
    negative_gaps.sort(key=lambda x: x[0], reverse=True)
    base_label = _product_compare_base_label(response)
    lines = ['🔍 Почему сформирован потенциал']
    if opportunity > 0:
        lines.append(f'Потенциал {_fmt_int(opportunity)} формируется за счёт отклонений от {base_label}.')
    else:
        lines.append(f'Потенциал формируется за счёт отклонений от {base_label}.')

    for money, reason in negative_gaps[:5]:
        name = _reason_display_name(reason)
        delta = _num(reason.get('delta_vs_business_percent', reason.get('delta_percent')))
        lines.append(f'{name}: {_fmt_int(money)}')
        lines.append(f'Объект хуже, чем {base_label}, на {_fmt_pp_delta(delta)}. Потенциальный эффект выравнивания: {_fmt_int(money)}.')
