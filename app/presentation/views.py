...
def _summary_metric_value(metric_entry: Any, *, money_key: str = 'fact_money', percent_key: str = 'fact_percent') -> Any:
    if isinstance(metric_entry, dict):
        ...
        fallback_money_key = 'value_money' if money_key == 'fact_money' else money_key
        fallback_percent_key = 'value_percent' if percent_key == 'fact_percent' else percent_key
        if fallback_money_key in metric_entry:
            return metric_entry.get(fallback_money_key)
        if fallback_percent_key in metric_entry:
            return metric_entry.get(fallback_percent_key)
    return metric_entry
...
def _structure_from_summary(payload: Dict[str, Any]) -> Dict[str, Any]:
    source = payload.get('structure') or {}
    ...
    for key, item in source.items():
        if not isinstance(item, dict):
            continue
        fact_money = item.get('fact_money', item.get('value_money'))
        fact_percent = item.get('fact_percent', item.get('value_percent'))
        result[key] = {
            **item,
            'fact_money': _round(fact_money),
            'fact_percent': _round(fact_percent),
            'base_percent': _round(item.get('base_percent')),
            'delta_percent': _round(item.get('delta_percent')),
            'effect_money': _round(item.get('effect_money')),
        }
    return result
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for field in metric_fields:
        ...
        entry = {
            ...
            'fact_money' if field not in PP_FIELDS else 'fact_percent': value,
            'delta_money': delta_money,
            'delta_percent': _round(delta_percent),
        }
        if field in PP_FIELDS:
            entry['prev_year_percent'] = base_value
        else:
            entry['prev_year_money'] = base_value
        if level != 'business':
    ...
    return {
        ...
        'structure': _structure_from_summary(payload),
