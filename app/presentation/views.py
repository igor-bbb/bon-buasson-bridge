...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    response: Dict[str, Any] = {
        ...
        'drain_block': payload.get('drain_block') if isinstance(payload.get('drain_block'), list) else [],
    ...
    for optional_key, default_value in optional_defaults.items():
        if optional_key in payload:
            response[optional_key] = payload.get(optional_key) or default_value
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    if isinstance(payload, dict) and all(key in payload for key in ('context', 'metrics', 'goal', 'navigation')):
        direct = {
        ...
        level = ((payload.get('context') or {}).get('level') or payload.get('level'))
        if level == 'sku':
            direct['structure'] = payload.get('structure') or {}
            direct['focus_block'] = payload.get('focus_block') or {}
            direct['decision_block'] = payload.get('decision_block') or []
            direct['provocation_block'] = payload.get('provocation_block') or []
            direct['drain_block'] = []
            return direct
        direct['structure'] = payload.get('structure') or {}
        direct['drain_block'] = payload.get('drain_block') or []
        if level == 'network':
            direct['focus_block'] = payload.get('focus_block') or {}
            direct['decision_block'] = payload.get('decision_block') or {}
        return direct
    ...
    if level == 'sku':
        ...
        direct['structure'] = {}
        direct['drain_block'] = drain_block
    return direct
...
def build_list_view(payload: Dict[str, Any], drill_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    return {
        ...
        'navigation': {
            ...
            'all': [item.get('object_name') for item in rows],
            'back': True,
        },
    }
...
def _screen_header(payload: Dict[str, Any]) -> Dict[str, Any]:
    context = payload.get('context') if isinstance(payload.get('context'), dict) else {}
    goal = payload.get('goal') if isinstance(payload.get('goal'), dict) else {}
    return {
        ...
        'goal': {
            'value_money': goal.get('value_money'),
            'type': goal.get('type'),
            'label': goal.get('label') or goal.get('goal_label'),
        },
    }


def _screen_drain(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    drain_items = []
    for item in (payload.get('drain_block') or []):
    ...
    navigation = payload.get('navigation') if isinstance(payload.get('navigation'), dict) else {}
    if payload.get('all_block') or navigation.get('has_all'):
        drain_items.append({'object_name': 'все', 'effect_money': None, 'type': 'all'})
    if payload.get('cause_block') or navigation.get('has_causes'):
        drain_items.append({'object_name': 'причины', 'effect_money': None, 'type': 'cause'})
    return drain_items


def _screen_micro_context(payload: Dict[str, Any]) -> Optional[List[str]]:
    items = []
    focus = payload.get('focus_block') if isinstance(payload.get('focus_block'), dict) else {}
    focus_items = focus.get('items') if isinstance(focus.get('items'), list) else []
    if focus_items:
        labels = []
        for item in focus_items[:2]:
            if isinstance(item, dict):
                label = item.get('article') or item.get('object_name') or item.get('sku')
                if label:
                    labels.append(str(label))
        if labels:
            items.append('Фокус: ' + ', '.join(labels))
    return items[:2] if items else None


def build_screen_view(payload: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        return payload
    screen = {
        'header': _screen_header(payload),
        'kpi': payload.get('metrics') if isinstance(payload.get('metrics'), dict) else {},
        'structure': payload.get('structure') if isinstance(payload.get('structure'), dict) else {},
        'drain': _screen_drain(payload),
    }
    micro_context = _screen_micro_context(payload)
    if micro_context:
        screen['micro_context'] = micro_context

    # preserve API data and navigation for runtime / GPT control flow
    screen['context'] = payload.get('context') if isinstance(payload.get('context'), dict) else {}
    screen['metrics'] = payload.get('metrics') if isinstance(payload.get('metrics'), dict) else {}
    screen['goal'] = payload.get('goal') if isinstance(payload.get('goal'), dict) else {}
    screen['navigation'] = payload.get('navigation') if isinstance(payload.get('navigation'), dict) else {}
    if 'all_block' in payload:
        screen['all_block'] = payload.get('all_block') or []
    if 'cause_block' in payload:
        screen['cause_block'] = payload.get('cause_block') or []
    if 'focus_block' in payload:
        screen['focus_block'] = payload.get('focus_block') or {}
    if 'decision_block' in payload:
        screen['decision_block'] = payload.get('decision_block') or []
    if 'provocation_block' in payload:
        screen['provocation_block'] = payload.get('provocation_block') or []
    return screen
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    if isinstance(payload, dict) and all(key in payload for key in ('context', 'metrics', 'goal', 'navigation')):
        ...
        for key in ('all_block', 'cause_block', 'focus_block', 'decision_block', 'provocation_block'):
            if key in payload:
                direct[key] = payload.get(key)
        return build_screen_view(direct)
