...
def _build_drain_rows(payload: Dict[str, Any], max_items: int = MAX_DRAIN_ITEMS) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        if not object_name or str(object_name).strip().lower() in {'пусто', 'без менеджера', 'none', 'null'}:
            continue
        ...
        if potential_money is None:
            biz_margin = _round(business_metrics.get('margin_pre'))
            potential_money = None if biz_margin is None or margin is None else _round((biz_margin - margin) / 100.0 * revenue)
        ...
        delta_prev_year = _prune_nones({'money': delta_py_money, 'percent': _round(delta_py_percent)})
        if delta_prev_year:
            row['delta_prev_year'] = delta_prev_year
        prepared.append(_prune_nones(row) or {})

    prepared.sort(key=lambda row: float(row.get('potential_money') or 0.0), reverse=True)
    negatives = prepared[:max(max_items, MIN_DRAIN_ITEMS)]
    if len(negatives) > max_items:
        negatives = negatives[:max_items]
    payload['_positive_items_preview'] = []
    payload['_negative_items_preview'] = negatives
    return negatives
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for field in metric_fields:
        ...
        base_money_field = 'finrez_final' if level == 'business' else 'finrez_pre'
        base_money_value = _round(metrics_raw.get(base_money_field))
        compare_money_value = _round((prev_raw if level == 'business' else business_raw).get(base_money_field))

        if field in {'margin_pre', 'markup'}:
            entry = {
                'value_percent': value,
                'delta_percent': _round(_pp_change(value, base_value)),
                'value_money': base_money_value,
                'delta_money': None if base_money_value is None or compare_money_value is None else _round(base_money_value - compare_money_value),
            }
        elif field == 'finrez_final' and level != 'business':
            continue
        else:
            entry = {
                'value_money': value,
