...
def _business_pnl_breakdown(metrics: Dict[str, Any], comparisons: Dict[str, Any]) -> List[Dict[str, Any]]:
    return _build_metric_rows('business', metrics, comparisons)
...
def _build_drain_rows(payload: Dict[str, Any], include_positive: bool = False, max_items: int = 5) -> List[Dict[str, Any]]:
    ...
    if include_positive:
        ...
        for bucket in (0, 1, 2, 3):
            ...
            if len(selected) >= max_items:
                break
        if len(prepared) >= MIN_DRAIN_ITEMS:
            selected = selected[:max(max_items if max_items < MIN_DRAIN_ITEMS else MIN_DRAIN_ITEMS, MIN_DRAIN_ITEMS)] if len(selected) >= MIN_DRAIN_ITEMS else selected
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    pnl_breakdown = _business_pnl_breakdown(metrics, comparisons) if level == 'business' else []
    return {
        'type': 'management',
        'mode': 'management',
        'view_mode': 'drain',
...
def build_list_view(scope_payload: Dict[str, Any], list_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    pnl_breakdown = _business_pnl_breakdown(metrics, comparisons) if level == 'business' else []
    full_items = list_payload.get('all_items') or list_payload.get('items') or []
    return {
        ...
        'items': _build_list_items(list_payload, max_items=len(full_items) or 100),
        'commands': _commands(level),
