...
def _build_context(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if goal_value is None:
        ...
        goal_text = f"закрыть {_format_money(goal_value)}"
    elif goal_value > 0:
        goal_text = f"удержать +{_format_money(goal_value).replace('−', '')}"
    else:
        goal_text = 'удержать текущий результат'
    return {
...
def _build_drain_rows(payload: Dict[str, Any], max_items: int = MAX_DRAIN_ITEMS) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        yoy_text = _compact_item_yoy(item)
        prepared.append({
            ...
            'signal_status': str(signal.get('status') or 'ok'),
            'signal_label': SIGNAL_LABELS.get(str(signal.get('status') or 'ok'), str(signal.get('status') or 'ok')),
            'yoy_display': yoy_text,
            'is_negative_for_business': float(gap_money or 0.0) > 0.0 or (finrez is not None and finrez < 0),
        })

    negatives = [row for row in prepared if row.get('is_negative_for_business')]
    negatives.sort(key=lambda row: float(row.get('gap_loss_money') or 0.0), reverse=True)
    total_negative = sum(float(row.get('gap_loss_money') or 0.0) for row in negatives)
    selected_negatives: List[Dict[str, Any]] = []
    covered = 0.0
    for row in negatives:
        selected_negatives.append(row)
        covered += float(row.get('gap_loss_money') or 0.0)
    ...
    positives = [row for row in prepared if not row.get('is_negative_for_business') and float(row.get('finrez_pre') or 0.0) > 0.0]
    positives.sort(key=lambda row: float(row.get('finrez_pre') or 0.0), reverse=True)
    positives = positives[:3]
    payload['_positive_items_preview'] = positives

    def _decorate(rows: List[Dict[str, Any]], total: float, positive: bool = False) -> List[Dict[str, Any]]:
        out = []
        for row in rows:
            value = abs(float(row.get('gap_loss_money') or row.get('finrez_pre') or 0.0))
            ...
            if positive:
                clean['line'] = f"{row['object_name']} | +{_format_money(abs(float(row.get('finrez_pre') or 0.0))).replace('−','')} | {row['margin_display']} | {row['yoy_display']} к ПГ"
            else:
                clean['line'] = f"{row['object_name']} | {row['gap_loss_money_display']} | {row['margin_display']} | {row['gap_to_business_pp_display']} к бизнесу"
            out.append(clean)
        return out

    return _decorate(selected_negatives, total_negative)
...
def _build_focus_block(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        'loss': _round(focus.get('loss')),
        'loss_display': _format_money(focus.get('loss')),
        'share': _round(focus.get('share')),
        'share_display': _format_percent(focus.get('share')),
        'line': f"Фокус: {focus_name} {_format_money(focus.get('loss'))} ({_format_percent(focus.get('share'))})",
    }
...
def _build_decision_view(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    goal_gap = block.get('goal_gap')
    if not actions and level != 'sku':
        return None
    for item in actions:
        item['effect_display'] = _format_money(item.get('effect'))
        item['delta_pp_display'] = _format_pp(item.get('delta_pp'))
        item['line'] = f"{item.get('action')} на {item.get('delta_pp_display')} → {item.get('effect_display')}"
    effect_total = _round(block.get('effect_total'))
    return {
        'actions': actions,
        'effect_total': effect_total,
        'effect_total_display': _format_money(effect_total),
        'goal_gap': _round(goal_gap),
        'goal_gap_display': _format_money(goal_gap),
        'goal_closed': bool(block.get('goal_closed')),
        'status_line': 'цель закрывается' if block.get('goal_closed') else 'цель не закрывается',
    }
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    drain_items = [] if level == 'sku' else _build_drain_rows(source, max_items=MAX_DRAIN_ITEMS)
    focus_block = _build_focus_block(payload)
    decision_block = _build_decision_view(payload)
    summary_lines = _build_summary_lines(level, payload, [item for item in drain_items if item.get('is_negative_for_business')])
