...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _metric_entry(field: str, percent_metric: bool = False) -> Dict[str, Any]:
        ...
        entry: Dict[str, Any] = {'delta_percent': _round(delta_percent)}
        if percent_metric:
            entry['value_percent'] = value
            # for percent metrics add money context as required by LOCK
        ...
        # additional informational delta_prev_year only below business and only when present
        if level != 'business':
