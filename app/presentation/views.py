...
def _ac_metric_money(field: str, current: Dict[str, Any]) -> Optional[float]:
    if field == 'markup':
        return _ac_num(current.get('revenue'))
    if field == 'margin_pre':
...
def _ac_gap_entry(current: Dict[str, Any], business: Dict[str, Any]) -> Dict[str, Any]:
    revenue = _ac_num(current.get('revenue'))
    obj_margin = _ac_num(current.get('margin_pre'))
    biz_margin = _ac_num(business.get('margin_pre'))
    effect = _ac_effect_money(biz_margin, obj_margin, revenue)
    delta_percent = None if biz_margin is None or obj_margin is None else _round(biz_margin - obj_margin)
    return _prune_nones({
        'value_money': effect,
        'value_percent': delta_percent,
        'delta_money': effect,
        'delta_percent': delta_percent,
    })
...
def _ac_metrics(payload: Dict[str, Any], level: str) -> Dict[str, Any]:
    ...
    metrics['gap'] = {} if level == 'business' else _ac_gap_entry(current, business)
...
def _ac_drain_block(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    ...
    for item in raw_items:
        ...
        out.append(_prune_nones({
            ...
            'gap_to_business': {'value_money': potential_money, 'value_percent': gap_percent},
