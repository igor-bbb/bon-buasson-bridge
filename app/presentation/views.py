...
def build_reasons_view(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for row in reason_rows:
        ...
        row['lines'] = [
            ...
            f"К бизнесу: {row['business_value_display']}",
            f"Потеря: {row['impact_value_display']}",
            f"Δ: {row['gap_pp_display']}",
        ]
        row['line'] = f"{row['factor_label']} факт {row['fact_value_display']} к бизнесу {row['business_value_display']} потеря {row['impact_value_display']} Δ {row['gap_pp_display']}"
