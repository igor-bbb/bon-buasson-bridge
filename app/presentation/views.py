...
def _build_top_summary(prepared_items: List[Dict[str, Any]]) -> Dict[str, Any]:
    if not prepared_items:
        return {'top_items': [], 'focus_object': None, 'concentration_share': None}

    negative_items = [item for item in prepared_items if float(item.get('finrez_pre', 0.0) or 0.0) < 0]
    ordered = negative_items if negative_items else prepared_items
    top_items = ordered[:5]
    total_negative = abs(sum(float(item.get('finrez_pre', 0.0) or 0.0) for item in negative_items))
    top_negative = abs(sum(float(item.get('finrez_pre', 0.0) or 0.0) for item in top_items if float(item.get('finrez_pre', 0.0) or 0.0) < 0))
    concentration_share = round(top_negative / total_negative, 4) if total_negative else None
    return {
        'top_items': [
            {
                ...
                'signal': item.get('signal', {}).get('status'),
                'priority': item.get('signal', {}).get('priority'),
            }
...
def build_management_view(comparison_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'action': {
            ...
            'next_step': _NEXT_STEP_LABELS.get(comparison_payload.get('level'), comparison_payload.get('action', {}).get('next_step')), 
        },
        'management': {
...
def _build_compact_drill_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'finrez_pre': management_item.get('basis', {}).get('finrez_pre'),
...
def build_comparison_management_view(query: Dict[str, Any], current: Dict[str, Any], previous: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'navigation': {'kpi_gap_current': current.get('navigation', {}).get('kpi_gap', 0.0), 'kpi_gap_previous': previous.get('navigation', {}).get('kpi_gap', 0.0), 'delta_kpi_gap': delta_kpi_gap},
        'context': {'margin_pre_current': current_margin_pre, 'margin_pre_previous': previous_margin_pre, 'margin_gap_current': current_margin_gap, 'margin_gap_previous': previous_margin_gap},
        'diagnosis_change': {'items': diagnosis_changes[:MAX_REASON_ITEMS], 'main_driver_metric': main_change.get('metric'), 'main_driver_label': main_change.get('label'), 'main_driver_delta': main_change.get('delta_value'), 'main_driver_direction': main_change.get('direction')},
        'impact': {'delta_finrez_pre': delta_finrez, 'delta_gap_loss_money': delta_gap_loss_money, 'main_driver_metric': main_change.get('metric')},
...
def build_signal_flow_view(summary_payload: Dict[str, Any], drilldown_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'action': {
            ...
            'next_step': summary_view.get('action', {}).get('next_step'),
        },
    }
