...
# ===== FINAL CORE PIPELINE OVERRIDES =====
CORE_METRIC_ORDER = ['revenue', 'markup', 'finrez_pre', 'margin_pre']
CORE_STRUCTURE_ORDER = ['markup', 'retro_bonus', 'logistics_cost', 'personnel_cost', 'other_costs']
CORE_STRUCTURE_KEYS = {
    'markup': 'markup',
    'retro_bonus': 'retro',
...
def _core_money_value(field: str, metrics: Dict[str, Any]) -> Optional[float]:
    if field == 'markup':
        gross_profit = _to_num(metrics.get('gross_profit'))
        if gross_profit is not None and gross_profit > 0:
            return gross_profit
        revenue = _to_num(metrics.get('revenue'))
        cost = _to_num(metrics.get('cost'))
        if revenue is not None and cost is not None:
            diff = _round(revenue - cost)
            if diff is not None:
                return diff
        markup_pct = _to_num(metrics.get('markup'))
        if revenue is not None and markup_pct is not None and markup_pct > -100:
            return _round(revenue * (markup_pct / (100.0 + markup_pct)))
        return None
    if field == 'margin_pre':
        return _to_num(metrics.get('finrez_pre'))
    if field == 'finrez_final':
        return _to_num(metrics.get('finrez_final'))
    return _to_num(metrics.get(field))


def _core_percent_value(field: str, metrics: Dict[str, Any]) -> Optional[float]:
    revenue = _to_num(metrics.get('revenue'))
    if field == 'revenue':
        return None
    if field in {'markup', 'margin_pre'}:
        return _to_num(metrics.get(field))
    value_money = _core_money_value(field, metrics)
    if value_money is None or revenue in (None, 0):
        return None
    return _round((value_money / revenue) * 100.0)


def _core_metric_entry(field: str, current: Dict[str, Any], baseline: Dict[str, Any], *, baseline_mode: str) -> Dict[str, Any]:
    value_money = _core_money_value(field, current)
    baseline_money = _core_money_value(field, baseline)
    value_percent = _core_percent_value(field, current)
    baseline_percent = _core_percent_value(field, baseline)
    delta_money = None if value_money is None or baseline_money is None else _round(value_money - baseline_money)
    if field in {'markup', 'margin_pre'}:
        delta_percent = _pp_change(value_percent, baseline_percent)
    else:
        if baseline_mode == 'previous_year':
            raw_current = _to_num(current.get(field if field != 'margin_pre' else 'finrez_pre'))
            raw_previous = _to_num(baseline.get(field if field != 'margin_pre' else 'finrez_pre'))
            if field == 'revenue':
                raw_current = _to_num(current.get('revenue'))
                raw_previous = _to_num(baseline.get('revenue'))
            delta_percent = _percent_change(raw_current if field not in {'markup', 'margin_pre'} else value_percent, raw_previous if field not in {'markup', 'margin_pre'} else baseline_percent)
        else:
            delta_percent = _pp_change(value_percent, baseline_percent)
    out = {'value_money': value_money, 'delta_money': delta_money, 'delta_percent': delta_percent}
    if value_percent is not None:
        out['value_percent'] = value_percent
    return _prune_nones(out)


def _core_gap_entry(current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
    revenue = _to_num(current.get('revenue'))
    baseline_revenue = _to_num(baseline.get('revenue'))
    markup = _to_num(current.get('markup'))
    margin_pre = _to_num(current.get('margin_pre'))
    baseline_markup = _to_num(baseline.get('markup'))
    baseline_margin_pre = _to_num(baseline.get('margin_pre'))
    value_percent = None if markup is None or margin_pre is None else _round(markup - margin_pre)
    baseline_percent = None if baseline_markup is None or baseline_margin_pre is None else _round(baseline_markup - baseline_margin_pre)
    value_money = None if value_percent is None or revenue is None else _round((value_percent / 100.0) * revenue)
    baseline_money = None if baseline_percent is None or baseline_revenue is None else _round((baseline_percent / 100.0) * baseline_revenue)
    return _prune_nones({
...
def _core_metrics(payload: Dict[str, Any], level: str) -> Dict[str, Any]:
    current = _safe_metrics(payload)
    previous = _safe_previous_metrics(payload)
    baseline = previous if level == 'business' else previous
    metrics = {field: _core_metric_entry(field, current, baseline, baseline_mode='previous_year') for field in CORE_METRIC_ORDER}
    metrics['gap'] = _core_gap_entry(current, baseline)
    if level == 'business':
        metrics['finrez_final'] = _core_metric_entry('finrez_final', current, previous, baseline_mode='previous_year')
    return _prune_nones(metrics)


def _core_structure_entry(field: str, current: Dict[str, Any], baseline: Dict[str, Any]) -> Dict[str, Any]:
    value_money = _core_money_value(field, current)
    baseline_money = _core_money_value(field, baseline)
    value_percent = _core_percent_value(field, current)
    baseline_percent = _core_percent_value(field, baseline)
...
def _core_structure(payload: Dict[str, Any], level: str) -> Dict[str, Any]:
    current = _safe_metrics(payload)
    baseline = _safe_previous_metrics(payload) if level == 'business' else _safe_business_metrics(payload)
    out = {}
    for field in CORE_STRUCTURE_ORDER:
        out[CORE_STRUCTURE_KEYS[field]] = _core_structure_entry(field, current, baseline)
    return _prune_nones(out)




def _core_effect_money(business_percent: Any, object_percent: Any, revenue: Any) -> Optional[float]:
    bp = _to_num(business_percent)
    op = _to_num(object_percent)
    rev = _to_num(revenue)
    if bp is None or op is None or rev is None:
        return None
    return _round(((bp - op) / 100.0) * rev)

def _core_drain_block(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> List[Dict[str, Any]]:
    source = drain_payload if drain_payload is not None else payload
    raw_items = source.get('all_items') or source.get('items') or []
    out: List[Dict[str, Any]] = []
    for item in raw_items:
        ...
        margin = _to_num(metrics.get('margin_pre'))
        biz_margin = _to_num(business.get('margin_pre'))
        effect_money = _core_effect_money(biz_margin, margin, revenue)
        if effect_money is None or effect_money <= 0:
            continue
        prev_finrez = _to_num(prev.get('finrez_pre'))
        prev_margin = _to_num(prev.get('margin_pre'))
        delta_py_money = None if finrez is None or prev_finrez is None else _round(finrez - prev_finrez)
        delta_py_percent = _pp_change(margin, prev_margin)
        gap_percent = None if biz_margin is None or margin is None else _round(biz_margin - margin)
        out.append(_prune_nones({
            ...
            'gap_to_business': {'value_percent': gap_percent},
            'effect_money': effect_money,
...
def _core_goal(payload: Dict[str, Any], level: str, drain_block: List[Dict[str, Any]]) -> Dict[str, Any]:
    current = _safe_metrics(payload)
    previous = _safe_previous_metrics(payload)
    if level == 'business':
        current_finrez = _to_num(current.get('finrez_final'))
        previous_finrez = _to_num(previous.get('finrez_final'))
        return _prune_nones({
            'delta_money': None if current_finrez is None or previous_finrez is None else _round(current_finrez - previous_finrez),
            'delta_percent': _percent_change(current_finrez, previous_finrez),
        })
    total = _round(sum(float(item.get('effect_money') or 0.0) for item in drain_block))
    revenue = _to_num(current.get('revenue'))
    return _prune_nones({
        'delta_money': total,
        'delta_percent': None if revenue in (None, 0) else _round((total / revenue) * 100.0),
    })


def _core_navigation(level: str, drain_block: List[Dict[str, Any]], drain_payload: Optional[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        'current_level': level,
        'next_level': _children_level_from_payload(drain_payload),
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    level = str(payload.get('level') or '')
    drain_block = [] if level == 'sku' else _core_drain_block(payload, drain_payload)
    return _prune_nones({
        'context': {
            'level': level,
            'object_name': payload.get('object_name'),
        ...
        'metrics': _core_metrics(payload, level),
        'structure': _core_structure(payload, level),
        'drain_block': drain_block,
        'goal': _core_goal(payload, level, drain_block),
        'navigation': _core_navigation(level, drain_block, drain_payload),
    })
