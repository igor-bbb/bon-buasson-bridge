...
def _build_context(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not goal:
        ...
        if level == 'business':
            ...
            base_finrez = _round(previous.get('finrez_pre'))
...
def _build_focus_block(payload: Dict[str, Any]) -> Any:
    direct_focus = payload.get('focus_block')
    if isinstance(direct_focus, list):
        return direct_focus
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    drain_block = _build_drain_rows(source, max_items=MAX_DRAIN_ITEMS)
    ...
    return {
        ...
        'drain_total': payload.get('drain_total'),
