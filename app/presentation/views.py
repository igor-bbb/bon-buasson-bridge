...
def _gap_markup_pp(payload: Dict[str, Any]) -> Optional[float]:
    object_markup = _safe_metrics(payload).get('markup')
    business_markup = _safe_business_metrics(payload).get('markup')
    return _pp_change(object_markup, business_markup)


def _gap_finrez_value(payload: Dict[str, Any]) -> Optional[float]:
    object_finrez = _safe_metrics(payload).get('finrez_pre')
    business_finrez = _safe_business_metrics(payload).get('finrez_pre')
    if object_finrez is None or business_finrez is None:
        return None
    return _round(float(object_finrez) - float(business_finrez))


def _markup_impact_value(payload: Dict[str, Any]) -> float:
    object_metrics = _safe_metrics(payload)
    business_metrics = _safe_business_metrics(payload)
    cost = float(object_metrics.get('cost') or 0.0)
    object_markup = object_metrics.get('markup')
    business_markup = business_metrics.get('markup')
    if cost <= 0 or object_markup is None or business_markup is None:
        return 0.0
    gap_pp = float(business_markup) - float(object_markup)
    return abs((gap_pp / 100.0) * cost)
...
def _build_vs_business(level: str, payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        'gap_markup_pp': _gap_markup_pp(payload),
        'gap_finrez_value': _gap_finrez_value(payload),
...
def _build_drain_rows(payload: Dict[str, Any], include_positive: bool = False, max_items: int = 5) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        if finrez is None:
            continue
        gap_pp = _gap_margin_pp(item)
        prepared.append({
            'object_name': item.get('object_name'),
            'finrez_pre': _round(finrez),
            'margin_pre': _round(metrics.get('margin_pre')),
            'revenue': _round(metrics.get('revenue')),
            'signal_status': str((item.get('signal') or {}).get('status') or 'ok'),
            'gap_to_business_pp': gap_pp,
            'gap_to_business_pp_display': _format_pp(gap_pp),
            '_bucket': _drain_priority_bucket(item),
        })

    prepared.sort(key=lambda row: row['_bucket'])
    negative_total = sum(abs(float(row.get('finrez_pre') or 0.0)) for row in prepared if (row.get('finrez_pre') or 0) < 0)

    if include_positive:
        selected = sorted(prepared, key=lambda row: float(row.get('finrez_pre') or 0.0))[:max_items]
    else:
        selected = [row for row in prepared if row['_bucket'][0] == 0][:max_items]
        if len(selected) < MIN_DRAIN_ITEMS:
            for bucket in (1, 2, 3):
                for row in prepared:
                    if row['_bucket'][0] != bucket or row in selected:
                        continue
                    selected.append(row)
    ...
    for row in selected:
        ...
        clean.update({
            ...
            'line': f"{row.get('object_name')} { _format_money(finrez) } { _format_percent(share) if finrez < 0 and negative_total else '—' } { _format_pp(row.get('gap_to_business_pp')) }",
        })
        out.append(clean)
    return out
...
def _drain_priority_bucket(item: Dict[str, Any]) -> tuple[int, float]:
    metrics = (item.get('metrics') or {}).get('object_metrics') or {}
    finrez = float(metrics.get('finrez_pre') or 0.0)
    status = str((item.get('signal') or {}).get('status') or 'ok')
    if status == 'critical' and finrez < 0:
        return (0, finrez)
    if status == 'risk' and finrez < 0:
        return (1, finrez)
    if finrez < 0:
        return (2, finrez)
    return (3, abs(finrez))
...
def _build_list_items(list_payload: Dict[str, Any], max_items: int = 100) -> List[Dict[str, Any]]:
    ...
    items = [item for item in raw_items if isinstance(item, dict)]
    items.sort(key=lambda it: float(((it.get('metrics') or {}).get('object_metrics') or {}).get('finrez_pre') or 0.0))
    out: List[Dict[str, Any]] = []
    for item in items[:max_items]:
        ...
        row['line'] = f"{row['object_name']} {row['finrez_pre_display']} {row['margin_pre_display']} {row['yoy_display']}"
        out.append(row)
    return out
...
def build_list_view(scope_payload: Dict[str, Any], list_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'items': _build_list_items(list_payload, max_items=100),
        'commands': _commands(level),
...
def build_reasons_view(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    negative_total = 0.0
    for factor in ['markup', 'retro_bonus', 'logistics_cost', 'personnel_cost', 'other_costs']:
        if factor == 'markup':
            fact_value = float(object_metrics.get('markup') or 0.0)
            business_percent = float(business_metrics.get('markup') or 0.0)
            fact_percent = fact_value
            impact_value = _markup_impact_value(payload)
            is_negative = fact_percent < business_percent
            business_value = business_percent
        else:
        ...
        reason_rows.append({
            ...
            'fact_value_display': _format_money(fact_value) if factor != 'markup' else _format_percent(fact_value),
    ...
    for idx, row in enumerate(reason_rows):
        ...
        if row['factor'] == 'markup':
            row['lines'] = [
                f"Факт {row['fact_percent_display']}",
