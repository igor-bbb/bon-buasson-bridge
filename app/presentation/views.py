...
def _build_context(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if level == 'business':
        current_finrez_pre = _round(metrics.get('finrez_pre'))
        base_finrez_pre = _round(previous.get('finrez_pre'))
    else:
        current_finrez_pre = _round(metrics.get('finrez_pre'))
        base_finrez_pre = _round(previous.get('finrez_pre'))

    goal_value = None if current_finrez_pre is None or base_finrez_pre is None else _round(current_finrez_pre - base_finrez_pre)
    if goal_value is None:
        goal = {'type': 'unknown', 'value_money': None}
    elif goal_value < 0:
        goal = {'type': 'close_gap', 'value_money': goal_value}
    else:
        goal = {'type': 'keep_growth', 'value_money': goal_value}
    return {
...
def _build_focus_block(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if not focus_type or not focus_name:
        return None
    raw_share = focus.get('share') or 0.0
    try:
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    money_source_map = {
        'margin_pre': 'finrez_pre',
        'markup': 'gross_profit',
    }

    for field in metric_fields:
        raw_field = 'kpi_gap' if field == 'gap' and 'gap' not in metrics_raw else field
        value = _round(metrics_raw.get(raw_field))
        base_raw = prev_raw if level == 'business' else business_raw
        base_value = _round(base_raw.get(raw_field))
        entry: Dict[str, Any] = {}

        if field in PP_FIELDS:
            entry['value_percent'] = value
            entry['delta_percent'] = _round(_pp_change(value, base_value))
            if field != 'gap':
                money_field = money_source_map.get(field, field)
                money_value = _round(metrics_raw.get(money_field))
                money_base = _round(base_raw.get(money_field))
                entry['value_money'] = money_value
                entry['delta_money'] = None if money_value is None or money_base is None else _round(money_value - money_base)
        else:
            entry['value_money'] = value
            entry['delta_money'] = None if value is None or base_value is None else _round(value - base_value)
            entry['delta_percent'] = _round(_percent_change(value, base_value))

        if level != 'business':
            py_base = _round(prev_raw.get(raw_field))
            if field in PP_FIELDS:
                py_percent = _round(_pp_change(value, py_base))
                delta_prev_year = {}
                if py_percent is not None:
                    delta_prev_year['percent'] = py_percent
                if field != 'gap':
                    money_field = money_source_map.get(field, field)
                    money_value = _round(metrics_raw.get(money_field))
                    py_money_base = _round(prev_raw.get(money_field))
                    if money_value is not None and py_money_base is not None:
                        delta_prev_year['money'] = _round(money_value - py_money_base)
                if delta_prev_year:
                    entry['delta_prev_year'] = delta_prev_year
            else:
                py_delta_money = None if value is None or py_base is None else _round(value - py_base)
                py_delta_percent = _round(_percent_change(value, py_base))
                delta_prev_year = {}
                if py_delta_money is not None:
                    delta_prev_year['money'] = py_delta_money
                if py_delta_percent is not None:
                    delta_prev_year['percent'] = py_delta_percent
                if delta_prev_year:
                    entry['delta_prev_year'] = delta_prev_year
...
def build_list_view(scope_payload: Dict[str, Any], list_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'items': items,
        'commands': ['1', '2', '3', 'назад'],
    }

def build_losses_view_from_children(comparison_payload: Dict[str, Any]) -> Dict[str, Any]:
    return build_object_view(comparison_payload)
