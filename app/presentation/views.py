...
PP_FIELDS = {'margin_pre', 'markup', 'kpi_gap', 'gap'}
...
def _metric_money_field(metric_name: str) -> Optional[str]:
    if metric_name == 'margin_pre':
        return 'finrez_pre'
    if metric_name == 'markup':
        return 'gross_profit'
    return None


def _metric_entry(field: str, value: Any, base_value: Any, metrics_raw: Dict[str, Any], base_raw: Dict[str, Any]) -> Dict[str, Any]:
    entry: Dict[str, Any] = {}

    if field in PP_FIELDS:
        entry['value_percent'] = _round(value)
        delta_percent = _pp_change(value, base_value)
        if delta_percent is not None:
            entry['delta_percent'] = _round(delta_percent)

        money_field = _metric_money_field(field)
        if money_field:
            value_money = _round(metrics_raw.get(money_field))
            base_money = _round(base_raw.get(money_field))
            ...
            if delta_money is not None:
                entry['delta_money'] = delta_money
    else:
        value_money = _round(value)
        base_money = _round(base_value)
        delta_money = None if value_money is None or base_money is None else _round(value_money - base_money)
        if value_money is not None:
            entry['value_money'] = value_money
        if delta_money is not None:
            entry['delta_money'] = delta_money
        delta_percent = _percent_change(value_money, base_money)
        if delta_percent is not None:
            entry['delta_percent'] = _round(delta_percent)

    return _drop_none(entry)
...
def _build_drain_rows(payload: Dict[str, Any], max_items: int = MAX_DRAIN_ITEMS) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        revenue = _round(metrics.get('revenue'))
        business_margin = _round(business_metrics.get('margin_pre'))
        gap_to_business_pp = None if margin is None or business_margin is None else _round(margin - business_margin)
        ...
        if not item.get('object_name'):
            continue
        if revenue is None or abs(float(revenue or 0.0)) < 1e-9:
            continue
        if float(potential_money or 0.0) <= 0.0:
            continue

        prepared.append(_drop_none({
    ...
    prepared.sort(key=lambda row: float(row.get('potential_money') or 0.0), reverse=True)
    total_negative = sum(float(row.get('potential_money') or 0.0) for row in prepared)
    selected: List[Dict[str, Any]] = []
    covered = 0.0
    for row in prepared:
        selected.append(row)
        covered += float(row.get('potential_money') or 0.0)
        if len(selected) >= MIN_DRAIN_ITEMS and (covered >= total_negative * DRAIN_SHARE_THRESHOLD or len(selected) >= max_items):
            break
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    for field in metric_fields:
        value = metrics_raw.get(field)
        base_raw = prev_raw if level == 'business' else business_raw
        base_value = base_raw.get(field)
        entry = _metric_entry(field, value, base_value, metrics_raw, base_raw)
        if level != 'business':
            py_entry = _metric_entry(field, value, prev_raw.get(field), metrics_raw, prev_raw)
            if py_entry:
    ...
    response = {
        ...
        'goal': _drop_none(context_block.get('goal') or {}),
        'focus_block': _drop_none(focus_block) if isinstance(focus_block, dict) else {},
        'navigation': _drop_none(navigation),
    }
