...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    def _metric_entry(field: str, percent_metric: bool = False) -> Dict[str, Any]:
        ...
        if percent_metric:
            ...
            base_money = _round(prev_raw.get('finrez_pre' if field == 'margin_pre' else 'revenue')) if level == 'business' else _round(business_raw.get('finrez_pre' if field == 'margin_pre' else 'revenue'))
            entry['value_money'] = value_money
            if value_money is not None and base_money is not None:
                entry['delta_money'] = _round(value_money - base_money)
            ...
            if value is not None and base_value is not None:
                entry['delta_money'] = _round(value - base_value)
            if delta_percent is not None:
                entry['delta_percent'] = _round(delta_percent)
        if level != 'business':
            ...
            py_entry = {}
            if not percent_metric and value is not None and py_base is not None:
                py_entry['money'] = _round(value - py_base)
            if py_delta_percent is not None:
                py_entry['percent'] = _round(py_delta_percent)
            py_entry = _prune_nones(py_entry)
            if py_entry:
                entry['delta_prev_year'] = py_entry
    ...
    gap_base = _round(prev_raw.get('kpi_gap') if prev_raw.get('kpi_gap') is not None else ((prev_raw.get('markup') or 0) - (prev_raw.get('margin_pre') or 0))) if level == 'business' else _round(business_raw.get('kpi_gap') if business_raw.get('kpi_gap') is not None else ((business_raw.get('markup') or 0) - (business_raw.get('margin_pre') or 0)))
    gap_entry = {}
    if gap_value is not None:
        gap_entry['value_percent'] = gap_value
    if gap_value is not None and gap_base is not None:
        gap_entry['delta_percent'] = _round(gap_value - gap_base)
    if gap_entry:
        metrics['gap'] = gap_entry
    ...
    for item in items:
        ...
        if potential is None and business_margin is not None and item_margin is not None:
        ...
        delta_prev_year = {}
        if item_finrez is not None and prev_finrez is not None:
            delta_prev_year['money'] = _round(item_finrez - prev_finrez)
        pct = _percent_change(item_finrez, prev_finrez)
        if pct is not None:
            delta_prev_year['percent'] = pct
        ...
        if delta_prev_year:
            drain_item['delta_prev_year'] = delta_prev_year
    ...
    if len(drain_block) > MAX_DRAIN_ITEMS:
        drain_block = drain_block[:MAX_DRAIN_ITEMS]

    goal = _prune_nones(context_block.get('goal') or {}) or {}
...
def build_list_view(scope_payload: Dict[str, Any], list_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    items = [item for item in raw_items if isinstance(item, dict)]
    drain_block: List[Dict[str, Any]] = []
    for item in items[:MAX_DRAIN_ITEMS]:
        name = item.get('object_name')
        if not name:
            continue
        metrics = (item.get('metrics') or {}).get('object_metrics') or {}
        ...
        entry = _prune_nones({
            'object_name': name,
            'fact': fact,
            'gap_to_business_pp': _round(metrics.get('margin_pre')),
            'potential_money': _round(((item.get('impact') or {}).get('gap_loss_money'))),
        }) or {}
        if entry:
            drain_block.append(entry)
    return {
        'context': _prune_nones({
            'path': context_block.get('path'),
            'period': context_block.get('period_label'),
        ...
        'metrics': {},
        'drain_block': [i for i in drain_block if i],
        'goal': {},
        'focus_block': {},
        'navigation': {
            'current_level': level,
            'next_level': list_payload.get('children_level') or list_payload.get('level'),
            'items': [i.get('object_name') for i in drain_block if i.get('object_name')],
            'all': True,
            'reasons': True,
            'back': True,
        },
