...
def _ac_metric_money(field: str, current: Dict[str, Any]) -> Optional[float]:
    if field == 'markup':
        ...
        cost = _ac_num(current.get('cost'))
        if revenue is not None and cost is not None:
            return _round(revenue - cost)
        markup_pct = _ac_num(current.get('markup'))
        if revenue is not None and markup_pct is not None and markup_pct > -100:
            # markup is a percent over cost, so gross profit = revenue * markup / (100 + markup)
...
def _ac_metric_entry(field: str, current: Dict[str, Any], previous: Dict[str, Any]) -> Dict[str, Any]:
    ...
    previous_value_money = _ac_metric_money(field, previous)
    value_percent = None if field == 'revenue' else _ac_metric_percent(field, current)
    if field in {'markup', 'margin_pre'}:
        delta_money = None if value_money is None or previous_value_money is None else _round(value_money - previous_value_money)
...
def _ac_gap_entry(current: Dict[str, Any], previous: Dict[str, Any]) -> Dict[str, Any]:
    revenue = _ac_num(current.get('revenue'))
    previous_revenue = _ac_num(previous.get('revenue'))
    markup = _ac_num(current.get('markup'))
    margin_pre = _ac_num(current.get('margin_pre'))
    previous_markup = _ac_num(previous.get('markup'))
    previous_margin_pre = _ac_num(previous.get('margin_pre'))

    value_percent = None if markup is None or margin_pre is None else _round(markup - margin_pre)
    previous_value_percent = None if previous_markup is None or previous_margin_pre is None else _round(previous_markup - previous_margin_pre)
    value_money = None if value_percent is None or revenue is None else _round((value_percent / 100.0) * revenue)
    previous_value_money = None if previous_value_percent is None or previous_revenue is None else _round((previous_value_percent / 100.0) * previous_revenue)
    delta_money = None if value_money is None or previous_value_money is None else _round(value_money - previous_value_money)
    delta_percent = None if value_percent is None or previous_value_percent is None else _round(value_percent - previous_value_percent)
...
def _ac_metrics(payload: Dict[str, Any], level: str) -> Dict[str, Any]:
    ...
    metrics['gap'] = _ac_gap_entry(current, previous if level == 'business' else business)
