...
def _ac_metric_money(field: str, current: Dict[str, Any]) -> Optional[float]:
    if field == 'markup':
        gross_profit = _ac_num(current.get('gross_profit'))
        if gross_profit is not None:
            return gross_profit
        revenue = _ac_num(current.get('revenue'))
        markup = _ac_num(current.get('markup'))
        if revenue is None or markup is None:
            return None
        return _round(revenue * (markup / (100.0 + markup))) if (100.0 + markup) != 0 else None
    if field == 'margin_pre':
        return _ac_num(current.get('finrez_pre'))
    return _ac_num(current.get(field))


def _ac_metric_entry(field: str, current: Dict[str, Any], previous: Dict[str, Any]) -> Dict[str, Any]:
    cur = _ac_num(current.get(field))
    prev = _ac_num(previous.get(field))
    value_money = _ac_metric_money(field, current)
    prev_value_money = _ac_metric_money(field, previous)
    value_percent = None if field == 'revenue' else _ac_metric_percent(field, current)
    if field in {'markup', 'margin_pre'}:
        delta_money = None if value_money is None or prev_value_money is None else _round(value_money - prev_value_money)
...
def _ac_gap_entry(current: Dict[str, Any], business: Dict[str, Any], previous: Dict[str, Any], level: str) -> Dict[str, Any]:
    revenue = _ac_num(current.get('revenue'))
    prev_revenue = _ac_num(previous.get('revenue'))
    current_markup = _ac_num(current.get('markup'))
    current_margin = _ac_num(current.get('margin_pre'))
    current_gap_pp = None if current_markup is None or current_margin is None else _round(current_markup - current_margin)
    current_gap_money = None if current_gap_pp is None or revenue is None else _round((current_gap_pp / 100.0) * revenue)

    previous_markup = _ac_num(previous.get('markup'))
    previous_margin = _ac_num(previous.get('margin_pre'))
    previous_gap_pp = None if previous_markup is None or previous_margin is None else _round(previous_markup - previous_margin)
    previous_gap_money = None if previous_gap_pp is None or prev_revenue is None else _round((previous_gap_pp / 100.0) * prev_revenue)

    if level == 'business':
        delta_money = None if current_gap_money is None or previous_gap_money is None else _round(current_gap_money - previous_gap_money)
        delta_percent = None if current_gap_pp is None or previous_gap_pp is None else _round(current_gap_pp - previous_gap_pp)
    else:
        business_markup = _ac_num(business.get('markup'))
        business_margin = _ac_num(business.get('margin_pre'))
        business_gap_pp = None if business_markup is None or business_margin is None else _round(business_markup - business_margin)
        business_gap_money = None if business_gap_pp is None or revenue is None else _round((business_gap_pp / 100.0) * revenue)
        delta_money = None if current_gap_money is None or business_gap_money is None else _round(current_gap_money - business_gap_money)
        delta_percent = None if current_gap_pp is None or business_gap_pp is None else _round(current_gap_pp - business_gap_pp)

    return _prune_nones({
        'value_money': current_gap_money,
        'value_percent': current_gap_pp,
        'delta_money': delta_money,
        'delta_percent': delta_percent,
    })


def _ac_reason_entry(metric: str, current: Dict[str, Any], business: Dict[str, Any], per_metric: Dict[str, Any]) -> Dict[str, Any]:
    revenue = _ac_num(current.get('revenue'))
    obj_pct = _ac_metric_percent(metric, current)
    biz_pct = _ac_metric_percent(metric, business)
    value_money = _ac_metric_money(metric, current)
    delta_percent = None if obj_pct is None or biz_pct is None else _round(obj_pct - biz_pct)
    if metric == 'markup':
    ...
    return _prune_nones({
        'value_money': value_money,
        'value_percent': obj_pct,
        'delta_money': effect_money,
        'delta_percent': delta_percent,
        'business_value_percent': biz_pct,
        'effect_money': effect_money,
    })
...
def _ac_metrics(payload: Dict[str, Any], level: str) -> Dict[str, Any]:
    ...
    previous = _safe_previous_metrics(payload)
    business = _safe_business_metrics(payload)
    metrics = {field: _ac_metric_entry(field, current, previous) for field in AC_KPI_ORDER}
    metrics['gap'] = _ac_gap_entry(current, business, previous, level)
...
def _ac_focus_block(payload: Dict[str, Any], level: str, drain_block: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    business = _safe_business_metrics(payload)
    per_metric = (payload.get('impact') or {}).get('per_metric_effects') or {}
    reasons = {metric: _ac_reason_entry(metric, current, business, per_metric) for metric in AC_REASON_ORDER}
    return _prune_nones(reasons)
...
def _ac_goal(payload: Dict[str, Any], level: str, drain_block: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if level == 'business':
        ...
        return _prune_nones({'delta_money': delta_money, 'delta_percent': delta_percent})
    ...
    return _prune_nones({'delta_money': total, 'delta_percent': delta_percent})
