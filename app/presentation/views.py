...
def _summary_metric_value(metric_entry: Any, *, money_key: str = 'fact_money', percent_key: str = 'fact_percent') -> Any:
    if isinstance(metric_entry, dict):
        if money_key in metric_entry:
            return metric_entry.get(money_key)
        if percent_key in metric_entry:
            return metric_entry.get(percent_key)
    return metric_entry


def _metrics_from_summary_contract(payload: Dict[str, Any]) -> Dict[str, Any]:
    metrics = payload.get('metrics') or {}
    if not isinstance(metrics, dict):
        return {}
    return {
        'revenue': _summary_metric_value(metrics.get('revenue')),
        'retro_bonus': _summary_metric_value((payload.get('structure') or {}).get('retro'), money_key='value_money', percent_key='value_percent'),
        'logistics_cost': _summary_metric_value((payload.get('structure') or {}).get('logistics'), money_key='value_money', percent_key='value_percent'),
        'personnel_cost': _summary_metric_value((payload.get('structure') or {}).get('personnel'), money_key='value_money', percent_key='value_percent'),
        'other_costs': _summary_metric_value((payload.get('structure') or {}).get('other'), money_key='value_money', percent_key='value_percent'),
        'margin_pre': _summary_metric_value(metrics.get('margin_percent')),
        'markup': _summary_metric_value(metrics.get('markup_percent')),
        'finrez_pre': _summary_metric_value(metrics.get('finrez_pre')),
        'finrez_final': _summary_metric_value(metrics.get('finrez_final')),
        'gross_profit': _summary_metric_value((payload.get('structure') or {}).get('markup'), money_key='value_money', percent_key='value_percent'),
    }


def _safe_metrics(payload: Dict[str, Any]) -> Dict[str, Any]:
    direct = payload.get('metrics_raw')
    if isinstance(direct, dict) and direct:
        return direct
    nested = (payload.get('metrics') or {}).get('object_metrics') or {}
    if nested:
        return nested
    return _metrics_from_summary_contract(payload)


def _safe_business_metrics(payload: Dict[str, Any]) -> Dict[str, Any]:
    direct = payload.get('business_metrics_raw')
    if isinstance(direct, dict) and direct:
        return direct
...
def _safe_previous_metrics(payload: Dict[str, Any]) -> Dict[str, Any]:
    direct = payload.get('previous_object_metrics') or {}
    if direct:
        return direct
    metrics = payload.get('metrics') or {}
    if not isinstance(metrics, dict):
        return {}
    return {
        'revenue': (metrics.get('revenue') or {}).get('prev_year_money'),
        'margin_pre': (metrics.get('margin_percent') or {}).get('prev_year_percent'),
        'markup': (metrics.get('markup_percent') or {}).get('prev_year_percent'),
        'finrez_pre': (metrics.get('finrez_pre') or {}).get('prev_year_money'),
        'finrez_final': (metrics.get('finrez_final') or {}).get('prev_year_money'),
    }
...
def _context_path_parts(payload: Dict[str, Any]) -> List[str]:
    filter_payload = payload.get('filter') or {}
...
def _context_path(payload: Dict[str, Any]) -> str:
    return ' / '.join(_context_path_parts(payload))
...
def _build_drain_rows(payload: Dict[str, Any], max_items: int = MAX_DRAIN_ITEMS) -> List[Dict[str, Any]]:
    direct_rows = payload.get('drain_block')
...
def _build_decision_view(payload: Dict[str, Any]) -> Any:
    direct = payload.get('decision_block')
    if isinstance(direct, list):
        return direct
    priority = payload.get('priority_action')
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    # Object screen top navigation must use the same source as the full "все" list.
    # Domain already guarantees all_block order by navigation_money DESC.
    all_block_source = payload.get('all_block') if isinstance(payload.get('all_block'), list) else []
    # BUG-006 FIX-002 / Navigation Contract v1.2:
    # all navigation views are sourced exclusively from all_block.
    # If all_block is empty or absent, navigation is empty; do not fall back
    # to drain_payload/drain_block/navigation.items/text parsing.
    drain_block = all_block_source[:MAX_DRAIN_ITEMS]
    ...
    navigation = {
        'current_level': level,
        'next_level': _children_level_from_payload(payload) or _children_level_from_payload(drain_payload),
    ...
    return {
        'context': {
            ...
            'period': context_block.get('period_label') or payload_context.get('period'),
            'level': level,
            'object_name': payload_context.get('object_name') or payload.get('object_name'),
        },
        'path': _context_path_parts(payload),
        'metrics': metrics,
        'structure': payload.get('structure') or {},
        # VECTRA Stage 2: money fields are owned by domain/summary.
        # Drain Block is only a top-route view; it must not define screen focus.
        'business_result_money': payload.get('business_result_money'),
        'business_result_rating': payload.get('business_result_rating') or [],
        'opportunity_rating': payload.get('opportunity_rating') or [],
        'business_reasons': payload.get('business_reasons') or [],
        'object_result_money': payload.get('object_result_money'),
        'opportunity_money': payload.get('opportunity_money'),
        'object_reasons': payload.get('object_reasons') or [],
        ...
        # Backend/API summary is the source of truth for full-list and reasons modes.
        # UI/state must not reconstruct these blocks from top-3 navigation.
        'all_block': payload.get('all_block') or [],
        'reasons_block': payload.get('reasons_block') or [],
        'decision_block': decision_block,
        # Product v3.0 / Contract Workspace: preserve API workspace blocks
        # through the legacy management view so /vectra/query can render a full
        # network contract desktop instead of losing it at presentation level.
        'decision_workspace': payload.get('decision_workspace') or {},
        'decision_workspace_block': payload.get('decision_workspace_block') or [],
        'product_layer_block': payload.get('product_layer_block') or [],
        ...
        'aggregation_level': payload.get('aggregation_level') or payload.get('grouping_type'),
        ...
        'losses': drain_block,
        'positive_items': _build_positive_items(source if not all_block_source else payload),
...
def build_reasons_view(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    object_name = context.get('object_name') or payload.get('object_name')
    return {
        'context': {
            'level': context.get('level') or payload.get('level'),
        ...
        'navigation': {
            'mode': 'reasons',
            'back': True,
            'back_label': f"назад к {object_name}" if object_name else "назад",
        },
    }
