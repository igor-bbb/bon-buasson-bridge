...
def _build_drain_rows(payload: Dict[str, Any], include_positive: bool = False, max_items: int = 5) -> List[Dict[str, Any]]:
    ...
    for row in selected[:max_items]:
        ...
        clean.update({
            ...
            'impact_share_display': _format_percent(share) if gap > 0 and total_gap > 0 else '—',
            'line': f"{row.get('object_name')} {_format_money(gap)} {_format_percent(share) if gap > 0 and total_gap > 0 else '—'} {_format_pp(row.get('gap_to_business_pp'))}",
        })
        out.append(clean)
    return out
...
def build_object_view(payload: Dict[str, Any], drain_payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if level != 'business':
        focus_rows.append({
            ...
            'line': f"Недозаработано {_format_money(impact.get('gap_loss_money'))}",
        })
        focus_rows.append({
            'field': 'signal',
            'label': 'Сигнал',
        ...
        focus_rows.append({
            ...
            'value_display': diagnosis.get('top_drain_metric') or '—',
            'line': f"Главная причина {diagnosis.get('top_drain_metric') or '—'}",
        })
