...
def _build_compact_drill_item(item: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'priority': management_item.get('focus', {}).get('priority') if isinstance(management_item.get('focus'), dict) else None,
...
def build_drilldown_management_view(drilldown_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    prepared_items = sorted(prepared_items, key=lambda x: x.get('finrez_pre', 0.0))
    return {
        'mode': 'drill_down',
        ...
        'top_summary': _compute_top_summary(prepared_items),
        'action': {
...
def _compute_top_summary(items: List[Dict[str, Any]], limit: int = 5) -> Dict[str, Any]:
    enriched = []
    for item in items or []:
        finrez = item.get('finrez_pre')
        if finrez is None:
            finrez = (((item.get('basis') or {}).get('finrez_pre')) if isinstance(item.get('basis'), dict) else None)
        if finrez is None:
            finrez = (((item.get('summary') or {}).get('basis') or {}).get('finrez_pre')) if isinstance(item.get('summary'), dict) else None
        try:
            finrez = _round(finrez or 0.0)
        except Exception:
            finrez = 0.0
        enriched.append({
            ...
            'priority': ((item.get('focus') or {}).get('priority') if isinstance(item.get('focus'), dict) else None),
        })

    enriched = [x for x in enriched if x.get('object_name')]
    enriched.sort(key=lambda x: x.get('finrez_pre', 0.0))

    negatives = [x for x in enriched if x.get('finrez_pre', 0.0) < 0]
    total_negative_loss = abs(sum(x.get('finrez_pre', 0.0) for x in negatives))
    top_negative = negatives[:limit]
    top_negative_loss = abs(sum(x.get('finrez_pre', 0.0) for x in top_negative))
    concentration = _round((top_negative_loss / total_negative_loss) if total_negative_loss else 0.0)

    top_items = top_negative if top_negative else enriched[:limit]
    return {
        'top_items': top_items,
...
def _validate_signal_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    if not payload.get('summary'):
        raise ValueError('invalid signal payload: summary is required')
    if 'items' not in payload:
        raise ValueError('invalid signal payload: items are required')
    top_summary = payload.get('top_summary') or {}
    if not top_summary.get('top_items') and payload.get('items'):
        raise ValueError('invalid signal payload: top_summary.top_items are required')
    return payload
...
def build_signal_flow_view(summary_payload: Dict[str, Any], drilldown_payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    payload = {
        ...
        'top_summary': drilldown_view.get('top_summary') or _compute_top_summary(drilldown_view.get('items', [])),
    ...
    return _validate_signal_payload(payload)
