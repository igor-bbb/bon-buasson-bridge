...
from app.development_journal import add_runtime_event, list_records
...
def _is_open_journal_record(record: Dict[str, Any]) -> bool:
    status = str(record.get("status") or "").strip().lower()
    if status in {"closed", "archived", "test", "dry run", "dry-run"}:
        return False
...
def _journal_open_snapshot() -> Dict[str, Any]:
    records = [r for r in list_records(include_test=False, include_archived=False) if _is_open_journal_record(r)]
...
def _make_product_owner_report(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    criticality = "критичных проблем не обнаружено" if failed == 0 else "найдены проблемы, которые нужно исправить перед спокойным использованием"
