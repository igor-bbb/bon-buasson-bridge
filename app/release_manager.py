...
from app.development_journal import (
    ...
    list_fixed_engineering_task_ids_for_release,
)
...
def run_release_acceptance(release_id: str = "manual-release", scenario_ids: Optional[List[str]] = None, release_brief: Any = None) -> Dict[str, Any]:
    ...
    # Final lifecycle rule: Release Manager does not accept manually supplied
    # fixed-task lists. Development Journal is the only source of tasks fixed
    # by the current release. Engineering must persist Open -> Fixed before
    # Release Brief is rendered; Release Manager only advances journal-backed
    # tasks to verification and closure.
    fixed_task_ids = sorted(set(
        list_fixed_engineering_task_ids_for_release(str(release_id), include_awaiting=True)
    ))
    brief.fixed_engineering_tasks = fixed_task_ids
