...
def _coerce_workspace_markdown(payload: Dict[str, Any]) -> None:
    ...
    `workspace_markdown`; if an internal renderer already produced
    `workspace_primary_block`, this function promotes that renderer output into
    the canonical public field before the response leaves Runtime.
    """
    markdown = payload.get('workspace_markdown')
    if isinstance(markdown, str) and markdown.strip():
        return
