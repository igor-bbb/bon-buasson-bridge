...
def classify_action_label(label: Any) -> str:
    ...
    if 'назад' in norm or 'вернуться' in norm:
        return 'back'
    if any(x in norm for x in ('выбрать другую сеть', 'выбрать другой контракт', 'выбрать сеть из портфеля', 'выбрать контракт из портфеля')):
        return 'select_network'
    if 'продуктов' in norm and any(x in norm for x in ('ветк', 'sku', 'позици')):
        return 'product_navigation'
    if 'формат' in norm and any(x in norm for x in ('фильтр', 'отфильтровать', 'выбрать')):
...
def _default_visible_actions(payload: Dict[str, Any]) -> List[str]:
    ...
    if level in {'network', 'contract'}:
        return ['подготовить переговоры — собрать переговорную позицию', 'собрать пакет позиций — подготовить SKU-пакет', 'открыть продуктовую ветку и выбрать SKU', 'создать задачи — зафиксировать действия', 'причины — разобрать факторы', 'все — открыть витрину текущего уровня', 'назад — вернуться назад']
    if level == 'category':
        return ['собрать пакет позиций — подготовить пакет развития', 'отфильтровать SKU по формату', 'подготовить переговоры — собрать аргументы', 'создать задачи — зафиксировать действия', 'причины — разобрать факторы', 'все — открыть витрину текущего уровня', 'назад — вернуться назад']
    if level in {'tmc_group', 'format', 'sku', 'product'}:
    ...
    if level == 'manager':
        return ['выбрать другой контракт / сеть из своего портфеля', 'все — открыть витрину текущего уровня', 'причины — разобрать факторы', 'kpi — показать KPI', 'назад — вернуться назад']
    if level in {'business', 'manager_top', 'top_manager'}:
...
def build_active_workspace_state(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Preserve Product Team research metadata attached by API-side research flow.
    research_flow = existing_state.get('research_flow') or payload.get('research_flow_status')
    if isinstance(research_flow, dict) and research_flow:
        state['research_flow'] = research_flow
    return state
