...
REPOSITORY_VERSION = "VECTRA-RUNTIME-0004"
...
def write_runtime_object(object_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if spec_type == 'dict':
        ...
        _write_json(path, current)
        written_id = current.get('memory_id') or current.get('bundle_id') or current.get('state_id') or object_name
    else:
        current = _read_json(path, [])
        if not isinstance(current, list):
            current = []
...
def verify_runtime_readback(object_name: str, written_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if written_id and isinstance(data, list):
        ...
        found = written_id in {data.get('memory_id'), data.get('bundle_id'), data.get('state_id'), object_name}
    else:
