...
def _seed_capability_registry() -> Dict[str, Any]:
    ...
    capabilities = [
        ...
        {
            'capability_id': 'laboratory_action_first_policy',
            'title': 'Laboratory Action First Policy',
            'professional_value': 'Удерживать VECTRA Laboratory в профессиональном Runtime-first режиме после длительного диалога: сначала действие через Runtime, потом вывод.',
            'responsibility': 'Laboratory Professional Behavior',
            'runtime_service': 'laboratory_behavior.determine_laboratory_next_action',
            'transport_endpoint': '/vectra/laboratory/behavior/next-action',
            'status': 'active',
...
def get_capability_registry() -> Dict[str, Any]:
    ...
    if not any(isinstance(c, dict) and c.get('capability_id') == 'business_domain_knowledge_runtime' for c in capabilities):
        ...
        changed = True
    if not any(isinstance(c, dict) and c.get('capability_id') == 'laboratory_action_first_policy' for c in capabilities):
        capabilities.append({
            'capability_id': 'laboratory_action_first_policy',
            'title': 'Laboratory Action First Policy',
            'professional_value': 'Удерживать VECTRA Laboratory в профессиональном Runtime-first режиме после длительного диалога: сначала действие через Runtime, потом вывод.',
        ...
        changed = True
    if not any(isinstance(c, dict) and c.get('capability_id') == 'laboratory_action_manifest' for c in capabilities):
        capabilities.append({
            'capability_id': 'laboratory_action_manifest',
            'title': 'Laboratory Action Manifest',
            'professional_value': 'Контролировать полную комплектацию GPT Actions: Capability -> Runtime Service -> GPT Action, чтобы релизы не теряли ранее реализованные действия.',
            'responsibility': 'Laboratory Actions Completeness',
            'runtime_service': 'laboratory_actions.get_action_manifest',
            'transport_endpoint': '/vectra/laboratory/actions/manifest',
            'status': 'active',
            'maturity_level': 'Production',
        })
        changed = True
    if not any(isinstance(c, dict) and c.get('capability_id') == 'laboratory_action_completeness_verification' for c in capabilities):
        capabilities.append({
            'capability_id': 'laboratory_action_completeness_verification',
            'title': 'Laboratory Action Completeness Verification',
            'professional_value': 'Автоматически сравнивать Runtime Capability Registry и GPT Actions Manifest перед релизом и блокировать релиз при отсутствующих Actions.',
            'responsibility': 'Laboratory Release Control',
            'runtime_service': 'laboratory_actions.verify_action_completeness',
            'transport_endpoint': '/vectra/laboratory/actions/verify',
            'status': 'active',
            'maturity_level': 'Production',
        })
        changed = True
    if changed:
        ...
        registry['release'] = 'LABORATORY-ACTIONS-0002'
        _write_json(base / 'runtime' / 'capabilities' / 'capability_registry.json', registry)
    return _with_workspace_markdown({
        'status': 'ok',
