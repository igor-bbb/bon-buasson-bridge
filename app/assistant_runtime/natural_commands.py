...
NATURAL_COMMAND_VERSION = "VOS-001"
...
def get_natural_command_model() -> Dict[str, Any]:
    return {
        ...
        "supported_human_requests": [
            ...
            "Покажи модель жизни VECTRA.",
            "Покажи операционную модель VECTRA.",
            "Как ты живёшь и работаешь?",
        ],
        "available_actions": [
            "memory_overview",
            "journal_read",
...
def classify_natural_command(payload: Any) -> Dict[str, Any]:
    ...
    if _contains_any(low, ["кто ты", "кто такая vectra", "кто такая вектра", "расскажи как ты работаешь", "расскажи, как ты работаешь", "покажи модель жизни vectra", "модель жизни vectra", "life model"]):
        return {"intent": "life_model_read", "confidence": 0.97, "message": text}
    if _contains_any(low, ["статус модели жизни", "life model status"]):
        return {"intent": "life_model_status", "confidence": 0.94, "message": text}
    if _contains_any(low, ["проверь модель жизни", "life model verify"]):
        return {"intent": "life_model_verify", "confidence": 0.94, "message": text}

    if _contains_any(low, ["операционная модель vectra", "операционную модель vectra", "vos", "vectra operating system", "как ты живешь", "как ты живёшь", "как устроена твоя жизнь"]):
        return {"intent": "vos_read", "confidence": 0.97, "message": text}
    if _contains_any(low, ["статус vos", "статус операционной модели", "operating system status"]):
        return {"intent": "vos_status", "confidence": 0.94, "message": text}
    if _contains_any(low, ["проверь vos", "проверить vos", "проверь операционную модель", "operating system verify"]):
        return {"intent": "vos_verify", "confidence": 0.94, "message": text}
...
def _render_result_markdown(intent: str, result: Dict[str, Any], human: Dict[str, Any]) -> str:
    ...
    if isinstance(result, dict):
        if intent == "life_model_read":
            md = result.get("life_model_markdown") if isinstance(result, dict) else None
            if md:
                lines += [str(md), ""]
            else:
                lines += ["## Модель жизни", _preview_value(result.get("life_model", result), 3000), ""]
        elif intent in {"life_model_status", "life_model_verify"}:
            lines += ["## Life Model Runtime", _preview_value(result, 2500), ""]
        elif intent in {"runtime_snapshot_read", "runtime_snapshot_refresh"}:
...
def _humanize(intent: str, result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if intent == "life_model_read":
        return {"title": "Модель жизни VECTRA", "short_answer": result.get("human_summary", "Я прочитала Life Model из Runtime Repository.")}
    if intent == "life_model_status":
        return {"title": "Статус Life Model VECTRA", "short_answer": f"Life Model status: {result.get('life_model_status', {}).get('status', result.get('status'))}."}
    if intent == "life_model_verify":
        return {"title": "Проверка Life Model VECTRA", "short_answer": f"Life Model Verification: {result.get('status')}."}
