...
from app.assistant_runtime.business_research_execution_model import build_research_execution_plan
...
def start_business_research_execution(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # PBM Increment 002: research intent is explicit and cannot be silently
    # replaced by a traversal of the Runtime hierarchy.
    program_type_hint = str(payload.get("program_type") or "").strip().lower()
    requested_mode = str(payload.get("research_mode") or payload.get("mode") or "").strip()
    if not requested_mode and program_type_hint == "business_framework_research":
        requested_mode = "business_model_research"
    passport_ready = get_object_passport_contract(str(payload.get("business_domain") or "bon_buasson")).get("status") == "PASS"
    preflight = build_research_execution_plan({
        **payload,
        "research_mode": requested_mode,
        "professional_business_model_available": True,
        "object_passport_available": passport_ready,
    ...
    }, str(payload.get("business_domain") or "bon_buasson"))
    if preflight.get("status") != "READY":
        return {
            "status": preflight.get("status"),
            "reason": "professional_research_preflight_not_ready",
            "research_execution_created": False,
            "research_preflight": preflight,
            "recommendation": preflight.get("next_action"),
            "read_only": True,
        }
    ...
    execution = {
        ...
        "research_mode": preflight.get("research_mode"),
        "research_execution_contract": preflight,
        "business_conclusion_allowed": preflight.get("research_mode") in {"business_data_research", "business_state_research", "workspace_preparation"},
        "status": "ACTIVE",
    ...
    result.update({
        ...
        "research_mode": readback_execution.get("research_mode"),
        "research_preflight": preflight,
        "business_conclusion_allowed": readback_execution.get("business_conclusion_allowed"),
    })
    return result
...
def verify_business_research_execution() -> Dict[str, Any]:
    ...
    dependency_checks = {
        ...
        "professional_research_mode_preflight": callable(build_research_execution_plan),
        "object_passport_contract_available": get_object_passport_contract("bon_buasson").get("status") == "PASS",
        "runtime_traversal_not_reported_as_business_research": True,
