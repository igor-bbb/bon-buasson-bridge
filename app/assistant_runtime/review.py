...
def verify_synchronization_execution_readback() -> Dict[str, Any]:
    ...
    checks = [
        ...
        {"object": "execution_report", "status": "PASS" if (isinstance(latest, dict) and latest.get("execution_status") in {"EXECUTED", "BLOCKED", "NOT_EXECUTED"}) or not reports else "FAIL", "note": "No execution report is acceptable before Product Owner Approved Session; Runtime Verification checks availability and boundaries."},
...
# GENESIS-0012 Evolution Journal & Runtime Verification Foundation.
# This section intentionally reuses the existing assistant_repository/journal/evolution_journal.json
# so the deploy package remains a full package without adding extra files beyond the GitHub web-upload limit.

EVOLUTION_JOURNAL_VERSION = "GENESIS-0012"


def _journal_items() -> List[Dict[str, Any]]:
    items = _read_json(_repo_path(Path("journal") / "evolution_journal.json"), [])
...
def _latest_execution_report_for_journal() -> Dict[str, Any]:
    reports = _read_list(EXECUTION_REPORTS_PATH)
    return reports[-1] if reports else {}


def _normalize_journal_entry(item: Dict[str, Any]) -> Dict[str, Any]:
    metadata = item.get("metadata") if isinstance(item.get("metadata"), dict) else {}
    execution = _latest_execution_report_for_journal()
    release_id = str(item.get("release_id") or metadata.get("release") or metadata.get("release_id") or item.get("review_release") or item.get("execution_release") or "unknown")
    entry_id = str(item.get("journal_entry_id") or item.get("entry_id") or f"EVJ-{uuid.uuid4().hex[:12]}")
    return {
        "journal_entry_id": entry_id,
        "entry_id": entry_id,
        "timestamp": item.get("timestamp") or item.get("created_at") or item.get("updated_at") or _now(),
        "release_id": release_id,
        "review_session_id": item.get("review_session_id") or metadata.get("review_session_id") or execution.get("review_session_id"),
        "execution_id": item.get("execution_id") or metadata.get("execution_id") or execution.get("execution_id"),
        "source": item.get("source") or "evolution_journal_repository",
        "status": item.get("status") or "confirmed",
        "new_professional_competence": item.get("new_professional_competence") or metadata.get("new_professional_competence") or item.get("object_changed") or "confirmed VECTRA evolution event",
        "new_professional_value": item.get("new_professional_value") or metadata.get("new_professional_value") or item.get("decision") or "confirmed professional value recorded in Evolution Journal",
        "change_boundaries": item.get("change_boundaries") or metadata.get("change_boundaries") or {
            "professional_model_auto_change": False,
            "automatic_reflection": False,
            "automatic_consolidation": False,
            "outside_release_scope": False,
        },
        "execution_report_ref": item.get("execution_report_ref") or metadata.get("execution_report_ref") or execution.get("execution_id"),
        "capitalization_status": item.get("capitalization_status") or metadata.get("capitalization_status") or "recorded",
        "raw_entry": item,
    }


def list_evolution_journal_entries(limit: int = 50) -> Dict[str, Any]:
    ensure_repository()
    entries = [_normalize_journal_entry(item) for item in _journal_items()]
    visible = entries[-max(1, int(limit or 50)):]
    payload = {
        "status": "ok",
        "render_mode": "vectra_evolution_journal",
        "identity_root": "VECTRA",
        "evolution_journal_release": EVOLUTION_JOURNAL_VERSION,
        "journal_status": "active",
        "entries_count": len(entries),
        "entries": visible,
        "human_summary": "Evolution Journal фиксирует подтверждённую эволюцию VECTRA и связывает её с Review / Execution контекстом.",
    }
    return _with_workspace_markdown(payload, "Evolution Journal VECTRA", {"entries_count": len(entries), "entries": visible})


def get_evolution_journal_latest(limit: int = 5) -> Dict[str, Any]:
    base = list_evolution_journal_entries(limit=limit)
    return {
        **base,
        "render_mode": "vectra_evolution_journal_latest",
        "latest_entries": base.get("entries", []),
        "human_summary": "Последние записи Evolution Journal доступны для Product Verification.",
    }


def get_evolution_journal_status() -> Dict[str, Any]:
    entries = list_evolution_journal_entries(limit=10).get("entries", [])
    latest = entries[-1] if entries else None
    payload = {
        "status": "ok",
        "render_mode": "vectra_evolution_journal_status",
        "identity_root": "VECTRA",
        "evolution_journal_release": EVOLUTION_JOURNAL_VERSION,
        "journal_status": "active",
        "entries_count": len(_journal_items()),
        "latest_entry_id": latest.get("journal_entry_id") if isinstance(latest, dict) else None,
        "latest_release_id": latest.get("release_id") if isinstance(latest, dict) else None,
        "capitalization_status": latest.get("capitalization_status") if isinstance(latest, dict) else None,
        "runtime_observable": True,
        "human_summary": "Evolution Journal активен и доступен через Runtime Observability.",
    }
    return _with_workspace_markdown(payload, "Evolution Journal Status VECTRA", payload)


def verify_evolution_journal_readback() -> Dict[str, Any]:
    journal = list_evolution_journal_entries(limit=5)
    latest = get_evolution_journal_latest(limit=3)
    status = get_evolution_journal_status()
    entries = journal.get("entries") if isinstance(journal.get("entries"), list) else []
    checks = [
        {"object": "evolution_journal_repository", "status": "PASS" if journal.get("status") == "ok" else "FAIL"},
        {"object": "journal_entries", "status": "PASS" if isinstance(entries, list) else "FAIL"},
        {"object": "latest_entries", "status": "PASS" if isinstance(latest.get("latest_entries"), list) else "FAIL"},
        {"object": "journal_status", "status": "PASS" if status.get("journal_status") == "active" else "FAIL"},
        {"object": "required_entry_fields", "status": "PASS" if all(k in (entries[-1] if entries else {}) for k in ["journal_entry_id", "timestamp", "release_id", "source", "status", "new_professional_competence", "new_professional_value", "change_boundaries", "capitalization_status"]) or not entries else "FAIL"},
    ]
    overall = "PASS" if all(item.get("status") == "PASS" for item in checks) else "FAIL"
    payload = {
        "status": overall,
        "render_mode": "vectra_evolution_journal_readback_verification",
        "identity_root": "VECTRA",
        "evolution_journal_release": EVOLUTION_JOURNAL_VERSION,
        "checks": checks,
        "entries_count": journal.get("entries_count"),
        "journal_status": status.get("journal_status"),
    }
    return _with_workspace_markdown(payload, "Evolution Journal Readback Verification VECTRA", payload)
