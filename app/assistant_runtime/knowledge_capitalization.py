...
KNOWLEDGE_RELEASE = "LABORATORY-KNOWLEDGE-0004"
KNOWLEDGE_STATUSES = [
    "DETECTED",
...
def auto_capitalize_confirmed_knowledge(payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not items:
        result = {
            ...
            "pipeline_steps": ["Analyze Context", "Extract Knowledge"],
            "final_status": "STOPPED",
            "reason": "no_confirmed_knowledge_detected",
            "message": "В текущем рабочем контексте не найдено подтверждённых знаний для капитализации.",
            "next_recommended_action": "Передайте подтверждённые знания в payload.confirmed_knowledge или подтвердите их в рабочем контексте.",
        }
    ...
    all_capitalized = bool(reports) and not errors and len(reports) == len(items)
    result = {
