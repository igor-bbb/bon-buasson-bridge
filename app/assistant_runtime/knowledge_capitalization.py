...
KNOWLEDGE_RELEASE = "LABORATORY-KNOWLEDGE-0006"
KNOWLEDGE_STATUSES = [
    "DETECTED",
...
def _items_from_prepared_knowledge_package(package: Dict[str, Any], domain: str) -> List[Dict[str, Any]]:
    ...
    for idx, item in enumerate(items, start=1):
        ...
        knowledge_id = str(item.get("knowledge_id") or item.get("id") or f"{kid_prefix}-{uuid.uuid4().hex[:6].upper()}").upper()
...
def auto_capitalize_confirmed_knowledge(payload: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    if not items:
        result = {
            ...
            "pipeline_steps": ["Accept Prepared Knowledge Package" if package_mode == "prepared_knowledge_package" else "Analyze Context", "Extract Knowledge", "Compare With Runtime Memory", "Completed"],
    ...
    if not approval:
        ...
        result = {
            ...
            "knowledge_input_mode": package_mode,
            "runtime_reanalysis_performed": package_mode != "prepared_knowledge_package",
            "pipeline_steps": ["Accept Prepared Knowledge Package" if package_mode == "prepared_knowledge_package" else "Analyze Context", "Extract Knowledge", "Create Candidate", "Approval Check"],
            "batch_id": batch_id,
            "summary": summary,
            "incremental_diff": diff,
    ...
    result = {
        ...
        "runtime_reanalysis_performed": package_mode != "prepared_knowledge_package",
        "pipeline_steps": [
            "Accept Prepared Knowledge Package" if package_mode == "prepared_knowledge_package" else "Analyze Context",
            "Extract Knowledge",
            "Create Candidate",
            "Create Package",
            "Approval Check",
