...
def _build_components() -> Dict[str, Dict[str, Any]]:
    ...
    professional_knowledge = get_professional_knowledge_overview()
    components["professional_knowledge"] = _component(
        "professional_knowledge",
        _status(professional_knowledge.get("repository_status") == "readable"),
        "Professional Knowledge repository is readable through Runtime." if professional_knowledge.get("repository_status") == "readable" else "Professional Knowledge repository is not readable.",
        {
            "documents_count": professional_knowledge.get("professional_documents", 0),
            "last_document": professional_knowledge.get("last_document"),
            "last_updated": professional_knowledge.get("last_updated"),
            "repository_status": professional_knowledge.get("repository_status"),
            "repository_path": professional_knowledge.get("repository_path"),
        },
    )
    ...
    components["business_domain_knowledge"] = _component(
        ...
        "Business Domain Knowledge repository is readable and restored for Бон Буассон." if business_knowledge.get("repository_status") == "readable" else "Business Domain Knowledge repository is not readable.",
        {
            ...
            "last_document": business_knowledge.get("last_document"),
            "last_updated": business_knowledge.get("last_updated"),
            "repository_status": business_knowledge.get("repository_status"),
            "repository_path": business_knowledge.get("repository_path"),
            "restored_in_domain_session": business_knowledge.get("restored_in_domain_session"),
        },
    )
...
def _build_runtime_snapshot(write: bool = True, reason: str = "runtime_observability_request") -> Dict[str, Any]:
    ...
    snapshot = {
        ...
        "professional_knowledge": components.get("professional_knowledge", {}).get("data", {}),
        "business_domain_knowledge": components.get("business_domain_knowledge", {}).get("data", {}),
