...
from app.development_journal import is_capture_command, is_export_command, is_show_command, is_dry_run_command, is_lifecycle_command
...
def is_development_journal_intent(message: Any) -> bool:
    ...
    if is_capture_command(message) or is_export_command(message) or is_show_command(message) or is_dry_run_command(message) or is_lifecycle_command(message):
        return True
    return any(
        norm == phrase
