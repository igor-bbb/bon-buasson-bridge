...
def resolve_period_from_message(message: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    range_match = re.search(r'\b([а-яё]+)\s*[-/]\s*([а-яё]+)\s+(20\d{2}|\d{2})\b', text)
    if range_match:
        ...
        if start_month and end_month:
            return f'{year}-{start_month}:{year}-{end_month}', None
...
def _score_alias_match(text_norm: str, alias_norm: str) -> Optional[Tuple[int, int, int, int, int]]:
    if not alias_norm:
        return None
    ...
    subset = 1 if text_tokens and text_tokens.issubset(alias_tokens) else 0
    token_overlap = len(text_tokens & alias_tokens)

    if not (exact or whole or partial or subset or token_overlap):
        return None

    return exact, whole, subset, token_overlap, len(alias_norm)
...
def _resolve_scope_entity(text: str, period: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    level_rank = {'manager': 5, 'manager_top': 4, 'network': 3, 'category': 2, 'tmc_group': 1, 'sku': 0}
    ...
    for level in priority:
        ...
        for alias, canonical in level_index.items():
            ...
            candidate = (score[0], score[1], score[2], score[3], score[4], level_rank.get(level, 0))
