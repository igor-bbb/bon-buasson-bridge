...
COMPARISON_MARKERS = ['сравни', 'сравнить', 'сравнение', 'vs', 'versus', 'против', 'по сравнению с', 'относительно']
FULL_VIEW_ALIASES = {'все', 'покажи все', 'полный список', 'full', 'show all'}
SHORT_DRILL_COMMANDS = {
    'топы': 'manager_top', 'топ менеджеры': 'manager_top', 'топ менеджер': 'manager_top', 'топ-менеджеры': 'manager_top', 'топ-менеджер': 'manager_top',
    'менеджеры': 'manager', 'менеджер': 'manager', 'сети': 'network', 'сеть': 'network', 'категории': 'category', 'категория': 'category',
    'группы': 'tmc_group', 'группа': 'tmc_group', 'группы тмц': 'tmc_group', 'группа тмц': 'tmc_group', 'товары': 'sku', 'товар': 'sku', 'sku': 'sku', 'скю': 'sku',
}
SPECIAL_QUERY_TYPES = {'причины': 'reasons', 'потери': 'losses', 'сигнал': 'summary'}
LEVEL_HINTS = {
    'manager_top': ['топ менеджеры', 'топ менеджер', 'топ-менеджеры', 'топ-менеджер', 'топы'], 'manager': ['менеджеры', 'менеджер'], 'network': ['сети', 'сеть'],
    'category': ['категории', 'категория'], 'tmc_group': ['группы тмц', 'группа тмц', 'группы', 'группа'], 'sku': ['товары', 'товар', 'sku', 'скю'], 'business': ['бизнес', 'business', 'компания', 'весь бизнес'],
}
SERVICE_PREFIXES = ['покажи мне', 'покажи', 'показать', 'дай', 'выведи', 'разложи', 'разложить', 'открой']
...
def _month_token_to_number(token: str) -> Optional[str]:
    ...
    month_stems = {'январ': '01', 'феврал': '02', 'март': '03', 'апрел': '04', 'ма': '05', 'июн': '06', 'июл': '07', 'август': '08', 'сентябр': '09', 'октябр': '10', 'ноябр': '11', 'декабр': '12'}
    for stem, month_num in month_stems.items():
...
def _extract_month_year_tokens(text: str) -> List[Tuple[int, str]]:
    ...
    for match in re.finditer(r'\b(0?[1-9]|1[0-2])[\/\.\-\s](20\d{2}|\d{2})\b', text):
        append_unique(match.start(), f"{_normalize_year(match.group(2))}-{int(match.group(1)):02d}")
    ...
    for match in re.finditer(rf'\b({month_names_pattern})\b(?:\s+(20\d{{2}}|\d{{2}}))?', text):
        month = _month_token_to_number(match.group(1)); year_raw = match.group(2)
        if month and year_raw:
            append_unique(match.start(), f'{_normalize_year(year_raw)}-{month}')
    for match in re.finditer(r'\b([а-яё]+)\b(?:\s+(20\d{2}|\d{2}))?', text):
        month = _month_token_to_number(match.group(1)); year_raw = match.group(2)
        if month and year_raw:
            append_unique(match.start(), f'{_normalize_year(year_raw)}-{month}')
    for match in re.finditer(r'\b(20\d{2}|\d{2})\b', text):
        append_unique(match.start(), _normalize_year(match.group(1)))
...
def extract_periods_from_text(message: str) -> List[str]:
    ...
    if len(split_parts) == 2:
        left_periods = [p for _, p in _extract_month_year_tokens(split_parts[0].strip())]
        right_periods = [p for _, p in _extract_month_year_tokens(split_parts[1].strip())]
        if left_periods and right_periods:
            return [left_periods[0], right_periods[0]]
    return [p for _, p in _extract_month_year_tokens(text)]


def _has_comparison_connector(message: str) -> bool:
    text = f' {normalize_user_message(message)} '
    return any(marker in text for marker in COMPARISON_MARKERS) or bool(re.search(r'\bк\b', text))
...
def _resolve_scope_entity(text: str, period: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    for level in priority:
        ...
        for alias, canonical in level_index.items():
            ...
            if not alias_norm:
                continue
            padded_text = f' {normalized_text} '; padded_alias = f' {alias_norm} '
...
def _resolve_base_level_for_target(target_level: str, scope_level: Optional[str]) -> Optional[str]:
    if target_level == 'manager_top': return 'business'
    if target_level == 'manager': return 'manager_top' if scope_level == 'manager_top' else 'business'
    if target_level == 'network': return 'manager' if scope_level == 'manager' else 'business'
    if target_level == 'category': return scope_level if scope_level in {'network', 'manager'} else None
    if target_level == 'tmc_group': return scope_level if scope_level in {'category', 'network'} else None
    if target_level == 'sku': return scope_level if scope_level in {'category', 'tmc_group', 'network'} else None
    return None
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    if text in FULL_VIEW_ALIASES:
        return {'status': 'ok', 'query': {'mode': 'diagnosis', 'query_type': 'full_view'}}
    if text in SHORT_DRILL_COMMANDS:
        return {'status': 'ok', 'query': {'mode': 'diagnosis', 'level': None, 'object_name': None, 'period_current': None, 'period_previous': None, 'query_type': 'drill_down', 'target_level': SHORT_DRILL_COMMANDS[text], 'period': None, 'object': None, 'full_view': False}}
    if text in SPECIAL_QUERY_TYPES:
        return {'status': 'ok', 'query': {'mode': 'diagnosis', 'level': None, 'object_name': None, 'period_current': None, 'period_previous': None, 'query_type': SPECIAL_QUERY_TYPES[text], 'period': None, 'object': None, 'full_view': False}}
    full_view = any(alias in text for alias in FULL_VIEW_ALIASES)
    ...
    if target_level:
        ...
        return {'status': 'ok', 'query': {'mode': mode, 'level': base_level, 'object_name': object_name, 'period_current': period_current, 'period_previous': period_previous, 'query_type': 'drill_down', 'target_level': target_level, 'period': period_current, 'object': object_name, 'full_view': full_view}}
    if not scope_level:
        scope_level = 'business'; scope_object_name = 'business'
    return {'status': 'ok', 'query': {'mode': mode, 'level': scope_level, 'object_name': scope_object_name, 'period_current': period_current, 'period_previous': period_previous, 'query_type': 'summary', 'period': period_current, 'object': scope_object_name, 'full_view': full_view}}
