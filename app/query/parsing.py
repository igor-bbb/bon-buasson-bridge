...
def normalize_user_message(message: str) -> str:
    ...
    if not text and ('бизнес' in original or 'business' in original):
        text = 'business'
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    # Sprint W14.2 — Aggregated period semantics.
    # Phrases like “к прошлому году / к прошлым годом” request the standard
    # YoY Workspace, not a separate comparison screen. The previous period is
    # still passed as context, but the route must build the same full Workspace
    # as for a single period. Explicit second periods (e.g. 2026-01 vs 2025-01)
    # remain true comparison queries.
    year_tokens = re.findall(r'\b20\d{2}\b', text)
    previous_year_phrase = bool(re.search(r'\bпрошл\w*\b', text))
    if previous_year_phrase and len(set(year_tokens)) <= 1:
        mode = 'diagnosis'
