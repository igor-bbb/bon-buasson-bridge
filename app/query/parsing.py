...
def _is_period_only_input(text: str) -> bool:
    stripped = _strip_period_tokens(text)
    return stripped == ''
...
def _resolve_scope_entity(text: str, period: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    for level in priority:
        ...
        for alias, canonical in level_index.items():
            ...
            candidate = (score[0], score[1], score[2], score[3], level, canonical)
...
def _period_only_after_cleanup(text: str) -> bool:
    cleaned = normalize_user_message(text)
    for token in ['бизнес', 'business', 'компания', 'весь бизнес']:
...
def _friendly_period_error() -> Dict[str, Any]:
    return error_response('Не распознал период. Примеры: 2026-02, Бизнес 2026-02, февраль 2026, 01-02 2026, 2026-01 → 2026-02, Иванов Иван 2026-02')


def _friendly_object_error() -> Dict[str, Any]:
    return error_response('Не распознал объект. Используй формат: Бизнес 2026-02 или ФИО + период')
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    if text in SPECIAL_QUERY_TYPES:
        return {
            ...
            'query': {
                'mode': 'diagnosis', 'level': None, 'object_name': None,
                'period_current': None, 'period_previous': None,
                'query_type': SPECIAL_QUERY_TYPES[text], 'period': None, 'object': None,
            },
        }
    ...
    if _period_only_after_cleanup(text):
        return {
            'status': 'ok',
            'query': {
                'mode': mode, 'level': 'business', 'object_name': 'business',
    ...
    if scope_level == 'business':
        return {
            ...
            'query': {
                'mode': mode, 'level': 'business', 'object_name': 'business',
                'period_current': period_current, 'period_previous': period_previous,
                'query_type': special_query_type or 'summary', 'period': period_current, 'object': 'business',
            },
        }
    ...
    if target_level:
        if scope_level == target_level and scope_object_name:
            return {
                'status': 'ok',
                'query': {
                    'mode': mode, 'level': scope_level, 'object_name': scope_object_name,
                    'period_current': period_current, 'period_previous': period_previous,
                    'query_type': special_query_type or 'summary', 'period': period_current, 'object': scope_object_name,
                },
            }
        base_level = _resolve_base_level_for_target(target_level, scope_level)
        if target_level in {'manager_top', 'manager'} and scope_level is None:
            ...
            scope_object_name = 'business'
        if scope_level is None and target_level in {'category', 'tmc_group', 'sku', 'network'}:
            if any(word in text for word in ['бизнес', 'business', 'компания']):
                scope_level = 'business'
                scope_object_name = 'business'
                if target_level == 'network':
                    target_level = 'manager_top'
        ...
        if not base_level:
            return error_response('Некорректная комбинация уровня и объекта')
        object_name = 'business' if base_level == 'business' else scope_object_name
        return {
            'status': 'ok',
            'query': {
                'mode': mode, 'level': base_level, 'object_name': object_name,
                'period_current': period_current, 'period_previous': period_previous,
                'query_type': 'drill_down', 'target_level': target_level,
                'period': period_current, 'object': object_name,
            },
        }
...
def _resolve_scope_entity_safe(text: str, period: str) -> Tuple[Optional[str], Optional[str]]:
    try:
        return _resolve_scope_entity(text, period)
    except Exception:
        return None, None


def _has_explicit_previous_reference(text: str) -> bool:
    text = normalize_user_message(text)
    return any(token in text for token in ['прошлому году', 'прошлого года', 'vs', 'versus', 'против', 'по сравнению'])
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    mode = detect_mode(periods, text)
    period_current, resolved_previous = resolve_period_from_message(text)
    period_previous = resolved_previous if (_has_explicit_previous_reference(text) or (period_current and ':' in period_current)) else None

    if not period_current:
        return _friendly_period_error()

    if _period_only_after_cleanup(text):
        return {'status': 'ok', 'query': {'mode': mode, 'level': 'business', 'object_name': 'business', 'period_current': period_current, 'period_previous': period_previous, 'query_type': 'summary', 'period': period_current, 'object': 'business'}}
    ...
    lookup_period = period_current.split('-', 1)[0] if ':' in period_current else period_current
    scope_level, scope_object_name = _resolve_scope_entity_safe(entity_text, lookup_period)

    if scope_level == 'business':
        return {'status': 'ok', 'query': {'mode': mode, 'level': 'business', 'object_name': 'business', 'period_current': period_current, 'period_previous': period_previous, 'query_type': special_query_type or 'summary', 'period': period_current, 'object': 'business'}}

    if target_level:
        if scope_level == target_level and scope_object_name:
            return {'status': 'ok', 'query': {'mode': mode, 'level': scope_level, 'object_name': scope_object_name, 'period_current': period_current, 'period_previous': period_previous, 'query_type': special_query_type or 'summary', 'period': period_current, 'object': scope_object_name}}
        base_level = _resolve_base_level_for_target(target_level, scope_level)
        if target_level in {'manager_top', 'manager'} and scope_level is None:
            base_level = 'business'; scope_object_name = 'business'
        if scope_level is None and target_level in {'category', 'tmc_group', 'sku', 'network'}:
            if any(word in text for word in ['бизнес', 'business', 'компания']):
                scope_level = 'business'; scope_object_name = 'business'
                if target_level == 'network':
                    target_level = 'manager_top'; base_level = 'business'
            else:
                return _friendly_object_error()
        if not base_level:
            return error_response('Некорректная комбинация уровня и объекта')
    ...
    if not scope_level:
        # fallback: if looks like a person name + period, keep manager level and let backend resolve later
        if entity_text and len(entity_text.split()) >= 2:
            return {'status': 'ok', 'query': {'mode': mode, 'level': 'manager', 'object_name': entity_text.title(), 'period_current': period_current, 'period_previous': period_previous, 'query_type': special_query_type or 'summary', 'period': period_current, 'object': entity_text.title()}}
        return _friendly_object_error()

    return {'status': 'ok', 'query': {'mode': mode, 'level': scope_level, 'object_name': scope_object_name, 'period_current': period_current, 'period_previous': period_previous, 'query_type': special_query_type or 'summary', 'period': period_current, 'object': scope_object_name}}


# ===== LOCK v2 RANGE HOTFIX =====

def _period_only_after_cleanup(text: str) -> bool:
    cleaned = normalize_user_message(text)
    for token in ['бизнес', 'business', 'компания', 'весь бизнес']:
        cleaned = re.sub(rf'\b{re.escape(token)}\b', ' ', cleaned)
    cleaned = re.sub(r'\b(20\d{2})-(0[1-9]|1[0-2])\s*(?:->|→|до|-)\s*(20\d{2})-(0[1-9]|1[0-2])\b', ' ', cleaned)
    cleaned = _strip_period_tokens(cleaned)
    cleaned = cleaned.replace('->', ' ').replace('→', ' ').replace('до', ' - ')
    return re.sub(r'\s+', ' ', cleaned).strip(' -') == ''
...
def _friendly_period_error() -> Dict[str, Any]:
    return error_response('Период не распознан. Используй формат вроде 2026-02, февраль 2026 или 01-02 2026')


def _friendly_object_error() -> Dict[str, Any]:
    return error_response('Объект не распознан. Укажи Бизнес + период или ФИО + период')


def _strip_period_tokens_v3(text: str) -> str:
    cleaned = f" {normalize_user_message(text)} "
    cleaned = re.sub(r'(20\d{2})-(0[1-9]|1[0-2])\s*(?:->|→|до|-)\s*(20\d{2})-(0[1-9]|1[0-2])', ' ', cleaned)
    cleaned = re.sub(r'(0?[1-9]|1[0-2])\s*[-/]\s*(0?[1-9]|1[0-2])\s+(20\d{2}|\d{2})', ' ', cleaned)
    cleaned = re.sub(r'[а-яё]+\s*[-/]\s*[а-яё]+\s+(20\d{2}|\d{2})', ' ', cleaned)
    cleaned = re.sub(r'(20\d{2})-(0[1-9]|1[0-2])', ' ', cleaned)
    cleaned = re.sub(r'(0?[1-9]|1[0-2])[\/\.\-\s](20\d{2}|\d{2})', ' ', cleaned)
    month_names_pattern = '|'.join(sorted(MONTHS_RU.keys(), key=len, reverse=True))
    cleaned = re.sub(rf'({month_names_pattern}|январ[а-яё]*|феврал[а-яё]*|март[а-яё]*|апрел[а-яё]*|ма[йя]|июн[а-яё]*|июл[а-яё]*|август[а-яё]*|сентябр[а-яё]*|октябр[а-яё]*|ноябр[а-яё]*|декабр[а-яё]*)', ' ', cleaned)
    cleaned = re.sub(r'(20\d{2}|\d{2})', ' ', cleaned)
    for token in list(SERVICE_PREFIXES) + ['бизнес', 'business', 'компания', 'весь бизнес']:
        cleaned = re.sub(rf'{re.escape(token)}', ' ', cleaned)
    cleaned = re.sub(r'\s+', ' ', cleaned).strip(' -')
    return cleaned


def _period_only_after_cleanup(text: str) -> bool:
    return _strip_period_tokens_v3(text) == ''
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    if special_query_type:
        stripped_text = _strip_special_query_types(stripped_text)
    entity_text = _strip_period_tokens_v3(stripped_text)
    ...
    if not scope_level:
        if entity_text and len(entity_text.split()) >= 2:
            guessed_name = ' '.join(part.capitalize() for part in entity_text.split())
            return {'status': 'ok', 'query': {'mode': mode, 'level': 'manager', 'object_name': guessed_name, 'period_current': period_current, 'period_previous': period_previous, 'query_type': special_query_type or 'summary', 'period': period_current, 'object': guessed_name}}
        return _friendly_object_error()

    return {'status': 'ok', 'query': {'mode': mode, 'level': scope_level, 'object_name': scope_object_name, 'period_current': period_current, 'period_previous': period_previous, 'query_type': special_query_type or 'summary', 'period': period_current, 'object': scope_object_name}}


# ===== FINAL LOCK v3 OVERRIDES 2 =====

def _remove_period_fragments_v3(text: str) -> str:
    cleaned = normalize_user_message(text)
    cleaned = re.sub(r'\b(20\d{2})-(0[1-9]|1[0-2])\s*(?:->|→|до|-)\s*(20\d{2})-(0[1-9]|1[0-2])\b', ' ', cleaned)
    cleaned = re.sub(r'\b(0?[1-9]|1[0-2])\s*[-/]\s*(0?[1-9]|1[0-2])\s+(20\d{2}|\d{2})\b', ' ', cleaned)
    cleaned = re.sub(r'\b[а-яё]+\s*[-/]\s*[а-яё]+\s+(20\d{2}|\d{2})\b', ' ', cleaned)
    cleaned = re.sub(r'\b(20\d{2})-(0[1-9]|1[0-2])\b', ' ', cleaned)
    cleaned = re.sub(r'\b(0?[1-9]|1[0-2])[\/\.\-\s](20\d{2}|\d{2})\b', ' ', cleaned)
    month_words = ['январь','января','февраль','февраля','март','марта','апрель','апреля','май','мая','июнь','июня','июль','июля','август','августа','сентябрь','сентября','октябрь','октября','ноябрь','ноября','декабрь','декабря']
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    stripped_for_period = _remove_period_fragments_v3(text)
    stripped_for_period = re.sub(r'\b(бизнес|business|компания|весь бизнес)\b', ' ', stripped_for_period)
    stripped_for_period = re.sub(r'\s+', ' ', stripped_for_period).strip()
    if stripped_for_period == '':
        return {'status': 'ok', 'query': {'mode': mode, 'level': 'business', 'object_name': 'business', 'period_current': period_current, 'period_previous': period_previous, 'query_type': 'summary', 'period': period_current, 'object': 'business'}}
    ...
    if special_query_type:
        stripped_text = _strip_special_query_types(stripped_text)
    entity_text = _remove_period_fragments_v3(stripped_text)
    entity_text = re.sub(r'\b(бизнес|business|компания|весь бизнес)\b', ' ', entity_text)
...
def _pre_normalize_entry_text(message: str) -> str:
    text = normalize_user_message(message)
    text = re.sub(r"\b(20\d{2})[./](0[1-9]|1[0-2])\b", r"\1-\2", text)
    text = re.sub(r"\b(0?[1-9]|1[0-2])[./](20\d{2})\b", lambda m: f"{int(m.group(2)):04d}-{int(m.group(1)):02d}", text)
    m = re.search(r"\b(20\d{2})-(0[1-9]|1[0-2])\s*(?:\+|плюс|и)\s*(20\d{2})-(0[1-9]|1[0-2])\b", text)
    if m and m.group(1) == m.group(3):
        a = f"{m.group(1)}-{m.group(2)}"
        b = f"{m.group(3)}-{m.group(4)}"
        start, end = sorted([a, b])
        text = re.sub(r"\b(20\d{2})-(0[1-9]|1[0-2])\s*(?:\+|плюс|и)\s*(20\d{2})-(0[1-9]|1[0-2])\b", f"{start}:{end}", text, count=1)
    return text


def parse_summary_entry(message: str) -> Dict[str, Any]:
    text = _pre_normalize_entry_text(message)
    direct_range = re.fullmatch(r"(?:бизнес\s+)?(20\d{2}-\d{2}:20\d{2}-\d{2})", text)
    if direct_range:
        return {'status': 'ok', 'level': 'business', 'object_name': 'business', 'period': direct_range.group(1)}
    parsed = parse_query_intent(text)
    if parsed.get('status') == 'ok':
        query = parsed.get('query') or {}
        period = query.get('period_current') or query.get('period')
        if period:
            level = query.get('level') or 'business'
            object_name = query.get('object_name') or ('business' if level == 'business' else None)
            return {'status': 'ok', 'level': level, 'object_name': object_name, 'period': period}

    period, _ = resolve_period_from_message(text)
    if period:
        cleaned = _remove_period_fragments_v3(text)
        cleaned = re.sub(r"\b(бизнес|business|компания|весь бизнес)\b", ' ', cleaned)
        cleaned = re.sub(r"\s+", ' ', cleaned).strip()
        if cleaned == '':
            return {'status': 'ok', 'level': 'business', 'object_name': 'business', 'period': period}
        guessed_name = ' '.join(part.capitalize() for part in cleaned.split())
        return {'status': 'ok', 'level': 'manager', 'object_name': guessed_name, 'period': period}

    return {'status': 'error', 'reason': 'Не удалось распознать вход. Используй период, Бизнес + период или ФИО + период'}

# ===== FINAL ENTRY LOCK OVERRIDES =====

_ENTRY_HELP_TEXT = 'Не удалось распознать вход. Примеры: 2026-02, Бизнес 2026-02, Иванов Иван 2026-02'
_MONTH_NAME_TO_NUM = {
    'январь': '01', 'января': '01',
...
_BUSINESS_WORDS = {'бизнес', 'business', 'компания', 'весь бизнес'}


def _entry_clean_text(message: str) -> str:
    text = clean_text(message).lower()
    text = text.replace('–', '→').replace('—', '→')
    text = text.replace('«', ' ').replace('»', ' ').replace('"', ' ').replace("'", ' ').replace('`', ' ')
    text = re.sub(r'\s+', ' ', text).strip()
...
def _entry_is_numeric_command(text: str) -> bool:
    return bool(re.fullmatch(r'\d+', text))


def _entry_expand_compact_months(text: str) -> str:
    # 2026.02 -> 2026-02 and 02.2026 -> 2026-02
    text = re.sub(r'\b(20\d{2})[./](0[1-9]|1[0-2])\b', r'\1-\2', text)
    text = re.sub(r'\b(0?[1-9]|1[0-2])[./](20\d{2}|\d{2})\b', lambda m: f"{_normalize_year(m.group(2))}-{int(m.group(1)):02d}", text)
    return text


def _entry_parse_period_range(text: str) -> Optional[Tuple[str, str]]:
    text = _entry_expand_compact_months(text)

    # explicit YYYY-MM -> YYYY-MM
    m = re.search(r'\b(20\d{2}-\d{2})\s*(?:→|->|до)\s*(20\d{2}-\d{2})\b', text)
    if m:
        a, b = sorted([m.group(1), m.group(2)])
        return a, b

    # YYYY-MM + YYYY-MM / плюс / и
    m = re.search(r'\b(20\d{2}-\d{2})\s*(?:\+|плюс|и)\s*(20\d{2}-\d{2})\b', text)
    if m:
        a, b = sorted([m.group(1), m.group(2)])
        return a, b
    ...
    if m:
        ...
        a = f"{year}-{int(m.group(1)):02d}"
        b = f"{year}-{int(m.group(2)):02d}"
        a, b = sorted([a, b])
        return a, b

    # monthname-monthname yyyy / январь-февраль 2026
    month_words = '|'.join(sorted(_MONTH_NAME_TO_NUM.keys(), key=len, reverse=True))
    m = re.search(rf'\b({month_words})\s*[-/]\s*({month_words})\s+(20\d{{2}}|\d{{2}})\b', text)
    if m:
        year = _normalize_year(m.group(3))
        a = f"{year}-{_MONTH_NAME_TO_NUM[m.group(1)]}"
        b = f"{year}-{_MONTH_NAME_TO_NUM[m.group(2)]}"
        a, b = sorted([a, b])
        return a, b

    # с января по февраль 2026
    m = re.search(rf'\bс\s+({month_words})\s+по\s+({month_words})\s+(20\d{{2}}|\d{{2}})\b', text)
    if m:
        year = _normalize_year(m.group(3))
        a = f"{year}-{_MONTH_NAME_TO_NUM[m.group(1)]}"
        b = f"{year}-{_MONTH_NAME_TO_NUM[m.group(2)]}"
...
def _entry_parse_single_period(text: str) -> Optional[str]:
    text = _entry_expand_compact_months(text)
    ...
    month_words = '|'.join(sorted(_MONTH_NAME_TO_NUM.keys(), key=len, reverse=True))
    m = re.search(rf'\b({month_words})\s+(20\d{{2}}|\d{{2}})\b', text)
    if m:
        return f"{_normalize_year(m.group(2))}-{_MONTH_NAME_TO_NUM[m.group(1)]}"

    return None


def _entry_extract_period_bounds(text: str) -> Tuple[Optional[str], Optional[str]]:
    rng = _entry_parse_period_range(text)
    if rng:
        return rng
    single = _entry_parse_single_period(text)
    if single:
        return single, single
    return None, None


def _entry_remove_period_and_business(text: str) -> str:
    cleaned = _entry_expand_compact_months(text)
    # remove ranges first
    cleaned = re.sub(r'\b20\d{2}-\d{2}\s*(?:→|->|до|\+|плюс|и)\s*20\d{2}-\d{2}\b', ' ', cleaned)
    cleaned = re.sub(r'\b(0?[1-9]|1[0-2])\s*[-/]\s*(0?[1-9]|1[0-2])\s+(20\d{2}|\d{2})\b', ' ', cleaned)
    month_words = '|'.join(sorted(_MONTH_NAME_TO_NUM.keys(), key=len, reverse=True))
    cleaned = re.sub(rf'\b({month_words})\s*[-/]\s*({month_words})\s+(20\d{{2}}|\d{{2}})\b', ' ', cleaned)
    cleaned = re.sub(rf'\bс\s+({month_words})\s+по\s+({month_words})\s+(20\d{{2}}|\d{{2}})\b', ' ', cleaned)
    cleaned = re.sub(r'\b20\d{2}-\d{2}\b', ' ', cleaned)
    cleaned = re.sub(rf'\b({month_words})\s+(20\d{{2}}|\d{{2}})\b', ' ', cleaned)
...
def _entry_resolve_object_name(entity_text: str, period_lookup: str) -> Tuple[Optional[str], Optional[str]]:
    if not entity_text:
        return 'business', 'business'

    norm = normalize_entity_text(entity_text)
    if norm in _BUSINESS_WORDS:
        return 'business', 'business'

    try:
        level, name = _resolve_scope_entity(entity_text, period_lookup)
    except Exception:
    ...
    # deterministic soft manager input: one or more tokens becomes manager, but never random network
    if tokens:
        guessed_name = ' '.join(part.capitalize() for part in tokens)
        return 'manager', guessed_name
    return None, None


def _entry_build_query(level: str, object_name: str, period_from: str, period_to: str, query_type: str = 'summary', target_level: Optional[str] = None) -> Dict[str, Any]:
    period_current = f"{period_from}:{period_to}" if period_from != period_to else period_from
    payload = {
        'mode': 'diagnosis',
        'level': level,
        'object_name': object_name,
    ...
    return {'status': 'ok', 'query': payload}


def parse_query_intent(message: str) -> Dict[str, Any]:
    text = _entry_clean_text(message)
    if not text:
        return error_response('Пустой запрос')

    if _entry_is_numeric_command(text):
        return {'status': 'ok', 'query': {'mode': 'diagnosis', 'level': None, 'object_name': None, 'period_current': None, 'period_previous': None, 'query_type': 'navigate_numeric', 'period': None, 'object': None, 'selection': int(text)}}

    if text in SHORT_DRILL_COMMANDS:
        return {'status': 'ok', 'query': {'mode': 'diagnosis', 'level': None, 'object_name': None, 'period_current': None, 'period_previous': None, 'query_type': 'drill_down', 'target_level': SHORT_DRILL_COMMANDS[text], 'period': None, 'object': None}}

    period_from, period_to = _entry_extract_period_bounds(text)
    if not period_from:
        return error_response(_ENTRY_HELP_TEXT)

    entity_text = _entry_remove_period_and_business(text)
    if entity_text == '':
        return _entry_build_query('business', 'business', period_from, period_to, 'summary')

    period_lookup = period_from
    level, object_name = _entry_resolve_object_name(entity_text, period_lookup)
    if not level or not object_name:
        return error_response(_ENTRY_HELP_TEXT)

    if level == 'business':
        return _entry_build_query('business', 'business', period_from, period_to, 'summary')

    return _entry_build_query(level, object_name, period_from, period_to, 'summary')


def parse_summary_entry(message: str) -> Dict[str, Any]:
    parsed = parse_query_intent(message)
    if parsed.get('status') != 'ok':
        return {'status': 'error', 'reason': _ENTRY_HELP_TEXT}
    query = parsed.get('query') or {}
    period = query.get('period_current') or query.get('period')
    level = query.get('level') or 'business'
    object_name = query.get('object_name') or ('business' if level == 'business' else None)
    if query.get('query_type') == 'navigate_numeric':
        return {'status': 'error', 'reason': _ENTRY_HELP_TEXT}
    if not period:
        return {'status': 'error', 'reason': _ENTRY_HELP_TEXT}
    return {'status': 'ok', 'level': level, 'object_name': object_name, 'period': period, 'period_from': query.get('period_from'), 'period_to': query.get('period_to'), 'query_type': query.get('query_type')}
