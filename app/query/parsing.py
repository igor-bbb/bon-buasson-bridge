...
def _extract_person_candidate_text(text: str) -> str:
    ...
    cleaned = re.sub(r'[а-яё]+\s+по\s+[а-яё]+', ' ', cleaned)
...
def _resolve_manager_candidates(text: str, period: Optional[str]) -> Tuple[List[str], Optional[str]]:
    ...
    tokens = [token for token in fuzzy_text.split() if token]
    if len(tokens) < 2:
    ...
    for name in pool:
        ...
        ratio = SequenceMatcher(None, fuzzy_text, name_fuzzy).ratio()
        token_overlap = len(set(tokens) & set(name_fuzzy.split()))
        if token_overlap >= 1:
            ratio = max(ratio, min(0.99, 0.62 + 0.15 * token_overlap))
        if ratio >= 0.70:
    ...
    shortlisted = [name for ratio, name in scored if ratio >= max(0.70, top_ratio - 0.08)]
...
def parse_query_intent(message: str) -> Dict[str, Any]:
    ...
    periods = extract_periods_from_text(text) if period_current else []
    mode = detect_mode(periods, text)
    period_previous = resolved_previous if mode == 'comparison' else None

    if not period_current:
        # If we can clearly see a person-like query but no period, ask for period instead of generic failure.
        manager_candidates, manager_candidate_text = _resolve_manager_candidates(text, None)
        if manager_candidates:
            return error_response(f'уточните период для {manager_candidates[0]}')
        if manager_candidate_text:
            ...
            if len(candidate_tokens) >= 2 and not has_business_lock:
                return error_response(f'уточните период для {manager_candidate_text}')
