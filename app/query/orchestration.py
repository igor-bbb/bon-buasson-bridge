...
def _build_screen_state_from_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    data = payload if isinstance(payload, dict) else {}
    context = data.get('context') or {}
    navigation = data.get('navigation') or {}
    items = navigation.get('items') or []
    return {
        'current_level': context.get('level') or data.get('level'),
        'current_object': context.get('object_name') or data.get('object_name'),
        'navigation_items': items if isinstance(items, list) else [],
        'has_all': bool(navigation.get('has_all')),
        'has_causes': bool(navigation.get('has_causes')),
        'has_back': bool(navigation.get('has_back', True)),
        'all_block': data.get('all_block') or [],
        'cause_block': data.get('cause_block') or [],
        'structure': data.get('structure') or {},
    }


def save_screen_state(session_id: str, payload: Dict[str, Any]) -> None:
    update_session(session_id, {'screen_state': _build_screen_state_from_payload(payload)})
...
def _handle_all_from_state(session_id: str) -> Dict[str, Any]:
    state = get_session(session_id)
    screen_state = (state.get('screen_state') or {})
    last_payload = state.get('last_payload') or {}
    if not screen_state or not isinstance(last_payload, dict):
        return {'status': 'error', 'reason': 'Нет активного экрана.'}
    all_block = screen_state.get('all_block') or []
    if not all_block:
        return {'status': 'error', 'reason': 'Нет активного экрана.'}
    payload = _clone_payload(last_payload)
    payload['drain_block'] = list(all_block)
    nav = dict(payload.get('navigation') or {})
    nav['items'] = [item.get('object_name') for item in all_block if item.get('object_name')]
    nav['has_all'] = bool(screen_state.get('has_all'))
    nav['has_causes'] = bool(screen_state.get('has_causes'))
    nav['has_back'] = True
    payload['navigation'] = nav
    save_screen_state(session_id, payload)
    save_last_payload(session_id, payload)
    save_session_state(session_id, last_list_items=_build_last_list_items(all_block, state.get('last_list_level')), view_mode='all')
    return payload


def _handle_causes_from_state(session_id: str) -> Dict[str, Any]:
    state = get_session(session_id)
    screen_state = (state.get('screen_state') or {})
    last_payload = state.get('last_payload') or {}
    if not screen_state or not isinstance(last_payload, dict):
        return {'status': 'error', 'reason': 'Нет активного экрана.'}
    cause_block = screen_state.get('cause_block') or []
    if not cause_block:
        return {'status': 'error', 'reason': 'Нет активного экрана.'}
    payload = _clone_payload(last_payload)
    payload['drain_block'] = [
        {
            'object_name': item.get('name') or item.get('article') or item.get('object_name'),
            'effect_money': item.get('effect_money'),
        }
        for item in cause_block
    ]
    nav = dict(payload.get('navigation') or {})
    nav['items'] = [item.get('name') or item.get('article') for item in cause_block if (item.get('name') or item.get('article'))]
    nav['has_all'] = bool(screen_state.get('has_all'))
    nav['has_causes'] = bool(screen_state.get('has_causes'))
    nav['has_back'] = True
    payload['navigation'] = nav
    ...
    save_last_payload(session_id, payload)
    cause_items = [
        {'object_name': item.get('name') or item.get('article'), 'effect_money': item.get('effect_money')}
        for item in cause_block
        if (item.get('name') or item.get('article'))
    ]
    save_session_state(session_id, last_list_items=_build_last_list_items(cause_items, state.get('scope_level')), view_mode='causes')
    return payload
...
def _route_drill_query(query: Dict[str, Any], session_ctx: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if response.get('status') == 'ok':
        ...
        save_screen_state(session_id, response)
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if normalized == 'все':
        return _handle_all_from_state(session_id)

    if normalized == 'причины':
        return _handle_causes_from_state(session_id)
