    ...
    if 0 <= index < len(items):
        ...
        return {
            ...
            'query': {
                ...
                'filter_payload': _merge_filter_payload(state.get('filter') or {}, selected.get('level'), selected.get('object_name'), state.get('period')),
            },
        }
...
def _summary_filter_for_query(level: str, object_name: Optional[str], period: str, session_ctx: Optional[Dict[str, Any]] = None, explicit_filter: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    state = get_session_state(session_ctx or {}) if session_ctx is not None else {}
    base_filter = dict(state.get('filter') or {})
    if explicit_filter:
        base_filter.update({k: v for k, v in explicit_filter.items() if v is not None})
    base_filter = _merge_filter_payload(base_filter, level, object_name, period)
    return base_filter


def _execute_summary(level: str, object_name: Optional[str], period: str, session_ctx: Optional[Dict[str, Any]] = None, explicit_filter: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    executor = SUMMARY_EXECUTORS.get(level)
    if executor is None:
        return {'error': 'base query not supported'}
    filter_payload = _summary_filter_for_query(level, object_name, period, session_ctx, explicit_filter=explicit_filter)
    if level == 'sku' and not filter_payload.get('network'):
        return {'error': 'no data after filtering', 'reason': 'sku requires parent filter'}
...
def _store_scope(session_id: str, level: str, object_name: str, period_current: str, period_previous: Any, mode: str, existing_filter: Optional[Dict[str, Any]] = None) -> None:
...
def _build_query_from_full_view(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not level or not object_name or not period:
        restored = _extract_state_from_last_payload(session_ctx.get('last_payload') or {})
        level = level or restored.get('level')
        object_name = object_name or restored.get('object_name')
        period = period or restored.get('period')
...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    drain_payload = None
    if target_level:
        source = _build_drill_from_scope(level, object_name, target_level, period, full_view=full_view, filter_payload=(current.get('filter') or query.get('filter_payload') or get_session_state(get_session(session_id)).get('filter') or {}))
    ...
    if response.get('status') == 'ok':
        _store_scope(session_id, level, object_name, period, period_previous, 'diagnosis', existing_filter=(current.get('filter') or query.get('filter_payload')))
...
def _route_base_query(query: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if query.get('query_type') == 'reasons':
        ...
        if response.get('status') == 'ok':
            _store_scope(session_id, level, resolved_object_name, resolved_period, query.get('period_previous'), 'diagnosis', existing_filter=(current.get('filter') or query.get('filter_payload')))
...
def _route_drill_query(query: Dict[str, Any], session_ctx: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    flow_target = DEFAULT_NEXT_LEVEL.get(scope_level)
    if flow_target and target_level != flow_target:
        target_level = flow_target
    ...
    payload = _build_drill_from_scope(scope_level, scope_object_name, target_level, period, full_view=full_view, filter_payload=(query.get('filter_payload') or state.get('filter') or {}))
    if 'error' in payload:
    ...
    if summary_executor is None:
        return not_implemented_response(query, 'base query not supported')
    current = _execute_summary(scope_level, scope_object_name, period, session_ctx, explicit_filter=(query.get('filter_payload') or state.get('filter') or {}))
    if 'error' in current:
        return error_response(current['error'], query)
    if scope_level == 'business':
        current = dict(current)
    ...
    current = _with_previous_metrics(current, _execute_summary(scope_level, scope_object_name, _previous_year_period(period), session_ctx, explicit_filter=(query.get('filter_payload') or state.get('filter') or {})) if _previous_year_period(period) and SUMMARY_EXECUTORS.get(scope_level) else None)
    ...
    if response.get('status') == 'ok':
        _store_scope(session_id, scope_level, scope_object_name, period, period_previous, 'diagnosis', existing_filter=(current.get('filter') or query.get('filter_payload') or state.get('filter') or {}))
