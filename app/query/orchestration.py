...
def sanitize_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'items_meta': payload.get('items_meta'),
    }
...
def enforce_contract(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # NEW SUMMARY CONTRACT: accept pure summary payload without legacy type field
    summary_keys = {'context', 'metrics', 'drain_block', 'goal', 'focus_block', 'navigation'}
    if summary_keys.issubset(set(data.keys())):
        if not isinstance(data.get('metrics'), dict):
            return {'status': 'error', 'reason': 'invalid summary metrics'}
        if not isinstance(data.get('drain_block'), list):
            return {'status': 'error', 'reason': 'invalid summary drain'}
        if not isinstance(data.get('navigation'), dict):
            return {'status': 'error', 'reason': 'invalid summary navigation'}
        return response
...
def _build_query_from_full_view(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if (not level or not object_name or not period) and isinstance(session_ctx.get('last_payload'), dict):
        ...
        if not state.get('last_list_level') and data.get('children_level'):
            state['last_list_level'] = data.get('children_level')
