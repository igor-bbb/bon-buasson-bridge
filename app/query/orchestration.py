...
def _execute_numeric_workspace_action(message: str, session_ctx: Dict[str, Any], session_id: str) -> Optional[Dict[str, Any]]:
    ...
    if level == 'network':
        # Contract Workspace rule:
        # numeric commands must follow the visible screen. In the analytical
        # contract desktop the first numbered option is the first category
        # (for example, 1 -> «Напитки»), not a global action package.
        # Action packages remain available by text aliases: «подготовить
        # переговоры», «собрать пакет позиций», «создать задачи».
        if n in {1, 2, 3}:
            return None  # let _build_query_from_numeric_selection open visible category/list item
        if n == 4:
            return show_reasons()
        if n == 5:
            result = _execute_action_command('tasks', session_ctx)
