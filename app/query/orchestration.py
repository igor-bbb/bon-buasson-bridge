...
def _voice_group_rating_rows(
    ...
    """Build Voice ratings from the same VECTRA Engine metrics as screens.

    BUG-021: percentage ratings must not derive their own formulas or average
    row percentages. For each displayed object, use metrics_raw from the domain
    summary layer. That makes a network/manager/SKU show the same margin and
    markup in Voice Rating as on its object screen for the same period.
    """
    ...
    for name in grouped.keys():
        if level and period:
            metrics = _voice_metrics_from_engine(level, name, period)
            previous_metrics = _voice_previous_metrics_from_engine(level, name, period)
        else:
            # Kept only as a defensive branch for tests. Runtime call always
            # passes level+period. The calculation still uses aggregate_metrics,
            # the same core helper used by the domain summary.
            chunk = grouped.get(name, [])
            metrics = aggregate_metrics(chunk)
            previous_grouped = _voice_group_map(previous_rows or [], group_field)
            previous_metrics = aggregate_metrics(previous_grouped.get(name, [])) if previous_grouped.get(name) else {}

        if not metrics:
            continue

        _voice_log_rating_metrics('rating_engine', name, metrics, previous_metrics)
