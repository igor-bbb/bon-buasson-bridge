...
def _execute_voice_management_request(message: str, intent: Dict[str, Any], session_ctx: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    try:
        entity = resolve_entity(message, get_entity_dictionary())
        level = entity.get('entity_type')
        object_name = entity.get('entity_name')
        if level and object_name:
            period = _voice_period_from_message_or_state(message, session_ctx, get_normalized_rows())
            if period:
                query = {
                    'level': level,
                ...
                return _route_base_query(query, 'voice_open')
    except Exception:
        pass
