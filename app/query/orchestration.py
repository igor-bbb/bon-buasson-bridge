...
def push_state(session_id: str) -> None:
    ...
    snapshot = {
        ...
        'branch_mode': current.get('branch_mode') or 'vector',
        'branch_meta': deepcopy(current.get('branch_meta') or {}),
    }
...
def get_session_state(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        'branch_mode': session_ctx.get('branch_mode') or 'vector',
        'branch_meta': session_ctx.get('branch_meta') or {},
    }
...
def save_session_state(
    ...
    *,
    ...
    if branch_meta is not None:
        payload['branch_meta'] = branch_meta
    if payload:
...
def _current_contract_from_session(session_ctx: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    payload = session_ctx.get('last_payload')
    if not isinstance(payload, dict):
        return None
...
def _prepare_branch_response(base_payload: Dict[str, Any], drain_items: List[Dict[str, Any]], list_level: Optional[str], branch_mode: str, branch_meta: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    response = dict(base_payload)
    response['drain_block'] = drain_items
    navigation = dict(response.get('navigation') or {})
    navigation['items'] = [item.get('object_name') or item.get('name') for item in drain_items if (item.get('object_name') or item.get('name'))]
    navigation['has_back'] = True
    response['navigation'] = navigation
    response['_branch_mode'] = branch_mode
    response['_branch_meta'] = branch_meta or {}
    response['_list_level'] = list_level
    return response


def _build_all_branch_response(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    current = _current_contract_from_session(session_ctx)
    if not current:
        return {'status': 'error', 'reason': 'Нет активного экрана для ветки все.'}
    all_block = current.get('all_block') or []
    list_level = ((current.get('navigation') or {}).get('next_level') or current.get('children_level'))
    return _prepare_branch_response(current, list(all_block), list_level, 'all')


def _build_causes_branch_response(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    current = _current_contract_from_session(session_ctx)
    if not current:
        return {'status': 'error', 'reason': 'Нет активного экрана для ветки причины.'}
    cause_block = current.get('cause_block') or []
    drain_items = [{'object_name': item.get('name'), 'effect_money': item.get('effect_money')} for item in cause_block]
    return _prepare_branch_response(current, drain_items, 'cause_article', 'causes')


def _build_cause_filtered_response(session_ctx: Dict[str, Any], article_name: str) -> Dict[str, Any]:
    current = _current_contract_from_session(session_ctx)
    if not current:
        return {'status': 'error', 'reason': 'Нет активного экрана для фильтра по причине.'}

    state = get_session_state(session_ctx)
    period = state.get('period') or current.get('period')
    current_level = state.get('level') or current.get('level')
    current_object = state.get('object_name') or current.get('object_name')
    next_level = ((current.get('navigation') or {}).get('next_level') or current.get('children_level'))
    all_block = current.get('all_block') or []
    structure_key = {
        'наценка': 'markup',
        'ретро': 'retro',
        ...
        'прочие': 'other',
        'прочее': 'other',
    }.get(normalize_entity_text(article_name), None)
    if not structure_key:
        return {'status': 'error', 'reason': 'Причина не распознана.'}
    if not next_level:
        return {'status': 'error', 'reason': 'Нет следующего уровня для фильтра по причине.'}

    ranked = []
    for item in all_block:
        child_name = item.get('object_name')
        if not child_name:
            continue
        child = _execute_summary(next_level, child_name, period, session_ctx, explicit_filter=state.get('filter') or current.get('filter'))
        if not isinstance(child, dict) or child.get('error'):
            continue
        effect_money = (((child.get('structure') or {}).get(structure_key) or {}).get('effect_money') or 0)
        ranked.append({'object_name': child_name, 'effect_money': effect_money})

    ranked = sorted(ranked, key=lambda x: _to_float(x.get('effect_money')))
    filtered = [x for x in ranked if _to_float(x.get('effect_money')) < 0]
    response = _prepare_branch_response(current, filtered or ranked, next_level, 'cause_filtered', {'article': article_name, 'article_key': structure_key})
    response['cause_filter'] = {'name': article_name}
    return response
...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 0 <= index < len(items):
        ...
        branch_mode = state.get('branch_mode') or 'vector'
        if branch_mode == 'causes':
            return {
                'status': 'ok',
                'query': {
                    'mode': 'diagnosis',
                    'query_type': 'cause_filter',
                    'article_name': selected.get('object_name'),
                },
            }
        return {
            'status': 'ok',
...
def _handle_all_branch(session_id: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    response = _build_all_branch_response(session_ctx)
    if response.get('status') == 'error':
        return response
    push_state(session_id)
    state = get_session_state(session_ctx)
    list_items = _build_last_list_items(response.get('drain_block', []), response.get('_list_level'))
    save_session_state(session_id, last_response_type='all', last_list_level=response.get('_list_level'), last_list_items=list_items, full_view=True, branch_mode='all', branch_meta={})
    save_last_payload(session_id, response)
    return response


def _handle_causes_branch(session_id: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    response = _build_causes_branch_response(session_ctx)
    if response.get('status') == 'error':
        return response
    push_state(session_id)
    list_items = _build_last_list_items(response.get('drain_block', []), response.get('_list_level'))
    save_session_state(session_id, last_response_type='causes', last_list_level=response.get('_list_level'), last_list_items=list_items, full_view=False, branch_mode='causes', branch_meta={})
    save_last_payload(session_id, response)
    return response


def _handle_cause_filter(session_id: str, session_ctx: Dict[str, Any], article_name: str) -> Dict[str, Any]:
    response = _build_cause_filtered_response(session_ctx, article_name)
    if response.get('status') == 'error':
        return response
    push_state(session_id)
    list_items = _build_last_list_items(response.get('drain_block', []), response.get('_list_level'))
    save_session_state(session_id, last_response_type='cause_filter', last_list_level=response.get('_list_level'), last_list_items=list_items, full_view=False, branch_mode='cause_filtered', branch_meta=response.get('_branch_meta') or {})
    save_last_payload(session_id, response)
    return response
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if normalized == 'все':
        return enforce_contract(_handle_all_branch(session_id, session_ctx))
    if normalized == 'причины':
        return enforce_contract(_handle_causes_branch(session_id, session_ctx))
    ...
    if query_type == 'cause_filter':
        return enforce_contract(_handle_cause_filter(session_id, get_session(session_id), query.get('article_name')))
