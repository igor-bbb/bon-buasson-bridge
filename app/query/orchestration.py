...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    # W14.3 Critical: Development Journal capture is a dedicated high-priority
    # route. It must never fall through to analytical, discovery or corporate
    # routing, otherwise Production feedback can be lost.
    ...
    if is_capture_command(message):
        record = add_development_journal_record(message, session_ctx, session_id=session_id)
        return _finalize_response(build_development_journal_capture_response(record))

    if is_development_journal_export_command(message):
        return _finalize_response(build_development_journal_response(export=True))

    if is_development_journal_show_command(message):
        return _finalize_response(build_development_journal_response(export=False))
    ...
    if is_product_review_command(message):
        return _finalize_response(build_product_review_response(list_development_journal_records()))

    if is_sprint_plan_command(message):
        return _finalize_response(build_sprint_plan_response(list_development_journal_records()))
