...
# ===== CLEAN ORCHESTRATION (NO INTERACTIVE) =====
import re
...
def orchestrate_vectra_query(message: str, session_ctx=None):
    ...
    from app.presentation.contracts import ok_response, error_response

    msg = message.lower().strip()

    period_match = re.search(r'\d{4}-\d{2}', msg)
    if not period_match:
        return error_response("period not found", {})

    period = period_match.group()

    if "менедж" in msg:
        level = "manager"
        obj = msg.replace(period, "").strip()
        data = get_manager_comparison(manager=obj, period=period)

    elif "сеть" in msg:
        level = "network"
        obj = msg.replace(period, "").strip()
        data = get_network_comparison(network=obj, period=period)

    elif "катег" in msg:
        level = "category"
        obj = msg.replace(period, "").strip()
        data = get_category_comparison(category=obj, period=period)

    elif "tmc" in msg:
        level = "tmc_group"
        obj = msg.replace(period, "").strip()
        data = get_tmc_group_comparison(tmc_group=obj, period=period)

    elif "sku" in msg:
        level = "sku"
        obj = msg.replace(period, "").strip()
        data = get_sku_comparison(sku=obj, period=period)
