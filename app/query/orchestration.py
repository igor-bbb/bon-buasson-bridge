...
def _execute_voice_analytical_request(
    ...
    if intent_type == 'find_money':
        return _build_find_money_response(screen)
    if intent_type == 'next_best_action':
        return _build_next_best_action_response(screen)
    if intent_type == 'teach_me':
        return _build_teach_me_response(screen, message)
    return _build_explain_current_object_response(screen)
