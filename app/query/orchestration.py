...
def save_session_state(
    ...
    *,
    level: Any = _CLEAR,
    object_name: Any = _CLEAR,
    period: Any = _CLEAR,
    period_previous: Any = _CLEAR,
    last_list_level: Any = _CLEAR,
    last_response_type: Any = _CLEAR,
    full_view: Any = _CLEAR,
    last_list_items: Any = _CLEAR,
    mode: Any = _CLEAR,
    last_payload: Any = _CLEAR,
) -> None:
    payload: Dict[str, Any] = {}
    if level is not _CLEAR:
        payload['scope_level'] = level
    if object_name is not _CLEAR:
        payload['scope_object_name'] = object_name
    if period is not _CLEAR:
        payload['period_current'] = period
    if period_previous is not _CLEAR:
        payload['period_previous'] = period_previous
    if last_list_level is not _CLEAR:
        payload['last_list_level'] = last_list_level
    if last_response_type is not _CLEAR:
        payload['last_response_type'] = last_response_type
    if full_view is not _CLEAR:
        payload['full_view'] = full_view
    if last_list_items is not _CLEAR:
        payload['last_list_items'] = last_list_items
    if mode is not _CLEAR:
        payload['mode'] = mode
    if last_payload is not _CLEAR:
        payload['last_payload'] = last_payload
...
ENTRY_START_COMMANDS = {'начать анализ', 'старт', 'start', 'подключись к api', 'подключись к апи'}
...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not items and isinstance(state.get('last_payload'), dict):
        items = _extract_list_items_from_payload(state.get('last_payload') or {})
    index = int(message) - 1
    if 0 <= index < len(items):
        selected = items[index]
        level = selected.get('level') or state.get('last_list_level')
        return {
            'status': 'ok',
            'query': {
                'mode': 'diagnosis',
...
def _extract_list_items_from_payload(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    if not isinstance(data, dict):
        return []
    items = data.get('items') if isinstance(data.get('items'), list) else []
    level = data.get('next_level') or data.get('children_level')
    prepared = _build_last_list_items(items, level)
    if prepared:
        return prepared
    top_items = ((data.get('top_summary') or {}).get('top_items') or [])
    inferred_level = level or data.get('next_level')
    return _build_last_list_items(top_items, inferred_level)


def _persist_successful_response(session_id: str, query: Dict[str, Any], response: Dict[str, Any]) -> None:
    ...
    save_session_state(
        ...
        last_response_type=data.get('mode') or query.get('query_type') or 'summary',
        last_list_level=data.get('next_level') or data.get('children_level'),
        last_list_items=_extract_list_items_from_payload(data),
...
def _build_query_from_short_command(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not _has_consistent_object_state(state):
        ...
        if isinstance(payload, dict) and payload.get('level') and payload.get('object_name') and payload.get('period'):
            state['level'] = payload.get('level')
            state['object_name'] = payload.get('object_name')
            state['period'] = payload.get('period')
        if not _has_consistent_object_state(state):
...
def _resolve_direct_object_entry(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    normalized = _normalize_message(message)
    only_period = normalize_entity_text(normalized.replace(period, ' ').replace(':', ' ').strip()) == ''
    if only_period:
        return {
            'status': 'ok',
            'query': {
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if query_type == 'drill_down':
        response = _route_drill_query(query, get_session(session_id), session_id)
        _persist_successful_response(session_id, query, response)
        return response

    response = _route_base_query(query, session_id)
    _persist_successful_response(session_id, query, response)
    return response
