...
ACTION_COMMAND_ALIASES = {
    ...
    'tasks': (
        'создать задачи', 'сформировать задачи', 'зафиксировать действия',
        'поставить задачи', 'задачи',
    ),
}
...
def _build_sku_package_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines = [
        f'📦 Пакет позиций для ввода — {ctx.get("object_name")}',
        f'Период: {ctx.get("period")}',
        '',
    ]
    if not sku_package:
        lines.append('В текущем рабочем столе нет подтверждённых отсутствующих позиций для формирования пакета.')
    else:
        lines.append('Собираю короткий пакет из позиций, которые уже являются лидерами бизнеса, но отсутствуют в текущем контракте.')
        lines.append('')
        lines.append('Позиция | Оборот бизнеса | Финрез до бизнеса | Почему в пакете')
        for item in sku_package:
            sku = item.get('sku') or 'Позиция'
            lines.append(f'{sku} | {_fmt_action_money(item.get("business_revenue"))} грн | {_fmt_action_money(item.get("business_finrez_pre"), signed=True)} грн | {item.get("reason") or "лидер бизнеса отсутствует в контракте"}')
        lines.extend([
            '',
            'Что делаем дальше:',
            '1. подготовить переговоры — собрать аргументы для байера',
            '2. создать задачи — зафиксировать ввод выбранных позиций',
            '3. назад — вернуться на рабочий стол контракта',
        ])
    return {
        'status': 'ok',
...
def _build_negotiation_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines = [f'🤝 Переговорная позиция — {ctx.get("object_name")}', f'Период: {ctx.get("period")}', '']
    goal = negotiation.get('goal') or 'развитие контракта на основании экономики, категорий и ассортимента'
    lines.append(f'Цель: {goal}.')
    if actions:
        lines.append('')
        lines.append('Приоритеты переговоров:')
        for idx, item in enumerate(actions[:3], 1):
            if isinstance(item, dict):
                label = item.get('label') or item.get('action') or item.get('reason') or 'направление работы'
                effect = item.get('expected_effect_money') or item.get('effect_money') or item.get('potential_money')
                lines.append(f'{idx}. {label} — эффект до {_fmt_action_money(effect)} грн')
    if sku_package:
        lines.append('')
        lines.append('Пакет позиций для обсуждения:')
        for sku in sku_package[:10]:
            lines.append(f'• {sku}')
    lines.extend(['', 'Что делаем дальше:', '1. собрать пакет позиций — уточнить позиции', '2. создать задачи — зафиксировать подготовку/ввод', '3. назад — вернуться к объекту'])
    return {
        'status': 'ok',
        'render_mode': 'action_package',
        'context': {'level': ctx.get('level'), 'object_name': ctx.get('object_name'), 'period': ctx.get('period'), 'parent_object': ctx.get('parent_object')},
        'path': screen.get('path') or [],
        'summary_block': 'Переговорная позиция подготовлена по текущему рабочему столу.',
        'kpi_block': lines,
        'structure_block': [],
        'drain_block_render': [],
        'drain_total': 0,
        'navigation_block': ['собрать пакет позиций — выбрать позиции', 'создать задачи — зафиксировать действия', 'назад — вернуться к объекту'],
        'screen_order': ['summary_block', 'kpi_block', 'navigation_block'],
    }
...
def _build_tasks_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    workspace = screen.get('decision_workspace') if isinstance(screen.get('decision_workspace'), dict) else {}
    actions = workspace.get('recommended_actions') if isinstance(workspace.get('recommended_actions'), list) else []
    lines = [f'📋 Черновик задач — {ctx.get("object_name")}', f'Период: {ctx.get("period")}', '']
    if not actions:
        lines.append('В текущем рабочем столе нет готового блока задач. Можно создать задачу вручную после выбора направления.')
    else:
        for idx, item in enumerate(actions[:5], 1):
            if not isinstance(item, dict):
                continue
            label = item.get('label') or item.get('action') or item.get('reason') or 'действие'
            effect = item.get('expected_effect_money') or item.get('effect_money') or item.get('potential_money')
            lines.append(f'{idx}. {label}')
            lines.append(f'   Основание: рабочий стол контракта {ctx.get("object_name")}.')
            lines.append(f'   Ожидаемый эффект: до {_fmt_action_money(effect)} грн.')
    lines.extend(['', 'Статус: черновик. Для назначения задач нужен следующий слой Task Engine.', 'назад — вернуться к объекту'])
    return {
        'status': 'ok',
        'render_mode': 'action_package',
        'context': {'level': ctx.get('level'), 'object_name': ctx.get('object_name'), 'period': ctx.get('period'), 'parent_object': ctx.get('parent_object')},
        'path': screen.get('path') or [],
        'summary_block': 'Черновик задач подготовлен по текущему рабочему столу.',
        'kpi_block': lines,
        'structure_block': [],
        'drain_block_render': [],
        'drain_total': 0,
...
def _render_all_screen_from_ready(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    obj_name = ctx.get('object_name') or out.get('object_name') or 'объект'
    period = ctx.get('period') or out.get('period') or ''
    parent_filter = _screen_filter_for_vitrine(out)
    lines = [
        f'Витрина объекта: {obj_name}' + (f' | {period}' if period else ''),
        '№ | Объект | Оборот | Финрез до | Δ прибыли | Доля объекта | Доля бизнеса | Сетей | Потенциал',
    ]
    list_items = []
    ...
    for idx, item in enumerate(block, start=1):
        ...
        lines.append(f'{idx} | {name} | {revenue_text} | {finrez_text} | {delta_text} грн | {parent_share} | {business_share} | {networks_text} | {potential_text}')
        list_items.append({
            'object_name': name,
...
def _handle_back(session_id: str) -> Dict[str, Any]:
    ...
    # hierarchy stack.
    if current.get('view_mode') in {'all', 'reasons', 'action'}:
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    elif _detect_action_command(normalized):
        action = _detect_action_command(normalized)
