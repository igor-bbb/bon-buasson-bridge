...
def _build_sku_package_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ctx.get('level') not in {'network', 'category', 'tmc_group'}:
        return {'status': 'error', 'reason': 'Сначала откройте контракт или категорию.'}
    ...
    # Category Workspace 2.0 fallback: category screens may not have a contract
    ...
    if not sku_package:
        ...
        lines.extend([
            ...
            'Логика пакета | позиции уже доказаны данными и используются как аргумент развития',
            '',
            'Позиция | Оборот в бизнесе | Финрез / эффект | Почему в пакете'
        ])
        for item in sku_package:
            ...
            lines.append(f'{sku} | {_fmt_action_money(item.get("business_revenue"))} грн | {_fmt_action_money(effect, signed=True)} грн | {item.get("reason") or "лидер бизнеса отсутствует в контракте"}')
        first_wave = sku_package[:5]
        if first_wave:
            lines.extend(['', 'Рекомендуемая первая волна'])
            for idx, item in enumerate(first_wave, 1):
                lines.append(f'{idx}. {item.get("sku") or "Позиция"}')
            lines.append('Комментарий ассистента: первая волна снижает риск отказа байера и даёт понятный тест результата без перегруза полки.')
        lines.extend([
            '',
            'Что делаем дальше:',
...
def _negotiation_strategy_type(screen: Dict[str, Any]) -> str:
    # Uses only already rendered/current workspace blocks. No KPI calculation here.
    text_parts = []
    for key in ('result_block', 'summary_block', 'diagnosis_block', 'kpi_block', 'structure_block', 'decision_workspace_block'):
    ...
    if any(token in text for token in ('сильный рост', 'демонстрирует рост', 'развит', 'растет', 'рост')):
        return 'развитие'
    return 'стабилизация'
...
def _build_negotiation_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines = [
        ...
        'Контракт открыт | ✅ | используется текущий Workspace',
        'Есть направления переговоров | ✅ | экономика / ассортимент / условия',
        'Есть пакет развития | ✅ | лидеры бизнеса и отсутствующие позиции' if sku_package else 'Есть пакет развития | ⚠️ | требуется собрать пакет позиций',
        'Есть критерии успеха | ✅ | минимум / норма / максимум',
        '',
    ...
    lines.extend([
        ...
        '## Цель встречи',
        'Уровень | Цель | Эффект',
        'Минимум | согласовать 2–3 позиции / следующий этап | быстрый тест без перегруза полки',
        'База | согласовать пакет развития | рост оборота и расширение матрицы',
        'Максимум | пакет развития + движение по условиям | дополнительный эффект по экономике',
        f'Контекст | {str(goal).rstrip(".")} | используется как общий вектор встречи',
        '',
    ...
    if sku_package:
        lines.extend([
            ...
            'Позиция | Почему именно она | Какой вопрос закрывает',
        ])
        for sku in sku_package[:10]:
            sku_text = str(sku)
            lines.append(f'{sku_text} | {_sku_argument_text(sku_text)} | расширение матрицы и рост оборота')

    lines.extend([
        '',
        '## Аргументы для байера',
        'Аргумент | Доказательство | Как использовать',
        'Контракт обсуждается на фактах | KPI и динамика текущего Workspace | начать встречу с подтверждённого результата',
        'Пакет развития не является гипотезой | позиции уже доказали силу на уровне бизнеса | предложить тестовую первую волну',
        'Условия обсуждаются вместе с ростом | ретро / логистика имеют денежный эффект | связывать уступки с расширением матрицы',
        '',
...
def _build_post_meeting_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        'context': {'level': ctx.get('level'), 'object_name': object_name, 'period': period, 'parent_object': ctx.get('parent_object')},
        'path': screen.get('path') or [],
        'summary_block': 'Post Meeting Workspace подготовлен по текущему объекту.',
        'kpi_block': lines,
        'structure_block': [],
        'drain_block_render': [],
        'drain_total': 0,
...
def _render_all_screen_from_ready(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Workspace here: large Contract/Manager screens can carry product,
...
def _render_kpi_screen_from_ready(screen: Dict[str, Any]) -> Dict[str, Any]:
    """Render focused KPI view from current Workspace state.

    KPI is a scope, not a new object analysis. It must not rebuild or expose the
    full Workspace when the user asks only for KPI.
    """
    if not isinstance(screen, dict):
    ...
    out = {
        ...
        'navigation_block': ['причины — разобрать факторы', 'все — витрина текущего уровня', 'назад — вернуться к предыдущему Workspace'],
...
def _is_kpi_view_command(message: str) -> bool:
    ...
    # API scope renderer, not hijack the current Workspace.
