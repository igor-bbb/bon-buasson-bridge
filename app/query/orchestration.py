...
def _extract_screen_state(payload: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    contract = enforce_contract(payload) if isinstance(payload, dict) else {}
    if not isinstance(contract, dict) or not isinstance(contract.get('context'), dict):
        return None
    navigation = contract.get('navigation') if isinstance(contract.get('navigation'), dict) else {}
    return {
        'current_level': contract.get('context', {}).get('level'),
        'current_object': contract.get('context', {}).get('object_name'),
        ...
        'all_block': list(contract.get('all_block') or []),
        'cause_block': list(contract.get('cause_block') or []),
        'structure': dict(contract.get('structure') or {}),
        'raw': contract,
    }


def save_screen_state(session_id: str, payload: Dict[str, Any]) -> None:
    state = _extract_screen_state(payload)
    if state is not None:
        update_session(session_id, {'screen_state': state})
...
def save_last_payload(session_id: str, payload: Dict[str, Any]) -> None:
    contract = enforce_contract(payload) if isinstance(payload, dict) else payload
    update_session(session_id, {'last_payload': contract})
    save_screen_state(session_id, contract)
...
def _build_local_all_response(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    screen_state = session_ctx.get('screen_state') or {}
    raw = screen_state.get('raw') if isinstance(screen_state, dict) else None
    if not isinstance(raw, dict):
        return {'status': 'error', 'reason': 'Нет активного экрана'}
    all_block = list(screen_state.get('all_block') or raw.get('all_block') or [])
    if not all_block:
        return {'status': 'error', 'reason': 'Нет активного экрана'}
    response = deepcopy(raw)
    response['drain_block'] = all_block
    nav = dict(response.get('navigation') or {})
    nav['items'] = [item.get('object_name') for item in all_block if item.get('object_name')]
    response['navigation'] = nav
    return response


def _build_local_reasons_response(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    screen_state = session_ctx.get('screen_state') or {}
    raw = screen_state.get('raw') if isinstance(screen_state, dict) else None
    if not isinstance(raw, dict):
        return {'status': 'error', 'reason': 'Нет активного экрана'}
    cause_block = list(screen_state.get('cause_block') or raw.get('cause_block') or [])
    if not cause_block:
        structure = dict(screen_state.get('structure') or raw.get('structure') or {})
        if structure:
            cause_block = [
                {'object_name': (item.get('name') or key), 'effect_money': item.get('effect_money')}
                for key, item in structure.items()
                if isinstance(item, dict)
            ]
            cause_block.sort(key=lambda x: float(x.get('effect_money') or 0))
    if not cause_block:
        return {'status': 'error', 'reason': 'Нет активного экрана'}
    response = deepcopy(raw)
    response['drain_block'] = cause_block
    nav = dict(response.get('navigation') or {})
    nav['items'] = [item.get('object_name') for item in cause_block if item.get('object_name')]
    response['navigation'] = nav
    return response
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if _is_full_reasons_command(normalized) or normalized == 'причины':
        return enforce_contract(_build_local_reasons_response(session_ctx))

    if _is_full_view_command(normalized):
        return enforce_contract(_build_local_all_response(session_ctx))
