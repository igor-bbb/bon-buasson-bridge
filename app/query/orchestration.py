...
def _store_scope(session_id: str, level: str, object_name: str, period_current: str, period_previous: Any, mode: str) -> None:
    filter_payload = _build_filter_from_scope(level, object_name, period_current)
    save_session_state(
        session_id,
        level=level,
        object_name=object_name,
...
def _store_list_context(
    ...
    filter_payload = _build_filter_from_scope(parent_level, parent_object_name, period_current)
    save_session_state(
        session_id,
        level=parent_level,
...
def _build_query_from_full_view(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    target_level = FULL_VIEW_NEXT_LEVEL.get(state.get('level')) or state.get('last_list_level')
    if not target_level:
        return {'status': 'error', 'reason': 'Нет данных для отображения.'}
