...
def _start_screen_response() -> Dict[str, Any]:
    return {
        'status': 'ok',
        'context': {
            'level': 'start',
...
def _finalize_response(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if response.get('status') == 'ok':
        return enforce_contract(response)
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if normalized in {'начать анализ', 'старт', 'start'}:
        return _start_screen_response()
