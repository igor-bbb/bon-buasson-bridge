...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if index < 0 or index >= len(items):
        return {'status': 'error', 'reason': 'Номер вне диапазона текущего списка.'}

    selected = items[index]
    period = state.get('period')
    if not period:
        return {'status': 'error', 'reason': 'Нет активного периода для выбора объекта.'}
...
def _build_query_from_period_only(message: str) -> Dict[str, Any]:
    normalized = _normalize_message(message)
    period_current, period_previous = resolve_period_from_message(normalized)
    if not period_current:
        return {}

    cleaned = normalize_entity_text(normalized)
    if cleaned in {'бизнес', 'business'}:
        return {}

    # if message contains only period tokens and service words, auto-open business
    stripped = cleaned
    for part in [period_current, period_previous]:
        if part:
            stripped = stripped.replace(normalize_entity_text(part), ' ')
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if _is_short_command(normalized):
        ...
        parsed = _build_query_from_period_only(message)
        if parsed.get('status') != 'ok':
