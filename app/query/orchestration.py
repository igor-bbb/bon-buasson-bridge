    ...
    if 0 <= index < len(items):
        ...
        return {
            ...
            'query': {
                ...
                'query_type': state.get('last_response_type') if state.get('last_response_type') in {'reasons','losses'} else 'summary',
...
def enforce_contract(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if response_type not in {'object', 'reasons', 'comparison'}:
        return {'status': 'error', 'reason': 'invalid response type'}
    if response_type == 'object' and ('object' not in data or 'drain' not in data):
        return {'status': 'error', 'reason': 'invalid object structure'}
...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    current = _with_previous_metrics(current, SUMMARY_EXECUTORS.get(level)(object_name, _previous_year_period(period)) if _previous_year_period(period) and SUMMARY_EXECUTORS.get(level) else None)
...
def _route_base_query(query: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if previous_same_period_key:
        try:
            previous_same_period = executor(object_name, previous_same_period_key)
    ...
    if query.get('query_type') == 'losses':
        ...
        response = ok_response(query, build_object_view(sanitize_payload(current), sanitize_payload(source)))
        if response.get('status') == 'ok':
...
def _route_drill_query(query: Dict[str, Any], session_ctx: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if summary_executor is None:
        return not_implemented_response(query, 'base query not supported')
    current = summary_executor(scope_object_name, period)
    if 'error' in current:
        return error_response(current['error'], query)
    if scope_level == 'business':
        current = dict(current)
    ...
    current = _with_previous_metrics(current, SUMMARY_EXECUTORS.get(scope_level)(scope_object_name, _previous_year_period(period)) if _previous_year_period(period) and SUMMARY_EXECUTORS.get(scope_level) else None)
    ...
    if response.get('status') == 'ok':
        ...
        list_items = _build_last_list_items(payload.get('items', []), target_level)
        _store_list_context(
            session_id,
            scope_level,
            scope_object_name,
