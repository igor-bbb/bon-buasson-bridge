...
def _voice_percent_anomaly(metrics: Dict[str, Any], metric_key: str) -> bool:
    # Margin above 100% almost always means an invalid or incomplete base in VECTRA rating output.
...
def _build_voice_rating_response(message: str, intent: Dict[str, Any], session_ctx: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    items = _voice_group_rating_rows(period_rows, group_field, metric_spec, direction, limit=limit, previous_rows=previous_rows)
    if not items:
        return {
            'status': 'ok',
