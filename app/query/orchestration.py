...
def _build_mode_response(session_id: str, mode: str) -> Dict[str, Any]:
    ...
    if mode == 'all':
        ...
        if not all_block:
            return {'status': 'error', 'reason': 'Нет данных для ветки все'}
        raw['drain_block'] = all_block
        raw['all_block'] = all_block
        items = [item.get('object_name') for item in all_block if isinstance(item, dict) and item.get('object_name')]
        ...
        raw['navigation']['items'] = [item.get('name') for item in reasons_block if isinstance(item, dict) and item.get('name')]
        save_session_state(session_id, last_list_items=[{'object_name': item.get('name'), 'level': state.get('current_level'), 'normalized_name': normalize_entity_text(item.get('name'))} for item in reasons_block if isinstance(item, dict) and item.get('name')], view_mode='reasons')
    else:
