...
def _blank_state() -> Dict[str, Any]:
    return {
        ...
        'full_view': False,
        'last_payload': None,
        'path_goal_money': None,
        'current_focus_money': None,
        'stack': [],
    }
...
def _extract_list_items_from_response(response: Dict[str, Any], fallback_level: Optional[str]) -> List[Dict[str, Any]]:
    ...
    items = (navigation.get('vector') or navigation.get('items')) if isinstance(navigation, dict) else []
...
def push_state(session_id: str) -> None:
    ...
    snapshot = {
        ...
        'path_goal_money': current.get('path_goal_money'),
        'current_focus_money': current.get('current_focus_money'),
    }
...
def get_session_state(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        'path_goal_money': session_ctx.get('path_goal_money'),
        'current_focus_money': session_ctx.get('current_focus_money'),
...
def save_session_state(
    ...
    *,
    ...
    path_goal_money: Any = None,
    current_focus_money: Any = None,
) -> None:
    ...
    if path_goal_money is not None:
        payload['path_goal_money'] = path_goal_money
    if current_focus_money is not None:
        payload['current_focus_money'] = current_focus_money
    if payload:
...
def _extract_item_effect_money(item: Dict[str, Any]) -> Optional[float]:
    if not isinstance(item, dict):
        return None
    for key in ('effect_money', 'potential_money', 'gap_loss_money'):
        value = item.get(key)
    ...
    for key in ('gap_loss_money', 'effect_money', 'potential_money'):
...
def _build_vector_block(path_goal_money: Any, current_focus_money: Any) -> Dict[str, Any]:
    def _to_float(value):
        try:
            if value is None:
                return None
    ...
    path_goal = _to_float(path_goal_money)
    current_focus = _to_float(current_focus_money)
    if path_goal is None or path_goal == 0 or current_focus is None:
        return {}

    coverage = round(abs(current_focus) / abs(path_goal) * 100)
    return {
        'path_goal_money': int(round(path_goal)),
        'current_focus_money': int(round(current_focus)),
...
def _build_last_list_items(items: List[Dict[str, Any]], level: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        prepared.append({
            ...
            'selected_focus_money': _extract_item_effect_money(item),
        })
        if len(prepared) >= MAX_LAST_LIST_ITEMS:
            break
    return prepared
...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 0 <= index < len(items):
        ...
        selected_focus_money = selected.get('selected_focus_money')
        path_goal_money = state.get('path_goal_money')
        if path_goal_money is None:
            path_goal_money = selected_focus_money
        return {
            'status': 'ok',
            'query': {
                'mode': 'diagnosis',
                ...
                'path_goal_money': path_goal_money,
                'selected_focus_money': selected_focus_money,
...
def _with_mode_payload(payload: Dict[str, Any], mode: str) -> Dict[str, Any]:
    data = deepcopy(_direct_payload(payload))
    if not isinstance(data, dict):
    ...
    if mode == "all":
        # Full list mode: do NOT reuse top-3 drain. Use the complete all_block
        # and expose it as selectable navigation items for the next numeric click.
        block = data.get("all_block") or []
        if not isinstance(block, list):
            block = []
        data["drain_block"] = block
        navigation["mode"] = "all"
        navigation["items"] = [item.get("object_name") for item in block if isinstance(item, dict) and item.get("object_name")]
        navigation["vector"] = navigation["items"]
        navigation["has_all"] = False
        navigation["has_causes"] = bool(data.get("reasons_block"))
        navigation["has_back"] = True
        data["navigation"] = navigation
        data["view_mode"] = "all"
    elif mode == "reasons":
        # Reasons mode keeps the same object context and structure, but removes drilldown
        # drain so the GPT screen can focus on explanation, not navigation noise.
        block = data.get("reasons_block") or []
        if not isinstance(block, list):
            block = []
        data["reasons_block"] = block
        data["drain_block"] = []
        navigation["mode"] = "reasons"
        navigation["items"] = []
        navigation["vector"] = []
        navigation["has_all"] = False
        navigation["has_causes"] = False
        navigation["has_back"] = True
        data["navigation"] = navigation
        data["view_mode"] = "reasons"
        data["summary_block"] = data.get("summary_block") or "Разбор причин по структуре отклонений от базы."
    else:
...
def _route_drill_query(query: Dict[str, Any], session_ctx: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if full_view:
        # Full list mode must keep the current object screen (KPI/structure/vector)
        # and only replace the selectable drain/navigation with the full child list.
        # build_list_view loses render-ready KPI fields, which caused zero KPI/structure.
        view_payload = build_object_view(sanitize_payload(current), sanitize_payload(source))
        all_items = current.get('all_block') if isinstance(current.get('all_block'), list) else []
        if not all_items:
            all_items = source.get('all_items') if isinstance(source.get('all_items'), list) else []
        if not all_items:
        ...
        for item in all_items:
            ...
            if not object_name:
                continue
            effect_money = item.get('effect_money')
            if effect_money is None:
                effect_money = item.get('potential_money')
            if effect_money is None and isinstance(item.get('impact'), dict):
                effect_money = item.get('impact', {}).get('gap_loss_money')
            normalized_all.append({
                'object_name': object_name,
                'effect_money': effect_money if effect_money is not None else 0,
            })

        view_payload['view_mode'] = 'all'
        view_payload['drain_block'] = normalized_all
        view_payload['drain_items'] = normalized_all
        view_payload['losses'] = normalized_all
        navigation = dict(view_payload.get('navigation') or {})
        navigation['mode'] = 'all'
        navigation['items'] = [item.get('object_name') for item in normalized_all]
        navigation['vector'] = navigation['items']
        navigation['all'] = False
        navigation['has_all'] = False
        navigation['reasons'] = True
        navigation['has_causes'] = True
        navigation['back'] = True
        navigation['has_back'] = True
        view_payload['navigation'] = navigation
        response = ok_response(query, view_payload)
        list_items_source = normalized_all
    else:
        response = ok_response(query, build_list_view(sanitize_payload(current), sanitize_payload(source)))
        list_items_source = source.get('items', [])

    if response.get('status') == 'ok':
        list_items = _build_last_list_items(list_items_source, child_level) or _extract_list_items_from_response(response, child_level)
        _store_list_context(
            session_id,
            scope_level,
            scope_object_name,
...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    drain_items = (view_payload.get('drain_block') or []) if isinstance(view_payload, dict) else []
    drain_total = sum(_extract_item_effect_money(item) or 0 for item in drain_items) if drain_items else 0
    selected_focus_money = query.get('selected_focus_money')
    current_focus_money = drain_total if drain_total else selected_focus_money
    vector_block = _build_vector_block(query.get('path_goal_money'), current_focus_money)
    if vector_block:
        view_payload['vector_block'] = vector_block
        view_payload['goal'] = {
            'type': 'close_gap',
            'effect_money': vector_block.get('current_focus_money'),
        }
    ...
    if response.get('status') == 'ok':
        ...
        if drain_payload is not None:
            ...
            list_items = _build_last_list_items(drain_payload.get('items', []), child_level) or _extract_list_items_from_response(response, child_level)
            save_session_state(
                session_id,
                ...
                full_view=False,
                view_mode='default',
                path_goal_money=query.get('path_goal_money'),
                current_focus_money=current_focus_money,
            )
        else:
            save_session_state(
                session_id,
...
def _route_base_query(query: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if query.get('query_type') == 'losses':
        ...
        if response.get('status') == 'ok':
            ...
            list_items = _build_last_list_items(source.get('items', []), child_level) or _extract_list_items_from_response(response, child_level)
            save_session_state(
                session_id,
