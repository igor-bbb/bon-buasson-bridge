...
def push_state(session_id: str) -> None:
    ...
    snapshot = {
        ...
        'screen_state': deepcopy(current.get('screen_state')),
    }
...
def _extract_direct_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
...
def _extract_navigation_items(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    navigation = data.get('navigation') if isinstance(data, dict) else {}
    items = navigation.get('items') if isinstance(navigation, dict) else []
    next_level = navigation.get('next_level') if isinstance(navigation, dict) else None
    prepared: List[Dict[str, Any]] = []
    if isinstance(items, list) and next_level:
        for item in items:
            if not item:
                continue
            prepared.append({
...
def _build_screen_state_from_payload(payload: Dict[str, Any]) -> Dict[str, Any]:
    data = _extract_direct_payload(payload)
    context = data.get('context') if isinstance(data, dict) else {}
    navigation = data.get('navigation') if isinstance(data, dict) else {}
    return {
        'current_level': context.get('level'),
        'current_object': context.get('object_name'),
        'navigation_items': _extract_navigation_items(data),
        'has_all': bool(data.get('all_block')) or bool((navigation or {}).get('has_all')),
        'has_causes': bool(data.get('cause_block')) or bool((navigation or {}).get('has_causes')),
        'has_back': True,
        'all_block': deepcopy(data.get('all_block') or []),
        'cause_block': deepcopy(data.get('cause_block') or []),
        'structure': deepcopy(data.get('structure') or {}),
        'raw': deepcopy(data),
    }


def save_last_payload(session_id: str, payload: Dict[str, Any]) -> None:
    update_session(session_id, {'last_payload': payload, 'screen_state': _build_screen_state_from_payload(payload)})
...
def _build_query_from_full_view(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw = deepcopy(screen_state.get('raw') or {})
    if not raw:
        return {'status': 'error', 'reason': 'Нет активного экрана'}
    raw['drain_block'] = deepcopy(screen_state.get('all_block') or [])
    return {'status': 'ok', 'local_response': raw}
...
def _build_local_reasons_response(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not raw:
        return {'status': 'error', 'reason': 'Нет активного экрана'}
    cause_block = deepcopy(screen_state.get('cause_block') or [])
    if not cause_block:
        structure = raw.get('structure') or {}
        if isinstance(structure, dict):
            cause_block = [
                {'object_name': key, 'effect_money': value.get('effect_money', 0)}
                for key, value in structure.items()
                if isinstance(value, dict)
            ]
            cause_block.sort(key=lambda x: x.get('effect_money', 0))
    raw['drain_block'] = cause_block
    return {'status': 'ok', 'local_response': raw}
...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    state = get_session_state(session_ctx)
    screen_state = state.get('screen_state') or {}
    items = state.get('last_list_items') or screen_state.get('navigation_items') or _extract_list_items_from_response(state.get('last_payload') or {}, state.get('last_list_level'))
    index = int(message) - 1
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if normalized.isdigit():
        ...
        parsed = _build_local_reasons_response(session_ctx)
        if parsed.get('status') != 'ok':
            return parsed
    ...
    if parsed.get('local_response') is not None:
        local = parsed['local_response']
        update_session(session_id, {'screen_state': _build_screen_state_from_payload(local)})
        return enforce_contract(local)
