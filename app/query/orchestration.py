...
from app.development_journal import (
    ...
    is_global_capture_command as is_development_journal_global_capture_command,
    is_lifecycle_command as is_development_journal_lifecycle_command,
    is_dialogue_review_command as is_development_journal_dialogue_review_command,
...
def _screen_active_workspace_state(screen: Dict[str, Any]) -> Dict[str, Any]:
    return screen.get('active_workspace_state') if isinstance(screen, dict) and isinstance(screen.get('active_workspace_state'), dict) else {}


def get_session_state(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    screen = session_ctx.get('current_screen') or session_ctx.get('last_payload') or {}
    active_state = _screen_active_workspace_state(screen)
    active_filter = active_state.get('filter') if isinstance(active_state.get('filter'), dict) else {}
    return {
        ...
        'level': active_state.get('workspace_level') or session_ctx.get('scope_level'),
        'object_name': active_state.get('object_name') or session_ctx.get('scope_object_name'),
        'period': active_state.get('period') or session_ctx.get('period_current'),
        'period_previous': session_ctx.get('period_previous'),
        'filter': active_filter or session_ctx.get('filter') or {},
        ...
        'last_payload': session_ctx.get('last_payload') or screen,
...
def _sync_session_from_active_workspace(session_id: str, screen: Dict[str, Any]) -> None:
...
def _has_active_workspace(session_ctx: Dict[str, Any]) -> bool:
    state = session_ctx.get('active_workspace_state') or _screen_active_workspace_state(session_ctx.get('current_screen') or {})
    return isinstance(state, dict) and bool(state.get('workspace_level')) and bool(state.get('object_name'))


def _explicit_context_change_intent(message: str) -> bool:
    n = _normalize_message(message)
    if not n:
        return False
    if re.search(r'\b(покажи|открой|разбери|бизнес)\b', n):
        return True
    if re.search(r'20\d{2}-\d{2}', n) and len(n.split()) >= 2:
        return True
    return False


def _contextual_free_dialogue_response(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    state = session_ctx.get('active_workspace_state') or _screen_active_workspace_state(session_ctx.get('current_screen') or {})
    obj = state.get('object_name') or session_ctx.get('object_name')
    level = state.get('workspace_level') or session_ctx.get('level')
    period = state.get('period') or session_ctx.get('period')
    return {
        'status': 'ok',
        'render_mode': 'voice_diagnostic',
        'context': {'level': 'assistant_dialogue', 'object_name': obj, 'parent_object': obj, 'period': period},
        'path': state.get('path') or [],
        'workspace_markdown': (
            f'Работаю в контексте открытого Workspace: {obj} | {period}.\n\n'
            'Этот свободный вопрос относится к текущему объекту. '
            'Для точного управленческого вывода используй данные открытого рабочего стола или выбери действие из текущего меню.'
        ),
...
def _append_dialogue_trace(session_id: str, *, message: Any = None, payload: Optional[Dict[str, Any]] = None) -> None:
    """Store bounded sanitized engineering trace, not raw conversation."""
    try:
        current = get_session(session_id)
        trace = list(current.get('dialogue_trace') or [])
        norm = _normalize_message(message or '')
        screen = payload if isinstance(payload, dict) else {}
        trace.append({
            'at': time.time(),
            'intent_hint': 'development_journal' if is_development_journal_intent(message) else ('workspace_opening' if is_workspace_opening_intent(message) else 'other'),
            'message_hash': hashlib.sha256(norm.encode('utf-8')).hexdigest()[:16],
            'message_length': len(str(message or '')),
            'render_mode': screen.get('render_mode'),
            'status': screen.get('status'),
            'reason': screen.get('reason'),
            'error_code': screen.get('error_code'),
        })
        update_session(session_id, {'dialogue_trace': trace[-200:]})
    except Exception:
        logger.exception('dialogue_trace_append_failed session_id=%s', session_id)
...
def save_last_payload(session_id: str, payload: Dict[str, Any]) -> None:
    ...
    if mode in {'list_only', 'all', 'reasons', 'kpi_only', 'action_package', 'negotiation_workspace', 'task_workspace', 'post_meeting_workspace', 'execution_workspace', 'voice_diagnostic'}:
        update_session(session_id, {'last_payload': screen})
        _append_dialogue_trace(session_id, message=None, payload=screen if isinstance(screen, dict) else {})
        return
...
def _cannot_parse_message_response(message: str) -> Dict[str, Any]:
    """Return a controlled NLI clarification instead of raw cannot_parse_message.

    DEV-0001: supported commands must not surface `cannot_parse_message`.
    For unsupported or ambiguous free text the router now returns a stable
    clarification payload, while still recording an engineering runtime event
    under a non-production-breaking code for later Laboratory analysis.
    """
    logger.warning('nli_unresolved_message raw_message=%r normalized_message=%r', message, _normalize_message(message))
    try:
        add_development_journal_runtime_event(
            event_type='nli_unresolved_message',
            component='nli_router',
            system_level='runtime',
            technical_reason='NLI router could not classify user intent into a supported Workspace, action or Discovery route.',
            suspected_root_cause='Unsupported or ambiguous object/period/action combination, or missing NLI alias.',
            error_code='nli_unresolved_message',
        ...
        logger.exception('development_journal_runtime_event_failed event=nli_unresolved_message')
    return {
        'status': 'error',
        'reason': 'nli_unresolved_message',
        'render_mode': 'nli_clarification',
        'message': 'Не смог однозначно определить рабочий объект или команду. Уточните объект и период, например: «Покажи Варус 2026-02» или «Бизнес 2026-02».',
    }
...
ACTION_COMMAND_ALIASES = {
    ...
    'execution': (
        'исполнение', 'перейти к исполнению', 'контроль выполнения', 'выполнение задач',
        'в работу', 'начать выполнение', 'как выполнить задачи',
    ),
}
...
ACTION_WORKSPACE_MODES = {'action_package', 'negotiation_workspace', 'task_workspace', 'post_meeting_workspace', 'execution_workspace'}


def _active_screen_for_action(session_ctx: Dict[str, Any], action: Optional[str] = None) -> Dict[str, Any]:
    """Return the correct source Workspace for an action.

    Stabilization S1 separates two concepts that were previously mixed:
    - analytical base Workspace: the Contract/Category/SKU screen that owns the data;
    - last displayed action Workspace: Negotiation/SKU Package/Task/Execution screen.

    Negotiation and SKU Package use the analytical base.
    Task, Post Meeting and Execution should prefer the last displayed action
    workspace when available, so the business flow Contract → Negotiation →
    SKU Package → Task → Execution does not collapse back to the old Contract
    menu or lose the package context.
    """
    state = get_session_state(session_ctx)
    current = state.get('current_screen') or session_ctx.get('current_screen') or {}
    last = session_ctx.get('last_payload') or state.get('last_payload') or {}

    def unwrap(screen: Any) -> Dict[str, Any]:
    ...
    last = unwrap(last)
    last_mode = str(last.get('render_mode') or '').strip().lower() if isinstance(last, dict) else ''
    action = str(action or '').strip().lower()

    if action in {'tasks', 'post_meeting', 'execution'} and last_mode in ACTION_WORKSPACE_MODES:
        return last
    if current:
        return current
    return last
...
def _build_sku_package_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not sku_package:
        ...
        lines.extend([
            ...
            '4. назад — вернуться на рабочий стол объекта',
        ])
    return {
        'status': 'ok',
        'render_mode': 'action_package',
...
def _build_negotiation_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if ctx.get('level') == 'sku':
        ...
        return {
            'status': 'ok',
            'render_mode': 'negotiation_workspace',
    ...
    lines.extend([
        ...
        '',
        '## Action Zone',
        '1. собрать пакет позиций — уточнить SKU первой волны',
        '2. создать задачи — зафиксировать подготовку, ввод и ответственных',
        '3. перейти к исполнению — контроль выполнения задач',
        '4. после встречи — зафиксировать итог переговоров',
        '5. назад — вернуться к рабочему столу контракта',
    ])
    ...
    return {
        ...
        'drain_block_render': [],
        'drain_total': 0,
        'navigation_block': ['собрать пакет позиций — уточнить первую волну', 'создать задачи — зафиксировать действия', 'перейти к исполнению — контроль выполнения задач', 'после встречи — зафиксировать итог', 'назад — вернуться к объекту'],
        'screen_order': ['workspace_markdown'],
        'workspace_markdown': '\n'.join(lines),
    }
...
def _simple_action_payload(screen: Dict[str, Any], title: str, lines: List[str], summary: str) -> Dict[str, Any]:
    ctx = _screen_context(screen)
    body = [title, f'Период: {ctx.get("period")}', ''] + lines
    body.extend(['', '## Action Zone', '1. собрать пакет позиций — перейти к пакету развития', '2. подготовить переговоры — собрать позицию', '3. назад — вернуться к объекту'])
    return {
        'status': 'ok',
        'render_mode': 'action_package',
        'context': {'level': ctx.get('level'), 'object_name': ctx.get('object_name'), 'period': ctx.get('period'), 'parent_object': ctx.get('parent_object')},
...
def _collect_task_candidates(screen: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    raw_lines = screen.get('kpi_block') if isinstance(screen.get('kpi_block'), list) else []
    # Stabilization S1: Task/Execution must be able to continue from action
    # workspaces, not only from the original analytical Contract screen.
    if render_mode == 'action_package':
        ...
        for line in raw_lines:
            ...
            if capture_package_rows and text.startswith('## '):
                break
            if not capture_package_rows or not text or '|' not in text:
                continue
            ...
            if len(parts) >= 2:
                ...
                if sku:
                    tasks.append({
                        ...
                        'basis': parts[-1] if parts[-1] else 'позиция входит в пакет развития',
                        'effect': 0,
                        'type': 'ассортимент',
                    })
        if tasks:
            # Keep the package focused: enough for execution without flooding the Workspace.
            return tasks[:8]

    if render_mode == 'task_workspace':
        for line in raw_lines:
            text = str(line or '').strip()
            if not text or '|' not in text or text.startswith('Задача |'):
                continue
            parts = [x.strip() for x in text.split('|')]
            if len(parts) >= 2 and parts[0]:
                tasks.append({
                    'title': parts[0],
                    'basis': parts[1] if len(parts) > 1 else 'задача из текущего Task Workspace',
                    'effect': 0,
                    'type': 'исполнение',
                })
        if tasks:
...
def _build_tasks_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines.extend([
        '',
        '## Action Zone',
        '1. перейти к исполнению — контроль выполнения задач',
        '2. после встречи — зафиксировать итог переговоров',
        '3. подготовить переговоры — вернуться к переговорной позиции',
        '4. назад — вернуться к рабочему столу объекта',
    ])
    return {
        'status': 'ok',
        ...
        'drain_block_render': [],
        'drain_total': 0,
        'navigation_block': ['перейти к исполнению — контроль выполнения задач', 'после встречи — зафиксировать итог', 'подготовить переговоры — вернуться к позиции', 'назад — вернуться к объекту'],
        'screen_order': ['workspace_markdown'],
        'workspace_markdown': '\n'.join(lines),
    }
...
def _build_post_meeting_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ...
    lines.extend([
        ...
        '',
        '## Action Zone',
        '1. создать задачи — сформировать черновик задач',
        '2. перейти к исполнению — контроль выполнения задач',
        '3. подготовить переговоры — вернуться к переговорной позиции',
        '4. назад — вернуться к рабочему столу объекта',
    ])
    return {
        ...
        'drain_block_render': [],
        'drain_total': 0,
        'navigation_block': ['создать задачи — сформировать черновик', 'перейти к исполнению — контроль выполнения задач', 'подготовить переговоры — вернуться к позиции', 'назад — вернуться к объекту'],
        'screen_order': ['workspace_markdown'],
        'workspace_markdown': '\n'.join(lines),
    }


def _build_execution_action(screen: Dict[str, Any]) -> Dict[str, Any]:
    ctx = _screen_context(screen)
    object_name = ctx.get('object_name') or 'объект'
    period = ctx.get('period') or ''
    tasks = _collect_task_candidates(screen)
    lines = [
        f'🚦 Рабочий стол исполнения — {object_name}',
        f'Период: {period}',
        '',
        'Этот Workspace нужен не для нового анализа, а для контроля выполнения решений по открытому объекту.',
        '',
        '## Что должно быть выполнено',
    ]
    if tasks:
        lines.append('Задача | Ответственный | Срок | Как проверить результат')
        for task in tasks[:6]:
            priority = _task_priority(task.get('effect'))
            owner = _task_owner(ctx, task.get('title') or '')
            due = _task_due(priority)
            lines.append(f'{task.get("title")} | {owner} | {due} | открыть объект после обновления DATA и проверить KPI / структуру / наличие SKU')
    else:
        lines.append('Нет подтверждённых задач для контроля исполнения. Сначала сформируй задачи по текущему Workspace.')
    lines.extend([
        '',
        '## Контроль отклонений',
        'Состояние | Что делает VECTRA',
        'Выполнено и эффект подтверждён | сохранить как успешную практику',
        'Выполнено, но эффекта нет | вернуть объект в разбор причин',
        'Не выполнено по блокеру | эскалировать руководителю с причиной',
        'Не выполнено без причины | отметить дисциплинарное отклонение',
        '',
        '## Action Zone',
        '1. после встречи — зафиксировать итог переговоров',
        '2. создать задачи — обновить черновик задач',
        '3. назад — вернуться к рабочему столу объекта',
    ])
    return {
        'status': 'ok',
        'render_mode': 'execution_workspace',
        'context': {'level': ctx.get('level'), 'object_name': object_name, 'period': period, 'parent_object': ctx.get('parent_object')},
        'path': screen.get('path') or [],
        'summary_block': 'Рабочий стол исполнения подготовлен по текущему объекту.',
        'kpi_block': lines,
        'structure_block': [],
        'drain_block_render': [],
        'drain_total': 0,
        'navigation_block': ['после встречи — зафиксировать итог', 'создать задачи — обновить черновик', 'назад — вернуться к объекту'],
        'screen_order': ['workspace_markdown'],
        'workspace_markdown': '\n'.join(lines),
    }
...
def _execute_action_command(action: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    screen = _active_screen_for_action(session_ctx, action)
    if not screen:
    ...
    if action == 'execution':
        return _build_execution_action(screen)
...
def _finalize_response(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if response.get('status') == 'error':
        ...
        for key in ('render_mode', 'message', 'workspace_markdown', 'workspace_primary_block', 'context', 'api_attempted', 'workspace_intent_detected'):
            if key in response:
                public_error[key] = response.get(key)
        return public_error
    return error_response('нет данных')

def _show_start_screen() -> Dict[str, Any]:
    # W15.0: Start Screen is a local UI entry point. It must never call the
    # analytical runtime before the user selects an object or period.
    markdown = (
        "📍 Старт\n\n"
    ...
    return {
        ...
        "start_screen_contract": {"local_only": True, "analytics_runtime_called": False},
    }
...
def _execute_numeric_workspace_action(message: str, session_ctx: Dict[str, Any], session_id: str) -> Optional[Dict[str, Any]]:
    ...
    screen = display_screen or source_screen
    if isinstance(screen, dict) and isinstance(screen.get('data'), dict):
        screen = screen.get('data')
    if isinstance(source_screen, dict) and isinstance(source_screen.get('data'), dict):
        source_screen = source_screen.get('data')
    if not isinstance(screen, dict):
        return None
    ctx = screen.get('context') if isinstance(screen.get('context'), dict) else {}
    level = str(ctx.get('level') or screen.get('level') or '').strip().lower()
    ...
    def execute_runtime_action(action: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if action_type == 'tasks':
            ...
            if isinstance(result, dict) and result.get('status') != 'error':
                save_session_state(session_id, view_mode='action', show_all=False)
            return result
        if action_type in {'post_meeting', 'execution'}:
            result = _execute_action_command(action_type, session_ctx)
            if isinstance(result, dict) and result.get('status') != 'error':
                save_session_state(session_id, view_mode='action', show_all=False)
            return result
    ...
    if level == 'network':
        ...
        if n == 5:
            ...
            return result
        if n == 6:
            result = _execute_action_command('execution', session_ctx)
            if isinstance(result, dict) and result.get('status') != 'error':
                save_session_state(session_id, view_mode='action', show_all=False)
            return result
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    _append_dialogue_trace(session_id, message=message, payload=None)
    session_ctx = get_session(session_id)
    ...
    if is_development_journal_dialogue_review_command(message):
        result = analyze_development_journal_dialogue(
            dialogue=None,
            session_ctx=session_ctx,
            session_id=session_id,
        )
        return _finalize_response(build_development_journal_dialogue_review_response(result))
    ...
    if is_development_journal_dry_run_command(message):
        record = add_development_journal_global_record(
            event_type='manual_engineering_registration_dry_run',
            component='global_development_journal_api',
            technical_description='Global Development Journal routing dry-run completed through Custom GPT query route.',
            suspected_root_cause='No production issue; smoke-test route validation.',
            proposed_fix_direction='No action. Dry-run record is not persisted.',
            priority='P2',
            runtime_context={'source_scope': 'any_vectra_assistant', 'route': 'vectra_query'},
            session_id=session_id,
            dry_run=True,
            is_test=True,
        )
    ...
    if is_development_journal_global_capture_command(message):
        record = add_development_journal_global_record(
            event_type='manual_engineering_registration',
            component='global_development_journal_api',
            technical_description='Manual global engineering registration was requested by a VECTRA assistant. User text was not persisted.',
            suspected_root_cause='Requires laboratory review of current runtime context and assistant behavior.',
            proposed_fix_direction='Laboratory must classify the issue, aggregate duplicates and create an engineering task if confirmed.',
            priority='P1',
            runtime_context={'source_scope': 'any_vectra_assistant', 'route': 'vectra_query'},
            session_id=session_id,
        )
    ...
    # Stabilization S1 — autonomous engineering loop commands. These are
    # engineering-control routes and must be evaluated before business Runtime.
    if any(x in normalized for x in ('release manager', 'релиз менеджер', 'проверить релиз', 'проверь релиз', 'проверь последний релиз', 'принять релиз', 'запусти тест план', 'выполнить тест план')):
        from app.release_manager import run_release_acceptance, build_release_manager_response
        release_id = 'query-command'
        m = re.search(r'(?:release|релиз)[:=-]+([A-Za-z0-9_.-]+)', message, re.I)
        if m:
            release_id = m.group(1)
        return _finalize_response(build_release_manager_response(run_release_acceptance(release_id=release_id)))

    if normalized in {'release brief', 'релиз бриф', 'показать release brief', 'показать релиз бриф'}:
        from app.release_brief import parse_release_brief, build_release_brief_markdown
        brief = parse_release_brief(None, fallback_release_id='query-command')
        return _finalize_response({'status': 'ok', 'render_mode': 'release_brief', 'context': {'level': 'release_brief', 'object_name': 'Release Brief'}, 'workspace_markdown': build_release_brief_markdown(brief), 'release_brief': brief.to_dict()})

    if normalized in {'test plan', 'тест план', 'показать тест план', 'открыть тест план', 'test plan engine'}:
        from app.release_manager import build_test_plan_response
        return _finalize_response(build_test_plan_response())

    if normalized in {'scenario library', 'библиотека сценариев', 'показать библиотеку сценариев', 'регрессионные сценарии'}:
        from app.release_manager import get_full_scenario_library
        lib = get_full_scenario_library()
        lines = ['# Scenario Library', '', f"Базовых сценариев: **{len(lib.get('base_scenarios') or [])}**", f"Regression Suite: **{len(lib.get('regression_scenarios') or [])}**"]
        return _finalize_response({'status': 'ok', 'render_mode': 'scenario_library', 'context': {'level': 'scenario_library', 'object_name': 'Scenario Library'}, 'workspace_markdown': '\n'.join(lines), 'scenario_library': lib})

    if normalized in {'scenario runner', 'запусти scenario runner', 'запусти сценарий', 'выполни сценарий'}:
        from app.release_manager import _get_scenario
        from app.scenario_runner import run_scenario
        scenario = _get_scenario('S1-START-SCREEN')
        result = run_scenario(scenario=scenario, release_id='query-scenario-runner', decision_callback=lambda step_result: 'PASS')
        return _finalize_response({'status': 'ok', 'render_mode': 'scenario_runner', 'context': {'level': 'scenario_runner', 'object_name': 'Scenario Runner'}, 'workspace_markdown': f"# Scenario Runner\n\nСценарий выполнен: **{result.get('scenario_id')}**. Шагов: **{result.get('steps_executed')}**.", 'scenario_runner': result})

    if normalized in {'анализ журнала', 'проанализируй журнал', 'лаборатория', 'laboratory', 'laboratory analysis', 'root cause analysis', 'сформировать инженерное тз'}:
        from app.laboratory_processor import build_laboratory_response
        return _finalize_response(build_laboratory_response())
    ...
    if normalized in {'начать анализ', 'начни анализ', 'начать анализ данных', 'старт', 'старт анализа', 'start analysis'}:
