...
def save_last_payload(session_id: str, payload: Dict[str, Any]) -> None:
    # Store the full screen atomically. UI commands must never rebuild it from
    # fragments after API has returned.
    screen = enforce_contract(payload) if isinstance(payload, dict) else payload
...
def _is_search_command(message: str) -> bool:
    return _normalize_message(message) in {'искать', 'поиск', 'найти деньги', 'искать деньги'}
...
def _with_mode_payload(payload: Dict[str, Any], mode: str) -> Dict[str, Any]:
    ...
    if mode == "all":
        ...
        # Reasons is a view of the current screen, not a new analytical query.
        # Keep current structure/metrics/vector so render can show real percentages.
        data = {
            "status": "ok",
...
def _handle_back(session_id: str) -> Dict[str, Any]:
    ...
    if not restored:
        ...
        if screen:
            return enforce_contract(screen)
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if normalized.isdigit():
        ...
        if view.get('status') != 'error':
            save_session_state(session_id, view_mode='all', show_all=True)
        return _finalize_response(view)
    else:
