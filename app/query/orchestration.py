...
def save_screen_state(session_id: str, payload: Dict[str, Any]) -> None:
    if not isinstance(payload, dict):
        return
    context = payload.get('context') or {}
    navigation = payload.get('navigation') or {}
    screen_state = {
        'current_level': context.get('level') or payload.get('level'),
        'current_object': context.get('object_name') or payload.get('object_name'),
        'navigation_items': list(navigation.get('items') or []),
        'has_all': bool(navigation.get('has_all')),
        'has_causes': bool(navigation.get('has_causes')),
        'has_back': bool(navigation.get('has_back')),
        'mode': navigation.get('mode') or 'default',
        'all_block': list(payload.get('all_block') or []),
        'reasons_block': list(payload.get('reasons_block') or payload.get('cause_block') or []),
...
def _build_mode_response(session_id: str, mode: str) -> Dict[str, Any]:
    state = get_session_state(get_session(session_id)).get('screen_state') or {}
    raw = deepcopy(state.get('raw') or {})
    if not raw:
    ...
    if mode == 'all':
        all_block = list(state.get('all_block') or raw.get('all_block') or [])
        if not all_block:
            all_block = list(raw.get('drain_block') or [])
        all_block = [
            {
            ...
            if isinstance(item, dict) and item.get('object_name') is not None
        ]
        all_block.sort(key=lambda x: (x.get('effect_money') is None, x.get('effect_money', 0)))
        raw['drain_block'] = all_block
        raw['all_block'] = all_block
        items = [item.get('object_name') for item in all_block if item.get('object_name')]
        raw['navigation']['items'] = items[:MAX_LAST_LIST_ITEMS]
        save_session_state(session_id, last_list_items=_build_last_list_items(all_block, navigation.get('next_level')), view_mode='all')
    elif mode == 'reasons':
        reasons_block = list(state.get('reasons_block') or raw.get('reasons_block') or raw.get('cause_block') or [])
        if not reasons_block:
            structure = raw.get('structure') or {}
            reasons_block = [
                {'name': name, 'effect_money': value.get('effect_money')}
                for name, value in structure.items() if isinstance(value, dict)
            ]
        reasons_block = [
            {'name': item.get('name'), 'effect_money': item.get('effect_money')}
            for item in reasons_block
            if isinstance(item, dict) and item.get('name') is not None
        ]
        reasons_block.sort(key=lambda x: (x.get('effect_money') is None, x.get('effect_money', 0)))
        raw['drain_block'] = reasons_block
        raw['reasons_block'] = reasons_block
        raw['cause_block'] = reasons_block
        raw['navigation']['items'] = [item.get('name') for item in reasons_block if item.get('name')]
        save_session_state(session_id, last_list_items=[{'object_name': item.get('name'), 'level': state.get('current_level'), 'normalized_name': normalize_entity_text(item.get('name'))} for item in reasons_block if item.get('name')], view_mode='reasons')
    else:
        raw['drain_block'] = list((raw.get('drain_block') or [])[:3])
        save_session_state(session_id, view_mode='drain')

    save_last_payload(session_id, raw)
    save_screen_state(session_id, raw)
    return raw
...
def _handle_back(session_id: str) -> Dict[str, Any]:
    ...
    save_screen_state(session_id, last_payload if isinstance(last_payload, dict) else {})
...
def _route_drill_query(query: Dict[str, Any], session_ctx: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if response.get('status') == 'ok':
        ...
        save_screen_state(session_id, enforce_contract(response))
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if normalized in FULL_VIEW_COMMANDS:
        return _build_mode_response(session_id, 'all')
    if normalized in {'причины'}:
        return _build_mode_response(session_id, 'reasons')
