...
def _add_context_path_and_summary(payload: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        return payload
    ...
    for key in ['markup', 'retro', 'logistics', 'personnel', 'other']:
        ...
        try:
            value = float(item.get('effect_money') or 0)
        except Exception:
            value = 0.0
        if worst_value is None or value < worst_value:
            worst_value = value
            worst_key = key
    labels = {'markup': 'наценку', 'retro': 'ретро', 'logistics': 'логистику', 'personnel': 'персонал', 'other': 'прочие'}
    payload['summary_block'] = f'Основное давление на результат через {labels.get(worst_key, worst_key)}' if worst_key else ''
    return payload
...
def _finalize_response(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if response.get('status') == 'ok':
        return _add_context_path_and_summary(enforce_contract(response))
