...
from app.query.parsing import normalize_user_message, parse_query_intent, resolve_period_from_message, _strip_period_tokens


SESSION_STORE: Dict[str, Dict[str, Any]] = {}
MAX_LAST_LIST_ITEMS = 100
START_COMMANDS = {'начать анализ', 'начать', 'старт', 'start'}
ENTRY_LEVEL_CHOICES = {'1': 'business', '2': 'manager_top', '3': 'manager'}
...
def _save_last_payload(session_id: str, response: Dict[str, Any]) -> None:
    if response.get('status') == 'ok':
        update_session(session_id, {'last_payload': response.get('data')})


def _build_start_screen(session_id: str) -> Dict[str, Any]:
    save_session_state(session_id, mode='entry_level', full_view=False, last_list_level=None, last_list_items=[])
    return {
        'status': 'ok',
        'data': {
            'mode': 'entry',
            'step': 'level',
            'title': 'VECTRA',
            'subtitle': 'Держу вектор на прибыль',
            'prompt': 'Выбери уровень: 1 — бизнес, 2 — топ-менеджер, 3 — менеджер',
            'items': [
                {'index': 1, 'object_name': 'business', 'level': 'business'},
                {'index': 2, 'object_name': 'manager_top', 'level': 'manager_top'},
                {'index': 3, 'object_name': 'manager', 'level': 'manager'},
            ],
        },
    }
...
def _handle_entry_flow(message: str, session_id: str, session_ctx: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if normalized in START_COMMANDS:
        return _build_start_screen(session_id)

    if mode == 'entry_level' and normalized in ENTRY_LEVEL_CHOICES:
        selected_level = ENTRY_LEVEL_CHOICES[normalized]
        save_session_state(session_id, mode='entry_period')
        update_session(session_id, {'entry_selected_level': selected_level})
        return {
            'status': 'ok',
            'data': {
                'mode': 'entry',
                ...
                'prompt': 'Укажи период в формате YYYY-MM',
            },
        }

    if mode == 'entry_period':
        ...
        selected_level = session_ctx.get('entry_selected_level') or 'business'
        save_session_state(session_id, mode='diagnosis')
        ...
        return {'status': 'error', 'reason': 'Для выбранного уровня нужен объект + период.'}
...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not period:
        return {'status': 'error', 'reason': 'Нет активного периода для выбора.'}
    return {
        'status': 'ok',
        'query': {
            'mode': 'diagnosis',
...
def _resolve_exact_object_in_dictionary(message: str, period: str) -> Optional[Dict[str, Any]]:
    message_wo_period = _strip_period_tokens(message)
    normalized_message = normalize_entity_text(message_wo_period)
    if not normalized_message:
        return None
    ...
    # manager stays above manager_top to prefer the more specific person-level match
    ...
    for level, level_rank in priority_rank.items():
        ...
        for alias, canonical in level_index.items():
            ...
            alias_tokens = set(alias_norm.replace('-', ' ').split())
            ...
            if best_score is None or score > best_score:
                ...
                tied_entities = {entity_key}
            elif score == best_score:
                tied_entities.add(entity_key)
...
def _fallback_parse_direct_object(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    object_text = _strip_period_tokens(normalized).strip()
    if not object_text:
    ...
    best_entity = None
    tied_entities = set()
    object_tokens = set(object_text_norm.replace('-', ' ').split())
    padded_object = f' {object_text_norm} '
    ...
    for level, level_rank in priority_rank.items():
        ...
        for alias, canonical in level_index.items():
            ...
            whole = 1 if f' {alias_norm} ' in padded_object else 0
            ...
            if best_score is None or score > best_score:
                best_score = score
                best_entity = (level, canonical)
                tied_entities = {entity_key}
            elif score == best_score:
                tied_entities.add(entity_key)

    if best_entity is None or len(tied_entities) > 1:
        return {'status': 'error', 'reason': 'object not recognized'}
    ...
    return {
        ...
        'query': {
            'mode': 'diagnosis',
            'level': best_entity[0],
            'object_name': best_entity[1],
            'period_current': period,
            'period_previous': None,
            'query_type': 'summary',
            'period': period,
            'object': best_entity[1],
        },
    }
...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if 'error' in source:
        ...
        _save_last_payload(session_id, response)
        return response
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    entry_response = _handle_entry_flow(message, session_id, session_ctx)
    if entry_response is not None:
        if entry_response.get('status') == 'ok' and entry_response.get('query'):
            parsed = entry_response
        else:
