...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if not items:
        ...
        if not items:
            data = (session_ctx.get('last_payload') or {}).get('data') or {}
            source_items = data.get('items') or ((data.get('top_summary') or {}).get('top_items') or [])
            items = _build_last_list_items(source_items, data.get('next_level') or data.get('children_level'))
...
def _handle_entry_flow(message: str, session_id: str, session_ctx: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    if entry_step == 'await_period':
        ...
        prompt = 'Напиши своё ФИО и период.\n\nПример:\nИонова 2026-02'
        if level == 'manager':
            prompt = 'Напиши своё ФИО и период.\n\nПример:\nИонова 2026-02'
        update_session(session_id, {'entry_step': None, 'entry_level': level, 'pending_period': period_current})
        return {'status': 'ok', 'data': {'mode': 'entry', 'step': 'await_object', 'level': level, 'title': prompt}}


    if entry_step == 'await_object':
        period_current = session_ctx.get('pending_period')
        normalized_name = normalize_user_message(message)
        if not normalized_name or not period_current:
            return {'status': 'error', 'reason': 'object not recognized'}
        level = session_ctx.get('entry_level') or 'manager'
        update_session(session_id, {
            'entry_step': None,
            'pending_period': None,
            'scope_level': level,
            'scope_object_name': message.strip(),
            'period_current': period_current,
            'full_view': False,
        })
        return {
            ...
            'query': {
                'mode': 'diagnosis',
                'level': level,
                'object_name': message.strip(),
                'period_current': period_current,
                'period_previous': None,
                'query_type': 'summary',
                'period': period_current,
                'object': message.strip(),
            },
        }
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if entry_result is not None:
        ...
        if parsed.get('status') != 'ok':
            period_only_current, _ = resolve_period_from_message(message)
            if period_only_current and normalize_entity_text(_strip_period_text(message)) == '':
                parsed = {
                    'status': 'ok',
                    'query': {
                        'mode': 'diagnosis',
                        'level': 'business',
                        'object_name': 'business',
                        'period_current': period_only_current,
                        'period_previous': None,
                        'query_type': 'summary',
                        'period': period_only_current,
                        'object': 'business',
                    },
                }
            else:
...
# ===== VECTRA PATCH: OBJECT-FIRST SAFE FLOW =====

def _safe_object_fallback(message: str):
    try:
        from app.query.parsing import resolve_period_from_message
        period, _ = resolve_period_from_message(message)
        if period:
            return {
                "level": "business",
...
def orchestrate_vectra_query(message: str, session_ctx=None):
    try:
        msg = message.lower()

        # причины shortcut
        if "причины" in msg:
            from app.presentation.views import build_reasons_view
            from app.presentation.contracts import ok_response

            # fallback через бизнес если нет state
            fallback = _safe_object_fallback(message)
            if fallback:
                from app.domain.comparison import get_business_comparison
                current = get_business_comparison(period=fallback["period"])
                return ok_response(fallback, build_reasons_view(current))

        # стандартный старый пайплайн
        return orchestrate(message, session_ctx)
