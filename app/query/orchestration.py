...
def _extract_goal_effect_money(payload: Dict[str, Any]) -> Optional[float]:
    if not isinstance(payload, dict):
        return None
    goal = payload.get('goal') if isinstance(payload.get('goal'), dict) else {}
    for key in ('effect_money', 'value_money', 'delta_money'):
        value = goal.get(key)
        if value is not None:
            try:
                return float(value)
...
def _build_vector_block(path_goal_money: Any, current_focus_money: Any) -> Dict[str, Any]:
    ...
    coverage = min(100, round(abs(current_focus) / abs(path_goal) * 100))
    return {
...
def _build_query_from_numeric_selection(message: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 0 <= index < len(items):
        ...
        # Stage 2 Money Logic:
        # A selected parent-list row effect is NOT the Path Goal for the new
        # object. Path Goal must be fixed from the selected object's own
        # internal_drain_money after its summary is built. Existing path goal
        # is carried deeper in the route; otherwise leave it empty here.
...
def _is_render_ready_screen(payload: Any) -> bool:
    return isinstance(payload, dict) and isinstance(payload.get('context'), dict) and any(
        key in payload for key in ('kpi_block', 'structure_block', 'navigation_block', 'drain_block_render', 'goal_block')
    )
...
def _with_mode_payload(payload: Dict[str, Any], mode: str) -> Dict[str, Any]:
    ...
    if mode == "all":
        ...
        data = {
            ...
            "vector_block": data.get("vector_block") or {},
            "reasons_block": block,
...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    # Stage 2 Money Logic:
    # Focus Money is the full internal drain of the current screen.
    # Drain Block is only a top-route (top-3) navigation view and must not
    # be summed to define focus/path/coverage.
    internal_drain_money = view_payload.get('internal_drain_money') if isinstance(view_payload, dict) else None
    if internal_drain_money is None and isinstance(current, dict):
        internal_drain_money = current.get('internal_drain_money')
    drain_items = (view_payload.get('drain_block') or []) if isinstance(view_payload, dict) else []
    top_route_total = sum(_extract_item_effect_money(item) or 0 for item in drain_items) if drain_items else 0
    try:
        drain_total = float(internal_drain_money) if internal_drain_money is not None else float(top_route_total or 0)
    except Exception:
        drain_total = top_route_total or 0
    selected_focus_money = query.get('selected_focus_money')
    is_management_object = level != 'business'
    current_focus_money = (abs(drain_total) if drain_total else selected_focus_money) if is_management_object else None

    # Goal-chain rule:
    # - business screen never creates or displays management vector fields.
    # - the first management object below business initializes path_goal_money
    #   from its own full internal drain.
    # - deeper screens inherit the same path_goal_money.
    display_path_goal_money = None
    path_goal_for_state = None
    if is_management_object:
        existing_path_goal_money = query.get('path_goal_money')
        path_goal_for_state = existing_path_goal_money
        if path_goal_for_state is None:
            path_goal_for_state = abs(drain_total) if drain_total else _extract_goal_effect_money(current)
        display_path_goal_money = path_goal_for_state

    vector_block = _build_vector_block(display_path_goal_money, current_focus_money) if is_management_object else {}
    if vector_block:
        ...
        view_payload.pop('vector_block', None)
        view_payload.pop('path_goal_money', None)
        if not is_management_object:
            view_payload.pop('focus_money', None)
    ...
    if response.get('status') == 'ok':
        ...
        if drain_payload is not None:
            ...
            save_session_state(
                ...
                view_mode='default',
                show_all=False,
                path_goal_money=path_goal_for_state,
