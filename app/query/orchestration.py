...
def _voice_rating_level(normalized: str) -> Optional[str]:
    if any(token in normalized for token in ('sku', 'скю', 'товар', 'товар', 'позици')):
        return 'sku'
...
def _voice_rating_metric(normalized: str) -> tuple[str, str]:
    if 'марж' in normalized:
        return 'margin_pre', 'марже'
    if 'нацен' in normalized:
        return 'markup', 'наценке'
    if any(token in normalized for token in ('оборот', 'выруч', 'продаж')):
        return 'revenue', 'обороту'
    return 'finrez_pre', 'прибыли'


def _voice_rating_direction(normalized: str, metric_key: str) -> str:
    if any(token in normalized for token in ('худш', 'слаб', 'просад', 'паден', 'потер')):
        return 'asc'
    if any(token in normalized for token in ('лучш', 'топ', 'рейтинг')):
        return 'desc'
    return 'desc'


def _voice_group_rating_rows(rows: List[Dict[str, Any]], group_field: str, metric_key: str, direction: str, limit: int = 10) -> List[Dict[str, Any]]:
    grouped: Dict[str, List[Dict[str, Any]]] = {}
    for row in rows:
        name = str(row.get(group_field) or '').strip()
        if not name or name.lower() in {'total', 'итого'}:
            continue
...
def _build_voice_rating_response(message: str, intent: Dict[str, Any], session_ctx: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    metric_key, metric_label = _voice_rating_metric(normalized)
    direction = _voice_rating_direction(normalized, metric_key)

    if not level:
        diagnostic = _build_voice_management_diagnostic(message, intent)
        diagnostic['summary_block'] = 'Рейтинг распознан, но уровень рейтинга не определён.'
        diagnostic['next_step_block'] = ['Уточните уровень: менеджеры, сети или SKU.']
        return diagnostic
    ...
    try:
        ...
        return {
            'status': 'error',
            'reason': f'Не удалось загрузить данные VECTRA Engine: {exc}',
        }
    period = _voice_period_from_message_or_state(message, session_ctx, rows)
    if not period:
        return {
            'status': 'error',
            'reason': 'Не удалось определить период для рейтинга.',
        }
    ...
    if not period_rows:
        return {
            'status': 'error',
            'reason': meta.get('empty_reason') if isinstance(meta, dict) else 'Нет данных за период.',
        }

    group_field = VOICE_RATING_FIELD_BY_LEVEL[level]
    items = _voice_group_rating_rows(period_rows, group_field, metric_key, direction, limit=10)
    if not items:
        return {
            'status': 'error',
            'reason': 'Нет объектов для построения рейтинга.',
        }

    direction_label = 'худшие' if direction == 'asc' else 'лучшие'
    title = f"{direction_label.capitalize()} {VOICE_RATING_LEVEL_LABELS[level]} по {metric_label}"
    lines = [title, f'Период: {period}', '']
    for idx, item in enumerate(items, start=1):
        value = item.get('metric_value')
        if metric_key in {'margin_pre', 'markup'}:
            rendered_value = _voice_fmt_percent(value)
        else:
            rendered_value = _voice_fmt_money(value, signed=True)
        lines.append(f"{idx}. {item.get('object_name')} → {rendered_value}")
    ...
    return {
        ...
        'render_mode': 'voice_diagnostic',
        'summary_block': f'Распознан свободный запрос: рейтинг. Построен рейтинг через VECTRA Engine.',
        'kpi_block': lines,
        'structure_block': [],
        'main_driver': '',
        'drain_block_render': [],
        ...
        'explanation_block': [
            f'Intent: rating_request',
            f'Уровень: {VOICE_RATING_LEVEL_LABELS[level]}',
            f'Показатель: {metric_label}',
            'Расчёты KPI не изменялись; рейтинг построен на агрегированных данных VECTRA Engine.',
        ],
        'next_step_block': ['Следующий шаг: открыть объект из рейтинга обычной навигацией или задать уточняющий запрос.'],
    }
