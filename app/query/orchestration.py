...
def get_session_state(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        'session_id': session_ctx.get('session_id') or 'default',
        'all_block_cache': session_ctx.get('all_block_cache') or [],
    }
...
def _rebuild_summary_from_screen_state(screen_state: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(screen_state, dict):
        return {}
    level = (screen_state.get('current_level') or '').strip()
    object_name = screen_state.get('current_object')
    period = ((screen_state.get('raw') or {}).get('context') or {}).get('period') or ((screen_state.get('raw') or {}).get('period'))
    if not level or not period:
        return {}
    try:
        if level == 'business':
            return get_business_summary(period=period)
        if level == 'manager_top' and object_name:
            return get_manager_top_summary(manager_top=object_name, period=period)
        if level == 'manager' and object_name:
            return get_manager_summary(manager=object_name, period=period)
        if level == 'network' and object_name:
            return get_network_summary(network=object_name, period=period)
        if level == 'category' and object_name:
            return get_category_summary(category=object_name, period=period)
        if level == 'tmc_group' and object_name:
            return get_tmc_group_summary(tmc_group=object_name, period=period)
        if level == 'sku' and object_name:
            return get_sku_summary(sku=object_name, period=period)
    except Exception:
        return {}
    return {}


def _get_all_block_from_state(session_ctx: Dict[str, Any]) -> List[Dict[str, Any]]:
    state = get_session_state(session_ctx)
    screen_state = state.get('screen_state') or {}
    raw = deepcopy(screen_state.get('raw') or {})
    all_block = deepcopy(screen_state.get('all_block') or raw.get('all_block') or state.get('all_block_cache') or [])
    if all_block:
        return all_block
    rebuilt = _rebuild_summary_from_screen_state(screen_state)
    rebuilt_data = _extract_direct_payload(rebuilt)
    all_block = deepcopy(rebuilt_data.get('all_block') or [])
    if all_block:
        update_session(session_ctx.get('session_id', 'default'), {
            'all_block_cache': all_block,
            'screen_state': _build_screen_state_from_payload(rebuilt_data),
            'last_payload': rebuilt_data,
        })
    return all_block
