...
def _build_last_list_items(items: List[Dict[str, Any]], level: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for item in items:
        ...
        # VECTRA VECTOR FIX: keep selected object effect from parent drain/list.
        ...
        if effect_money is None:
            ...
            effect_money = impact.get('gap_loss_money')

        prepared.append({
            'object_name': object_name,
            'level': level,
            'normalized_name': normalize_entity_text(object_name),
            'inherited_goal_money': effect_money,
        })
        if len(prepared) >= MAX_LAST_LIST_ITEMS:
            break
    return prepared
    ...
    if 0 <= index < len(items):
        ...
        return {
            ...
            'query': {
                ...
                'inherited_goal_money': selected.get('inherited_goal_money'),
...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    # VECTRA VECTOR FIX: goal follows selected parent drain/list effect.
    inherited_goal_money = query.get('inherited_goal_money')
    if inherited_goal_money is not None:
        current = dict(current)
        current['goal'] = {
            'type': 'close_gap',
            'effect_money': inherited_goal_money,
        }
