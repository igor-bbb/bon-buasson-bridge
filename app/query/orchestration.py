...
def _ensure_workspace_markdown_for_action(payload: Dict[str, Any], *, render_mode: str = '') -> Dict[str, Any]:
    """W15.0 Negotiation Workspace completion guard.

    Action/Negotiation responses are user Workspaces, not partial technical
    calculation payloads. They must always include a complete workspace_markdown
    built from the user-facing lines before leaving orchestration.
    """
    if not isinstance(payload, dict) or payload.get('status') == 'error':
        return payload
    lines: List[str] = []
    for key in ('workspace_primary_block', 'negotiation_workspace_block', 'kpi_block', 'summary_block'):
        value = payload.get(key)
        if isinstance(value, list):
            lines.extend(str(x) for x in value if str(x or '').strip())
        elif isinstance(value, str) and value.strip() and key != 'summary_block':
            lines.append(value.strip())
    if not lines and isinstance(payload.get('summary_block'), str) and payload.get('summary_block').strip():
        lines.append(payload.get('summary_block').strip())
    markdown = '\n'.join(lines).strip()
    if not markdown:
        payload['status'] = 'error'
        payload['reason'] = 'workspace_markdown_missing'
        payload['error_code'] = 'negotiation_workspace_markdown_missing'
        return payload
    payload['workspace_primary_block'] = lines
...
def _execute_action_command(action: str, session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if action == 'negotiation':
        return _ensure_workspace_markdown_for_action(_build_negotiation_action(screen), render_mode='negotiation_workspace')
...
def _build_contextual_assistant_response(message: str, session_ctx: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """W15.0 Context Engine: free dialogue remains inside active Workspace.

    When a user asks a non-opening follow-up after a Workspace is open, do not
    erase context or ask for object/period again. The API returns a contextual
    assistant bridge so GPT can answer over the current workspace_markdown.
    """
    state = get_session_state(session_ctx)
    screen = state.get('current_screen') or state.get('last_payload')
    if not isinstance(screen, dict):
        return None
    ctx = screen.get('context') if isinstance(screen.get('context'), dict) else {}
    level = ctx.get('level') or state.get('level')
    object_name = ctx.get('object_name') or state.get('object_name')
    period = ctx.get('period') or state.get('period')
    if not level or not period:
        return None
    normalized = _normalize_message(message)
    # Explicit object-opening or discovery commands must continue through the
    # normal parser. This guard is only for follow-up assistant questions.
    if is_workspace_opening_intent(message) or normalized.isdigit() or _is_back_command(normalized) or _is_full_view_command(normalized):
        return None
    markdown = screen.get('workspace_markdown') if isinstance(screen.get('workspace_markdown'), str) else ''
    lines = [
        f'Контекст сохранён: {object_name or level} | {period}',
        'Этот вопрос относится к открытому рабочему столу. Используй active_workspace_state и последний workspace_markdown как источник контекста.',
        '',
        'Что можно сделать дальше:',
        '1. Ответить на вопрос пользователя в рамках текущего объекта.',
        '2. Показать причины текущего объекта.',
        '3. Показать полную витрину текущего уровня.',
        '4. Назад — вернуться к предыдущему рабочему столу.',
    ]
    return {
        'status': 'ok',
        'render_mode': 'assistant_context',
        'context': {'level': level, 'object_name': object_name, 'period': period, 'parent_object': ctx.get('parent_object')},
        'path': screen.get('path') if isinstance(screen.get('path'), list) else [],
        'workspace_markdown': '\n'.join(lines),
        'workspace_primary_block': lines,
        'active_workspace_state': screen.get('active_workspace_state') if isinstance(screen.get('active_workspace_state'), dict) else {},
        'workspace_action_map': screen.get('workspace_action_map') if isinstance(screen.get('workspace_action_map'), list) else [],
        'last_workspace_markdown_available': bool(markdown.strip()),
        'navigation_block': ['причины — разобрать факторы', 'все — витрина текущего уровня', 'назад — вернуться к рабочему столу'],
        'screen_order': ['workspace_markdown'],
    }
    ...
    if normalized.isdigit():
        ...
        if full_path.get('status') == 'ok':
            ...
            if named_selection.get('status') == 'ok':
                ...
                if parsed.get('status') != 'ok':
                    contextual = _build_contextual_assistant_response(message, session_ctx)
                    if contextual is not None:
                        return _finalize_response(contextual)
                    ...
                    return _finalize_response(parsed)
                # Parsed free text opens a new analytical object/path, so reset only
                # after a valid new context was detected. Follow-up assistant
                # questions above keep the active Workspace untouched.
