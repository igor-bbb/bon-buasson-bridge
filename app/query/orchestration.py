...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    current_focus_money = abs(drain_total) if drain_total else selected_focus_money

    # Goal-chain rule:
    # - business screen keeps its own YoY goal; no management vector is shown.
    # - direct object entry creates the root path goal from the object's goal,
    #   but does not display coverage on that first screen.
    # - numeric drilldown carries path_goal_money and renders vector/coverage.
    display_path_goal_money = query.get('path_goal_money')
    path_goal_for_state = display_path_goal_money
    if path_goal_for_state is None and level != 'business':
        # Direct object entry starts a new management path.
        # Its path goal is the current object's child drain total (where to go),
        # not an old business vector and not the local YoY KPI delta.
        path_goal_for_state = abs(drain_total) if drain_total else _extract_goal_effect_money(current)

    vector_block = _build_vector_block(display_path_goal_money, current_focus_money)
    if vector_block:
