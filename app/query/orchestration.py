...
SESSION_STORE_PATH = os.getenv('VECTRA_SESSION_STORE_PATH', '/tmp/vectra_session_store.json')
MAX_LAST_LIST_ITEMS = 500


def _load_persistent_sessions() -> None:
    ...
    try:
        ...
        if isinstance(data, dict):
            SESSION_STORE = {str(k): v for k, v in data.items() if isinstance(v, dict)}
...
def _persist_sessions() -> None:
    try:
        with open(SESSION_STORE_PATH, 'w', encoding='utf-8') as fh:
            json.dump(SESSION_STORE, fh, ensure_ascii=False)
    except Exception:
        pass


def derive_session_id(seed: Optional[str]) -> str:
...
def enforce_contract(response: Dict[str, Any]) -> Dict[str, Any]:
    ...
    required = {'context', 'metrics', 'structure', 'drain_block', 'goal', 'navigation'}
