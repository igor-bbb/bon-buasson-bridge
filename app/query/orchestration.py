...
from app.query.entity_resolution import resolve_entity
...
def _resolve_voice_open_filter(
    ...
    if _voice_filter_matches_rows(current_filter, matches):
        payload = {'period': period}
        for field in VOICE_OPEN_PARENT_FIELDS.get(level, []):
...
def _execute_voice_management_request(
    ...
    try:
        rows = get_normalized_rows()
        period = _voice_period_from_message_or_state(message, session_ctx, rows)
        entity = resolve_entity(message, get_entity_dictionary(period))
        level = entity.get('entity_type')
        object_name = entity.get('entity_name')
        if level and object_name and period:
            resolved = _resolve_voice_open_filter(level, object_name, period, rows, session_ctx=session_ctx)
