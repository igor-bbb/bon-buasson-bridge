...
def push_state(session_id: str) -> None:
    ...
    snapshot = {
        ...
        'screen_state': deepcopy(current.get('screen_state') or {}),
    }
...
def get_session_state(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        'screen_state': session_ctx.get('screen_state') or {},
...
def _extract_payload_contract(payload: Any) -> Dict[str, Any]:
    if not isinstance(payload, dict):
        return {}
    if all(k in payload for k in ('context', 'metrics', 'structure', 'drain_block', 'goal', 'navigation')):
        return dict(payload)
    data = payload.get('data')
    if isinstance(data, dict) and all(k in data for k in ('context', 'metrics', 'structure', 'drain_block', 'goal', 'navigation')):
        return dict(data)
    return dict(payload)


def _build_screen_state_from_payload(payload: Dict[str, Any], session_ctx: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    contract = _extract_payload_contract(payload)
    ...
    return {
        'current_level': context.get('level') or (session_ctx or {}).get('scope_level'),
        'current_object': context.get('object_name') or (session_ctx or {}).get('scope_object_name'),
        'navigation_items': navigation.get('items') or (session_ctx or {}).get('last_list_items') or [],
        'has_all': bool(navigation.get('has_all', contract.get('all_block') is not None)),
        'has_causes': bool(navigation.get('has_causes', contract.get('cause_block') is not None)),
        'has_back': bool(navigation.get('has_back', True)),
        'all_block': contract.get('all_block') or [],
        'cause_block': contract.get('cause_block') or [],
        'structure': contract.get('structure') or {},
        'raw': contract,
    }


def _save_screen_state(session_id: str, payload: Dict[str, Any]) -> None:
    current = get_session(session_id)
    screen_state = _build_screen_state_from_payload(payload, current)
    update_session(session_id, {'screen_state': screen_state})


def _screen_state(session_ctx: Dict[str, Any]) -> Dict[str, Any]:
    state = session_ctx.get('screen_state') or {}
    if isinstance(state, dict) and state:
        return state
    payload = session_ctx.get('last_payload') or {}
    return _build_screen_state_from_payload(payload, session_ctx)


def _build_mode_response(session_id: str, mode: str) -> Dict[str, Any]:
    session_ctx = get_session(session_id)
    screen_state = _screen_state(session_ctx)
    raw = _extract_payload_contract(screen_state.get('raw') or session_ctx.get('last_payload') or {})
    if not raw:
        return {'status': 'error', 'reason': 'Нет активного экрана'}

    response = deepcopy(raw)
    navigation = response.get('navigation') if isinstance(response.get('navigation'), dict) else {}
    ...
    if mode == 'all':
        all_block = screen_state.get('all_block') or raw.get('all_block') or []
        if not isinstance(all_block, list):
            all_block = []
        response['drain_block'] = list(all_block)
        response['all_block'] = list(all_block)
        list_items = _build_last_list_items(list(all_block), session_ctx.get('last_list_level'))
        save_session_state(session_id, view_mode='all', last_list_items=list_items or (session_ctx.get('last_list_items') or []), full_view=True)
    elif mode == 'reasons':
        cause_block = screen_state.get('cause_block') or raw.get('cause_block') or []
        if not cause_block:
            structure = raw.get('structure') if isinstance(raw.get('structure'), dict) else {}
            cause_block = []
            for key, item in structure.items() if isinstance(structure, dict) else []:
                if not isinstance(item, dict):
                    continue
                cause_block.append({'name': item.get('label') or key, 'effect_money': item.get('effect_money', 0)})
            cause_block = sorted(cause_block, key=lambda x: x.get('effect_money', 0))
        response['drain_block'] = list(cause_block)
        response['cause_block'] = list(cause_block)
        save_session_state(session_id, view_mode='reasons', full_view=False)
    else:
        save_session_state(session_id, view_mode='drain', full_view=False)
...
def _route_drill_query(query: Dict[str, Any], session_ctx: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    if response.get('status') == 'ok':
        ...
        save_last_payload(session_id, response)
        _save_screen_state(session_id, response)
...
def orchestrate_vectra_query(message: str, session_id: str = 'default') -> Dict[str, Any]:
    ...
    if _is_full_view_command(normalized):
        push_state(session_id)
        return enforce_contract(_build_mode_response(session_id, 'all'))

    if normalized in {'причины'} or _is_full_reasons_command(normalized):
        push_state(session_id)
        return enforce_contract(_build_mode_response(session_id, 'reasons'))
