...
def _route_signal_flow(query: Dict[str, Any], current: Dict[str, Any], session_id: str) -> Dict[str, Any]:
    ...
    # v1.2 Money Contract Lock:
    # Object Result, Opportunity Money and Navigation Money are explicit fields.
    # Legacy Path Goal / Focus / Coverage / Vector Block are not rebuilt here.
    internal_drain_money = view_payload.get('internal_drain_money') if isinstance(view_payload, dict) else None
    opportunity_money = view_payload.get('opportunity_money') if isinstance(view_payload, dict) else None
    if opportunity_money is None and internal_drain_money is not None:
        try:
            opportunity_money = abs(float(internal_drain_money)) if float(internal_drain_money) < 0 else 0
        except Exception:
            opportunity_money = 0
    view_payload['opportunity_money'] = opportunity_money or 0
    view_payload['navigation_money'] = view_payload.get('navigation_money', opportunity_money or 0)
    view_payload.pop('vector_block', None)
    view_payload.pop('focus_money', None)
    view_payload.pop('path_goal_money', None)
    if level == 'business':
        view_payload.pop('goal', None)
