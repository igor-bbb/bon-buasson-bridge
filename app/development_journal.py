...
def _is_open_engineering_task_record(record: Dict[str, Any]) -> bool:
    return _is_engineering_task_record(record) and _status_norm(record) not in {'closed', 'archived', 'logged'}


def _is_closed_engineering_task_record(record: Dict[str, Any]) -> bool:
    return _is_engineering_task_record(record) and _status_norm(record) == 'closed'
...
def build_journal_response(export: bool = False, include_test: bool = False) -> Dict[str, Any]:
    ...
    if include_test or export:
        ...
        if not closed_tasks:
            ...
            for r in closed_tasks[:50 if not export else len(closed_tasks)]:
                lines += ['', f'### {r.get("id")} — {r.get("event_type") or r.get("type")}', f'- **Закрыто:** {r.get("updated_at") or "—"}', f'- **Компонент:** {r.get("component")}', f'- **Описание:** {r.get("technical_reason")}']
    if export:
        lines += [
            '',
    ...
    return {
        ...
        'development_journal': {
            'schema_version': 'DevelopmentJournalExport/v2.1',
            'summary': summary,
