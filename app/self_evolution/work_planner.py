...
def get_professional_activity_plan() -> Dict[str, Any]:
    ...
    # DEV-0011B: enrich the plan with value-aware ranking after the raw
    # work blocks are built.  The import is local to avoid a circular import
    # because the Value Engine consumes the Work Planner output.
    from app.self_evolution.value_priority_engine import evaluate_professional_activity_value
    ...
    plan = {
        ...
        "principle": "Product Team Assistant must manage its professional work as value-ranked logical blocks, not as disconnected autonomous cycles.",
        "planning_rules": [
            ...
            "The Orchestrator conducts the selected block through value, dependency, consolidation, execution discipline and replanning gates.",
            "Value and priority are evaluated together before selecting the next block.",
            "Dependencies are evaluated before execution so blocked work is not started prematurely.",
            "Related work items are consolidated into logical cycles before new directions are opened.",
    ...
    value_evaluation = evaluate_professional_activity_value(plan)
    plan["value_priority_engine"] = {
        "value_engine_version": value_evaluation.get("value_engine_version"),
        "next_value_block": value_evaluation.get("next_value_block"),
        "decision_reason": value_evaluation.get("decision_reason"),
        "criteria_weights": value_evaluation.get("criteria_weights"),
    }
    plan["work_blocks"] = value_evaluation.get("work_blocks") or blocks
    plan["activity_items"] = value_evaluation.get("activity_items") or items
    plan["next_work_block"] = value_evaluation.get("next_value_block") or next_block
    plan["next_recommended_action"] = value_evaluation.get("next_recommended_action") or plan["next_recommended_action"]
    ...
    plan["dependency_manager"] = {
        "dependency_manager_version": dependency_evaluation.get("dependency_manager_version"),
        "dependency_summary": dependency_evaluation.get("dependency_summary"),
        "next_dependency_action": dependency_evaluation.get("next_dependency_action"),
        "consolidation_groups": dependency_evaluation.get("consolidation_groups"),
        "blocked_items": dependency_evaluation.get("blocked_items"),
    }
    plan["next_recommended_action"] = dependency_evaluation.get("next_dependency_action") or plan["next_recommended_action"]
    ...
    manager["professional_activity_engine"] = {
        ...
        "value_priority_engine": plan.get("value_priority_engine"),
        "dependency_manager": plan.get("dependency_manager"),
...
def build_professional_activity_response(plan: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    lines = [
        ...
        "- выбирать следующий блок работы по ценности, приоритету и зависимостям;",
        "- не прыгать между разрозненными задачами;",
        "- сохранять план работы в своём состоянии.",
        "",
        f"Следующий блок: {next_block.get('title') or '—'}",
        f"Причина выбора: {(plan.get('value_priority_engine') or {}).get('decision_reason') or '—'}",
