...
ASSISTANT_STATE_FILE = REPOSITORY_ROOT / "assistant_identity" / "assistant_state.json"
VERSIONS_DIR = REPOSITORY_ROOT / "versions"
LOCK = Lock()

MODEL_VERSION = "PAE-0011D.1"
REPOSITORY_SCHEMA_VERSION = "1.8"
...
def ensure_repository() -> Dict[str, Any]:
    ...
    with LOCK:
        ...
        if not ASSISTANT_STATE_FILE.exists():
            try:
                from app.self_evolution.identity import default_assistant_state
                _write_json(ASSISTANT_STATE_FILE, default_assistant_state())
            except Exception:
                _write_json(ASSISTANT_STATE_FILE, {"schema_version": "1.0", "assistant_entity": "Product Team Assistant", "center_of_system": "Product Team Assistant professional model", "created_at": now_iso(), "updated_at": now_iso()})
        return manifest
...
def repository_status() -> Dict[str, Any]:
    ...
    return {
        ...
        "primary_entity": manifest.get("primary_entity"),
        "repository_role": manifest.get("repository_role"),
        "identity_recovery_required": manifest.get("identity_recovery_required"),
...
def _default_manifest() -> Dict[str, Any]:
    return {
        ...
        "state_manager_required": True,
        "professional_activity_engine_required": True,
        "value_priority_engine_required": True,
        "dependency_manager_required": True,
        "professional_activity_orchestrator_required": True,
...
def _upgrade_manifest(manifest: Any) -> Dict[str, Any]:
    ...
    manifest.setdefault("primary_entity", "Product Team Assistant professional model")
    ...
    manifest.setdefault("identity_recovery_required", True)
    manifest.setdefault("state_manager_required", True)
    manifest.setdefault("professional_activity_engine_required", True)
    manifest.setdefault("value_priority_engine_required", True)
    manifest.setdefault("dependency_manager_required", True)
    manifest.setdefault("professional_activity_orchestrator_required", True)
    manifest.setdefault("repository_role", "infrastructure")
