...
def test_margin_and_markup_are_derived_from_aggregated_money():
    metrics = aggregate_metrics([
        {"revenue": 100.0, "cost": 80.0, "gross_profit": 20.0, "retro_bonus": 0.0, "logistics_cost": 0.0, "personnel_cost": 0.0, "other_costs": 0.0, "finrez_pre": 10.0, "margin_pre": 999.0, "markup": 999.0},
        {"revenue": 300.0, "cost": 200.0, "gross_profit": 100.0, "retro_bonus": 0.0, "logistics_cost": 0.0, "personnel_cost": 0.0, "other_costs": 0.0, "finrez_pre": 30.0, "margin_pre": -999.0, "markup": -999.0},
    ])
    assert metrics["revenue"] == 400.0
    assert metrics["cost"] == 280.0
    assert metrics["gross_profit"] == 120.0
    assert metrics["margin_pre"] == 10.0
    assert metrics["markup"] == 42.86
    assert metrics["kpi_gap"] == 32.86
