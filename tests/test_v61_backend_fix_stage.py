...
def test_reasons_view_returns_all_existing_reasons_without_truncation():
    payload = {
        ...
        'period': '2026-02',
        'metrics': {
            'object_metrics': {'revenue': 1000, 'cost': 800, 'markup': 25, 'margin_pre': 5, 'retro_bonus': 200, 'logistics_cost': 50, 'personnel_cost': 40, 'other_costs': 10, 'finrez_pre': 50},
            'business_metrics': {'revenue': 5000, 'markup': 35, 'margin_pre': 12, 'retro_bonus': 500, 'logistics_cost': 100, 'personnel_cost': 150, 'other_costs': 20},
        },
    ...
    assert view['reasons'][0]['factor'] == 'markup'
...
def test_vs_business_contains_markup_and_finrez_gaps():
    payload = {
        'level': 'manager',
        'object_name': 'M1',
        'period': '2026-02',
        'metrics': {
            ...
            'business_metrics': {'revenue': 10000, 'margin_pre': 12, 'markup': 35, 'finrez_pre': 2000},
        },
        'impact': {'gap_loss_money': 20.0},
        'previous_object_metrics': {},
        'consistency': {'status': 'ok'},
    }
    view = build_object_view(payload)
    assert view['vs_business']['business_markup'] == 35.0
    assert view['vs_business']['gap_markup_pp'] == -15.0
    assert view['vs_business']['gap_finrez_value'] == -1900.0


def test_drain_rows_include_gap_to_business_display():
    scope = {
        'level': 'manager',
        'object_name': 'Сененко',
        'period': '2026-02',
        'metrics': {'object_metrics': {'revenue': 1000, 'margin_pre': 10, 'markup': 20, 'finrez_pre': 100}, 'business_metrics': {'revenue': 1000, 'margin_pre': 12, 'markup': 25, 'finrez_pre': 300}},
        'previous_object_metrics': {},
        'consistency': {'status': 'ok'},
    }
    drain_payload = {
        'children_level': 'network',
        'all_items': [
            {'object_name': 'A', 'signal': {'status': 'critical'}, 'metrics': {'object_metrics': {'finrez_pre': -100, 'margin_pre': -10}, 'business_metrics': {'margin_pre': 12}}},
            {'object_name': 'B', 'signal': {'status': 'risk'}, 'metrics': {'object_metrics': {'finrez_pre': -50, 'margin_pre': -5}, 'business_metrics': {'margin_pre': 12}}},
            {'object_name': 'C', 'signal': {'status': 'ok'}, 'metrics': {'object_metrics': {'finrez_pre': -10, 'margin_pre': 1}, 'business_metrics': {'margin_pre': 12}}},
        ],
        'consistency': {'status': 'ok'},
    }
    view = build_object_view(scope, drain_payload)
    assert view['drain_items'][0]['gap_to_business_pp_display'] == '-22.00 п.п.'
    assert 'п.п.' in view['drain_items'][0]['line']
