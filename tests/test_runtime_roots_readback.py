...
def test_runtime_roots_readback_returns_exact_twelve_roots(monkeypatch):
    ...
    monkeypatch.setattr(roots, "initialize_runtime_supervisor", lambda force=False: _publication("Runtime Supervisor", "VECTRA-RUNTIME-SUPERVISOR-001", supervisor_status="READY"))
    monkeypatch.setattr(roots, "initialize_runtime_recovery", lambda force=False: _publication("Runtime Recovery", "VECTRA-RUNTIME-RECOVERY-001", recovery_status="READY", supervisor_available=True))
    monkeypatch.setattr(roots, "initialize_runtime_capability_registry", lambda force=False: _publication("Runtime Capability Registry", "VECTRA-RUNTIME-CAPABILITY-REGISTRY-001", registry_status="READY"))
    monkeypatch.setattr(roots, "initialize_runtime_dependency_graph", lambda force=False: _publication("Runtime Dependency Graph", "VECTRA-RUNTIME-DEPENDENCY-GRAPH-001", graph_status="READY"))
    monkeypatch.setattr(roots, "initialize_runtime_observability", lambda force=False: _publication("Runtime Observability", "VECTRA-RUNTIME-OBSERVABILITY-001", observability_status="READY"))
    monkeypatch.setattr(roots, "initialize_runtime_health", lambda force=False: _publication("Runtime Health", "VECTRA-RUNTIME-HEALTH-001", health_status="HEALTHY"))
    ...
    assert result["operational_completeness_status"] == "LIMITED"
    ...
    assert result["root_components"]["Runtime Supervisor"]["connection_status"] == "NOT_EXPLICITLY_REPORTED"
...
def test_memory_facade_publishes_and_reads_snapshot_without_refresh(monkeypatch):
    ...
    assert public_schema["info"]["version"] == "VECTRA-RUNTIME-ROOTS-READBACK-001"
