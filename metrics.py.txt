...
def aggregate_metrics(rows: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    # ?? Ќаценка считаетс€ агрегированно по объекту в периоде
...
def detect_next_step(level: str) -> str:
    chain = {
        'business': 'спуститьс€ до дивизиональных менеджеров',
        'manager_top': 'спуститьс€ до менеджеров',
        'manager': 'спуститьс€ до сетей',
        'network': 'спуститьс€ до SKU',
        'category': 'спуститьс€ до SKU',
        'tmc_group': 'спуститьс€ до SKU',
        'sku': 'прин€ть решение по SKU',
    }
...
def detect_suggested_action(status: str, priority: str, top_drain_metric: Optional[str], level: str) -> str:
    ...
    if status == 'critical':
        return 'провалитьс€ глубже по дренажу'
